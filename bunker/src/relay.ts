      ...
      ws.on('open', () => {
        ...
        ws.send(
          JSON.stringify([
            'REQ',
            SUB_ID,
            // `since` bounds the reconnect replay: strfry STORES 24133,
            // so a no-since REQ replayed every recent stored request and
            // the bunker re-signed them on each reconnect (2026-08-23
            // review). 60s grace still covers requests sent during a
            // brief disconnect; the handler's seen-id set dedupes those.
            {
              kinds: [NIP46_KIND],
              '#p': this.subscribePubkeys,
