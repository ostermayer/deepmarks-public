...
Box C signing service. Holds operational nsecs behind a NIP-46
interface so the keys never live on the api box.
...
|---|---|---|
| `brand` (`zap@deepmarks.org`) | `/run/secrets/brand.nsec` | 9735 (zap receipts), 1985 (lifetime labels), 39701 (legacy Pinboard seed bookmarks) |
| `personal` (public brand/social in production) | `/run/secrets/personal.nsec` | 9735 (legacy zap receipts), 39701 (daily Pinboard bookmark), 1 (matching social note) |

Unauthorized client pubkeys and disallowed kinds both get audited as
`rejected`. There is no way to sign `kind:0` (profile), `kind:5`
(deletion), relay lists, contacts, etc. through this service. The only
`kind:1` allowance is the public-profile daily Pinboard cross-post.
...
- `BUNKER_CLIENT_PUBKEY` — the single api pubkey this bunker
...
- Bunker ↔ api traffic traverses Box A's strfry over the VPC,
