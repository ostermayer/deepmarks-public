...
BLOSSOM="${BLOSSOM:-https://blossom.deepmarks.org}"
VERBOSE="${VERBOSE:-0}"
...
section "[2] Archive blob playback CSP on blossom.deepmarks.org"

archive_headers=$(curl -sI "$BLOSSOM/__deepmarks_security_probe__")
archive_csp=$(printf '%s' "$archive_headers" | grep -i "^content-security-policy:" | tr -d '\r')
if printf '%s' "$archive_csp" | grep -q "script-src 'none'"; then
  pass "archive playback blocks script execution"
else
  fail "archive playback CSP does not contain script-src 'none' (got: '$archive_csp')"
fi
if printf '%s' "$archive_csp" | grep -q "sandbox"; then
  pass "archive playback uses CSP sandbox"
else
  fail "archive playback CSP does not contain sandbox (got: '$archive_csp')"
fi
if printf '%s' "$archive_csp" | grep -q "connect-src 'none'"; then
  pass "archive playback blocks network fetch/connect"
else
  fail "archive playback CSP does not contain connect-src 'none' (got: '$archive_csp')"
fi
if printf '%s' "$archive_csp" | grep -q "img-src 'self' data: blob:" && ! printf '%s' "$archive_csp" | grep -q "img-src .*https:"; then
  pass "archive playback blocks external image beacons"
else
  fail "archive playback CSP allows external image loads (got: '$archive_csp')"
fi

# ─── 3. SSRF guard on /archive/purchase ──────────────────────────────────

section "[3] SSRF guard on /archive/purchase"
...
ssrf_url() {
  ...
  case "$resp" in
      ...
      402) pass "$label blocked by lifetime-only archive policy (402) — metered purchase route disabled" ;;
      429) pass "$label rate-limited (429) — guard untestable but rate limit alive" ;;
      *)   fail "$label accepted with HTTP $resp (expected 400/401/402/429)" ;;
  esac
}
...
# ─── 5. NIP-98 — body required, replay rejected, missing-payload caught ─

section "[5] NIP-98 enforcement on /account/nsec-ciphertext"
...
# ─── 6. Worker callback HMAC ──────────────────────────────────────────────

section "[6] Archive callback HMAC"

# Probe each variant. Treat 503 specially — it means
# WORKER_CALLBACK_SECRET isn't loaded into api's env, which
...
probe_callback() {
  ...
  if [ "$code" = "401" ]; then
    ...
    fail "$label → 503 (env WORKER_CALLBACK_SECRET not loaded into api — fix compose.yml passthrough)"
  else
...
# ─── 7. Lifetime price tampering ──────────────────────────────────────────

section "[7] /account/lifetime ignores body.amountSats"
...
# ─── 8. CORS — array origins, not CSV string ──────────────────────────────
