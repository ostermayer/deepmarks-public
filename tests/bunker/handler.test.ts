...
describe('handleRequest — reconnect replay dedup (2026-08-23 review)', () => {
  it('processes an event once and drops the identical replay', async () => {
    // strfry stores kind:24133 and a no-since resubscribe replays recent
    // requests — the bunker used to re-sign them on every reconnect.
    const f = makeFixture();
    const req = buildClientRequest(
      f,
      f.brandPub,
      JSON.stringify({ id: 'replay-1', method: 'connect', params: [f.brandPub] }),
    );
    const deps = { vault: f.vault, audit: f.audit, authorizedClient: f.authorizedClient };

    const first = await handleRequest(deps, req);
    expect(first).not.toBeNull();

    const replay = await handleRequest(deps, req);
    expect(replay).toBeNull();
    // Exactly one audit entry — the replay produced no second signing.
    expect(readAudit(f).filter((e) => e.outcome === 'accepted')).toHaveLength(1);
  });
});

describe('handleRequest — envelope freshness (2026-08-23 follow-up)', () => {
  it('rejects a backdated request envelope (captured-event replay after restart)', async () => {
    // The seen-id set is process-local: a captured kind:24133 re-POSTed
    // to strfry after a bunker restart would be re-signed without this
    // created_at bound.
    const f = makeFixture();
    const convKey = deriveKey(f.clientSk, f.brandPub);
    const cipher = encryptPayload(
      convKey,
      JSON.stringify({ id: 'old-1', method: 'connect', params: [f.brandPub] }),
    );
    ...
    expect(resp).toBeNull();
    const entries = readAudit(f);
    expect(entries.some((e) => e.outcome === 'accepted')).toBe(false);
  });
});
