...
import { allowedKindsFor, checkPermission } from '@src/permissions.js';
...
describe('checkPermission', () => {
  ...
  it('accepts personal signing kind 1 for the daily public-profile cross-post', () => {
    const r = checkPermission(
      { authorizedClient: AUTHORIZED },
      { clientPubkey: AUTHORIZED, identity: 'personal', kind: 1 },
    );
    expect(r.ok).toBe(true);
  });

  it('rejects kind 1 for the admin/operational brand identity', () => {
    const r = checkPermission(
      { authorizedClient: AUTHORIZED },
      { clientPubkey: AUTHORIZED, identity: 'brand', kind: 1 },
    );
...
describe('allowedKindsFor', () => {
  ...
  it('exposes the personal allowlist (9735 + 39701 + 1)', () => {
...
describe('kind 39701 (public web bookmark) allowed for both server publishing roles', () => {
  const AUTHORIZED = 'a'.repeat(64);
  it('brand can still sign legacy 39701 events', () => {
    const r = checkPermission(
      { authorizedClient: AUTHORIZED },
  ...
  it('personal can sign 39701 for the public-profile daily Pinboard post', () => {
    const r = checkPermission(
      { authorizedClient: AUTHORIZED },
