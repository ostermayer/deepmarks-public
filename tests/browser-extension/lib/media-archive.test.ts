...
describe('media archive extension detection', () => {
  ...
  it('suppresses on the queued: sentinel without stashing a key or polling it', async () => {
    // 2026-07-17 duplicate-flood regression: the server answers a
    // duplicate enqueue with jobId 'queued:already-pending'. The loop
    // must remember the URL (no re-enqueue) but must NOT bind the
    // freshly-generated key to the sentinel, and must never poll the
    // sentinel as if it were a real jobId (the status route 400s it).
    const videoUrl = 'https://www.youtube.com/watch?v=AbCdEfGhI12';
    mocks.startMediaArchive.mockResolvedValue({ jobId: 'queued:already-pending' });

    const first = await queueEligibleMediaArchiveBackfill({
      bookmarks: [{ url: videoUrl, eventId: 'event-1', savedAt: 123 }],
      archives: [],
      nsecHex: '1'.repeat(64),
      pubkey: '2'.repeat(64),
    ...
    expect(first).toEqual({ queued: 0, skipped: 1 });
    expect(mocks.startMediaArchive).toHaveBeenCalledTimes(1);
    expect(mocks.stashPendingKey).not.toHaveBeenCalled();
    expect(mocks.publishPendingArchiveKey).not.toHaveBeenCalled();
    expect(storage.get('deepmarks-media-archive-queued:v1:yt:AbCdEfGhI12')).toEqual(
      expect.objectContaining({ jobId: 'queued:already-pending' }),
    );

    const second = await queueEligibleMediaArchiveBackfill({
      bookmarks: [{ url: videoUrl, eventId: 'event-1', savedAt: 123 }],
      archives: [],
      nsecHex: '1'.repeat(64),
      pubkey: '2'.repeat(64),
    ...
    // Still exactly one enqueue attempt, and the sentinel was never
    // handed to the status endpoint.
    expect(mocks.startMediaArchive).toHaveBeenCalledTimes(1);
    expect(mocks.archiveStatus).not.toHaveBeenCalledWith('queued:already-pending');
  });
});
