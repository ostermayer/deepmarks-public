...
import { LifetimeStore } from '@src/lifetime.js';
import { sanitizeRedirectUrl } from '@src/routes/lifetime.js';
...
class FakeRedis {
  ...
  // markPaid LPUSHes onto a queue on first settlement (lifetime-
  // archive backfill trigger) and SADDs to the registry. Both no-op
  // for the tests in this file.
  lists = new Map<string, string[]>();
  async lpush(k: string, ...vals: string[]) {
    const arr = this.lists.get(k) ?? [];
    arr.unshift(...vals);
    this.lists.set(k, arr);
    return arr.length;
  }
...
describe('sanitizeRedirectUrl', () => {
  it('accepts public frontend returns when the API origin is api.deepmarks.org', () => {
    expect(
      sanitizeRedirectUrl('https://deepmarks.org/app/bookmarks?upgraded=1', 'https://api.deepmarks.org'),
    ).toBe('https://deepmarks.org/app/bookmarks?upgraded=1');
  });

  it('accepts the native app upgrade return scheme', () => {
    expect(
      sanitizeRedirectUrl('deepmarks://upgrade?done=1', 'https://api.deepmarks.org'),
    ).toBe('deepmarks://upgrade?done=1');
  });

  it('rejects unrelated hosts and custom-scheme paths', () => {
    expect(sanitizeRedirectUrl('https://evil.example/app/bookmarks', 'https://api.deepmarks.org')).toBeUndefined();
    expect(sanitizeRedirectUrl('deepmarks://evil?done=1', 'https://api.deepmarks.org')).toBeUndefined();
  });
});
