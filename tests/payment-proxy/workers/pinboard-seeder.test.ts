...
import { PinboardSeederWorker, seederIntervalFromEnv } from '@src/workers/pinboard-seeder.js';
import type { SeedResult } from '@src/seed/runner.js';
import type { RemoteSigner } from '@src/signer.js';
...
describe('PinboardSeederWorker', () => {
  ...
  it('skips when the daily idempotency guard says today already ran', async () => {
    const log = mkLogger();
    const seedFn = vi.fn(async () => OK_RESULT);
    const setTimer = vi.fn(() => 0 as unknown as ReturnType<typeof setTimeout>);
    const claimDailyRun = vi.fn(async () => ({ ok: false }));
    const worker = new PinboardSeederWorker(
      { pool: fakePool, logger: log, signer: fakeSigner, seedFn, setTimer, claimDailyRun },
      24,
    );
    await worker.start();
    await new Promise((r) => setTimeout(r, 0));
    expect(claimDailyRun).toHaveBeenCalledTimes(1);
    expect(seedFn).not.toHaveBeenCalled();
    expect(log.logs.some((l) => l.includes('already ran today'))).toBe(true);
    expect(setTimer).toHaveBeenCalledTimes(1);
  });

  it('releases the daily claim when seedOnce throws', async () => {
    const log = mkLogger();
    ...
    const worker = new PinboardSeederWorker(
      {
        ...
        seedFn,
        setTimer,
        claimDailyRun: async () => ({ ok: true, release }),
      },
      24,
    );
    await worker.start();
