...
import { bookmarkEventToJson, bookmarkSortScore } from '@src/api-helpers.js';
...
describe('bookmarkEventToJson', () => {
  ...
  it('uses published_at as savedAt when a delayed publish preserves the original save time', () => {
    const sk = generateSecretKey();
    const event = finalizeEvent(
      {
        kind: 39701,
    ...
    const json = bookmarkEventToJson(event);
    expect(json.publishedAt).toBe(1_700_000_000);
    expect(json.savedAt).toBe(1_700_000_000);
  });

  it('uses published_at_ms as the stable same-second sort score', () => {
    const sk = generateSecretKey();
    const event = finalizeEvent(
      {
        kind: 39701,
    ...
    const json = bookmarkEventToJson(event);
    expect(json.savedAt).toBe(1_700_000_000);
    expect(json.savedAtMs).toBe(1_700_000_000_123);
    expect(bookmarkSortScore(json)).toBe(1_700_000_000_123);
  });
