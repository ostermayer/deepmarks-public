...
<div id="popular_bookmarks">
  ...
  <div class="bookmark" id="bm-1">
    ...
    <a href="/url:abc" class="bookmark_count">42</a>
...
describe('parsePinboardPage', () => {
  ...
  it('captures Pinboard popularity counts when present', () => {
    const [first] = parsePinboardPage(FIXTURE);
    expect(first?.count).toBe(42);
  });

  it('parses Pinboard popular rows that only expose title and count', () => {
    const entries = parsePinboardPage(`
      <div class="bookmark" id="abc">
        <div class="display">
          <a class="bookmark_title" href="https://popular.example/post">Popular thing</a>
          <a href="/url:abc" class="bookmark_count">20</a>
        </div>
      </div>
    `);
    expect(entries).toEqual([
      {
        url: 'https://popular.example/post',
        title: 'Popular thing',
        tags: [],
        count: 20,
      },
...
describe('dedupe', () => {
  const a: PinboardEntry = { url: 'https://x', title: 'A', tags: ['a'], count: 1 };
  const b: PinboardEntry = { url: 'https://x', title: 'A', tags: ['a', 'b'], description: 'desc', count: 2 };
