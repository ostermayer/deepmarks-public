...
import {
  ...
  listCachedPublicBookmarksByUrl,
  removeCachedPublicBookmarksForDeletion,
} from '@src/public-bookmark-cache.js';
...
class FakeRedis {
  ...
  multi(): { set: (key: string, value: string) => unknown; zadd: (key: string, score: number, id: string) => unknown; zremrangebyrank: (key: string, start: number, stop: number) => unknown; zrem: (key: string, id: string) => unknown; del: (key: string) => unknown; exec: () => Promise<unknown[]> } {
...
describe('public bookmark cache deletion', () => {
  it('indexes bookmarks by canonical URL for metadata fallback lookups', async () => {
    const redis = new FakeRedis();
    const first = bookmark({
      id: '1'.repeat(64),
      created_at: 10,
      tags: [['d', 'https://Example.com/report/?utm_source=news#top'], ['title', 'Report']],
    });
    const second = bookmark({
      id: '2'.repeat(64),
      pubkey: 'b'.repeat(64),
      created_at: 11,
      tags: [['d', 'https://example.com/report'], ['title', 'Updated report']],
    });

    await cachePublicBookmarkEvent(redis as never, first);
    await cachePublicBookmarkEvent(redis as never, second);

    await expect(listCachedPublicBookmarksByUrl(redis as never, 'https://example.com/report', 10))
      .resolves.toMatchObject([
        { id: second.id, title: 'Updated report' },
        { id: first.id, title: 'Report' },
      ]);
  });
  ...
  it('removes cached kind:39701 rows targeted by kind:5', async () => {
    ...
    await expect(listCachedPublicBookmarksByUrl(redis as never, event.tags[0]![1]!, 10)).resolves.toEqual([]);
  });
