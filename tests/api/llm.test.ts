...
describe('deepInfraConfigFromEnv', () => {
  ...
  it('configures the Cerebras chat fallback and defaults to gpt-oss-120b', () => {
    const config = deepInfraConfigFromEnv({ DEEPINFRA_TOKEN: 'token', CEREBRAS_TOKEN: 'cb' });
    expect(config?.chatFallbackToken).toBe('cb');
    expect(config?.chatFallbackModel).toBe('gpt-oss-120b');
  });

  it('treats empty-string fallback env (compose ${VAR:-}) as unset, not invalid', () => {
    // Regression: compose passes unset vars as '' — `??` let '' through and the
    // policy check then rejected the empty model, disabling the whole LLM.
    const config = deepInfraConfigFromEnv({
      DEEPINFRA_TOKEN: 'token', CEREBRAS_TOKEN: 'cb', CEREBRAS_CHAT_MODEL: '', CEREBRAS_BASE_URL: '',
    });
    expect(config?.chatFallbackModel).toBe('gpt-oss-120b');
    expect(config?.chatFallbackBaseUrl).toBe('https://api.cerebras.ai/v1');
  });

  it('rejects an unapproved chat-fallback model', () => {
    expect(() => deepInfraConfigFromEnv({
      DEEPINFRA_TOKEN: 'token', CEREBRAS_TOKEN: 'cb', CEREBRAS_CHAT_MODEL: 'closed-fallback',
    })).toThrow(DeepInfraModelPolicyError);
  });
});
...
describe('DeepInfraClient', () => {
  ...
  it('fails over to the Cerebras chat fallback when the primary provider errors', async () => {
    const fetchMock = vi.fn(async (url: string, init: RequestInit) => {
      if (url.includes('deepinfra')) throw new Error('primary down');
      // Cerebras fallback request: fast open-weight model, low reasoning effort.
      const body = JSON.parse(String(init.body)) as Record<string, unknown>;
      ...
      return new Response(JSON.stringify({
        choices: [{ message: { content: JSON.stringify({
          title: 'Fallback title', description: 'Fallback desc', summary: 'Fallback summary',
          tags: ['fallback'], category: 'Research', language: 'en', confidence: 0.8,
        }) } }],
      }), { status: 200 });
    });
    vi.stubGlobal('fetch', fetchMock);

    const client = new DeepInfraClient(deepInfraConfigFromEnv({ DEEPINFRA_TOKEN: 'token', CEREBRAS_TOKEN: 'cb-token' }));
    const result = await client.enrichBookmark({ url: 'https://example.com/post' });

    expect(result).toMatchObject({ title: 'Fallback title', summary: 'Fallback summary' });
    expect(fetchMock).toHaveBeenCalledTimes(2); // primary failed, fallback succeeded
  });

  it('does not fall back when no fallback token is configured', async () => {
    const fetchMock = vi.fn(async () => { throw new Error('primary down'); });
    vi.stubGlobal('fetch', fetchMock);
    const client = new DeepInfraClient(deepInfraConfigFromEnv({ DEEPINFRA_TOKEN: 'token' }));
    await expect(client.enrichBookmark({ url: 'https://example.com/post' })).rejects.toThrow('primary down');
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
});
