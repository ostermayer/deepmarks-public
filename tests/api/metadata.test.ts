...
describe('extractMetadata', () => {
  ...
  it('does not fetch preview URLs whose host resolves to a private address', async () => {
    // Persistent (not Once): the blocked-fetch fallback gate re-resolves the
    // host before running any network fallback leg, and real DNS would
    // answer private both times.
    dnsLookupMock.mockResolvedValue([{ address: '10.0.0.4', family: 4 }]);
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);
...
describe('direct media metadata', () => {
  ...
  it('recovers title/description from a Wayback snapshot when a host bot-walls the fetch', async () => {
    const url = 'https://www.wsj.com/business/white-line-road-invention-america-250-8ce6bb89';
    const snapshotUrl = 'https://web.archive.org/web/20260704110715/' + url;
    const snapshotHtml = `<html><head>
      <title>ignored</title>
      <meta property="og:title" content="This Simple White Line Is America's Greatest Unsung Innovation">
      <meta property="og:description" content="A history of the painted road line.">
      <meta property="og:image" content="https://web.archive.org/web/2026/im_/https://images.wsj.net/x.jpg">
      <meta name="lightning" content="stale@snapshot.example">
    </head><body></body></html>`;
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL) => {
      const target = String(input);
      if (target.startsWith('https://archive.org/wayback/available')) {
        return new Response(JSON.stringify({
          archived_snapshots: { closest: { available: true, status: '200', url: snapshotUrl } },
        }), { status: 200, headers: { 'content-type': 'application/json' } });
      }
      if (target.startsWith('https://web.archive.org/')) {
        return new Response(snapshotHtml, { status: 200, headers: { 'content-type': 'text/html' } });
      }
      // The live page itself is bot-walled.
      return new Response('challenge', { status: 401, headers: { 'content-type': 'text/html' } });
    }));

    const search = vi.fn();
    const meta = await new MetadataStore(null, undefined, { enabled: true, search }).resolve(url);

    expect(meta).toMatchObject({
      url,
      title: "This Simple White Line Is America's Greatest Unsung Innovation",
      description: 'A history of the painted road line.',
    });
    // Snapshot-only fields must not leak: media URLs are rewritten to
    // web.archive.org and a stale lightning tag must never feed zap splits.
    expect(meta?.image).toBeUndefined();
    expect(meta?.lightning).toBeUndefined();
    // Wayback answered, so the flakier web-search leg is never consulted.
    expect(search).not.toHaveBeenCalled();
  });

  it('recovers title/description from web search when a host bot-walls the fetch', async () => {
    // WSJ/DataDome answer 401 challenge shells to any datacenter fetch, so
    // the direct fetch, bookmark fallback, and Crossref all come up empty —
    // the rescue web search is the last-ditch title source.
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response('<html><p>Please enable JS</p></html>', {
        status: 401,
        headers: { 'content-type': 'text/html' },
      }),
    ));
    const url = 'https://www.wsj.com/business/white-line-road-invention-america-250-8ce6bb89?st=dnd4bo';
    const search = vi.fn().mockResolvedValue([
      // SEO spam pointing elsewhere must be ignored...
      { url: 'https://spam.example.com/whatever', title: 'Buy pills now' },
      // ...same host but a different article too...
      { url: 'https://www.wsj.com/business/other-story-11223344', title: 'Wrong article' },
      // ...only the same host+path result may title this URL (query string
      // differences like the ?st= share token don't matter).
      {
        url: 'https://www.wsj.com/business/white-line-road-invention-america-250-8ce6bb89',
        title: "This Simple White Line Is America's Greatest Unsung Innovation",
        snippet: 'A history of the painted road line.',
      },
    ]);
    ...
    // Query is host + slug words with the opaque trailing hex id dropped.
    expect(search.mock.calls[0]![0]).toBe('wsj.com business white line road invention america 250');
    expect(meta).toMatchObject({
      title: "This Simple White Line Is America's Greatest Unsung Innovation",
      description: 'A history of the painted road line.',
      suggestedTags: [],
  ...
  it('falls back to a last-segment query when the full-path query finds nothing', async () => {
    // Some engines return zero results for longer queries; the article slug
    // is the final path segment, so retry with just that.
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
    ...
    expect(search).toHaveBeenNthCalledWith(1, 'wsj.com business white line road invention america 250');
    expect(search).toHaveBeenNthCalledWith(2, 'wsj.com white line road invention america 250');
    expect(meta).toMatchObject({
      title: "This Simple White Line Is America's Greatest Unsung Innovation",
    });
  });

  it('returns the empty stub when no search result matches the exact host+path', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response('blocked', { status: 403, headers: { 'content-type': 'text/html' } }),
    ));
    const url = 'https://www.wsj.com/business/white-line-road-invention-america-250-8ce6bb89';
    const search = vi.fn().mockResolvedValue([
      { url: 'https://mirror.example.net/wsj/white-line-road', title: 'Scraped copy' },
    ]);

    const meta = await new MetadataStore(null, undefined, { enabled: true, search }).resolve(url);

    expect(meta).toEqual({ url, suggestedTags: [] });
  });

  it('does not search for opaque URLs whose path has no slug words', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      new Response('blocked', { status: 403, headers: { 'content-type': 'text/html' } }),
    ));
    const search = vi.fn();

    const meta = await new MetadataStore(null, undefined, { enabled: true, search })
      .resolve('https://example.com/a1b2c3d4e5f6');

    expect(search).not.toHaveBeenCalled();
    expect(meta).toMatchObject({ suggestedTags: [] });
  });
