...
describe('archive rescue', () => {
  ...
  it('does not rescue media archive failures (best-effort, not webpages)', async () => {
    ...
    // Reason string is shared with the terminal-retry sweep now
    // (archiveFailureReplayEligibility) — one policy, one spelling.
    expect(result).toMatchObject({ eligible: false, skippedReason: 'non-replayable-archive' });
    expect(fetchMock).not.toHaveBeenCalled();
  });
