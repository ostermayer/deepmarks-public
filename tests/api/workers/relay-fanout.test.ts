...
import { planPublishRelayRetry, shouldAlertPublishRelayDeadLetter } from '@src/workers/relay-fanout.js';
...
describe('planPublishRelayRetry', () => {
  ...
  it('drops replaced replaceable events without alerting operators', () => {
    const err = new Error('replaced: have newer event');

    expect(planPublishRelayRetry(1, err)).toEqual({
      action: 'drop',
      reason: 'policy-rejected',
    });
    expect(shouldAlertPublishRelayDeadLetter('attempts-exhausted', err)).toBe(false);
    expect(shouldAlertPublishRelayDeadLetter('attempts-exhausted', new Error('connection reset'))).toBe(true);
  });
