...
import { privateItemSetNameForUrl } from '$lib/nostr/private-set-core.js';
...
describe('per-item private writes never touch the existing set', () => {
  it('a save publishes one per-item event without fetching or decrypting chunks', async () => {
    ...
    const template = await addToPrivateSet({ url: URL, title: 'new' }, OWNER);

    expect(fetchEvents).not.toHaveBeenCalled();
    expect(template.tags.find((t) => t[0] === 'd')?.[1])
      .toBe(await privateItemSetNameForUrl(URL));
    expect(template.tags.some((t) => t[0] === 'dm-set-version')).toBe(false);
  ...
  it('an edit publishes one per-item event for the same d-tag', async () => {
    installNdk();
    const template = await updatePrivateSetEntry({ url: URL, title: 'edited' }, OWNER);
  ...
  it('a delete publishes only the tombstone item', async () => {
    ...
    await removeFromPrivateSet(URL, OWNER);
