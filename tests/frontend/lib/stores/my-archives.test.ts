...
describe('archive record lookup preferences', () => {
  ...
  it('indexes a rescued archive under its original failed URL', async () => {
    const { myArchives, replaceMyArchiveRecords } = await import('$lib/stores/my-archives.js');
    const rescued = archive({
    ...
    replaceMyArchiveRecords('pubkey', [rescued]);

    expect(get(myArchives).get('https://gomix.com/community/community-picks')).toBe(rescued);
  });
