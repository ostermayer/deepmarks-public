...
describe('media archive tracking', () => {
  ...
  it('treats common yt-dlp video/audio platforms as media candidates', () => {
    expect(isPotentialMediaUrl('https://soundcloud.com/artist/track-name')).toBe(true);
    expect(isPotentialMediaUrl('https://artist.bandcamp.com/track/song')).toBe(true);
    expect(isPotentialMediaUrl('https://www.mixcloud.com/dj/a-set/')).toBe(true);
    expect(isPotentialMediaUrl('https://www.dailymotion.com/video/x8abcde')).toBe(true);
    expect(isPotentialMediaUrl('https://dai.ly/x8abcde')).toBe(true);
    expect(isPotentialMediaUrl('https://streamable.com/abcde')).toBe(true);
    expect(isPotentialMediaUrl('https://www.bitchute.com/video/abcDEF/')).toBe(true);
    expect(isPotentialMediaUrl('https://kick.com/streamer/clips/clip_abc')).toBe(true);
    expect(isPotentialMediaUrl('https://example.com/just-an-article')).toBe(false);
  });
