...
const archiveKeyMocks = vi.hoisted(() => ({
  ...
  // nip44 probe passes by default; the gating test overrides per-case.
...
import {
  ...
  rememberArchiveQueueFailure,
  type ArchiveProgress,
} from '$lib/nostr/archive.js';
import { api } from '$lib/api/client';
import { isArchiveBackoffActive } from '$lib/nostr/archive-backoff';
import {
  addArchiveKeyToSet,
...
} from '$lib/nostr/archive-keys.js';
...
describe('archivePage', () => {
  ...
  it('persists immediate archive enqueue failures for cleanup', () => {
    const url = 'https://dead.example/imported';

    rememberArchiveQueueFailure('pub', url, 'domain lookup failed');

    const stats = archiveQueueStats('pub');
    expect(stats.failedUrls.has(url)).toBe(true);

    expect(forgetArchiveQueueUrls('pub', [url])).toBe(1);
    expect(archiveQueueStats('pub').failedUrls.has(url)).toBe(false);
  });

  it('records failure backoff when the server answers with a recent-failure sentinel', async () => {
    // 2026-08-21 loop fix: the server's cooldown gate replies with
    // `queued:recent-failure` instead of enqueueing — the client must
    // remember that and stop re-asking on every backfill pass.
    const url = 'https://dead.example/gated';
    mockedPurchase.mockResolvedValueOnce({
      paymentHash: 'queued:recent-failure',
      jobId: 'queued:recent-failure',
      amountSats: 0,
    });

    await enqueueArchivePage({ url, tier: 'public', pubkey: 'pub', lifetime: true, dedupe: true });

    expect(isArchiveBackoffActive('pub', url)).toBe(true);
    expect(isArchiveBackoffActive('pub', 'https://dead.example/other')).toBe(false);
  });

  it('starts a backoff when a failed queue slot is released for retry', () => {
    const url = 'https://dead.example/failing';
    rememberArchiveQueueFailure('pub', url, 'archive job failed', 'lifetime:' + 'a'.repeat(32));

    expect(releaseFailedArchiveQueueSlots('pub')).toBe(1);

    // The slot is free again (a later pass MAY retry), but the backoff
    // ledger keeps that retry from firing immediately.
    expect(archiveQueueStats('pub').failedUrls.has(url)).toBe(false);
    expect(isArchiveBackoffActive('pub', url)).toBe(true);
  });
});
