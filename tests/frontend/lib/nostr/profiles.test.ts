...
describe('parseProfileContent', () => {
  it('extracts the standard NIP-01 profile fields', () => {
    const json = JSON.stringify({
      ...
      lud06: 'lnurl1dp68gurn8ghj7mrww4exctnrdakj7m3wv4uxzmtsd3jjucm0d5q',
  ...
  it('accepts lud06 LNURL-pay values when no lud16 address is present', () => {
    const p = parseProfileContent(
      JSON.stringify({ lud06: 'lnurl1dp68gurn8ghj7mrww4exctnrdakj7m3wv4uxzmtsd3jjucm0d5q' }),
      'pub-1'
    );
    expect(p?.lud16).toBeUndefined();
    expect(p?.lud06).toBe('lnurl1dp68gurn8ghj7mrww4exctnrdakj7m3wv4uxzmtsd3jjucm0d5q');
  });

  it('selects lud16 before lud06 for zap routing', () => {
    expect(profileLightningAddress({
      lud16: 'alice@getalby.com',
      lud06: 'lnurl1fallback',
...
describe('getProfile (reactive store)', () => {
  ...
  it('resolveProfile waits for the in-flight profile lookup', async () => {
    fetchEvent.mockResolvedValue({
      content: JSON.stringify({ lud16: 'alice@getalby.com' }),
    });
    await expect(resolveProfile('pub-4')).resolves.toMatchObject({
      lud16: 'alice@getalby.com',
    });
  });
});
