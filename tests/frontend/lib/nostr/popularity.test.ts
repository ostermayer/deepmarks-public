...
describe('applyPopularityFloor', () => {
  ...
  it('gives every editorial pubkey the same unconditional pass', () => {
    const out = applyPopularityFloor(
      [
        ranked({ curator: BRAND, score: 0, totalZapSats: 0 }),
        ranked({ curator: SEEDER, score: 0, totalZapSats: 0 }),
        ranked({ curator: OTHER, score: 0, totalZapSats: 10_000 }),
      ],
      { editorialPubkeys: [BRAND, SEEDER] },
    );
    expect(out.map((b) => b.curator)).toEqual([BRAND, SEEDER]);
  });
