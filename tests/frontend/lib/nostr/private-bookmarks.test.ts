...
vi.mock('$lib/nostr/ndk.js', () => ({
  getNdk: vi.fn(),
  // No-op connect helper — the real one connects to relays, which the
  // private-bookmarks tests don't need (the mocked NDK already serves
  // events directly).
...
} from '$lib/nostr/private-set-core.js';
import { getNdk } from '$lib/nostr/ndk.js';
import { KIND } from '$lib/nostr/kinds.js';
import type { SignedEventLike, UnsignedEventTemplate } from '$lib/nostr/bookmarks.js';
...
describe('tryDecryptPrivateSet', () => {
  ...
  it('returns ok with the parsed entries on success', async () => {
    ...
    if (r.ok) {
      ...
      expect(r.set.createdAt).toBe(1700000000);
    }
  });
});
...
describe('chunk set events (via the replacement stream)', () => {
  it('throws when no signer is attached', async () => {
    mockedGetNdk.mockReturnValue(makeNdkMock(null));
    const stream = buildPrivateSetReplacementEventStream({ entries: [] }, 'me');
    await expect(stream.next()).rejects.toThrow(/Sign in/);
  });
  ...
  it('builds a kind:30003 event with d=deepmarks-private and ciphertext content', async () => {
    ...
    expect(template.tags[0]).toEqual(['d', 'deepmarks-private']);
...
describe('private set chunking', () => {
  ...
  it('merges mobile private item events with chunked set entries', async () => {
    const signer = makeSignerMock();
    signer.decrypt
      .mockResolvedValueOnce(JSON.stringify([[['d', 'https://chunk.test'], ['title', 'chunk']]]))
      .mockResolvedValueOnce(JSON.stringify([[['d', 'https://item.test'], ['title', 'item']]]));
    mockedGetNdk.mockReturnValue(makeNdkMock(signer, [
      makeEvent('me', 'chunk', 200, 'deepmarks-private'),
      makeEvent('me', 'item', 250, 'deepmarks-private-item:abc'),
    ]));

    const set = await fetchOwnPrivateSet('me');
    expect(set.chunkNames).toEqual(['deepmarks-private']);
    expect(set.entries.map((entry) => entry[0]?.[1])).toEqual([
      'https://chunk.test',
      'https://item.test',
    ]);
  ...
  it('uses the latest item event for each mobile private item d-tag', () => {
    const events = [
      makeEvent('me', 'old-item', 100, 'deepmarks-private-item:abc'),
      makeEvent('me', 'new-item', 300, 'deepmarks-private-item:abc'),
      makeEvent('me', 'chunk', 200, 'deepmarks-private'),
    ];
    expect(selectPrivateBookmarkSetEvents(events).map((e) => e.content))
      .toEqual(['chunk', 'new-item']);
    expect(selectCompletePrivateSetEvents(events).map((e) => e.content))
      .toEqual(['chunk']);
  });

  it('merges decrypted chunks, mobile items, and tombstones in one place', () => {
    const set = mergePrivateSetEventEntries([
      {
        id: 'chunk',
        createdAt: 100,
        tags: [['d', 'deepmarks-private']],
        entries: [
          [['d', 'https://a.test'], ['title', 'chunk a']],
          [['d', 'https://b.test'], ['title', 'chunk b']],
        ],
      },
      {
        id: 'old-item',
        createdAt: 150,
        tags: [['d', 'deepmarks-private-item:old']],
        entries: [[['d', 'https://a.test'], ['title', 'old item']]],
      },
      {
        id: 'delete',
        createdAt: 200,
        tags: [['d', 'deepmarks-private-item:delete']],
        entries: [[['d', 'https://a.test'], ['deleted', '1']]],
      },
    ]);

    expect(set.baseEventId).toBe('chunk');
    expect(set.createdAt).toBe(100);
    expect(set.chunkNames).toEqual(['deepmarks-private']);
    expect(set.entries.map((entry) => entry[0]?.[1])).toEqual(['https://b.test']);
  });

  it('hashes a URL into a stable private item d-tag', async () => {
    await expect(privateItemSetNameForUrl('https://example.com/x'))
      .resolves.toMatch(/^deepmarks-private-item:[0-9a-f]{64}$/);
  });
