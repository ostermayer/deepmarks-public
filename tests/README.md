...
| `shared-core-parity.test.ts` | structural | passing | Every PARITY-COPIED module must stay byte-identical between frontend and extension — sha256 comparison of both copies. Four pairs: `private-set-core.ts` (chunk selection incl. cross-version union, per-item selection, tombstone-aware merge — drift here caused the original wipe bug), `bookmark-merge-core.ts` (the canonical replaceable-conflict rule), `nwc.ts` (NIP-47 wallet client), `nsec-backup.ts` (recovery text + QR). |
...
| `shared-module-parity.test.ts` | 2026-08-23 simplification backlog | structural | The backend counterpart of the frontend/extension `shared-core-parity` convention. Every generated copy of a `packages/*` canonical module (`safe-url-core` SSRF predicates, `archive-wire` Redis job/file/delete shapes, `youtube-id` video-ID parsing across all four packages) must stay byte-identical to its source — drift is how SSRF fixes were hand-ported between copies, how the worker's `ArchiveJob` lost `eventId`, and how the client's YouTube parser disagreed with the server's. The manifest is imported from `scripts/sync-shared-modules.mjs`, so new shared modules are covered automatically; a twin test runs in the archive-worker suite. |
...
- `lib/nostr/` — the Nostr core: kind:39701 build/parse + URL safety
  ...
  `popularity*`).
- `lib/api/` — api client: NIP-98 auth headers, zod response
...
- `shared-module-parity.test.ts` — generated `packages/*` copies must match
  their canonical source (twin of the api regression test of the same name).
