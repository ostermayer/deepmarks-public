...
|---|---|---|---|
| `private-set-destructive-rewrite.test.ts` | PRIV-F1 | guard (fixed 2.2.0) | A failed chunk decrypt (remote signer hiccup, missing nip44, denied prompt) must abort private-set rewrites instead of silently republishing the set without those entries (which erased them relay-wide). Baseline test pins the fetch→merge→append happy path. |
| | SYNC-F2 | guard (fixed 2.2.0) | A URL covered by a newer delete tombstone must not be resurrected into a rewrite from this device's stale localStorage cache. Baseline pins that the relay-fetched merge itself honors tombstones. |
...
| `archive-crypto-cross-impl.test.ts` | ARCH gap | passing | Cross-implementation vector: archive-worker `encryptBlob` (node:crypto) output decrypts with the frontend's `decryptArchiveBlob` (WebCrypto) — layout `[12-byte nonce][ct][16-byte tag]`, base64 key format, GCM tamper/wrong-key rejection, 1 MiB blob. Drift here bricks every paid private/media archive. |

### `browser-extension/regression/`

| Test file | Finding | Status | What it guards |
|---|---|---|---|
| `private-set-wipe-on-failed-read.test.ts` | EXT-F1 / PRIV-F3 | guard (fixed 2.2.0) | The single most severe finding: a timed-out relay read or an undecryptable chunk is swallowed into an empty set, and the next private save publishes a complete 1-entry replacement — wiping the user's entire private library on all devices. The save must refuse the whole-set rewrite. Baseline pins the append-and-republish happy path **and** the `dm-set-version`/`dm-set-count` wire format the web app's reader requires. |

### `payment-proxy/regression/`

| Test file | Finding | Status | What it guards |
|---|---|---|---|
| `relay-fanout-retry-classification.test.ts` | RELAY-F1/F5 | guard (fixed 2.2.0) | Deterministic strfry policy rejections ("pubkey not registered", "kind not accepted", "too old", future clock skew) must be dead-lettered, not retried 8× as "transient" and silently dropped after the client already got its 202. Baselines pin rate-limit backoff, transient backoff, attempts-exhausted drop. |
| `strfry-write-policy.test.ts` | SYNC-F10 / RELAY-F1 | guard (fixed 2.2.0) | Drives `deploy/box-a/strfry/deepmarks.js` as a real subprocess over the strfry plugin line protocol. Private-item delete tombstones (`d=deepmarks-private-item:<hash>`) share the private-state rate budget like the chunks they accompany (in the small general bucket, bulk deletes lost tombstones and deleted bookmarks resurrected). Baselines pin the private-state scope for private/archive-key chunk rewrites and the general limit for kind:39701. |
