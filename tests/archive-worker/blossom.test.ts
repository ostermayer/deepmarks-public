...
import { BlossomClient } from '@src/blossom.js';
...
describe('BlossomClient.upload', () => {
  it('hashes the blob with SHA-256 for content-addressed storage', async () => {
...
describe('BlossomClient.mirror', () => {
  ...
  it('falls back to direct upload when a mirror endpoint is unsupported', async () => {
    fetchMock
    ...
    const results = await client.mirror(hash, ['https://mirror.example'], blob, 'text/html');

    expect(results).toEqual([{ url: 'https://mirror.example', ok: true }]);
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(fetchMock.mock.calls[0]![0]).toBe('https://mirror.example/mirror');
    expect(fetchMock.mock.calls[1]![0]).toBe('https://mirror.example/upload');
    expect((fetchMock.mock.calls[1]![1] as RequestInit).method).toBe('PUT');
  });

  it('includes both mirror and direct-upload errors when both paths fail', async () => {
    fetchMock
    ...
    const blob = Buffer.from('payload');
    const hash = bytesToHex(sha256(blob));
    const results = await client.mirror(hash, ['https://mirror.example'], blob);

    expect(results[0]).toMatchObject({ url: 'https://mirror.example', ok: false });
    expect(results[0]?.error).toContain('mirror HTTP 401');
    expect(results[0]?.error).toContain('direct upload failed');
  });
});

describe('BlossomClient.deleteFrom', () => {
  it('DELETEs /<blobHash> with delete auth bound to the target server', async () => {
    fetchMock.mockResolvedValue(new Response('', { status: 200 }));
    const client = new BlossomClient('https://blossom.deepmarks.org', PRIVKEY, PUBKEY);
    const result = await client.deleteFrom('https://mirror.example/', 'deadbeef');

    expect(result).toEqual({ url: 'https://mirror.example', ok: true, status: 200 });
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe('https://mirror.example/deadbeef');
    expect((init as RequestInit).method).toBe('DELETE');
    const auth = decodeAuth(((init as RequestInit).headers as Record<string, string>).Authorization);
    expect(auth.tags.find((t) => t[0] === 't')?.[1]).toBe('delete');
    expect(auth.tags.find((t) => t[0] === 'x')?.[1]).toBe('deadbeef');
    expect(auth.tags.find((t) => t[0] === 'server')?.[1]).toBe('https://mirror.example');
  });

  it('treats already-missing blobs as deleted', async () => {
    fetchMock.mockResolvedValue(new Response('', { status: 404 }));
    const client = new BlossomClient('https://blossom.deepmarks.org', PRIVKEY, PUBKEY);
    await expect(client.deleteFrom('https://mirror.example', 'deadbeef')).resolves.toEqual({
  ...
  it('returns a failed result instead of throwing on delete rejection', async () => {
