...
import { tryResolvePodcastEpisodeArchive } from '@src/podcast.js';

vi.mock('@src/safe-url.js', () => ({
  assertSafePublicHttpUrl: vi.fn(async (raw: string) => new URL(raw)),
  // The real safeFetch resolves + pins the IP; in tests it simply delegates
  // to the stubbed global fetch so podcast-resolution logic is exercised
  // without real DNS.
