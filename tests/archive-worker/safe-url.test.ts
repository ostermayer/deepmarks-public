...
describe('safeFetch — SSRF resolve guard', () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  });

  it('resolves + private-checks a hostname, then fetches it', async () => {
    // Hostname URL: dns.lookup returns a single public IP. safeFetch must
    // resolve + validate the host is public, then fetch. (Connect-level IP
    // pinning against DNS rebinding is a tracked follow-up; the fetch here
    // is the plain global fetch — no dispatcher.)
    vi
      .spyOn(dns, 'lookup')
  ...
  it('resolves DNS once inside safeFetch (no redundant double lookup)', async () => {
    // dns.lookup is called once inside safeFetch (resolve + validate). The
    // guarded fetch that follows is the plain global fetch; callers walk
    // redirects manually and re-validate each hop.
    let lookupCalls = 0;
    vi
      .spyOn(dns, 'lookup')
