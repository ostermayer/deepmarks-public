...
describe('direct PDF archives', () => {
  ...
  it('downloads a PDF file directly and preserves a safe filename', async () => {
    ...
    const archive = await tryDownloadDirectFileArchive('https://93.184.216.34/files/report.pdf');
  ...
  it('validates redirect targets before fetching the next hop', async () => {
    ...
    await expect(tryDownloadDirectFileArchive('https://93.184.216.34/start.pdf')).rejects.toThrow('unsafe url');
  });
  ...
  it('tries direct PDF fallback for renderer PDF/download failures', () => {
    ...
    expect(shouldAttemptDirectFileArchive('https://example.com/anything', new Error('Download is starting'))).toBe(true);
  });
  ...
  it('honors a caller-specific max byte limit for media add-on downloads', async () => {
    const bytes = Buffer.alloc(1024 * 1024 + 1, 7);
    ...
    await expect(
      tryDownloadDirectFileArchive('https://93.184.216.34/video/movie.mp4', { maxBytes: 1024 * 1024 }),
    ).rejects.toThrow(/exceeds 1048576/);

    const archive = await tryDownloadDirectFileArchive('https://93.184.216.34/video/movie.mp4', { maxBytes: 2 * 1024 * 1024 });
    expect(archive?.bytes.byteLength).toBe(bytes.byteLength);
    expect(archive?.contentType).toBe('video/mp4');
  });

  it('can write direct media to disk for large archive processing', async () => {
    const bytes = Buffer.from('direct media bytes');
    vi.stubGlobal('fetch', vi.fn(async () => new Response(bytes, {
      status: 200,
      headers: {
        'content-type': 'video/mp4',
    ...
    const archive = await tryDownloadDirectFileArchiveToFile('https://93.184.216.34/video/clip.mp4', {
      maxBytes: 1024 * 1024,
    });

    expect(archive?.contentType).toBe('video/mp4');
    expect(archive?.byteLength).toBe(bytes.byteLength);
    expect(archive?.fileName).toBe('clip.mp4');
    expect(Buffer.from(await readFile(archive!.filePath)).equals(bytes)).toBe(true);
