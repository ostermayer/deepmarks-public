...
function decryptChunked(layout: Buffer, base64Key: string): Buffer {
  expect(layout.subarray(0, CHUNKED_BLOB_MAGIC.length).equals(CHUNKED_BLOB_MAGIC)).toBe(true);
  const key = Buffer.from(base64Key, 'base64');
  const chunkBytes = layout.readUInt32BE(CHUNKED_BLOB_MAGIC.length);
  const frameBytes = NONCE_BYTES + chunkBytes + TAG_BYTES;
  const out: Buffer[] = [];
  let offset = CHUNKED_BLOB_MAGIC.length + 4;
  let index = 0;
  while (offset < layout.byteLength) {
    const nextOffset = Math.min(offset + frameBytes, layout.byteLength);
    const frame = layout.subarray(offset, nextOffset);
    const nonce = frame.subarray(0, NONCE_BYTES);
    const tag = frame.subarray(frame.byteLength - TAG_BYTES);
    const ciphertext = frame.subarray(NONCE_BYTES, frame.byteLength - TAG_BYTES);
    const dec = createDecipheriv('aes-256-gcm', key, nonce);
    ...
    aad.writeUInt8(nextOffset >= layout.byteLength ? 1 : 0, CHUNKED_BLOB_MAGIC.length + 4);
    dec.setAAD(aad);
    dec.setAuthTag(tag);
    out.push(Buffer.concat([dec.update(ciphertext), dec.final()]));
    offset = nextOffset;
    index += 1;
  }
...
describe('encryptFileChunked', () => {
  it('round-trips file-backed media without loading the final format as plaintext', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'deepmarks-crypto-test-'));
    try {
      const input = join(dir, 'media.bin');
      const output = join(dir, 'media.dmchunk');
      const key = genKeyB64();
      const plaintext = randomBytes(1024 * 1024 + 123);
      await writeFile(input, plaintext);

      const result = await encryptFileChunked(input, output, key, 64 * 1024);
      const ciphertext = await readFile(output);

      expect(result.plaintextBytes).toBe(plaintext.byteLength);
      expect(result.encryptedBytes).toBe(ciphertext.byteLength);
      expect(decryptChunked(ciphertext, key).equals(plaintext)).toBe(true);
    } finally {
