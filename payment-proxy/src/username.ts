...
export class UsernameStore {
  ...
  /**
   * Release the pubkey's current handle. Normal user-initiated releases
   * enter the 30-day cooldown; full account deletion for a lifetime
   * member can skip cooldown so the name is immediately claimable again.
   */
  async release(
    pubkey: string,
    ...
    const cooldown = opts.cooldown ?? true;
    const pipeline = this.redis.multi();
