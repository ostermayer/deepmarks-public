...
export interface PinboardEntry {
  ...
  description?: string;
  tags: string[];
  /** Pinboard's public popularity count, when the page exposes it. */
  count?: number;
}
...
export function parsePinboardPage(html: string): PinboardEntry[] {
  ...
  $('div.bookmark').each((_, el) => {
    ...
    const countText = $bm.find('a.bookmark_count').first().text().trim();
    const count = /^\d+$/.test(countText) ? Number.parseInt(countText, 10) : undefined;
    const tags: string[] = [];
...
/** De-dupe a flat list by canonical URL, preferring the entry with the most Pinboard signal. */
export function dedupe(entries: PinboardEntry[]): PinboardEntry[] {
  ...
  for (const e of entries) {
    ...
    const score = (x: PinboardEntry) =>
      (x.count ?? 0) * 1000 + x.tags.length + (x.description ? 1 : 0);
