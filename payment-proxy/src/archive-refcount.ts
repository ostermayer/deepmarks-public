...
// two users archive the same URL — or the same YouTube video — the
...
// Model:
//
//   dm:archive-refs:<contentKey>  (Redis SET, members = pubkey)
//
// `contentKey` is the dedup unit. For webpage archives it's the blob
// hash itself. For YouTube videos it's "yt:<videoId>" so the same
// video URL in different forms (youtu.be/X, watch?v=X, embed/X)
// collapses to one ref set.
//
// Invariants:
...
/** Build the Redis key for a content-key. Public so callers don't
 *  hard-code the prefix and drift from this module. */
export function refsKeyFor(contentKey: string): string {
  return `${REFS_PREFIX}${contentKey}`;
}

/** Record that `pubkey` references `contentKey`. Returns the new
...
export async function addArchiveRef(
  ...
  const key = refsKeyFor(contentKey);
...
export async function getArchiveRefCount(
  ...
  return redis.scard(refsKeyFor(contentKey));
}
...
export async function isLastReference(
  ...
  const members = await redis.smembers(refsKeyFor(contentKey));
