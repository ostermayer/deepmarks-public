...
export function makeRequireNip98(nip98: Nip98Fn, redis: Redis): RequireNip98Fn {
  return async function requireNip98(
    ...
    // is by definition a Deepmarks user — register them so their
    // events can flow through the relay's writePolicy. Cheap no-op
    // when already in the set (positive cache fast-path).
    void registerPubkey(redis, result.pubkey);
...
export function makeRequireAdmin(opts: {
  ...
  return async function requireAdmin(
    ...
    // via the admin pubkey.
    void registerPubkey(opts.redis, check.pubkey);
