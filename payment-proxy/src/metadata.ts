...
async function fetchOembedMetadata(url: string, html: string): Promise<UrlMetadata | null> {
  const endpoint = extractOembedEndpoint(url, html);
  if (!endpoint || !parseAllowedUrl(endpoint)) return null;
  try {
    const res = await fetch(endpoint, {
...
export class MetadataStore {
  ...
  async resolve(rawUrl: string): Promise<UrlMetadata | null> {
    ...
    const meta = fetched.kind === 'media'
      ? fetched.meta
      : mergeMetadata(extractMetadata(url, fetched.html), await fetchOembedMetadata(url, fetched.html));
