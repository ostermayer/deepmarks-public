...
describe('UsernameStore.release + cooldown', () => {
  ...
  it('can release without cooldown for lifetime account deletion', async () => {
    const { store } = makeStore();
    await store.claim(PK_A, 'alice', true);
    const out = await store.release(PK_A, { cooldown: false });
    expect(out).toEqual({ released: 'alice' });
    expect(await store.lookup('alice')).toBeNull();
    expect(await store.claim(PK_B, 'alice', true)).toEqual({ ok: true, name: 'alice' });
  });
