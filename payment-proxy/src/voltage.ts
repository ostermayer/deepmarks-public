...
/** YouTube video archive — per-video purchase. Pricier than a webpage
 *  archive because yt-dlp downloads, optional ffmpeg re-mux, and the
 *  resulting blob (a video file capped at 720p) is materially larger
 *  in object storage than an HTML snapshot. */
export const YOUTUBE_ARCHIVE_COST_SATS = 150_000;
...
/**
 * Legacy metered archive invoice helper. New archive routes are lifetime-only.
 */
...
/**
 * Per-video YouTube-archive invoice. Same shape as createArchiveInvoice
 * but with the 150k-sat price tag and a description that makes the
 * purchase identifiable in the user's Lightning wallet history.
 */
export async function createYoutubeArchiveInvoice(
  lnd: AuthenticatedLnd,
  videoLabel: string,
): Promise<{ paymentHash: string; invoice: string; expiresAt: Date }> {
  const shortLabel = videoLabel.length > 80 ? `${videoLabel.slice(0, 77)}…` : videoLabel;
  const description = `deepmarks youtube archive: ${shortLabel}`;
  const expiresAt = new Date(Date.now() + INVOICE_EXPIRY_SECONDS * 1000);
  const result = await createInvoice({
    lnd,
    tokens: YOUTUBE_ARCHIVE_COST_SATS,
    description,
    expires_at: expiresAt.toISOString(),
  });
  return { paymentHash: result.id, invoice: result.request, expiresAt };
}
