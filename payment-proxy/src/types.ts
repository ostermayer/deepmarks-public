...
export interface PurchaseRecord {
  ...
  mirrorUrls?: string[];
  /** Job category. 'webpage' (default) or 'youtube'. The invoice
   *  settlement handler reads this to enqueue the right job shape. */
  kind?: 'webpage' | 'youtube';
  /** YouTube 11-char video id when kind='youtube'. */
  videoId?: string;
}
...
export interface ArchiveJob {
   ...
   *  archive path. 'youtube' triggers the worker's yt-dlp branch, which
   *  downloads the video at ≤720p, encrypts client-side with AES-256-GCM
   *  (always private regardless of caller tier), and uploads to the
   *  ciphertext bucket rather than the public Blossom server. */
  kind?: 'webpage' | 'youtube';
  /** YouTube video ID (11-char base64-URL alphabet) when kind='youtube'.
   *  The worker's dedup pivot is `yt:<videoId>` so the same video
   *  pasted as youtu.be/X, watch?v=X, and embed/X all collapse to one
   *  refcount bucket. */
  videoId?: string;
}
