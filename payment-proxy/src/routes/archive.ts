...
export function register(deps: Deps): void {
  ...
  app.post<{ Body: unknown }>(
    ...
    async (request, reply) => {
      ...
      const gate = await rateLimit('archive-browser-capture-pk', pubkey, 500, 24 * 60 * 60);
      if (!gate.ok) {
        reply.header('Retry-After', String(gate.retryAfter));
        return reply.status(429).send({ error: 'rate limit (browser capture)', retryAfter: gate.retryAfter });
      }
