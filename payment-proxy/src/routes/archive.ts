...
const ArchiveCallbackSchema = z.object({
  ...
  // YouTube-archive extras. Worker sets these when kind='youtube' so
  // the user's archive list can render "Title — Channel" without an
  // extra round-trip to youtube.com from the frontend. videoId echoes
  // the canonical 11-char id back to the proxy so refcount keying
  // doesn't have to re-parse the URL.
  kind: z.enum(['webpage', 'youtube']).optional(),
  ...
  app.post('/archive/callback', async (request, reply) => {
    ...
    if (payload.status === 'ok') {
      ...
      const archiveRecord = {
        ...
        // YouTube archives carry a title + channel so the user's
      ...
      // Refcount the content so a future delete from this user only
      // tears down primary storage when no other user still references
      // the same bytes. For YouTube the dedup pivot is the canonical
      // video id (so youtu.be/X and watch?v=X collapse to one
      // bucket); for webpage archives it's the blob hash.
      const contentKey = payload.kind === 'youtube' && payload.videoId
        ? `yt:${payload.videoId.toLowerCase()}`
        : payload.blobHash;
      await addArchiveRef(redis, contentKey, payload.ownerPubkey);
      // Stash the resolved metadata under the dedup key so a future
      // buyer of the same video can be served the existing blob
      // without a fresh yt-dlp run.
      if (payload.kind === 'youtube' && payload.videoId) {
        await redis.set(
          `dm:archive-meta:yt:${payload.videoId.toLowerCase()}`,
          JSON.stringify({
            blobHash: payload.blobHash,
            videoTitle: payload.videoTitle,
            videoChannel: payload.videoChannel,
            videoDurationSeconds: payload.videoDurationSeconds,
            firstArchivedAt: Math.floor(Date.now() / 1000),
          }),
        ).catch(() => undefined);
      }
