...
async function archiveAuditSummary(redis: Deps['redis']): Promise<ArchiveAuditSummary | null> {
  try {
    const raw = await redis.get('dm:archive-audit:last');
    if (!raw) return null;
...
export function register(deps: Deps, workers: Workers): void {
  ...
  app.get('/admin/dashboard', async (request, reply) => {
    ...
    const [
      ...
      archiveStatus, eventCounts, registered, usernameClaims, watched, queues, alerts,
      lifetimeMembers, mediaArchiveMembers, archiveAudit,
    ] = await Promise.all([
      pingRedis(redis),
      pingMeili(meili),
