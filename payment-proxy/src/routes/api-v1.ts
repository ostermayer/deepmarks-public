...
import { compareArchiveRecordsNewest, parseArchiveRecord } from '../archive-lifecycle.js';
import {
  SignedEventSchema,
  bookmarkEventToJson,
  publishToRelays,
    ...
    async (request, reply) => {
      ...
      if (doneRaw) {
        try {
          ...
          if (done.status === 'ok') {
            return { jobId: request.params.jobId, state: 'done', blobHash: done.blobHash };
          }
          if (done.status === 'failed') {
            return { jobId: request.params.jobId, state: 'failed', error: done.error ?? 'archive job failed' };
          }
        } catch {
