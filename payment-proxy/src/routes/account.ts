...
export function register(deps: Deps): void {
  ...
  app.get('/account/archive-queue', async (request, reply) => {
    ...
    return { pending, running, archivedTotal };
  });

  // ── GET/PUT /account/settings — cross-client app preferences ───────
  // This is intentionally Deepmarks-server coordinated rather than
  // relay-coordinated: settings need predictable last-write-wins sync
  // across web, mobile, and extensions. The document excludes secrets
  // and signer grants; NWC and "always allow this site" permissions stay
  // device-local.
  ...
  app.delete<{ Params: { blobHash: string } }>(
    ...
    async (request, reply) => {
      ...
      // Peek at the raw record for fields we don't normally parse —
      // the YouTube videoId determines the refcount key so the same
      // dedup bucket the worker callback added to is the one we
      // decrement now.
      let videoIdRaw: string | undefined;
      let kindRaw: string | undefined;
      try {
        const raw = JSON.parse(entry) as { videoId?: string; kind?: string };
        videoIdRaw = typeof raw.videoId === 'string' ? raw.videoId : undefined;
        kindRaw = typeof raw.kind === 'string' ? raw.kind : undefined;
      } catch { /* legacy record without these fields — treat as webpage */ }

      // Step 1 — drop from the user's archive list.
      await deps.redis.hdel(`dm:archives:${auth.pubkey}`, blobHash);

      // Step 2 — decrement the cross-user refcount on this content.
      // For YouTube the pivot is the canonical video id (so the same
      // video saved via different URL forms collapses to one bucket).
      // For webpage archives it's the blob hash itself.
      const contentKey = kindRaw === 'youtube' && videoIdRaw
        ? `yt:${videoIdRaw.toLowerCase()}`
        : blobHash;
      const remaining = await removeArchiveRef(deps.redis, contentKey, auth.pubkey);
      ...
      if (remaining === 0) {
        primary = await deletePrimaryArchiveBlobs(deps.blossomBlobStore, parsed);
        ...
        mirrorDeleteQueued = await enqueueArchiveMirrorDelete(
          deps.redis,
          parsed,
          'archive-delete',
        );
...
async function countArchiveProcessingOwner(
  ...
  do {
    ...
    for (const key of keys) {
      total += countOwnerInArchiveJobJson(await redis.lrange(key, 0, -1), pubkey);
    }
  } while (cursor !== '0');
  return total;
}

function countOwnerInArchiveJobJson(items: string[], pubkey: string): number {
  let count = 0;
  for (const raw of items) {
    try {
      const job = JSON.parse(raw) as { ownerPubkey?: unknown };
      if (job.ownerPubkey === pubkey) count += 1;
    } catch {
