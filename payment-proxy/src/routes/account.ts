...
export function register(deps: Deps): void {
  ...
  app.delete<{ Params: { blobHash: string } }>(
    ...
    async (request, reply) => {
      ...
      // Step 2 — best-effort S3 teardown of our primary copy, including
      // any public screenshot thumbnail.
      const primary = await deletePrimaryArchiveBlobs(deps.blossomBlobStore, parsed);
      for (const err of primary.errors) {
        app.log.warn({ blobHash, err }, 'archive primary delete warning');
      }
      // Step 3 — known mirror teardown via the archive-worker, which owns
      // the Blossom signing key that uploaded/mirrored the bytes.
      const mirrorDeleteQueued = await enqueueArchiveMirrorDelete(
        deps.redis,
        parsed,
        'archive-delete',
      );
