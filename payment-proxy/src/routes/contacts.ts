...
export function register(deps: Deps): void {
  ...
  app.get<{ Querystring: { q?: string; limit?: string } }>('/account/people-search', async (request, reply) => {
    ...
    const gate = await rateLimit('people-search', auth.pubkey, 60, 60);
    if (!gate.ok) {
      reply.header('Retry-After', String(gate.retryAfter));
      return reply.status(429).send({ error: 'rate limit', retryAfter: gate.retryAfter });
    }
...
async function exactRegisteredCandidate(redis: Deps['redis'], pubkey: string): Promise<string[]> {
  const exists = await redis.exists(`dm:pk:${pubkey}`).catch(() => 0);
  return exists ? [pubkey] : [];
}
...
async function scanRegisteredPubkeys(redis: Deps['redis']): Promise<string[]> {
  ...
  let cursor = '0';
  do {
    const [next, keys] = await redis.scan(cursor, 'MATCH', 'dm:pk:*', 'COUNT', 500);
    cursor = next;
    for (const key of keys) {
      const pubkey = key.slice('dm:pk:'.length).toLowerCase();
