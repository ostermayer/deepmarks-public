...
const ACCEPTED_KINDS = new Set([0, 1, 3, 5, 10000, 10002, 10003, 30000, 30003, 39701, 9735, 24133]);
...
export function register(deps: Deps): void {
  ...
  app.post('/publish', async (request, reply) => {
    ...
    let gate: Awaited<ReturnType<typeof rateLimit>>;
    try {
      gate = await rateLimit('publish-pubkey', auth.pubkey, 200, 60);
    } catch (err) {
...
async function warmFriendCacheForAcceptedEvents(
  ...
  for (const event of events) {
    ...
    if (event.kind === 39701 || event.kind === 10003 || event.kind === 30003) {
      pubkeys.add(event.pubkey);
    }
  }
...
async function queueDefaultArchivesForPublishEvents(
  ...
  const archiveEvents = events.filter((event) => event.kind === 39701 || event.kind === 10003 || event.kind === 30003);
...
function archiveCandidates(events: z.infer<typeof SignedEventSchema>[]): Array<{
  ...
  for (const event of events) {
    ...
    if (event.kind !== 10003 && event.kind !== 30003) continue;
...
function nostrNoteArchiveUrl(eventId: string): string | null {
