...
const PUBLISH_RELAY_ATTEMPT_TTL_SECONDS = 60 * 60;
...
export class RelayFanoutWorker {
  ...
  async start(): Promise<void> {
    ...
    this.sub = this.pool.subscribeMany(
      [this.deps.relayUrl],
      { kinds: [0, 3, 5, 10000, 10002, 10003, 30003, 39701], since },
      {
    ...
    // Drain the server-mediated publish queue — every signed event
  ...
  private async handleEvent(event: NostrEvent): Promise<void> {
    ...
    // (our private + archive-keys chunks) and any kind:30003 that
  ...
  private async requeuePublishRelayEvent(event: NostrEvent, err: unknown): Promise<void> {
    ...
    try {
      ...
      await this.deps.redis.multi()
        .lpush(PUBLISH_RELAY_QUEUE, JSON.stringify(event))
        .ltrim(PUBLISH_RELAY_QUEUE, 0, PUBLISH_RELAY_QUEUE_CAP - 1)
        .exec();
    } catch (redisErr) {
      this.deps.logger.error(
