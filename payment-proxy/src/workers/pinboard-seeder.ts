// Runs the Pinboard daily importer in the background on a fixed cadence
// (production: every 24 hours). seedOnce() chooses one fresh popular
// Pinboard bookmark; this worker makes sure restarts do not publish more
// than once per UTC day.
//
...
export interface PinboardSeederDeps {
  pool: SimplePool;
  logger: SeedLogger;
  /** Public-profile signer — publishes kind 39701 + kind 1 daily posts. */
  signer: RemoteSigner;
  /** Optional daily idempotency guard. Return ok=false to skip this run. */
  claimDailyRun?: () => Promise<{ ok: boolean; release?: () => Promise<void> }>;
...
export class PinboardSeederWorker {
  ...
  async start(): Promise<void> {
    if (!this.enabled) {
      this.deps.logger.info('pinboard daily importer disabled (SEED_PINBOARD_INTERVAL_HOURS unset or 0)');
      return;
    }
    this.stopped = false;
  ...
  private async runOnce(): Promise<void> {
    ...
    this.running = true;
    let dailyClaim: { ok: boolean; release?: () => Promise<void> } | null = null;
    try {
      if (this.deps.claimDailyRun) {
        dailyClaim = await this.deps.claimDailyRun();
        if (!dailyClaim.ok) {
          this.deps.logger.info('pinboard daily importer already ran today; skipping');
          return;
        }
      }
      ...
      this.deps.logger.info(
        `pinboard daily importer: +${result.ok} bookmark, +${result.socialOk} note (${result.failed + result.socialFailed} failed, ${result.alreadyPublished} already present)`,
      );
    } catch (e) {
      if (dailyClaim?.release) await dailyClaim.release().catch(() => undefined);
      this.deps.logger.error(`pinboard daily importer threw: ${(e as Error).message}`);
    } finally {
      this.running = false;
      this.scheduleNext();
    }
