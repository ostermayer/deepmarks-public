...
const BOOKMARKED_NOTE_TARGET_TTL_S = 30 * 24 * 60 * 60;
...
const WATCHED_CONTENT_KINDS = [39701, 30000, 30003, 10003] as const;
...
export class FollowsIngesterWorker {
  ...
  private async fetchBookmarkedNoteTargets(listEvents: NostrEvent[], curatorRelays: string[]): Promise<NostrEvent[]> {
    const targets = this.bookmarkedNoteTargetIds(listEvents);
    ...
    const batchSize = 50;
    const ids = targets.ids.slice(0, MAX_NOTE_TARGETS_PER_CURATOR);
    ...
    for (const event of events) {
      ...
      if (!targets.idSet.has(event.id.toLowerCase())) continue;
      byId.set(event.id.toLowerCase(), event);
    }
    return Array.from(byId.values());
  }

  private bookmarkedNoteTargetIds(events: NostrEvent[]): { ids: string[]; idSet: Set<string>; relays: string[] } {
