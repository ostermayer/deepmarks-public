...
// 30003 / 10003 events into our local strfry.
//
...
//
// Resource caps:
//   - Max relays open at once (shared SimplePool reuses connections)
...
/** How often we re-fetch each registered user's kind:3 + kind:0 from
...
const REGISTERED_USER_SYNC_INTERVAL_MS = 6 * 60 * 60 * 1000;
const REGISTERED_SYNC_LAST_PREFIX = 'dm:contacts:sync:last:';
const REGISTERED_USERS_SET = 'dm:registered:pubkeys';
/** Don't try to sync every registered user every cycle — cap so the
 *  worker stays responsive even if the registered set grows large. */
const MAX_REGISTERED_SYNCS_PER_CYCLE = 100;
...
export class FollowsIngesterWorker {
  ...
  async start(): Promise<void> {
    ...
    // Watch kind:3 contact lists from any registered user. The
    // writePolicy gate means strfry only persists kind:3 from
    // registered pubkeys, so anything we see here is something we
    // want to act on.
  ...
  private async runBatch(): Promise<void> {
    ...
    try {
      ...
      // Second pass: refresh each registered user's own kind:3
      ...
      await this.syncRegisteredUserContacts().catch((err) =>
        this.deps.logger.warn({ err }, 'follows-ingester syncRegisteredUserContacts failed'),
      );
    } catch (err) {
  ...
  /** Refresh kind:3 + kind:0 for registered users from their NIP-65
   *  relays. Skips users whose last sync was within
   *  REGISTERED_USER_SYNC_INTERVAL_MS. Caps total per cycle so a
   *  growing registered set doesn't extend the batch indefinitely. */
  private async syncRegisteredUserContacts(): Promise<void> {
    const all = await this.deps.redis.smembers(REGISTERED_USERS_SET).catch(() => [] as string[]);
    if (all.length === 0) return;
    const now = Date.now();
    const pubkeys = all.filter((pk) => /^[0-9a-f]{64}$/.test(pk));
    const pipeline = this.deps.redis.pipeline();
    for (const pk of pubkeys) pipeline.get(REGISTERED_SYNC_LAST_PREFIX + pk);
    ...
    const due = stamped
      .filter((s) => now - s.lastSync >= REGISTERED_USER_SYNC_INTERVAL_MS)
      .sort((a, b) => a.lastSync - b.lastSync)
      .slice(0, MAX_REGISTERED_SYNCS_PER_CYCLE);

    if (due.length === 0) return;
    this.deps.logger.info({ count: due.length }, 'follows-ingester: syncing registered-user kind:3 + kind:0');
    for (const { pubkey } of due) {
      if (this.stopping) break;
      try {
        await this.syncOneRegisteredUser(pubkey);
      } catch (err) {
        this.deps.logger.warn({ err, pubkey }, 'follows-ingester syncOneRegisteredUser failed');
      }
    }
  }

  private async syncOneRegisteredUser(pubkey: string): Promise<void> {
    ...
    if (relays.length === 0) {
      ...
      await this.deps.redis
        .set(REGISTERED_SYNC_LAST_PREFIX + pubkey, String(Date.now()))
        .catch(() => undefined);
      return;
    }
  ...
  private async curatorRelays(pubkey: string): Promise<string[]> {
    ...
    try {
      // Try our own relay first — if the user is registered + has
