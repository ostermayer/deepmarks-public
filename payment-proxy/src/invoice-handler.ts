      ...
      try {
        ...
        await purchases.rollbackToPending(inv.id).catch((rollbackErr) => {
          ...
          void alerter.alert({
            ...
            body: `User ${rec.userPubkey} has a legacy metered archive invoice at paymentHash ${inv.id}. Both the queue rpush AND the rollback to 'pending' failed. Their record is stuck in 'paid' state with no archive job. Reconcile manually:\n\n1. Verify the payment with LND (lncli lookupinvoice ${inv.id})\n2. Either issue a manual archive job or refund\n\nRollback error: ${(rollbackErr as Error).message ?? rollbackErr}`,
          });
        });
      }
    } catch (err) {
