...
1. **Lifetime archives** — verify lifetime membership/API-key access, enqueue archive jobs for Box B, and record Box B completion callbacks.
...
Only zap/lifetime-upgrade payment flows terminate at the Voltage-hosted Lightning node. We use the `invoice`-only macaroon — if this service is compromised, the attacker cannot move funds.
...
├── archive-purchase.ts  Lifetime archive enqueue + legacy metered invoice helper
└── workers/
...
Every module has `*.test.ts` tests except `email.ts` and `queue.ts` (stubs for services that are exercised end-to-end against Redis + Blossom in integration — unit tests against those depend on real external state).

## Endpoints

| Method | Path | Purpose | Auth |
| --- | --- | --- | --- |
| `POST` | `/archive/purchase` | Legacy metered-archive guard; returns lifetime-required | none |
| `POST` | `/archive/lifetime` | Enqueue archive for lifetime member | NIP-98 |
| `POST` | `/archive/callback` | Box B archive completion callback | HMAC headers `X-Worker-Timestamp` + `X-Worker-Signature` |
| `GET`  | `/archive/status/:hash` | Poll archive job state | none |
...
| `GET`  | `/account/archives` | List signed-in user's completed archives | NIP-98 |
| `GET`  | `/account/archive-queue` | Count signed-in user's pending/running archive jobs | NIP-98 |
| `GET`/`PUT` | `/account/settings` | Sync non-secret app preferences | NIP-98 |
...
| `POST` | `/api/v1/archives` | Enqueue lifetime archive job | Bearer `dmk_live_…` |
...
## Lifetime archive flow

```
Lifetime user checks "archive when saved"
  ↓
Frontend/extension signs NIP-98 → POST /archive/lifetime {url, eventId?}
  ↓
proxy verifies lifetime membership + safe URL
  ↓
proxy → RPUSH dm:archive:queue {jobId, paymentHash, ownerPubkey, url, tier, archiveKey?}
  ↓
Box B renders page, uploads to primary Blossom, fans out to mirror Blossom servers
  ↓
Box B → POST /archive/callback, proxy stores archive metadata and mirror status
  ↓
Frontend/extension refreshes GET /account/archives and /account/archive-queue
```

### Example

For API keys, lifetime membership is already enforced when the key is created:
...
# → { "jobId": "...", "paymentHash": "...", "amountSats": 0 }

curl -H 'authorization: Bearer dmk_live_...' \
  https://deepmarks.org/api/v1/archives/<jobId>
# → { "jobId": "...", "state": "done", "status": "archived", ... }
```
...
4. Session is now 'email' tier — can decrypt private bookmarks, cannot
   sign. If the Deepmarks browser extension is unlocked on deepmarks.org,
   the first-party signer path can attach automatically; otherwise write
   actions still ask for a signer.
5. Connecting a signer upgrades to 'full' tier on the client side. No
...
└─────┬──────────────────┬────┘    └──────────┬──────────────────┘
      │ POST             │                    │ GET /.well-known/
      │ /archive/lifetime│                    │   lnurlp/zap
      │ /api/v1/archives │                    │ GET /lnurlp/zap/callback
      ▼                  ▼                    ▼
┌────────────────────────────────────────────────────┐
...
└──────┬────────────────────────────────┬────────────┘
       │ createInvoice /                │ RPUSH archive job
...
- **Replay of invoice_updated events.** The zap-receipt handler uses `zaps.consume(hash)` which atomically deletes the Redis key, so duplicate events can't double-publish receipts. Legacy metered archive invoices still use a status state machine that no-ops if already `enqueued`.
- **Invoices on the node that aren't ours** (e.g. manual test invoices from the Voltage dashboard). Ignored — if the payment hash isn't in the zap store or legacy archive store, we do nothing.
