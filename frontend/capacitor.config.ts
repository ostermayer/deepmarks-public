...
const config: CapacitorConfig = {
  ...
  ios: {
    scheme: 'Deepmarks',
    // 'never' so WKWebView never auto-applies safe-area insets. With
    // 'always', iOS applied the inset on scrollable routes but not on
    // short non-scrollable ones, so `bottom: 0` anchored to two
    // different places (above vs through the home indicator) and the
    // tab bar appeared to resize per route. We handle safe areas
    // ourselves via env(safe-area-inset-*) instead.
    contentInset: 'never',
  },
  android: {
