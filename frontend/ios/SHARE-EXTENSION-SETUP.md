...
`https://api.deepmarks.org/publish`. It also writes the edited payload to
the shared App Group queue with `published: "1"`, so the host app can
update the local bookmark list later without posting the event twice.

For private saves, missing local keys, or a native publish failure, the
extension keeps the edited payload in the App Group queue and opens the
host app at `deepmarks://save?pendingShareId=…`. The host app's
`appUrlOpen` listener routes that to `/app/save`, the SvelteKit form
loads the queued payload, and the app signs/publishes it when the local
signer is ready. If iOS does not launch the host app immediately, the
queued request is picked up the next time Deepmarks opens.
...
For public saves with a shared nsec available, `NostrEvent.swift` and
...
inside the extension. The extension then marks the queued payload as
already published and dismisses.
