...
class ShareViewController: SLComposeServiceViewController {
        ...
        for attachment in attachments {
            if attachment.hasItemConformingToTypeIdentifier(textType) {
                attachment.loadItem(forTypeIdentifier: textType, options: nil) { (data, _) in
                    ...
                    let firstUrl = text.split(whereSeparator: { $0.isWhitespace })
    ...
    private func openInHostApp(sharedUrl: URL) {
        let encoded = sharedUrl.absoluteString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
        guard let appUrl = URL(string: "deepmarks://save?url=\(encoded)") else { return }
