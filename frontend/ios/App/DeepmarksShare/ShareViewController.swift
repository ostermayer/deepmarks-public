...
class ShareViewController: UIViewController, UITextViewDelegate {
    ...
    /// fast path. Public bookmarks can be posted directly from the
    /// extension; private bookmarks need NIP-44 chunked-set rewrites
    /// that aren't implemented natively.
    private func shouldNativePublish(_ pendingShare: [String: String]) -> Bool {
        let visibility = pendingShare["visibility"] ?? "default"
        let resolvesPublic = visibility == "public" || (visibility == "default" && defaultVisibility == "public")
        guard resolvesPublic else { return false }
