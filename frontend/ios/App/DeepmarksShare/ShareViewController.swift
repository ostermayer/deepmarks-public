...
class ShareViewController: UIViewController, UITextViewDelegate {
    ...
    /** Page title offered by the sharing app — Safari passes the page title in
     *  the extension item's title/content text. Fills the Title field when
     *  /metadata can't: bot-walled hosts (wsj.com & co.) 401 every datacenter
     *  fetch, so the on-device share payload is the only title source.
     *  (Android reads the equivalent EXTRA_TITLE/EXTRA_SUBJECT already.) */
    private var sharedItemTitle: String?
    ...
    override func viewDidLoad() {
        ...
        extractSharedURL { [weak self] url in
            DispatchQueue.main.async {
                ...
                if let url {
                    self.captureSharedItemTitle(excluding: url)
    ...
    private func fetchMetadata() {
        ...
        // enrich=0 — skip the server's inline LLM round-trip so the share
        // sheet autofills as fast as possible. The saved bookmark still
        // gets LLM-enriched by the backend backfill after it publishes.
        components?.queryItems = [
            URLQueryItem(name: "url", value: raw),
            URLQueryItem(name: "enrich", value: "0"),
        ]
        ...
        URLSession.shared.dataTask(with: metadataUrl) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                // Server couldn't see the page (bot-walled host, offline,
                // non-HTML) — fall back to the title the sharing app gave us.
                if self.trimmed(self.titleView.text).isEmpty,
                   let fallback = self.sharedItemTitle, !fallback.isEmpty {
                    self.titleView.text = fallback
                }
    ...
    @objc private func save() {
        let raw = trimmed(urlView.text)
        guard let inputURL = URL(string: raw), inputURL.scheme == "http" || inputURL.scheme == "https" else {
            showInvalidURL()
            return
        }
    ...
    /** Pull a human page title out of the share payload: first non-URL line
     *  of the items' title/content text, with the shared URL itself stripped.
     *  Mirrors Android's extractTitle (EXTRA_TITLE/EXTRA_SUBJECT + first
     *  non-URL line of the shared text). */
    private func captureSharedItemTitle(excluding url: URL?) {
        let items = extensionContext?.inputItems.compactMap { $0 as? NSExtensionItem } ?? []
        let candidates = items
            .flatMap { [$0.attributedTitle?.string, $0.attributedContentText?.string] }
            .compactMap { $0 }
        for candidate in candidates {
            ...
            for line in text.split(whereSeparator: \.isNewline) {
                ...
                if title.isEmpty || title.hasPrefix("http://") || title.hasPrefix("https://") { continue }
                sharedItemTitle = String(title.prefix(240))
                return
            }
        }
    }
    ...
    private func extractSharedURL(completion: @escaping (URL?) -> Void) {
        ...
        guard !items.isEmpty || !attachments.isEmpty else {
            completion(nil)
            return
        }
        ...
        let urlProviders = attachments.filter { $0.hasItemConformingToTypeIdentifier(urlType) }

        if !urlProviders.isEmpty {
            loadFirstWebURL(from: urlProviders, typeIdentifier: urlType, index: 0) { [weak self] url in
                if let url {
                    completion(url)
                } else {
                    self?.extractTextSharedURL(from: items, attachments: attachments, textTypes: textTypes, completion: completion)
                }
            }
            return
        }

        extractTextSharedURL(from: items, attachments: attachments, textTypes: textTypes, completion: completion)
    }

    private func extractTextSharedURL(
        ...
        let itemText = items
            ...
            .compactMap { $0 }
            .joined(separator: "\n")
        if let url = extractFirstWebURL(from: itemText) {
            completion(url)
            return
        }

        let textProviders = attachments.flatMap { attachment in
            textTypes
                .filter { attachment.hasItemConformingToTypeIdentifier($0) }
                .map { (attachment, $0) }
        }
        loadFirstTextURL(from: textProviders, index: 0, completion: completion)
    }

    private func loadFirstWebURL(
        ...
        providers[index].loadItem(forTypeIdentifier: typeIdentifier, options: nil) { [weak self] data, _ in
            if let url = self?.webURL(from: data) {
                completion(url)
            } else {
                self?.loadFirstWebURL(
                    from: providers,
    ...
    private func loadFirstTextURL(
        from providers: [(NSItemProvider, String)],
        index: Int,
        completion: @escaping (URL?) -> Void
    ) {
        guard index < providers.count else {
        ...
        let (provider, textType) = providers[index]
        provider.loadItem(forTypeIdentifier: textType, options: nil) { [weak self] data, _ in
            ...
            if let url = self?.extractFirstWebURL(from: text) {
                completion(url)
            } else {
                self?.loadFirstTextURL(from: providers, index: index + 1, completion: completion)
            }
        }
    }

    private func webURL(from data: Any?) -> URL? {
        if let url = data as? URL, url.scheme == "http" || url.scheme == "https" {
            return url
        }
        if let text = data as? String {
            return extractFirstWebURL(from: text)
        }
        return nil
    }
    ...
    private func showInvalidURL() {
        let alert = UIAlertController(
            title: "Enter a URL",
            message: "Deepmarks can save http and https links. If this PDF was shared as a local file, paste the web URL into the URL field.",
    ...
    private func extractFirstWebURL(from text: String) -> URL? {
        ...
        // reference with no web URL at all. Wrap it as the canonical
        // social URL the app bookmarks notes under (the JS save path
        // normalizes the same forms — keep behavior identical).
        for token in tokens {
            let stripped = token.lowercased().hasPrefix("nostr:")
                ? String(token.dropFirst("nostr:".count))
                : token
            let lower = stripped.lowercased()
            if (lower.hasPrefix("note1") || lower.hasPrefix("nevent1")),
               lower.count > 12,
               lower.allSatisfy({ "qpzry9x8gf2tvdw0s3jn54khce6mua7l1".contains($0) || $0.isNumber }) {
                return URL(string: "https://primal.net/e/\(lower)")
            }
        }
        return nil
    }
