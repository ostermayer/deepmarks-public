...
class ShareViewController: UIViewController, UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        guard textView === tagsView else { return }
        updateAutoCompleteSuggestions()
    }
    ...
    private func scrollActiveTextViewIntoView() {
        ...
        let rectInScroll = scrollView.convert(textView.bounds, from: textView)
        let padded = rectInScroll.insetBy(dx: 0, dy: -16)
    ...
    @objc private func save() {
        ...
        // Fast path: for explicitly-public shares with a nsec in the
        // shared Keychain, sign + POST /publish RIGHT HERE inside the
        // extension. The kind:39701 event lands on relay.deepmarks.org
    ...
    /// fast path. Explicitly-public visibility only — "default" is
    /// resolved by the JS-side user settings, which the extension
    /// doesn't have visibility into. Private bookmarks need NIP-44
    /// chunked-set rewrites that aren't yet implemented natively.
    private func shouldNativePublish(_ pendingShare: [String: String]) -> Bool {
        guard pendingShare["visibility"] == "public" else { return false }
        guard KeychainSharedStore.loadNsecHex() != nil else { return false }
        return true
    }
