...
class DeepmarksSecureStorePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "writeShareDefaults", returnType: CAPPluginReturnPromise)
    ]
    ...
    @objc func writeShareDefaults(_ call: CAPPluginCall) {
        guard let defaults = UserDefaults(suiteName: appGroupIdentifier) else {
            call.resolve()
            return
        }
        let visibility = call.getString("defaultVisibility") == "public" ? "public" : "private"
        let readLater = call.getBool("defaultReadLater") ?? false
        let tags = call.getArray("defaultTags") as? [String] ?? []
        let activePubkey = normalizePubkey(call.getString("activePubkey") ?? "")
        ...
        defaults.set([
            ...
            "defaultTags": Array(cleanedTags.prefix(40)),
            "activePubkey": activePubkey
        ], forKey: shareDefaultsKey)
        defaults.synchronize()
        call.resolve()
    }
