...
Clients sign user events locally, POST writes through api's
...
│   ├── api/
│   │   ├── client.ts     api HTTP client + account/archive/api.keys schemas
...
| Mobile | Native bottom tab bar for bookmarks, collections, friends, search, and save (tags and other destinations live in the overflow "more" menu) |
...
api. Every response is parsed through a `zod` schema — a backend that drifts
