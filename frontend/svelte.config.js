...
const config = {
  ...
  kit: {
    ...
    // The frontend talks to relays + api directly from the browser.
    adapter: adapter({
      pages: 'build',
      assets: 'build',
      fallback: 'index.html',
    ...
    csp: {
      ...
      directives: {
        'default-src': ["'self'"],
        // 'unsafe-eval' is required by @nostr-dev-kit/ndk-cache-dexie:
        // bakeCollection() builds optimized filter predicates with
        // `new Function(...)` at runtime. Blocking eval makes every
        // event lookup miss the IndexedDB cache and fall through to
        // a live relay query — observed as 'EvalError: Refused to
        // evaluate a string as JavaScript' spam on iOS Capacitor
        // builds with NDK Dexie effectively non-functional. Our
        // threat model doesn't change much here: the eval string is
        // hardcoded NDK source, not user input, and the rest of the
        // CSP still blocks cross-origin script injection.
        ...
        'base-uri':    ["'self'"],
        'form-action': ["'self'"],
        // service worker (src/service-worker.ts) runs from same origin.
        'worker-src':  ["'self'"],
      },
    },
    ...
    // Register manually from +layout so native Capacitor shells do not
    // install a service worker for capacitor://localhost. A stale native
    // SW cache can strand the WebView on app.html's loading shell after
    // an app update if it serves HTML that points at old immutable chunks.
    serviceWorker: {
      register: false,
    },
  },
