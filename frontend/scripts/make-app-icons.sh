...
#
# Keep the pennant inside Android's adaptive-icon safe zone. The full
# foreground canvas is 108dp; launchers may clip outside the central
# 72dp and often visually scale foreground layers compared with the
# legacy square icon. Keep Android's adaptive foreground a little
# smaller than the iOS/legacy master. The SVG's transparent bounds are
# geometrically centered by ImageMagick, but the red pennant mass sits
# up and left inside that box; nudge the foreground so the visible mark
# reads centered in Android's launcher mask.
echo "→ android adaptive foreground"
ANDROID_ADAPTIVE_PENNANT_PX=230
ANDROID_ADAPTIVE_X_OFFSET=19
ANDROID_ADAPTIVE_Y_OFFSET=12
magick -background none -density 600 "$SVG" \
  -resize "${ANDROID_ADAPTIVE_PENNANT_PX}x${ANDROID_ADAPTIVE_PENNANT_PX}" "$TMP/foreground-penn.png"
magick -size 432x432 xc:none "$TMP/foreground-penn.png" \
  -gravity center -geometry "+${ANDROID_ADAPTIVE_X_OFFSET}+${ANDROID_ADAPTIVE_Y_OFFSET}" \
  -composite "$TMP/foreground.png"
for i in "${!ANDROID_DPIS[@]}"; do
  dpi="${ANDROID_DPIS[$i]}"
  size="${ANDROID_SIZES[$i]}"
