...
# 72dp. Rendering the pennant at 60% of the canvas leaves a little
# breathing room even under a circular launcher mask.
echo "→ android adaptive foreground"
magick -background none -density 600 "$SVG" -resize 259x259 "$TMP/foreground-penn.png"
magick -size 432x432 xc:none "$TMP/foreground-penn.png" \
  -gravity center -composite "$TMP/foreground.png"
for i in "${!ANDROID_DPIS[@]}"; do
  dpi="${ANDROID_DPIS[$i]}"
  size="${ANDROID_SIZES[$i]}"
