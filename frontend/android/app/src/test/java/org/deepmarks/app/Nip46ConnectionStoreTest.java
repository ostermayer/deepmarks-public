...
public class Nip46ConnectionStoreTest {
    ...
    public void parsesOnlyValidConnections() {
        String raw = "[" +
            ...
            "\",\"relays\":[\"wss://relay.deepmarks.org/\",\"ftp://bad\"],\"secret\":\"pair-secret\",\"perms\":[\"sign_event:1\"],\"trustLevel\":\"medium\",\"name\":\"Amethyst\",\"createdAt\":1,\"lastSeenAt\":1}," +
        ...
        assertEquals("medium", connections.get(0).trustLevel);
        assertEquals("Amethyst", connections.get(0).name);
    }

    @Test
    public void mergesPermissionListsInOrder() {
        assertEquals(
            Arrays.asList("sign_event:1", "nip44_decrypt", "sign_event:0"),
            Nip46ConnectionStore.mergePerms(
                Arrays.asList("sign_event:1", "nip44_decrypt"),
                Arrays.asList(" sign_event:1 ", "sign_event:0")
            )
        );
    }
}
