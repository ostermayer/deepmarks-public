...
final class NostrNativePublisher {
    ...
    private static final String PRIVATE_ITEM_PREFIX = "deepmarks-private-item:";
    ...
    static void publishBookmark(Context context, JSONObject share) throws Exception {
        String visibility = share.optString("visibility", "private");
        switch (visibility) {
            case "public":
                publishPublicBookmark(context, share);
                return;
            case "private":
                publishPrivateBookmark(context, share);
                return;
            default:
                throw new IllegalArgumentException("unsupported bookmark visibility");
        }
    }

    private static void publishPublicBookmark(Context context, JSONObject share) throws Exception {
        Account account = requireShareAccount(context, share);
        long createdAt = parseLong(share.optString("createdAt"), System.currentTimeMillis() / 1000L);
        List<List<String>> tags = bookmarkTags(share, createdAt);
        NostrEventSigner.SignedEvent event = NostrEventSigner.signEvent(account.nsecHex, 39701, createdAt, tags, "");
        postSignedEvent(account.nsecHex, event);
    }

    private static void publishPrivateBookmark(Context context, JSONObject share) throws Exception {
        Account account = requireShareAccount(context, share);
        long createdAt = parseLong(share.optString("createdAt"), System.currentTimeMillis() / 1000L);
        String url = safeUrl(share);
        List<List<String>> innerTags = bookmarkTags(share, createdAt);
        String plaintext = "[" + NostrEventSigner.jsonTags(innerTags) + "]";
        byte[] conversationKey = NostrNip44.conversationKey(account.nsecHex, account.pubkey);
        String ciphertext = NostrNip44.encrypt(plaintext, conversationKey);
        List<List<String>> eventTags = new ArrayList<>();
        addTag(eventTags, "d", privateItemName(url));
    ...
    private static void postSignedEvent(String nsecHex, NostrEventSigner.SignedEvent event) throws Exception {
        String eventJson = event.toJson();
        String body = "{\"events\":[" + eventJson + "]}";
        String auth = NostrEventSigner.buildNip98AuthHeader(nsecHex, PUBLISH_URL, "POST", body);
        postJson(PUBLISH_URL, body, auth);
    }

    static boolean canNativePublish(Context context, JSONObject share) {
        String visibility = share.optString("visibility", "private");
        if (!"public".equals(visibility) && !"private".equals(visibility)) return false;
    ...
    private static Account requireShareAccount(Context context, JSONObject share) throws Exception {
        Account account = loadAccount(context);
        requireMatchingAccount(account, share);
        return account;
    }

    private static void requireMatchingAccount(Account account, JSONObject share) {
    ...
    private static List<List<String>> bookmarkTags(JSONObject share, long createdAt) {
    ...
    private static String safeUrl(JSONObject share) {
    ...
    private static String privateItemName(String url) throws Exception {
        byte[] hash = NostrCrypto.sha256(url.getBytes(StandardCharsets.UTF_8));
        return PRIVATE_ITEM_PREFIX + NostrCrypto.hex(hash);
    }
