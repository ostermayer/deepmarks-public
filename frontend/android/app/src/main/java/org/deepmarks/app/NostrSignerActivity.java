...
public class NostrSignerActivity extends BridgeActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (shouldDeclineSignerRequest(getIntent())) {
            // The user has not turned on "use Deepmarks as my Nostr signer
            // for other apps". Decline instead of foregrounding our approval
            // UI — otherwise merely opening another app that has Deepmarks
            // selected as its external signer (e.g. Amethyst) yanks the user
            // into Deepmarks asking them to sign in. RESULT_CANCELED lets the
            // caller fall back to the user's real signer.
    ...
    protected void onNewIntent(Intent intent) {
        if (shouldDeclineSignerRequest(intent)) {
            setResult(RESULT_CANCELED);
            finish();
            return;
        }
    ...
    /**
     * True when the incoming intent is a NIP-55 signer request but the user
     * has not enabled Deepmarks-as-signer. We must never foreground the app
     * for an unsolicited signer request from another app.
     */
    private boolean shouldDeclineSignerRequest(Intent intent) {
        return NostrSignerIntentStore.isNostrSignerIntent(intent)
            && !Nip46ConnectionStore.isEnabled(this);
    }
