...
public class NostrSignerProvider extends ContentProvider {
    ...
    public Cursor query(
        ...
        // Only act as a signer for other apps when the user has explicitly
        // enabled it. Otherwise stay invisible — don't hand out the pubkey
        // or sign for anyone, even previously-trusted apps.
        if (!Nip46ConnectionStore.isEnabled(context)) return empty(columns);
