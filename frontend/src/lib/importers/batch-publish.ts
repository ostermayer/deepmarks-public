    ...
    const results = await Promise.allSettled(
      slab.map(async (input) => {
        ...
        const result = await withImportSignerRetry(
          () => publishEventQueued(template, pubkey, { failureSubject: 'bookmark' }),
          pubkey,
...
async function* publishPrivateBatch(
  ...
  // Bulk import is the one remaining whole-set rewrite — refuse to
  // build on an incomplete view (undecryptable chunks) just like the
  // old per-bookmark paths did.
  assertPrivateSetRewriteSafe(existing);
  ...
  const publishCount = itemPreparedCount > 0 ? privateSetReplacementCount(entries, existing.chunkNames) : 0;
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    try {
      const result = await withImportSignerRetry(
        () => publishEventQueued(templates[i]!, pubkey, { failureSubject: 'private bookmark' }),
        pubkey,
...
function parsedBookmarkFromInput(
  ...
  const savedAt = input.publishedAt ?? fallbackSavedAt;
  const savedAtMs = input.publishedAtMs &&
    Number.isSafeInteger(input.publishedAtMs) &&
    Math.floor(input.publishedAtMs / 1000) === savedAt
      ? input.publishedAtMs
      : undefined;
  return {
    url: input.url,
