...
export function parsePinboard(content: string): BookmarkInput[] {
  ...
  for (const raw of data as PinboardEntry[]) {
    // Malformed entries (null rows, non-string href/tags) used to escape
    // as raw TypeErrors instead of the ImportError contract (2026-08-23
    // review) — skip them like any other unusable row.
    if (!raw || typeof raw !== 'object' || typeof raw.href !== 'string' || !raw.href) continue;
