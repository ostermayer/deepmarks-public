...
const MEDIA_HOST_PATTERNS = [
  ...
  /(^|\.)tube\.tchncs\.de$/i,
  /(^|\.)video\.nostr\.build$/i,
  // Other common yt-dlp-supported video/audio platforms users bookmark.
  // A non-media URL on one of these fails once and is then skipped by the
  // failed-media cooldown, so widening coverage is safe.
  /(^|\.)dailymotion\.com$/i,
  /^dai\.ly$/i,
  /(^|\.)streamable\.com$/i,
  /(^|\.)bitchute\.com$/i,
...
export async function queueEligibleMediaArchives(
  ...
  for (const bookmark of byUrl.values()) {
    ...
    if (
      ...
      || isMediaQueued(bookmark.url)
      || isRecentlyFailedMedia(bookmark.url)
    ) {
      skipped += 1;
      continue;
    }
  ...
  await Promise.all(candidates.map(async (item) => {
    try {
      ...
      if (status.state === 'done') {
        ...
        // Record the terminal failure — deleting the entry made the next
        // backfill pass re-enqueue the same dead video (deleted upload,
        // geo-block) forever, burning yt-dlp attempts each time. Failed
        // URLs are skipped for FAILED_MEDIA_RETRY_MS, then eligible again.
        rememberFailedMediaArchive(item.entry.canonicalUrl ?? mediaUrlFromStorageKey(item.storageKey), status.error);
        removeMediaStorageKey(item.storageKey);
        failed += 1;
      } else {
...
const FAILED_MEDIA_LS_KEY = 'deepmarks-media-failed:v1';
const FAILED_MEDIA_RETRY_MS = 30 * 24 * 60 * 60 * 1000; // genuinely-gone media: retry monthly
const FAILED_MEDIA_RETRY_FIXABLE_MS = 2 * 24 * 60 * 60 * 1000; // recoverable failures: retry ~every 2 days
const FAILED_MEDIA_CAP = 500;

function readFailedMediaMap(): Record<string, { error?: string; failedAt: number }> {
  if (typeof localStorage === 'undefined') return {};
  try {
    const raw = localStorage.getItem(FAILED_MEDIA_LS_KEY);
    return raw ? (JSON.parse(raw) as Record<string, { error?: string; failedAt: number }>) : {};
  } catch {
    return {};
  }
}

function rememberFailedMediaArchive(url: string | undefined, error?: string): void {
  if (!url || typeof localStorage === 'undefined') return;
  try {
    const map = readFailedMediaMap();
    map[mediaArchiveIdentity(url)] = { error, failedAt: Date.now() };
    const entries = Object.entries(map)
      .sort(([, a], [, b]) => b.failedAt - a.failedAt)
      .slice(0, FAILED_MEDIA_CAP);
    localStorage.setItem(FAILED_MEDIA_LS_KEY, JSON.stringify(Object.fromEntries(entries)));
  } catch {
    // Quota — worst case we re-attempt, which is the old behavior.
  }
}

// A media failure is "permanent" when the video is genuinely gone — retrying
// soon is pointless. Everything else (bot walls, missing PO token, "format not
// available", interrupted/lost jobs) is recoverable once the server side is
// fixed, so it gets a much shorter cooldown to pick the fix up quickly.
function isPermanentMediaFailure(error?: string): boolean {
  const t = (error ?? '').toLowerCase();
  return /no video|no media|private video|video unavailable|removed|deleted|unsupported url|not a valid url/.test(t);
}

function isRecentlyFailedMedia(url: string): boolean {
  const entry = readFailedMediaMap()[mediaArchiveIdentity(url)];
  if (!entry) return false;
  const cooldown = isPermanentMediaFailure(entry.error) ? FAILED_MEDIA_RETRY_MS : FAILED_MEDIA_RETRY_FIXABLE_MS;
  return Date.now() - entry.failedAt < cooldown;
}

function mediaUrlFromStorageKey(storageKey: string): string | undefined {
  // Storage keys embed the identity; the canonicalUrl on the entry is
  // preferred — this is the fallback when an old entry lacks it.
  const idx = storageKey.lastIndexOf(':');
  return idx >= 0 ? storageKey.slice(idx + 1) || undefined : undefined;
}
...
function mediaArchiveIdentity(raw: string): string {
  try {
    ...
    // never queued (2026-08-23 review). Old lowercased localStorage
    // identities simply re-check once; the server dedupe absorbs it.
