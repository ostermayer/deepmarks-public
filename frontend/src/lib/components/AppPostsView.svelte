<script lang="ts">
  ...
  $: searchSummary = activeSearchQuery
    ...
    : '';
  // `postSearchRevision` is referenced so titles refresh as notes resolve.
  $: overlaySearchResults = activeSearchQuery && postSearchRevision >= 0
    ? [
        ...searchedBookmarkResults.map(bookmarkToSearchResult),
        ...searchedPostEntries
          .map(postEntryToResult)
  ...
  // A post opens at its canonical Nostr viewer (same target as the card's
  // "open it on primal" link); the row title is the note's cached content.
  function postEntryToResult(entry: PostEntry): SearchResultItem | null {
    const href = nostrNoteArchiveUrl(entry.data.targetEventId);
    if (!href) return null;
    const content = (get(resolveEvent(entry.data.targetEventId))?.content ?? '')
      .replace(/\s+/g, ' ')
      .trim();
    const title = content
      ? content.length > 90 ? `${content.slice(0, 90)}…` : content
      : 'Nostr post';
    return { id: `note:${entry.data.targetEventId}`, title, subtitle: 'post', href, external: true };
  }
