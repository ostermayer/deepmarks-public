<script lang="ts">
  ...
  type TabId = 'bookmarks' | 'friends' | 'search' | 'save' | 'collections' | 'tags' | 'posts' | 'readlater';
  ...
  const DEFAULT_VISIBLE_IDS: TabId[] = ['bookmarks', 'collections', 'friends', 'search', 'save'];
  ...
  const appTabs: Array<NavItem & { id: TabId }> = [
    ...
    { id: 'save', label: 'save', href: '/app/save', icon: Plus, match: (route) => route.startsWith('/app/save') },
    {
      ...
      icon: Folder,
      match: (route) => route.startsWith('/app/collections'),
    },
...
<style>
  ...
  .native-tabbar a.active::before,
  .native-tabbar button.active::before {
    content: '';
    position: absolute;
    top: 3px;
    left: 50%;
  ...
  .native-tabbar a.active :global(svg),
  .native-tabbar button.active :global(svg) {
    color: var(--coral);
    stroke-width: 2.6;
  }
