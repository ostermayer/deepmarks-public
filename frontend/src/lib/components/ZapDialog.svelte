<script lang="ts">
  ...
  import { getProfile, resolveProfile } from '$lib/nostr/profiles';
  ...
  $: if ($curatorProfile?.lud16) resolvedCuratorLightning = $curatorProfile.lud16;
  $: curatorLightning = resolvedCuratorLightning ?? $curatorProfile?.lud16 ?? null;
  ...
  async function refreshCuratorLightning() {
    ...
    if (profile?.lud16) resolvedCuratorLightning = profile.lud16;
  }
        ...
        <div class="manual">
          <div class="manual-head">
            <div>
              ...
              {/if}
              <p>
                Pay the invoice with any Lightning wallet. The dialog closes when the zap receipt confirms.
              </p>
            </div>
