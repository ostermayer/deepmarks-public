<script lang="ts">
  ...
  $: displayedBookmarks = paginate ? renderSource.slice(0, visibleLimit) : renderSource;
  $: hasMore = paginate && visibleLimit < renderSource.length;
  $: shownCount = Math.min(visibleLimit, renderSource.length);
  $: summarySource = summaryBookmarks ?? bookmarks;
  $: tagCloud = frozenCloud ?? tagCloudFrom(summarySource);
  ...
  type RelayModules = {
    ensureCoreRelaysConnected: () => void;
  ...
  function loadRelayModules(): Promise<RelayModules> {
    relayModulesPromise ??= Promise.all([
      ...
      ensureCoreRelaysConnected: ndkModule.ensureCoreRelaysConnected,
  ...
  async function refreshRelayStatus(): Promise<void> {
    try {
      const { ensureCoreRelaysConnected, getNdk, NDKRelayStatus } = await loadRelayModules();
      ensureCoreRelaysConnected();
      const pool = getNdk().pool;
      relayStatus = [...pool.relays.values()].map((r) => ({
        url: r.url,
        ok: r.status >= NDKRelayStatus.CONNECTED,
        status: statusLabel(r.status, NDKRelayStatus),
      }));
    } catch {
      relayStatus = [];
    }
  ...
  function loadMore() {
    visibleLimit = Math.min(visibleLimit + pageSize, renderSource.length);
  }
</script>
...
<div class="container">
    ...
    {:else}
      ...
      {#if hasMore}
        <div class="load-more-wrap">
          ...
          <span>showing {shownCount.toLocaleString()} of {renderSource.length.toLocaleString()}</span>
        </div>
      {/if}
    {/if}
  ...
  <Sidebar {tagCloud} {stats} {showStats} {archivedCountOverride} relays={relayStatus} />
</div>
