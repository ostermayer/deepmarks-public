<script lang="ts">
  ...
  import { primeEvents } from '$lib/nostr/event-resolver';
  ...
  let lastFreezeKey = paginationKey;
  ...
  $: visibleNoteTargets = renderNostrNoteBookmarks
    ? displayedBookmarks.map(noteBookmarkTarget).filter(Boolean)
    : [];
  $: nextNotePrimeSignature = visibleNoteTargets.join(',');
  $: if (nextNotePrimeSignature && nextNotePrimeSignature !== notePrimeSignature) {
    notePrimeSignature = nextNotePrimeSignature;
    void primeEvents(visibleNoteTargets);
  }
