<script lang="ts">
  ...
  import { api, ApiError, type ArchiveQueueStatus, type MediaArchiveAddonStatus } from '$lib/api/client';
  ...
  const QUEUE_STATUS_REFRESH_MS = 30_000;
  ...
  let archiveQueueStatus: ArchiveQueueStatus | null = null;
  ...
  $: serverMediaQueued = (archiveQueueStatus?.mediaPending ?? 0) + (archiveQueueStatus?.mediaRunning ?? 0);
  $: visibleMediaQueued = Math.max(counts.queued, serverMediaQueued);
  $: if (purchased && counts.eligible > counts.archived + visibleMediaQueued) scheduleBacklogQueue();
  ...
  async function refreshStatus(opts: { quiet?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      isLifetime = lifetime;
      status = remoteStatus;
      if (remoteStatus.purchased) {
        if (opts.quiet) void refreshArchiveQueueStatus();
        else await refreshArchiveQueueStatus({ force: true });
      } else {
        archiveQueueStatus = null;
      }
  ...
  async function refreshArchiveQueueStatus(opts: { force?: boolean } = {}): Promise<void> {
    const now = Date.now();
    if (!opts.force && now - queueStatusFetchedAt < QUEUE_STATUS_REFRESH_MS) return;
    queueStatusFetchedAt = now;
    try {
      archiveQueueStatus = await api.archives.queueStatus();
    } catch {
      archiveQueueStatus = null;
    }
  }
  ...
  async function queueBacklog(): Promise<void> {
    ...
    try {
      ...
      if (result.queued > 0) {
        ...
        await refreshArchiveQueueStatus({ force: true });
      }
    } catch {
  ...
  <article class="addon">
    <header>
      <div>
        <h3>archive media</h3>
        <p class="addon-desc">
          One-time add-on for automatic private archives of primary image, video, or audio
          from bookmarked pages, direct media files, streaming manifests, podcast episodes,
          and supported media sites.
          Media archives are encrypted and only listed for the npub that saved the bookmark.
        </p>
        <p class="addon-disclaimer">
          Deepmarks makes a best-effort attempt to archive associated media, but some providers
          block automated capture or require account sessions, so not every page can be fully archived.
        </p>
      </div>
      {#if purchased}
    ...
    {:else if purchased}
      ...
      {#if visibleMediaQueued > 0}
        <p class="status">{visibleMediaQueued} media archive{visibleMediaQueued === 1 ? '' : 's'} queued or processing.</p>
      {:else if counts.eligible === 0}
        <p class="status">bookmark a page with primary media and Deepmarks will queue the private media archive automatically.</p>
      {/if}
