<script lang="ts">
  ...
  // 402 on POST /api/v1/keys, which we surface without exposing purchase UI
  // in iOS/App Store builds.
  ...
  $: apiV1Base = `${config.apiBase.replace(/\/$/, '')}/api/v1`;
  ...
  async function create() {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 402) {
        error = IS_APPLE_BUILD
          ? 'API access is available to lifetime-tier accounts. Existing access works in this app.'
          : 'API access is available to lifetime-tier members (21,000 sats). See /pricing.';
...
<SettingsSection title="api access">
  <p class="settings-section-copy">
    {IS_APPLE_BUILD
      ? 'Programmatic access for lifetime-tier accounts.'
      : 'Programmatic access for lifetime-tier members (21,000 sats).'}
    Use keys as <code>Bearer</code> tokens against <code>{apiV1Base}</code>.
    Endpoint details, examples, and signing rules live on the
    <a href="/api">API details page</a>. Keys can be revoked any time.
  </p>

  {#if !$canSign}
    <div class="hint">Sign in with {nativeShell ? 'your recovery key' : 'a signer (browser extension or nsec)'} to manage API keys.</div>
  {:else}
