<script lang="ts">
  ...
  $: showAction = !isPrivateBookmark || $session.pubkey === bookmark.curator;
...
{#if showAction}
  <button
    ...
    disabled={!$canSign}
    title={$canSign ? 'post this link to Nostr' : 'connect your signer to post to Nostr'}
