<script lang="ts">
  ...
  $: isPrivate = isPrivateBookmark(bookmark);
  $: isNostrNoteLink = (bookmark as BookmarkSource).source === 'nostr-note-link';
  $: canReferenceBookmark = !isPrivate && !isNostrNoteLink;
  $: showAction = !!$session.pubkey && (!isPrivate || $session.pubkey === bookmark.curator);
  ...
  function shareNaddrUrl(): string {
    if (!canReferenceBookmark) return bookmark.url;
    try {
  ...
  async function copyNostrLink(): Promise<void> {
    ...
    try {
      ...
      message = canReferenceBookmark ? 'Nostr bookmark link copied' : 'URL copied';
      error = '';
    } catch {
      message = url;
      error = '';
    ...
    try {
      const result = await publishEvent(
        buildSocialPostEvent({
          ...
          description: bookmark.description,
          content: text,
          bookmarkEventId: canReferenceBookmark ? bookmark.eventId : undefined,
          bookmarkAuthor: canReferenceBookmark ? bookmark.curator : undefined,
        }),
        $session.pubkey,
      );
...
{#if showAction && open}
  ...
  >
    ...
    <p class="post-note">
      ...
      {#if isPrivate}Your private save stays private, but this note will reveal the link.{/if}
    </p>
    <MentionTextarea
      rows={7}
      bind:value={text}
