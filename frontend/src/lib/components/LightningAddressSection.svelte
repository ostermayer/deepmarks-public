<script lang="ts">
  ...
  //   • lnaddr is where curator zaps are paid, so it's central to how
  //     value flows through deepmarks
...
<SettingsSection title="lightning address">
  {#if !pubkey}
    <p class="settings-section-copy">sign in to manage your lightning address.</p>
  {:else}
    ...
    {#if needsReconnect}
      <p class="reconnect">
        your Deepmarks account is signed in. to publish lightning address changes,
