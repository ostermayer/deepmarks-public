<script lang="ts">
  ...
  //   2. Call DELETE /account on api to release the user's
  //      deepmarks handle + clear server-side state (settings, api keys,
  //      archives, private marks, account record). Lifetime-payment
  //      record stays, so the same nsec can start fresh later without
  //      re-paying.
  //
  ...
  let result: {
    ...
    releasedUsernameCooldown: boolean;
    revokedApiKeys: number;
    privateMarksRemoved: number;
    archivesRemoved: number;
    archiveMirrorDeleteJobs: number;
  } | null = null;
  let error = '';

  $: armed = typed.trim() === CONFIRMATION_PHRASE && $canSign && !working;
  $: needsReconnect = !!$session.pubkey && !$canSign && !$sessionRestoring;
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      result = {
        ...
        releasedUsernameCooldown: server.releasedUsernameCooldown ?? true,
        revokedApiKeys: server.revokedApiKeys,
        privateMarksRemoved: server.privateMarksRemoved ?? 0,
        archivesRemoved: server.archivesRemoved ?? 0,
        archiveMirrorDeleteJobs: server.archiveMirrorDeleteJobs ?? 0,
      };
      step = '';
  ...
  {#if result}
    <p class="ok">
      ...
      {#if result.releasedUsername}
        your handle <code>{result.releasedUsername}</code> was released{result.releasedUsernameCooldown ? ' into cooldown' : ' immediately'}.
      {/if}
      {#if result.archivesRemoved > 0}
        removed {result.archivesRemoved} archive record{result.archivesRemoved === 1 ? '' : 's'} and queued {result.archiveMirrorDeleteJobs} mirror delete job{result.archiveMirrorDeleteJobs === 1 ? '' : 's'}.
      {/if}
      signing you out…
    </p>
  {:else}
    <p class="explainer">
      ...
      your <strong>deepmarks handle</strong> (lifetime handles are released immediately; free handles
      enter the 30-day cooldown), revoke all api keys, remove synced settings, delete our archive
      blobs, queue mirror deletes, and drop any cached private-bookmark ciphertexts.
    </p>
    <ul class="caveats">
      <li>
    ...
    {#if !$canSign}
      <p class="explainer">
        {#if needsReconnect}
          your Deepmarks account is signed in. to delete it,
