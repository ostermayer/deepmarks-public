<script lang="ts">
  ...
  let privacyChoice: 'default' | 'public' | 'private' = prefillVisibility;
  let readLater = prefillReadLater === true || tags.includes('toread');
  ...
  $: if (nativeMode && privacyChoice === 'default') isPublic = $userSettings.defaultVisibility === 'public';
  ...
  function applyIncomingPrefill() {
    ...
    readLater = prefillReadLater === true || tags.includes('toread');
    privacyChoice = prefillVisibility;
    setPrivacyChoice(prefillVisibility);
  ...
  function applyDefaultTagsIfNeeded() {
    ...
    if (url.trim() && tags.length === 0 && $userSettings.defaultTags.length > 0) {
      ...
      nativeTagsText = tags.filter((tag) => tag !== 'toread').join(' ');
      if (nativeMode && tags.includes('toread')) readLater = true;
    }
  }
  ...
  function setPrivacyChoice(choice: 'default' | 'public' | 'private') {
...
{:else if nativeMode}
  <div class="native-save-form" class:disabled={!$canSign}>
    ...
    <section class="native-section">
      <h2>TITLE</h2>
      <div class="native-card">
        <textarea rows="2" placeholder="Optional" bind:value={title} disabled={saving}></textarea>
      </div>
    </section>
    ...
    <section class="native-section">
      <h2>DESCRIPTION</h2>
      <div class="native-card">
        <textarea rows="3" placeholder="Optional" bind:value={description} disabled={saving}></textarea>
      </div>
    </section>
      ...
      <div class="native-card">
        ...
        <div class="native-row privacy-row">
          ...
          <div class="native-segmented" aria-label="privacy">
            <button type="button" class:active={privacyChoice === 'default'} on:click={() => setPrivacyChoice('default')} disabled={saving}>Default</button>
            <button type="button" class:active={privacyChoice === 'public'} on:click={() => setPrivacyChoice('public')} disabled={saving}>Public</button>
            <button type="button" class:active={privacyChoice === 'private'} on:click={() => setPrivacyChoice('private')} disabled={saving}>Private</button>
          </div>
        </div>
