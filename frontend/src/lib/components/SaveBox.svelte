<script lang="ts">
  ...
  import { normalizeBookmarkSaveInput } from '$lib/nostr/social-refs';
  ...
  async function fetchMetadata() {
    ...
    try {
      ...
      const [meta, popular] = await Promise.all([
        api.metadata(trimmed, { fast: isNativeShell() }),
  ...
  async function save() {
    ...
    // Pasted note1/nevent1/nostr: references become the canonical
    // social URL so "bookmark this note" works straight from a Nostr
    // client's copy-note-id action.
    url = normalizeBookmarkSaveInput(url);
    error = '';
    success = '';
    warning = '';
    signerPrompt = '';
