<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    invalid: { targetEventId: string; kind: number };
  ...
  // Only a resolved kind:1 renders the full card — mirrors NoteCard so we
  // never show an orphaned action row under a missing/non-note target.
  $: isNote = $event?.kind === 1;

  // A target that stays unresolved used to render NOTHING — the bookmark
  // looked lost. After a short grace period (cheap stand-in for "the
  // fetch settled"), show a placeholder instead; if the note gets
  // mirrored later, resolveEvent's session retry swaps the real card in.
  let unresolvedLongEnough = false;
  onMount(() => {
    const timer = setTimeout(() => { unresolvedLongEnough = true; }, 4000);
    return () => clearTimeout(timer);
  });
  $: showMissingPlaceholder = unresolvedLongEnough && $event === null;
  ...
  async function onOpenArchive(): Promise<void> {
    ...
    archiveError = '';
    try {
      const blob = await fetchArchiveBlob(ownArchive, { pubkey: $session.pubkey });
...
{#if isNote}
  <div class="post-card" class:read-later={isReadLater}>
    <NoteCard {targetEventId} {zapSats} embedded on:invalid={forwardInvalid} />
...
{:else if showMissingPlaceholder}
  <div class="post-card post-missing">
    <p>
      this bookmarked post isn't reachable right now — it may not be
      mirrored to the relay yet, or its author deleted it. your bookmark
      is still saved.
      {#if noteUrl}
        <a href={noteUrl} target="_blank" rel="noopener noreferrer">open it on primal ↗</a>
      {/if}
    </p>
    <small>saved {new Date(savedAt * 1000).toLocaleDateString()}</small>
  </div>
{/if}
