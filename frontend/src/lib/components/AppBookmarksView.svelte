<script lang="ts">
  ...
  const SYNC_BANNER_SHOW_DELAY_MS = 6000;
  const SYNC_BANNER_HIDE_DELAY_MS = 3000;
  ...
  function updateSyncBanner(count: number): void {
    ...
    syncBannerTimer = setTimeout(() => {
      showSyncBanner = want;
      syncBannerTimer = null;
    }, want ? SYNC_BANNER_SHOW_DELAY_MS : SYNC_BANNER_HIDE_DELAY_MS);
  }
  ...
  $: visibleBaseBookmarks = filterBookmarks(allMineBookmarks, view);
