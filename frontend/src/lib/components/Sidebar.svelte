<script lang="ts">
  ...
  //   - relays:    configured Deepmarks storage relays with live connection status
  //
  ...
  export let tagScope: 'network' | 'mine' = 'mine';
  ...
  $: lifetimeKnownNonMember = !!isLifetime && $isLifetime === false;
  ...
  $: showUpgrade = !IS_APPLE_BUILD && (!$session.pubkey || lifetimeKnownNonMember);
  ...
  {#if showStats && stats !== null}
    <div class="sidebar-section">
      ...
      <div class="stats">
        ...
        <a class="stat-link" href="/app/tags"><span>my tags</span><strong class="num-retro">{formatNumber(stats.tagsUsed)}</strong></a>
        <a class="stat-link" href="/app/bookmarks?view=media"><span>media</span><strong class="num-retro">{formatNumber(mediaCountOverride)}</strong></a>
    ...
    <details class="sidebar-section relay-status">
      <summary>
        <span class="status-label">
          ...
          {relays.filter((r) => r.ok).length}/{relays.length} storage relays connected
        </span>
      </summary>
      <div class="stats">
        {#each relays as r}
