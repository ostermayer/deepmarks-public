<script lang="ts">
  ...
  import { toSyncedAccountSettings, userSettings } from '$lib/stores/user-settings';
  ...
  async function saveThemePreference(next: Theme): Promise<void> {
    ...
    try {
      ...
      userSettings.update((value) => ({ ...value, ...saved, syncedAt: saved.updatedAt }));
  ...
  async function logout() {
    ...
    // during the next page mount. Then send the user to /login —
    // landing on / after logout on native used to render a near-empty
    // hero that looked like a blank screen, and on web it dropped
    // them on the marketing landing which felt disorienting.
  ...
  $: nativeRootPath = ['/app', '/app/bookmarks', '/app/search', '/app/save', '/app/archives', '/app/settings']
    .includes(pathname);
