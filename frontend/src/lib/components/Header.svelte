<script lang="ts">
  ...
  import { session, npub, sessionRestoring } from '$lib/stores/session';
  import { theme, type Theme } from '$lib/stores/theme';
  import { mergeSyncedAccountSettings, toSyncedAccountSettings, userSettings } from '$lib/stores/user-settings';
  ...
  async function saveThemePreference(next: Theme): Promise<void> {
    const current = get(userSettings);
    const optimistic = {
      ...current,
      theme: next,
      pendingSync: true,
      syncedAt: Math.max(current.syncedAt, Date.now()),
    };
    ...
    try {
      ...
      userSettings.update((value) => mergeSyncedAccountSettings(value, saved));
      theme.set(saved.theme);
    } catch {
      // Settings page exposes sync state; header theme changes remain local until the next sync.
    }
  }
  ...
  async function logout() {
    ...
    // Await the actual signer teardown before navigating so any
    // in-flight request that depends on it doesn't fire a stale ping
    // during the next page mount. Then send the user to the public
    // landing page, where logged-out users can choose login or signup.
  ...
  function goBack() {
    ...
    void goto($session.pubkey ? '/app/bookmarks' : '/');
  }
  ...
  onDestroy(() => {
    ...
    if (keyListenerActive) document.removeEventListener('keydown', onGlobalKey);
  ...
  $: showSearch = pathname.startsWith('/app/search');
  $: homeHref = $session.pubkey ? '/app/bookmarks' : '/';
  $: isActive = (href: string) =>
    href === '/app' ? pathname === '/app' : pathname.startsWith(href);
  $: nativeRootPath = ['/app', '/app/bookmarks', '/app/friends', '/app/posts', '/app/search', '/app/save', '/app/tags', '/app/explore', '/app/settings']
    .includes(pathname);
  $: showNativeBack = nativeShell && !nativeRootPath;
</script>

<div class="header" class:native-shell={nativeShell} class:menu-open={menuOpen}>
  <div class="masthead">
    {#if showNativeBack}
      <button class="native-back" type="button" aria-label="go back" on:click={goBack}>
        <ChevronLeft size={19} strokeWidth={2.4} />
        <span>back</span>
      </button>
    {/if}
    <a href={homeHref} class="masthead-link" aria-label="Deepmarks home">
      <Logo size={24} />
    </a>
    <div class="title-block">
      <a href={homeHref} class="wordmark">Deepmarks</a>
      {#if !nativeShell}
  ...
  {#if showSearch}
    <form class="header-search" on:submit={onSubmitSearch}>
      ...
      <input
        ...
        bind:value={query}
        type="text"
        placeholder={nativeShell ? 'search bookmarks...' : 'search your bookmarks + the network...'}
        aria-label="search"
      />
      {#if !nativeShell}
        <span class="shortcut">/</span>
        <span class="search-help-wrap">
          <button type="button" class="search-help" aria-label="search syntax" title="Search syntax">?</button>
          <span class="search-help-popover" role="tooltip">
            <strong>search syntax</strong>
            <code>#bitcoin</code>
            <code>site:paulgraham.com</code>
  ...
  <div class="user-info" bind:this={menuRoot}>
    {#if $session.pubkey}
      {#if nativeShell}
        <span class="avatar-btn mobile-avatar" aria-label="signed in">
          <span class="avatar-wrap">
            <Avatar pubkey={$session.pubkey} size={34} />
            <span class="lifetime-overlay"><LifetimeBadge pubkey={$session.pubkey} size={12} /></span>
          </span>
          {#if !$session.signer && !$sessionRestoring}
            <span class="avatar-lock" title="signer locked — unlock to publish">🔒</span>
          {/if}
        </span>
      {:else}
        <button
  ...
  <!-- Top nav removed: it duplicated semantics with the in-page sort
       row + the new Global panel in the right sidebar. /app/friends,
       /app/follows, /app/popular, /app/recent, /app/explore, /app/tags routes still exist for
...
<style>
  ...
  .native-shell .header-search {
    margin-left: 0;
    margin-right: 54px;
    max-width: none;
  }
  .header.native-shell .masthead {
    align-items: center;
    padding-right: 54px;
  }
  .header.native-shell .native-back {
    display: inline-flex;
    align-items: center;
    gap: 2px;
    border: 1px solid var(--rule);
  ...
  .header.native-shell .native-back:active {
  ...
  .native-shell .header-search input {
    font-size: 16px;
  }
  ...
  .search-help-wrap:hover .search-help-popover,
  .search-help:focus + .search-help-popover {
    opacity: 1;
    visibility: visible;
  ...
  /* ── Mobile breakpoints ──────────────────────────────────────────
     ...
     rail + main column + masthead). Below that we collapse:
       ...
       - tagline hides under 480px (the lede on the home page already
         covers it)
  */
  @media (max-width: 720px) {
    ...
    .native-shell .user-info { top: calc(env(safe-area-inset-top, 0px) + 12px); }
  }
