<script lang="ts">
  ...
  function scrollRoot(): HTMLElement | null {
    return document.querySelector<HTMLElement>('[data-native-scroll-root="true"]');
  }
  ...
  function onTouchEnd(): void {
    ...
    if (active && pull >= THRESHOLD) {
      ...
      pull = THRESHOLD;
      try {
        sessionStorage.setItem('deepmarks-native-refresh-scroll-top', '1');
      } catch {
        // Non-fatal; the immediate scroll below still handles this view.
      }
      scrollToTopSoon();
      window.setTimeout(() => {
  ...
  onMount(() => {
    ...
    if (!enabled) return;
    try {
      if (sessionStorage.getItem('deepmarks-native-refresh-scroll-top') === '1') {
        sessionStorage.removeItem('deepmarks-native-refresh-scroll-top');
        scrollToTopSoon();
      }
    } catch {
