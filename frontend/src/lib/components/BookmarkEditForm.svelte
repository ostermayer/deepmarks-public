<script lang="ts">
  ...
  import { finalizeBookmarkArchive, type ArchiveDoneDetail, type BookmarkArchiveSnapshot } from '$lib/archives/finalize';
  ...
  let pendingArchive: (BookmarkArchiveSnapshot & { savedEventId: string }) | null = null;
  ...
  async function swapVisibility() {
    ...
    try {
      ...
      if (isPrivate) {
        ...
        const { templates } = await removeFromPrivateSet(bookmark.url, pubkey);
        for (const template of templates) await publishEvent(template, pubkey);
  ...
  async function save() {
    ...
    try {
      ...
      let eventId = '';
      if (isPrivate) {
        const { templates } = await updatePrivateSetEntry(bookmarkInput, pubkey);
        for (const template of templates) {
          const result = await publishEvent(template, pubkey);
          eventId = result.eventId;
        }
  ...
  async function onArchiveDone(detail: ArchiveDoneDetail) {
    ...
    try {
      ...
      const pubkey = sessionState.pubkey;
      if (!pubkey) return;
      eventId = (await finalizeBookmarkArchive(pubkey, snap, detail)).eventId;
    } catch {
      // Archive succeeded; swallow the retag failure.
    } finally {
      if (eventId) dispatch('updated', { eventId });
      cancel();
    }
  }

  function closeArchiveDialog() {
    const savedEventId = pendingArchive?.savedEventId;
    archiveDialogOpen = false;
    pendingArchive = null;
    if (savedEventId) dispatch('updated', { eventId: savedEventId });
    cancel();
  }
</script>
...
<div class="edit-form">
  ...
  {:else if canUseArchiveFlow}
    <label class="archive-row">
      ...
      <span>
        archive when saved
        <small>save opens the archive confirmation</small>
      </span>
    </label>
