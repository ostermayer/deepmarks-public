<script lang="ts">
  ...
  import { publishAll, publishEvent } from '$lib/nostr/publish';
  ...
  async function swapVisibility() {
    ...
    try {
      ...
      if (isPrivate) {
        ...
        await publishAll(templates, pubkey);
        ...
        const results = await publishAll(templates, pubkey);
  ...
  async function performDelete() {
    ...
    try {
      ...
      if (isPrivate) {
        const { templates } = await removeFromPrivateSet(bookmark.url, pubkey);
        const results = await publishAll(templates, pubkey);
        dispatch('deleted', { eventId: results.at(-1)?.eventId ?? '' });
      } else {
