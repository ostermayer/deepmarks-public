<script lang="ts">
  ...
  import { publishAllQueued, publishEventQueued } from '$lib/nostr/publish';
  ...
  async function swapVisibility() {
    ...
    try {
      ...
      if (isPrivate) {
        ...
        await publishAllQueued(templates, pubkey, { failureSubject: 'private bookmark' });
        ...
        const results = await publishAllQueued(templates, pubkey, { failureSubject: 'private bookmark' });
  ...
  async function save() {
    ...
    try {
      ...
      if (isPrivate) {
        ...
        // Publish all chunks concurrently. Used to be a sequential
        // for-loop, which made simple tag edits on libraries with
        // many private chunks take 20-40s on a NIP-07 signer (each
        // chunk = sign + publish round-trip per loop iteration).
        // Parallel publishes drop that to a couple of seconds.
        const results = await publishAllQueued(templates, pubkey, { failureSubject: 'private bookmark' });
        eventId = results.at(-1)?.eventId ?? '';
      } else {
  ...
  async function performDelete() {
    ...
    try {
      ...
      if (isPrivate) {
        const { templates } = await removeFromPrivateSet(bookmark.url, pubkey);
        const results = await publishAllQueued(templates, pubkey, { failureSubject: 'private bookmark' });
        commitDeleted(results.at(-1)?.eventId ?? '');
