<script lang="ts">
  ...
  /** Optional already-cached picture URL from a parent list join. */
  export let picture: string | undefined = undefined;
  /** Some dense lists already carry profile data; skip row-by-row
   * profile lookups there and let the parent refresh the cache in bulk. */
  export let eagerProfile: boolean = true;

  const emptyProfile = { subscribe: (run: (value: null) => void) => { run(null); return () => undefined; } };
  $: profile = eagerProfile ? getProfile(pubkey) : emptyProfile;
  $: npub = (() => {
    try { return nip19.npubEncode(pubkey); } catch { return undefined; }
  })();
  let failedPictureUrl: string | undefined;
  $: rawPictureUrl = picture || $profile?.picture;
  $: pictureUrl = rawPictureUrl && failedPictureUrl !== rawPictureUrl ? rawPictureUrl : undefined;
...
>
  {#if pictureUrl}
    <img
      ...
      on:error={() => (failedPictureUrl = pictureUrl)}
    />
  {:else}
    <svg
      width={size}
