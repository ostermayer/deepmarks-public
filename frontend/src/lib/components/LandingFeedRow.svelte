<script lang="ts">
  ...
  import { isSavedByMe } from '$lib/stores/my-saved-urls';
  ...
  /** Profile pages can expose the full bookmark description on demand. */
  export let allowDescription: boolean = false;
   ...
   *  instant feedback. */
  let hidden = false;
  $: signedIn = !!$session.pubkey;
  $: isOwner = $session.pubkey === bookmark.curator;
  $: isReadLater = bookmark.tags.includes('toread');
  // 'saved ✓' indicator. We only flip the button for users who own the
  // URL via THEIR own kind:39701 — owning someone else's curator event
  // doesn't count as saved on our list.
  $: alreadySaved = signedIn && !isOwner && $isSavedByMe(bookmark.url);
  $: hasDescription = allowDescription && bookmark.description.trim().length > 0;
  $: if (!hasDescription && descriptionOpen) descriptionOpen = false;
  $: profile = getProfile(bookmark.curator);
  $: curatorLabel = resolveLabel($profile?.displayName, bookmark.curator);
  $: displayTitle = readableNostrText(bookmark.title);
  $: curatorHref = (() => {
...
<div
  class="row"
  class:is-private={bookmark.eventId.startsWith('private:')}
  class:read-later={isReadLater}
>
  <Favicon url={bookmark.url} size={22} />
  <div class="body">
    <a class="title" href={safeExternalHref(bookmark.url)} target="_blank" rel="noreferrer">{displayTitle}</a>
    <div class="meta">
      {#if isReadLater}
        <span class="vis-tag readlater" title="saved for later reading">📖 read later</span>
        <span class="dot">·</span>
      {/if}
      ...
      {#if hasDescription}
        ...
        <button
          ...
          on:click|stopPropagation={() => (descriptionOpen = !descriptionOpen)}
        >
          {#if descriptionOpen}
    ...
    {#if hasDescription && descriptionOpen}
      <p class="description"><NostrText text={bookmark.description} /></p>
    {/if}
  </div>
  ...
  {#if signedIn && !isOwner}
    {#if alreadySaved}
      <span class="saved-tag" title="already on your list">saved ✓</span>
    {:else}
...
<style>
  ...
  .row.read-later {
    ...
    border-left: 2px solid var(--toread-accent);
    padding-left: 10px;
    margin-left: -12px;
    border-radius: 4px;
  }
  .row.read-later.is-private {
    /* Private + read-later: keep the orange tint, switch the border to
       indicate private wins for the privacy stripe color. */
