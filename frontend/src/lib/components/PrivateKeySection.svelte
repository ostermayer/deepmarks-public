...
<section>
  ...
  <ul class="recovery-list" aria-label="account recovery checklist">
    ...
    {#if nativeShell}
      <li class:done={!!signer} class:warn={!signer && !!pubkey}>
        <strong>Secure storage</strong>
        <span>{signer ? 'recovery key is stored in this device secure store' : pubkey ? 'unlock by signing in with your recovery key' : 'sign in first'}</span>
      </li>
    {:else}
