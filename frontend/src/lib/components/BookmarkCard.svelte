<script lang="ts">
  ...
  async function onToggleReadLater() {
    if (!$session.pubkey || togglingReadLater) return;
    // Optimistic — toggle the local cache synchronously so the row's
    // pill + tint flip on this click. The relay publish runs in the
    // background; we only flag the button as busy while awaiting it
    // so a second tap doesn't race the first.
    const { toggleReadLater } = await import('$lib/nostr/toggle-read-later');
    let result;
    try {
