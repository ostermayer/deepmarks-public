<script lang="ts">
  ...
  import { Archive, ArchiveX, Download, File, FileText, Play } from 'lucide-svelte';
  import { type ParsedBookmark, safeExternalHref } from '$lib/nostr/bookmarks';
  ...
  import { archiveLookupKeys, myArchiveFailures, myArchives, myArchivesByVideoKey } from '$lib/stores/my-archives';
  import {
    archiveBlobUrl,
    archiveFileAsRecord,
    archiveFileLabel,
  ...
  import BookmarkCollectionAction from './BookmarkCollectionAction.svelte';
  ...
  const dispatch = createEventDispatcher<{
    readLaterChanged: { bookmark: ParsedBookmark; isReadLater: boolean };
  }>();
  ...
  // NIP-51 imports can carry privacy in `visibility` rather than in the
  // `private:` event id prefix. Centralize that test so labels and first
  // actions preserve the source privacy.
  $: derivedIsPrivate = isPrivate ?? isPrivateBookmark(bookmark);
  ...
  function onToggleReadLater() {
    ...
    try {
      ...
      dispatch('readLaterChanged', {
        bookmark: result.bookmark,
        isReadLater: result.bookmark.tags.includes('toread'),
      });
    } catch (e) {
    ...
    result.publish
      .catch((e) => {
        dispatch('readLaterChanged', {
          bookmark: previous,
          isReadLater: previous.tags.includes('toread'),
        });
        alert(`couldn't update: ${(e as Error).message ?? 'unknown'}`);
      })
      .finally(() => {
  ...
  $: ownArchiveFailure = resolveOwnArchiveFailure(isOwner, bookmark.url, $myArchiveFailures);
  ...
  $: showArchiveFailure = !showArchive && !!ownArchiveFailure;
  $: archiveFailureLabel = ownArchiveFailure?.reason === 'site-blocked'
    ? 'site blocked'
    : 'archive failed';
  $: archiveFailureTitle = ownArchiveFailure
    ? `${ownArchiveFailure.message}${ownArchiveFailure.error ? ` ${ownArchiveFailure.error}` : ''}`
    : '';
  ...
  function resolveOwnArchiveFailure(
    owner: boolean,
    url: string,
    failures: Map<string, ArchiveFailureRecord>,
  ): ArchiveFailureRecord | undefined {
    if (!owner) return undefined;
    for (const key of archiveLookupKeys(url)) {
  ...
  async function openPrivateArchive(record = ownArchive): Promise<void> {
    ...
    decryptError = '';
    try {
      const blob = await fetchArchiveBlob(record, { pubkey: $session.pubkey });
  ...
  async function watchMediaArchive(): Promise<void> {
    ...
    try {
      // Streaming first: fragmented-MP4 archives play as the first
      // decrypted chunks arrive instead of after the full download.
      const stream = await openArchiveMediaStream(record, { pubkey: $session.pubkey }).catch(() => null);
      if (stream) {
        mediaPlayerKind = (record.mseCodecs ?? '').startsWith('audio/') ? 'audio' : 'video';
        archiveChooserOpen = false;
        mediaPlayerUrl = stream.objectUrl;
        stream.done.catch((e: Error) => {
          mediaPlayerError = e.message ?? 'streaming failed';
        });
        return;
      }
      const blob = await fetchArchiveBlob(record, { pubkey: $session.pubkey });
...
<div
  ...
  class:compact
>
  <span class="favicon-column">
    <Favicon url={bookmark.url} size={compact ? 18 : 24} />
  </span>
  <div class="body">
    <div class="title">
      <a href={safeExternalHref(bookmark.url)} target="_blank" rel="noreferrer">{displayTitle}</a>
      {#if isOwner}
    ...
    <div class="meta meta-actions">
      {#if showArchive}
        ...
        {/if}
      {/if}
      {#if showArchiveFailure}
        <span
          class="archive-failed-chip"
          title={archiveFailureTitle}
          aria-label={archiveFailureTitle}
        >
          <ArchiveX size={14} strokeWidth={2.2} />
          <span>{archiveFailureLabel}</span>
        </span>
      {/if}
      {#if showSaveLink}
        <a
      ...
      {/if}
      {#if isOwner}
        <BookmarkCollectionAction {bookmark} />
      {/if}
      {#if isOwner}
        <button
          type="button"
...
<style>
  ...
  .archive-failed-chip {
    ...
    min-height: 19px;
    padding: 0 7px 0 4px;
    border: 1px solid color-mix(in srgb, var(--coral-deep) 52%, var(--rule));
    border-radius: 5px;
    color: var(--coral-deep);
  ...
  .archive-failed-chip :global(svg) {
    display: block;
    flex: 0 0 auto;
  }
  .archive-download {
