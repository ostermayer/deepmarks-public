<script lang="ts">
  ...
  // An imported NIP-51 bookmark carries its privacy in `visibility`, not
  // in a `private:` eventId prefix — without this an imported PRIVATE
  // bookmark reads as public and editing it would republish a public
  // kind:39701, leaking it.
  $: derivedIsPrivate = isPrivate ?? (
    bookmark.eventId.startsWith('private:') ||
    (isImportedUrlBookmark(bookmark) && bookmark.visibility === 'private')
  );
