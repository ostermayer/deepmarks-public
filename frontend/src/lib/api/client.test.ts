...
describe('api.archives', () => {
  it('paginates the NIP-98 account archive list', async () => {
    ...
    expect(fetchMock.mock.calls[0]![0]).toContain('/account/archives?limit=500&offset=0');
    expect(fetchMock.mock.calls[1]![0]).toContain('/account/archives?limit=500&offset=500');
    const init = fetchMock.mock.calls[0]![1] as RequestInit;
    expect(((init.headers ?? {}) as Record<string, string>)['Authorization']).toMatch(/^Nostr /);
  });
});
