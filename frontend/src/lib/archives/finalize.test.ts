...
import { finalizeBookmarkArchive } from './finalize.js';
...
describe('finalizeBookmarkArchive', () => {
  beforeEach(() => {
    ...
    vi.mocked(updatePrivateSetEntry).mockResolvedValue({
      template: { kind: 30003, content: 'set', tags: [], created_at: 1 },
      templates: [
        { kind: 30003, content: 'one', tags: [], created_at: 1 },
        { kind: 30003, content: 'two', tags: [], created_at: 1 },
      ],
      entries: [],
    });
  });
  ...
  it('retags public bookmarks and records the archive locally', async () => {
    const result = await finalizeBookmarkArchive(
      'pubkey',
      { url: 'https://example.com', title: 'Example', tags: ['web'], isPublic: true },
      { hash: 'blob-hash', wayback: 'https://web.archive.org/example' },
    );

    expect(result.eventId).toBe('event-1');
    expect(rememberArchiveRecord).toHaveBeenCalledWith('pubkey', {
      jobId: 'blob-hash',
