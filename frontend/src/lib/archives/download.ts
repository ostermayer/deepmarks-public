...
export async function fetchArchiveBytes(
  ...
  if (isChunkedArchiveBlob(bytes)) {
    const parts = await decryptChunkedArchiveBlob(bytes, key);
...
export async function fetchArchiveBlob(
  ...
  for (const url of sources) {
    try {
      ...
      if (head.byteLength === 8 && isChunkedArchiveBlob(new Uint8Array([...head, 0]))) {
      ...
      return new Blob([await decryptArchiveBlob(bytes, key) as BlobPart], { type: mime });
    } catch (err) {
      lastError = err;
    }
  }
