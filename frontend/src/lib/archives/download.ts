...
import {
  createArchiveByteSource,
  decryptChunkedArchiveBlob,
  decryptChunkedArchiveBody,
  decryptChunkedArchiveChunks,
  isChunkedArchiveBlob,
  isChunkedArchiveHead,
...
export async function fetchArchiveBytes(
  ...
  const key = await resolveArchiveKey(rec, opts);
  if (isChunkedArchiveBlob(bytes)) {
    return concatBytes(await decryptChunkedArchiveBlob(bytes, key));
  }
  return decryptArchiveBlob(bytes, key);
}

/** Run the full key-recovery chain (cached map → forced relay re-read →
 *  stash → reconcile → extension bridge) and throw the standard
 *  missing-key message when nothing pans out. */
async function resolveArchiveKey(rec: ArchiveRecord, opts: FetchArchiveOptions): Promise<string> {
...
/**
 * Fetch + decrypt an archive into a Blob with BOUNDED memory. For
 * chunked (v2, media) blobs the ciphertext is consumed straight off the
 * network stream and decrypted ~8 MiB at a time; the plaintext parts go
 * into a Blob, which browsers back with disk for large payloads. This
 * is what makes a 1 GB+ paid media archive actually playable on iOS —
 * the old whole-file path needed ~3× the file size in memory at once.
 * Legacy (v1) and public blobs fall back to the buffered path.
 */
export async function fetchArchiveBlob(
  rec: ArchiveRecord,
  opts: FetchArchiveOptions = {},
): Promise<Blob> {
  const mime = archiveMime(rec);
  if (rec.tier !== 'private') {
  ...
  for (const url of sources) {
    try {
      ...
      if (!res.body) {
        ...
        const plaintext = isChunkedArchiveBlob(bytes)
          ? await decryptChunkedArchiveBlob(bytes, key)
          : [await decryptArchiveBlob(bytes, key)];
        return new Blob(plaintext as BlobPart[], { type: mime });
      }
      const source = createArchiveByteSource(res.body);
      const head = (await source.read(8)) ?? new Uint8Array(0);
      if (isChunkedArchiveHead(head)) {
        const parts = await decryptChunkedArchiveBody(source, key);
        return new Blob(parts as BlobPart[], { type: mime });
      }
      // Legacy v1 — buffer the remainder and decrypt whole-file.
      const chunks: Uint8Array[] = [head];
      for (;;) {
        const more = await source.read(1024 * 1024);
        if (!more) break;
        chunks.push(more);
      }
      return new Blob([await decryptArchiveBlob(concatBytes(chunks), key) as BlobPart], { type: mime });
    } catch (err) {
      lastError = err;
    }
  }
...
/**
 * Streaming playback: pipe a chunked (v2) fragmented-MP4 media archive
 * straight into a MediaSource — playback starts as soon as the first
 * decrypted chunks land instead of after the whole file downloads.
 * Returns null when streaming isn't possible (no codecs string, no MSE,
 * not a v2 blob) — the caller falls back to fetchArchiveBlob, which
 * always works.
 */
export async function openArchiveMediaStream(
  rec: ArchiveRecord,
  opts: FetchArchiveOptions = {},
): Promise<{ objectUrl: string; done: Promise<void>; cancel: () => void } | null> {
  if (rec.tier !== 'private') return null;
  if (!rec.mseCodecs || typeof MediaSource === 'undefined') return null;
  try {
    if (!MediaSource.isTypeSupported(rec.mseCodecs)) return null;
  } catch {
    return null;
  }

  const key = await resolveArchiveKey(rec, opts);
  for (const url of archiveBlobSourceUrls(rec)) {
    let res: Response;
    try {
      res = await fetch(url);
    } catch {
    ...
    const head = (await source.read(8)) ?? new Uint8Array(0);
    if (!isChunkedArchiveHead(head)) {
      // Legacy v1 blob — not streamable; abandon this response and let
      // the caller take the Blob path.
    ...
    const done = (async () => {
      ...
      const buffer = mediaSource.addSourceBuffer(rec.mseCodecs!);
      const appendAndWait = (bytes: Uint8Array) => new Promise<void>((resolve, reject) => {
        buffer.addEventListener('updateend', () => resolve(), { once: true });
        buffer.addEventListener('error', () => reject(new Error('media append failed')), { once: true });
        buffer.appendBuffer(bytes as BufferSource);
      });
      for await (const part of decryptChunkedArchiveChunks(source, key)) {
        if (cancelled) return;
        await appendAndWait(part);
      }
...
export async function downloadArchiveRecord(
  ...
  // Blob path: chunked media downloads stream-decrypt with bounded
  // memory instead of materializing the whole file three times.
  const blob = await fetchArchiveBlob(rec, opts);
  downloadBlobAs(blob, archiveFilename(rec));
}
...
export async function downloadArchivesZip(
  ...
  for (let i = 0; i < records.length; i++) {
    const rec = records[i]!;
    const recFiles = archiveFiles(rec);
    let recordFailed = false;
    try {
      for (let fileIndex = 0; fileIndex < recFiles.length; fileIndex += 1) {
        const file = recFiles[fileIndex]!;
        const fileRec = archiveFileAsRecord(rec, file);
        try {
          const bytes = await fetchArchiveBytes(fileRec, { ...opts, archiveKeys });
          const label = recFiles.length > 1 ? `${archiveFileLabel(file).toLowerCase()}-` : '';
          const path = uniquePath(
            `archives/${String(i + 1).padStart(4, '0')}-${String(fileIndex + 1).padStart(2, '0')}-${label}${archiveFilename(fileRec)}`,
            used,
          );
          ...
          const label = recFiles.length > 1 ? `${archiveFileLabel(file).toLowerCase()}-` : '';
          const path = uniquePath(
            `errors/${String(i + 1).padStart(4, '0')}-${String(fileIndex + 1).padStart(2, '0')}-${label}${safeName(hostOf(fileRec.url) || 'archive')}.txt`,
            used,
          );
          files.push({
            path,
            data: utf8(`Failed to include archive file\n\nURL: ${fileRec.url}\nBlob: ${file.blobHash}\nError: ${error}\n`),
          });
          manifest.push({
            url: fileRec.url,
            blobHash: file.blobHash,
...
function downloadBytes(bytes: Uint8Array, filename: string, mime: string): void {
  downloadBlobAs(new Blob([bytes as BlobPart], { type: mime }), filename);
}
