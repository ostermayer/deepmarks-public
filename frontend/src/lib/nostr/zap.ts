...
export function buildZapRequestTags(
  ...
  if (bookmark.eventId) {
    ...
    tags.push(['k', String(KIND.webBookmark)]);
  }
  return tags;
}
 ...
 *      explicitly opted in.
 *   2. WebLN (Alby, Mutiny browser extension, etc.) — convenient
 *      when the user already has an extension installed.
 *   3. Deepmarks extension bridge — reaches the extension's NWC even
...
export async function payInvoices(invoices: ZapInvoice[]): Promise<string[]> {
  ...
  if (hasSiteNwc) {
    ...
    return paySequentialInvoices(invoices, async (inv) => (await payInvoice(inv.invoice)).preimage);
  }

  if (window.webln) {
    await window.webln.enable();
    return paySequentialInvoices(invoices, async (inv) => {
