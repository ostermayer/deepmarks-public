...
type ZapBookmarkTarget = Pick<ParsedBookmark, 'eventId'> & {
  ...
  /** Bookmark author — lets addressable (kind:39701) targets carry the
   *  NIP-57 `a` coordinate so receipts survive edits (2026-08-23 #9). */
  curator?: string;
};
...
export function buildZapRequestTags(
  ...
  if (target) {
    ...
    // kind:39701 is ADDRESSABLE — an edit mints a new event id and the
    // old one drops off relays, orphaning every receipt keyed on it
    // (2026-08-23 review #9). The `a` coordinate survives edits;
    // receipts copy request tags, so future aggregation can match on it.
    if (target.address) tags.push(['a', target.address]);
  }
  return tags;
}

function zapTarget(bookmark: ZapBookmarkTarget): { eventId: string; kind: number; address?: string } | null {
  if (
  ...
  if (HEX_PUBKEY_RE.test(bookmark.eventId)) {
    return {
      ...
      address: bookmark.curator && HEX_PUBKEY_RE.test(bookmark.curator) && bookmark.url
        ? `${KIND.webBookmark}:${bookmark.curator}:${bookmark.url}`
        : undefined,
    };
  }
  return null;
