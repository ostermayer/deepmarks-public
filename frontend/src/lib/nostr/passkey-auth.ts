...
//             → encrypt nsec → upload ciphertext to api.
...
async function fetchCiphertext(pubkey: string, token: string): Promise<string> {
  const url = `${config.apiBase}/account/nsec-ciphertext?pubkey=${encodeURIComponent(pubkey)}`;
...
/** 30s upper bound on any single navigator.credentials.* call. Real
 *  users take ~2-5s to approve; 30s keeps password-manager interception
 *  failures from feeling like a frozen login screen. */
...
async function withAbortTimeout<T>(
  ...
  const abortPromise = new Promise<never>((_, reject) => {
    const reason = () => {
      const raw = ctrl.signal.reason;
      return raw instanceof DOMException
        ? raw
  ...
  try {
    ...
    if (e instanceof DOMException && e.name === 'AbortError') {
      if (e.message === 'passkey-timeout') {
        throw new Error(
          label === 'create'
            ? 'passkey prompt timed out - Bitwarden or another password-manager extension may be intercepting it. continue without a passkey, then add one from Settings.'
            : 'passkey prompt timed out - Bitwarden or another password-manager extension may be intercepting it.',
        );
      }
      throw new Error(
        label === 'create'
          ? 'passkey cancelled - continue without one or retry.'
          : 'passkey cancelled.',
      );
    }
    throw e;
...
export async function registerPasskeyAndStoreNsec(
  ...
  // PRF-capable AND ask the authenticator to evaluate PRF inline during
  // create(). iCloud Keychain (macOS 14+) and Windows Hello return the
  // PRF output in the registration response, letting us skip the second
  // get() call entirely. That second call is what tripped Firefox on
  // macOS — Firefox enforces "transient user activation" per WebAuthn
  // call, and the gesture from the user's button click was consumed by
  // create(). The immediate get() then fired without a fresh gesture
  // and the platform returned NotAllowedError. Authenticators that
  // can't evaluate PRF during create() (older Android, some hardware
  // keys) just don't include the result; we fall back to a get() then.
  ...
  // 4. Derive the AES key from the PRF output.
  //    Preferred path: the authenticator returned PRF inline in the
  //    create() extensions (iCloud Keychain on macOS 14+, Windows Hello,
  //    recent Android). No second prompt = no Firefox transient-
  //    activation issue.
  //    Fallback: return a "finish setup" state. Some browsers and
  //    password-manager extensions consume the user's activation during
  //    create(), then suppress the immediate get() prompt. A second user
  //    click gives the native passkey picker a fresh activation.
  const inlinePrf: ArrayBuffer | undefined = createExt?.prf?.results?.first;
  if (!inlinePrf) {
    return { credentialID: regResult.credentialID, needsSecondStep: true };
  }

  // 5. Encrypt the nsec + upload.
  const key = await deriveKeyFromPrfOutput(inlinePrf);
...
/**
 * Complete a passkey setup after registration when PRF output was not
 * returned inline. Must be called from a fresh user click so browsers
 * that require transient activation can show the native passkey prompt.
 */
export async function finishPasskeyNsecStorage(
  pubkeyHex: string,
  nsecHex: string,
): Promise<void> {
...
export async function unlockNsecWithPasskey(pubkeyHex: string): Promise<string> {
  ...
  const ciphertextB64 = await fetchCiphertext(pubkeyHex, sessionToken.token);
...
export async function unlockNsecWithPasskeyDiscoverable(): Promise<{
  ...
  const ciphertextB64 = await fetchCiphertext(pubkey, token);
...
/** Check whether the server has a usable passkey sign-in for this pubkey. */
export async function passkeyExistsForPubkey(pubkeyHex: string): Promise<boolean> {
  return (await passkeyStatusForPubkey(pubkeyHex)).exists;
}

export async function passkeyStatusForPubkey(pubkeyHex: string): Promise<PasskeyStatus> {
  const res = await fetch(
    `${config.apiBase}/account/passkey/exists?pubkey=${encodeURIComponent(pubkeyHex)}`,
  );
  if (!res.ok) return { exists: false, hasCredential: false, hasCiphertext: false };
  const body = (await res.json()) as Partial<PasskeyStatus>;
  return {
    exists: body.exists === true,
    hasCredential: body.hasCredential ?? body.exists === true,
    hasCiphertext: body.hasCiphertext ?? body.exists === true,
  };
}
