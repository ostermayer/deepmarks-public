...
// bridge should be used instead. Native iOS/Android builds use the platform
// secure store instead, so external signer sessions can still save a local
// wallet connection. NWC secrets are intentionally device-local: the app
// secret is a wallet spending credential, so web, extension, iOS, and
// Android clients must each connect their wallet separately.
//
...
//
// Native builds consume the same module via the SvelteKit-as-Capacitor
// wrapper, but store NWC in Keychain / Android Keystore-backed secure
// storage instead of WebView localStorage. The browser-extension's
// chrome.storage.local variant stays separate because chrome.storage.* APIs
// only exist inside extension contexts.
...
interface NativeStoredNwcConnection {
  schemaVersion: 1;
  storage: 'native-secure-store';
  connection: NwcConnection;
}

interface NativeNwcStore {
...
export async function loadNwc(): Promise<NwcConnection | null> {
  ...
  const nativeStore = await nativeNwcStore();
  if (nativeStore) return loadNativeNwc(nativeStore);
...
export async function saveNwc(conn: NwcConnection): Promise<void> {
  ...
  const nativeStore = await nativeNwcStore();
  if (nativeStore) {
    await saveNativeNwc(conn, nativeStore);
    clearLocalNwc();
    return;
  }

  const key = await deriveSessionNwcKey();
  if (!key) throw new NwcLockedError('Sign in with a passkey or recovery key before saving NWC on this device');
...
export async function clearNwc(): Promise<void> {
  if (!browser) return;
  const nativeStore = await nativeNwcStore();
  if (nativeStore) await nativeStore.remove(KEY);
  clearLocalNwc();
}
...
async function nativeNwcStore(): Promise<NativeNwcStore | null> {
  if (!browser) return null;
  const { isNativeShell } = await import('$lib/native/runtime');
  if (!isNativeShell()) return null;
  const { secureGet, secureSet, secureRemove } = await import('$lib/mobile/secure-store');
  return {
    get: secureGet,
    set: secureSet,
    remove: secureRemove,
...
async function loadNativeNwc(store: NativeNwcStore): Promise<NwcConnection | null> {
  const raw = await store.get(KEY);
  if (raw) {
    const saved = parseNativeStoredNwc(raw);
    if (saved) return saved;
    await store.remove(KEY);
  }

  // Migrate native records written by earlier builds. Recovery-key-backed
  // sessions used localStorage encryption; signer-backed sessions cannot
  // unlock those records, so treat them as disconnected and let the user
  // add a fresh platform-secure NWC connection.
  const legacy = readStoredNwc();
  if (!legacy) return null;
  if (isEncryptedNwc(legacy)) {
    try {
      const migrated = await decryptStoredNwc(legacy);
      await saveNativeNwc(migrated, store);
      clearLocalNwc();
      return migrated;
    } catch {
      return null;
  ...
  await saveNativeNwc(legacy, store);
  clearLocalNwc();
  return legacy;
}

async function saveNativeNwc(conn: NwcConnection, store: NativeNwcStore): Promise<void> {
  const record: NativeStoredNwcConnection = {
    schemaVersion: 1,
    storage: 'native-secure-store',
    connection: conn,
  };
...
function parseNativeStoredNwc(raw: string): NwcConnection | null {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
  ...
  if (!parsed || typeof parsed !== 'object') return null;
  if (isPlainNwc(parsed)) return parsed;
  const v = parsed as Partial<NativeStoredNwcConnection>;
  if (
    v.schemaVersion === 1 &&
    v.storage === 'native-secure-store' &&
    v.connection &&
    typeof v.connection === 'object' &&
    isPlainNwc(v.connection)
  ) {
    return v.connection;
  }
  return null;
