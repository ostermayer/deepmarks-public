...
export async function saveBookmark(input: SaveBookmarkInput): Promise<SaveBookmarkResult> {
  ...
  const initialTitle = input.title?.trim() || undefined;
  const initialDescription = input.description?.trim() || undefined;
  const { title, description } = await fillMissingMetadata(url, initialTitle, initialDescription);
...
async function fillMissingMetadata(
  ...
  if (!isHttpUrl(url)) return { title, description };
  try {
    const { api } = await import('$lib/api/client');
    const meta = await withTimeout(api.metadata(url), 3_000, null);
    return {
      title: title || cleanMetadataText(meta?.title),
      description: description || cleanMetadataText(meta?.description),
    };
  } catch {
