...
export interface PopularityFloorOpts {
  /** Legacy single curator pubkey that bypasses the firehose zap threshold. */
  brandPubkey?: string;
  /** Editorial curator pubkeys that bypass the firehose zap threshold.
   *  This includes Deepmarks-owned daily-import and legacy seed keys. */
  editorialPubkeys?: Iterable<string>;
 ...
 *
 * The "freshest representative" rule is: latest `savedAt` wins, then
 * public event `created_at` for metadata edits, then lexicographic event id.
 */
  ...
  for (const b of bookmarks) {
    ...
    if (
      ...
      (b.savedAt === slot.rep.savedAt && (
        ...
        (incomingEditTime === currentEditTime && b.eventId > slot.rep.eventId)
      ))
    ) {
      slot.rep = b;
    }
 ...
 *   2. Firehose — entries NOT authored by an editorial pubkey must
 *      additionally clear totalZapSats > firehoseMinZapSats (default
 *      500). Real sats on the table is how open-Nostr content earns a
 *      popular slot.
 *
...
export function applyPopularityFloor(
  ...
  const editorialPubkeys = new Set(opts.editorialPubkeys ?? []);
  if (opts.brandPubkey) editorialPubkeys.add(opts.brandPubkey);
  return ranked.filter((b) => {
    ...
    if (editorialPubkeys.has(b.curator)) return true;
