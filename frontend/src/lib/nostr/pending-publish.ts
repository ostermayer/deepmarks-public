...
function templateKey(pubkey: string, template: UnsignedEventTemplate): string {
  ...
  if (template.kind === 5) {
    const targets = template.tags
      .filter((t) => t[0] === 'e' || t[0] === 'a' || t[0] === 'k')
      .map((t) => t.join('='))
      .sort()
      .join('|');
    return `${pubkey}:k5:${template.created_at}:${targets}`;
  }
...
/** Reactive count of queued-but-unpublished saves for the active user —
 *  drives the "waiting to sync" indicator so local-only state is visible
 *  instead of indistinguishable from relay-confirmed saves. */
export const pendingPublishes = writable(0);

function writeQueue(pubkey: string, items: PendingPublishItem[]): boolean {
  ...
  try {
    ...
    pendingPublishes.set(items.length);
    return true;
  } catch {
...
export function removePendingPublish(template: UnsignedEventTemplate, pubkey: string): void {
  if (!browser) return;
  const key = templateKey(pubkey, template);
  // Match the exact template, not just the dedupe key: a newer queued
  // edit shares the (kind, d-tag) key with the save it replaced, and the
  // older save's success acknowledgement must not delete it.
  const next = readQueue(pubkey).filter(
    (item) => item.key !== key || !sameTemplate(item.template, template),
  );
  writeQueue(pubkey, next);
}

function sameTemplate(a: UnsignedEventTemplate, b: UnsignedEventTemplate): boolean {
  return (
    a.kind === b.kind &&
...
/** Prime the reactive counter from disk (writeQueue keeps it fresh after
 *  that). Call on view mount / session change. */
export function refreshPendingPublishCount(pubkey: string | null | undefined): void {
  pendingPublishes.set(pubkey ? readQueue(pubkey).length : 0);
}

let draining = false;
let publishImpl: ((template: UnsignedEventTemplate, pubkey: string) => Promise<{ relays: string[] }>) | null = null;

/** Replaceable (d-tag) templates are state snapshots, not history — the
 *  relay keeps only the newest copy per d-tag. Replaying one with its
 *  original created_at after any newer save of the same key gets it
 *  silently discarded as "older" while the drain reports success. Stamp
 *  drain time so the queued state actually lands. */
export function drainTemplate(template: UnsignedEventTemplate): UnsignedEventTemplate {
  const hasDTag = template.tags.some((tag) => tag[0] === 'd');
  if (!hasDTag) return template;
  return {
    ...template,
    created_at: Math.max(Math.floor(Date.now() / 1000), template.created_at + 1),
  };
}
...
export async function drainPendingPublishes(pubkey: string): Promise<DrainResult> {
  ...
  // Offline drains can't succeed — don't burn the bounded attempt budget
  // on them (a tab left open on a flight used to expire queued saves).
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const remaining = readQueue(pubkey).length;
    pendingPublishes.set(remaining);
    return { attempted: 0, ok: 0, failed: 0, remaining };
  }
  draining = true;
  try {
    ...
    for (let i = 0; i < slice.length; i += DRAIN_CONCURRENCY) {
      ...
      const results = await Promise.all(batch.map(async (item) => {
        try {
          const res = await publishImpl!(drainTemplate(item.template), item.pubkey);
      ...
      for (const { item, success, error } of results) {
        if (success) {
          ...
          stillFailing.push({
            ...item,
            // A locked/disconnected signer is a precondition failure, not
            // a publish attempt — don't burn the bounded budget on it
            // (45 min of passkey-locked tab used to silently expire saves).
            attempts: /no signer/i.test(error ?? '') ? item.attempts : item.attempts + 1,
            lastError: error,
          });
        }
      }
