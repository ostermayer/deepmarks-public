...
export interface MuteList {
  ...
  baseEventId?: string;
  /** True only after a definitive load — see contacts.ts (2026-08-23
   *  review #10): mutations refuse to republish over an unloaded list. */
  loaded?: boolean;
}

const empty = (loaded = false): MuteList => ({
  pubkeys: new Set(),
  hashtags: new Set(),
  words: new Set(),
  loaded,
...
export async function loadMuteList(ownerPubkey: string): Promise<void> {
  try {
    ...
    if (!event) {
      // Connected relay answering "no event" = genuinely empty; a cold
      // pool proves nothing (see contacts.ts, 2026-08-23 review #10).
...
function assertMuteListLoaded(): void {
  let loaded = false;
  internal.subscribe((v) => { loaded = v.loaded === true; })();
  if (!loaded) {
    throw new Error('mute list could not be loaded — not publishing over it; try again in a moment');
  }
}
