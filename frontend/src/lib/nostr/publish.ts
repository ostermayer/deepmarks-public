...
export interface PublishEventOptions {
  /**
   * Called after the event itself and the NIP-98 request auth event have
   * both been signed, immediately before the network POST starts.
   */
    ...
    try {
      ...
      if (!readyNotified) {
        if (options.queueBeforePost) {
          enqueuePendingPublish(template, pubkey, 'queued before network publish');
          queuedBeforePost = true;
        }
