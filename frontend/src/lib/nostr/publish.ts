...
export async function publishAllQueued(
  templates: readonly UnsignedEventTemplate[],
  pubkey: string,
  options: Omit<PublishEventOptions, 'queueBeforePost' | 'onReadyToPost'> = {},
): Promise<PublishResult[]> {
  if (templates.length === 0) return [];
  return Promise.all(templates.map((template) => publishEventQueued(template, pubkey, options)));
}
