...
// Tag schema follows the Deepmarks NIP-B0 bookmark conventions.
...
/** Adapt an NDKEvent (optional kind/created_at) to SignedEventLike.
 *  Use this instead of `as unknown as SignedEventLike`, which silently
 *  lets undefined fields through the type. */
...
/** Non-throwing render guard: returns the URL only if it's a safe http(s)
 *  link, otherwise '#'. Use at EVERY place a bookmark URL becomes an
 *  `<a href>`. Only the relay-parse path enforces http(s); the API /
 *  search / public-cache conversions type `url` as a bare string, so a
 *  kind:39701 published straight to the relay with a `javascript:` /
 *  `data:` d-tag could otherwise reach an href and execute in our origin —
 *  which can read the localStorage nsec (account takeover). */
...
// The canonical replaceable-conflict rule lives in the parity-copied
// bookmark-merge-core.ts (shared byte-identically with the extension);
// re-exported here because bookmarks.ts is the hub its 12 consumers
// already import from.
export {
  bookmarkSortTimeMs,
  ...
  type MergeableBookmark,
} from './bookmark-merge-core.js';
