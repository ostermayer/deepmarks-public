 ...
 * one answer shared by the relay feed and the own-bookmarks store, and
 * mirrored by the extension's popup merge. Three implementations of
 * this policy once drifted apart (different tie-break directions);
 * everything now delegates here. Tiers:
 ...
 */
export function replaceableBookmarkWins(existing: ParsedBookmark, incoming: ParsedBookmark): boolean {
  if (
...
export function mergeReplaceableBookmark(
