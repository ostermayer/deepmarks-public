...
const missingProfileCache = createCachedKv<true>({
  prefix: 'deepmarks-profile-missing',
  version: 'v1',
  ttlMs: 6 * 60 * 60_000,
});
// Names, handles, and avatars change rarely. Keep the localStorage prime
// fresh for a week so friend-feed rows do not re-open profile lookups on
// every normal app session; explicit profile edits still call
// invalidateProfile() and bypass this interval.
...
export function getProfile(pubkey: string): Readable<Profile | null> {
  ...
  const persisted = profileCache.loadWithMeta(pubkey);
  if (persisted) store.__set(persisted.value);
  const missing = missingProfileCache.loadWithMeta(pubkey);
  const shouldFetch = !browser || shouldRefreshProfile(persisted?.at ?? missing?.at);
  const promise = (shouldFetch
    ? (browser ? deferProfileFetch(pubkey, store) : fetchAndPopulate(pubkey, store))
...
/** Synchronously read profile data already present in memory/localStorage. */
export function peekProfile(pubkey: string): Profile | null {
  const current = cache.get(pubkey);
  if (current) {
    const profile = get(current.store);
    if (profile) return profile;
  }
  return profileCache.load(pubkey);
}

/** Seed the profile cache from trusted server-side joins such as
 * /account/contacts, then update any live stores already mounted. */
export function primeProfileCache(profiles: Iterable<Profile | null | undefined>): void {
  for (const profile of profiles) {
    if (!profile?.pubkey) continue;
    const pubkey = profile.pubkey.toLowerCase();
    if (!hasRenderableProfile(profile)) continue;
    const normalized = {
      ...profile,
      pubkey,
      picture: isLikelyImageUrl(profile.picture) ? profile.picture : undefined,
    };
    profileCache.save(pubkey, normalized);
    missingProfileCache.remove(pubkey);
    const hit = cache.get(pubkey);
    if (hit) hit.store.__set(normalized);
  }
}
...
function shouldRefreshProfile(cachedAt: number | undefined): boolean {
  return cachedAt === undefined || Date.now() - cachedAt > PROFILE_REFRESH_MS;
}

function hasRenderableProfile(profile: Profile): boolean {
  return !!(
    profile.displayName
    || profile.name
    || profile.picture
...
async function fetchAndPopulate(pubkey: string, store: ProfileStore): Promise<void> {
  ...
  if (!event) {
    if (!get(store)) missingProfileCache.save(pubkey, true);
    return;
  }
  const profile = parseProfileContent(event.content, pubkey);
  store.__set(profile);
  if (profile && hasRenderableProfile(profile)) {
    profileCache.save(pubkey, profile);
    missingProfileCache.remove(pubkey);
  }
}
