...
import { api, type ArchiveStatus } from '$lib/api/client.js';
...
export async function enqueueArchivePage(opts: {
  ...
  try {
    ...
    if (archiveKey) stashPendingArchiveKey(result.jobId, archiveKey);
...
export async function* archivePage(opts: {
  ...
  while (Date.now() < deadline) {
    ...
    if (status.state === 'done') {
      ...
      // localStorage stash and reconcileArchiveKeys will retry on the
      // next /app/archives mount.
      if (archiveKey && status.blossomHash) {
        try {
          await addArchiveKeyToSet(status.blossomHash, archiveKey, opts.pubkey);
