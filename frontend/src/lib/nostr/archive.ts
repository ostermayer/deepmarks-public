...
import { recordArchiveFailureBackoff } from './archive-backoff.js';
import {
  addArchiveKeyToSet,
...
export async function enqueueArchivePage(opts: {
  ...
  try {
    ...
    // The server's terminal-failure cooldown answered instead of enqueueing —
    // remember that locally so backfill passes stop re-asking until the
    // escalating window has a chance of having lapsed.
    if (result.jobId === 'queued:recent-failure' || result.jobId === 'queued:permanent-failure') {
      recordArchiveFailureBackoff(opts.pubkey, opts.url);
    }
...
/** Client-side mirror of the server's static URL gate (api parseAllowedUrl /
 *  validateSafePublicHttpUrl minus DNS): non-http(s), IP literals,
 *  single-label hosts, and localhost bookmarks would 400 at the enqueue API
 *  anyway — filtering here keeps them from burning backfill queue slots and
 *  showing up as spurious failures. */
export function isServerArchivableUrl(rawUrl: string): boolean {
  let u: URL;
  try {
    u = new URL(rawUrl);
  } catch {
  ...
  if (!host || host.includes(':')) return false; // bracketed IPv6 literal
  if (/^(\d{1,3}\.){3}\d{1,3}$/.test(host)) return false; // IPv4 literal
...
export function releaseFailedArchiveQueueSlots(pubkey: string): number {
  ...
  for (const [url, entry] of Object.entries(map)) {
    ...
    delete map[url];
    released += 1;
    // Releasing the slot is what lets a later pass retry the URL — feed the
    // failure into the backoff ledger so that retry waits out the escalating
    // window instead of firing on the very next pass.
    recordArchiveFailureBackoff(pubkey, url);
  }
  if (released > 0) saveQueuedArchiveMap(pubkey, map);
  return released;
}

export function rememberArchiveQueueFailure(pubkey: string, url: string, error: string, jobId?: string): void {
  if (typeof localStorage === 'undefined') return;
  const map = loadQueuedArchiveEntries(pubkey);
  map[url] = {
...
export function forgetArchiveQueueUrls(pubkey: string, urls: Iterable<string>): number {
  if (typeof localStorage === 'undefined') return 0;
  const map = loadQueuedArchiveEntries(pubkey);
  let removed = 0;
  for (const url of urls) {
    if (!(url in map)) continue;
    delete map[url];
    removed += 1;
  }
  if (removed > 0) saveQueuedArchiveMap(pubkey, map);
  return removed;
}
    ...
    try {
      ...
      if (status.state === 'done') {
        ...
        map[url] = {
          ...
          state: 'failed',
          lastCheckedAt: Date.now(),
          error: status.message ?? status.error ?? 'archive job failed',
        };
        failed += 1;
      } else {
        map[url] = {
...
export async function* archivePage(opts: {
  ...
  while (Date.now() < deadline) {
    ...
    if (status.state !== lastState) {
      yield {
        state: status.state,
        detail: status.state === 'failed' ? (status.message ?? status.error) : undefined
      };
    ...
    if (status.state === 'failed') {
      throw new Error(status.message ?? status.error ?? 'archive job failed');
    }
