...
// frontend/src/lib/nostr/nwc.ts and browser-extension/src/lib/nwc.ts,
// guarded by tests/frontend/regression/shared-core-parity.test.ts.
// Each surface resolves `./nwc-store.js` to its own connection store
// (same exported shape), and both run NWC traffic on a local
// SimplePool — the channel is single-relay and short-lived, which
// keeps the wallet relay isolated from the user's bookmark relays.
//
