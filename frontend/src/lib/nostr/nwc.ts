...
//
// Mirrors browser-extension/src/lib/nwc.ts. Same wire protocol, same
// timeout policy, same client-side preimage verification. Diffs:
//
//   - Uses the SvelteKit-side NwcConnection shape from ./nwc-store.ts
//   - Operates on a local SimplePool (NWC traffic is single-relay
//     and short-lived, so we don't share NDK's pool — keeps the
//     wallet relay isolated from the user's bookmark relays)
//
...
// the message — wallets typically write user-friendly reasons like
// "INSUFFICIENT_BALANCE" / "RATE_LIMITED" / "QUOTA_EXCEEDED" which
// the UI can render verbatim.

import { finalizeEvent, nip04, SimplePool, type Event as NostrEvent } from 'nostr-tools';
...
export async function payInvoiceWith(
  ...
  // SimplePool isn't reused — the NWC channel is single-relay and
  // short-lived per request. Cleanup is done in the finally block so
  // a thrown await doesn't leak a subscription.
    ...
    const responsePromise = new Promise<NostrEvent>((resolve, reject) => {
      const sub = pool.subscribeMany(
        [conn.relayUrl],
        {
          ...
          '#e': [event.id],
          // Don't require #p here. NIP-47 says response events SHOULD
          // include it, but the encrypted response plus request #e and
          // wallet author are enough to validate the reply.
