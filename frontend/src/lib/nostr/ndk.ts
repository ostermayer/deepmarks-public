...
/** Best-effort: whether the pool currently has at least one connected
 *  relay. Replaceable-list loaders (contacts/friends/mute) use this to
 *  distinguish "a relay answered: no event exists" from "cold pool
 *  answered nothing" before treating a null fetch as an empty list — a
 *  cold-pool null used to let follow()/mutePubkey() republish a
 *  near-empty replaceable list over the user's real one (2026-08-23
 *  review #10). Unknown API shape defers to the old behavior (true)
 *  rather than deadlocking mutations. */
export function hasConnectedRelays(): boolean {
  try {
    const pool = (getNdk() as unknown as { pool?: { connectedRelays?: () => unknown[] } }).pool;
    if (!pool || typeof pool.connectedRelays !== 'function') return true;
    return pool.connectedRelays().length > 0;
  } catch {
    return true;
  }
}
