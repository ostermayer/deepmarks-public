...
// Conflicts resolve via the canonical rule in bookmarks.ts
// (replaceableBookmarkWins): newest wins, ties keep the LOWEST event id
// to match NIP-01 relay retention.
...
/** Pure decision: should `incoming` replace `existing`? Delegates to
 *  THE canonical rule in bookmarks.ts — see replaceableBookmarkWins. */
export function shouldReplace(existing: Entry, incoming: ParsedBookmark): boolean {
  return replaceableBookmarkWins(existing.bookmark, incoming);
}

const mergeReplacement = mergeReplaceableBookmark;
  ...
  return readable<ParsedBookmark[]>(initial, (set) => {
    ...
    try {
      ...
      sub.on('event', (event: NDKEvent) => {
        const signed = ndkEventAsSigned(event);
        if (signed.kind === KIND.deletion) {
          const observed = rememberBookmarkDeletion(deletionMemory, signed);
          if (applyBookmarkDeletion(byKey, signed)) emit();
          notifyDeletionObservers(observed);
          return;
        }
        const parsed = parseBookmarkEvent(signed);
        if (!parsed) return;
