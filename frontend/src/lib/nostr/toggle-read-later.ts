 ...
 * background and reverts the optimistic update only when we fail before
 * the signed event reaches the durable retry queue.
 */
export function toggleReadLater(
  ...
  const isPrivate = isPrivateBookmark(bookmark);
  ...
  const publishedAt = bookmark.publishedAt ?? bookmark.savedAt;
  const editCreatedAt = Math.max(
    Math.floor(Date.now() / 1000),
    (bookmark.eventCreatedAt ?? bookmark.savedAt) + 1,
  );
  ...
  // this durable; if it fails we revert via the snapshot-rollback —
  // re-remembering the ORIGINAL was a no-op revert because the merge
  // keeps the newest eventCreatedAt, i.e. the failed optimistic copy
  // (2026-08-23 review).
  const rollbackOptimistic = rememberOwnBookmarkWithRollback(optimistic, !isPrivate);
  ...
  const input = {
    ...
    publishedAt,
    publishedAtMs: bookmark.savedAtMs,
  };
  ...
  const publish = (async () => {
    try {
      ...
      if (isPrivate) {
        ...
        template.created_at = editCreatedAt;
        const result = await publishEventQueued(template, pubkey, { failureSubject: 'bookmark' });
        eventId = result.eventId;
      ...
      // Revert only when signing/encryption failed before a publishable
      // event reached the durable queue.
      rollbackOptimistic();
      throw err;
    }
  })();
