...
// map of { blobHash|job:<jobId>: archiveKeyPlaintextBase64 }. Encrypted to self —
...
export interface ArchiveKeyPlaintextChunk {
  plaintext: string;
  index: number;
  versionMatches: boolean;
}
...
export async function getArchiveKeyMap(
  ...
  const parsedSetCount = Number(tagValue(chunkZero.tags, SET_COUNT_TAG) ?? 1);
  const setCount = Number.isInteger(parsedSetCount) && parsedSetCount > 0 ? parsedSetCount : 1;
  ...
  const allChunks: ArchiveKeyChunkRef[] = [{
    tags: chunkZero.tags,
  ...
  if (setCount > 1) {
    ...
    for (const event of extra) {
      ...
      // A relay rate-limit or network interruption can leave chunk 0
      // from a newer generation alongside stale later chunks. Keep the
      // stale chunks as fallback because archive keys are immutable per
      // blob hash, but merge matching-version chunks first below.
      const versionMatches = !setVersion || tagValue(event.tags, SET_VERSION_TAG) === setVersion;
      allChunks.push({
        tags: event.tags,
  ...
  const plaintextChunks: ArchiveKeyPlaintextChunk[] = [];
  for (const chunk of allChunks) {
    let plaintext: string;
    try {
      plaintext = await withTimeout(
        ndk.signer.decrypt(me, chunk.content, 'nip44'),
        SIGNER_OP_TIMEOUT_MS,
        'archive key decrypt',
      );
    } catch (err) {
      // A timed-out remote signer will hang every remaining chunk the
      // same way — fail the whole read instead of burning the timeout
      // budget per chunk with the UI frozen.
      if (err instanceof TimeoutError) throw err;
      // Otherwise skip this chunk — bad ciphertext or signer-rejected.
      // Other chunks still contribute their keys instead of nuking the
      // whole map.
      continue;
    }
    plaintextChunks.push({
      plaintext,
      index: chunk.index,
      versionMatches: chunk.versionMatches,
    });
  }

  const merged = mergeArchiveKeyPlaintextChunks(plaintextChunks);
  cached = { pubkey: ownPubkey, map: merged };
  return merged;
}

export function mergeArchiveKeyPlaintextChunks(chunks: ArchiveKeyPlaintextChunk[]): ArchiveKeyMap {
  const merged: ArchiveKeyMap = {};
  const ordered = [...chunks].sort((a, b) => {
    if (a.versionMatches !== b.versionMatches) return a.versionMatches ? -1 : 1;
    return a.index - b.index;
  });
  for (const chunk of ordered) {
    try {
      const parsed = JSON.parse(chunk.plaintext) as unknown;
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) continue;
      for (const [k, v] of Object.entries(parsed as Record<string, unknown>)) {
        if (typeof k === 'string' && typeof v === 'string' && !(k in merged)) merged[k] = v;
      }
...
async function publishArchiveKeyMap(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const ciphertext = await withTimeout(
      ndk.signer.encrypt(me, JSON.stringify(chunks[i]!), 'nip44'),
      SIGNER_OP_TIMEOUT_MS,
      'archive key encrypt',
    );
    ...
    const result = await publishEventQueued(
      {
      ...
      ownPubkey,
      { failureSubject: 'archive key sync' },
    );
...
interface PendingMap {
  // jobId → { archiveKey: base64, createdAt: unix-seconds }
  [jobId: string]: { archiveKey: string; createdAt: number; publishedAt?: number };
}
...
/** Stash a freshly-generated archive key under its jobId after the
 *  archive has been accepted. Survives tab close. Once the archive completes
...
export function stashPendingArchiveKey(jobId: string, archiveKey: string): void {
  const map = readPendingMap();
  map[jobId] = { archiveKey, createdAt: Math.floor(Date.now() / 1000) };
  writePendingMap(map);
}

export async function publishPendingArchiveKey(
  jobId: string,
  archiveKey: string,
  ownPubkey: string,
): Promise<void> {
  const jobKey = archiveKeyJobMapKey(jobId);
  if (!jobKey) return;
  await addArchiveKeyToSet(jobKey, archiveKey, ownPubkey);
  // Stamp the stash so the sweep knows this key is recoverable from
  // relays — only published entries are safe to bound by age.
  const map = readPendingMap();
  if (map[jobId] && !map[jobId].publishedAt) {
    map[jobId] = { ...map[jobId], publishedAt: Math.floor(Date.now() / 1000) };
    writePendingMap(map);
  }
}

/** Probe the signer's NIP-44 support with a tiny self-encrypt. Private
 *  archives are unrecoverable cross-device without a published key wrap,
 *  so an enqueue from a signer that can't encrypt must fail loudly
 *  instead of orphaning the key in this device's localStorage. */
export async function assertArchiveKeySignerReady(ownPubkey: string): Promise<void> {
  const ndk = getNdk();
  if (!ndk.signer) throw new Error('No signer attached. Sign in first.');
  const me = ndk.getUser({ pubkey: ownPubkey });
  try {
    await withTimeout(
      ndk.signer.encrypt(me, 'deepmarks-nip44-probe', 'nip44'),
      SIGNER_OP_TIMEOUT_MS,
      'archive key encrypt probe',
    );
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    if (/nip-?44/i.test(message) && /not (?:available|supported)/i.test(message)) {
      throw new Error(
        'Your signer does not support NIP-44 encryption, which private archives need to sync their keys. ' +
        'Use a signer with NIP-44 support (or your nsec) and try again.',
      );
    }
    if (err instanceof TimeoutError) {
      throw new Error('Your remote signer did not respond — check that it is online and try again.');
    }
    throw err;
  }
}
...
export function getPendingArchiveKey(jobId: string): string | null {
  return readPendingMap()[jobId]?.archiveKey ?? null;
}

/** Drop a stashed key once it's been promoted to the NIP-51 set. */
export function clearPendingArchiveKey(jobId: string): void {
  const map = readPendingMap();
  if (jobId in map) {
    delete map[jobId];
    writePendingMap(map);
...
export async function reconcileArchiveKeys(
  ...
  // Retry the relay-side wrap for stash entries that never confirmed a
  // publish (relay down / signer asleep at enqueue time). Without this
  // the only copy of the key lives in THIS device's localStorage.
  let republished = 0;
  for (const [jobId, entry] of Object.entries(next)) {
    if (entry.publishedAt || republished >= 5) continue;
    try {
      await publishPendingArchiveKey(jobId, entry.archiveKey, ownPubkey);
      next[jobId] = { ...entry, publishedAt: Math.floor(Date.now() / 1000) };
      republished++;
    } catch {
      // Still unpublished — keep the stash; a future pass retries.
    }
  }

  // Sweep: a PUBLISHED stash entry is just a cache of the relay-side
  // wrap — safe to age out after 14d. An UNPUBLISHED entry is the only
  // copy of the key in existence; keep it much longer (90d hard cap to
  // bound storage for jobs the proxy expired or refunded).
  let abandoned = 0;
  const nowSeconds = Math.floor(Date.now() / 1000);
  const publishedCutoff = nowSeconds - 14 * 24 * 60 * 60;
  const unpublishedCutoff = nowSeconds - 90 * 24 * 60 * 60;
  for (const [jobId, entry] of Object.entries(next)) {
    const cutoff = entry.publishedAt ? publishedCutoff : unpublishedCutoff;
  ...
  if (reconciled > 0 || abandoned > 0 || republished > 0) writePendingMap(next);
