...
async function buildPrivateSetEvent(
  set: PrivateSet,
  ownerPubkey: string,
  setName = SET_NAME,
...
/** Number of replaceable chunk events a whole-set rewrite publishes.
 *  Covers every previously-used chunk name (not just what the entries
 *  need) so stale higher-index chunks get overwritten with empty sets
 *  instead of resurrecting deleted bookmarks. */
export function privateSetReplacementCount(
  entries: string[][][],
  ...
  return Math.max(chunkPrivateSetEntries(entries).length, previousMaxIndex + 1, 1);
}
...
export async function* buildPrivateSetReplacementEventStream(
  ...
  const count = privateSetReplacementCount(set.entries, previousChunkNames);
 ...
 *  Only the bulk importer still rewrites the WHOLE chunked set; building
 *  that rewrite on a partially-decrypted fetch silently erases whatever
 *  the unreadable chunks held — on every device, not just this one. */
...
export async function addToPrivateSet(
  ...
  return buildPrivateItemEvent(input, ownerPubkey);
}

/**
 * Delete the private bookmark for `url` by publishing its encrypted
 * per-item tombstone — NIP-09 kind:5 is the wrong primitive because the
 * bookmark data lives *inside* replaceable kind:30003 events. Every
 * reader's merge drops a URL whose newest record is a tombstone.
 * Idempotent: deleting an absent URL is a no-op success.
 */
export async function removeFromPrivateSet(
  ...
  return buildPrivateItemTombstoneEvent(url, ownerPubkey);
}

/**
 * Replace the private bookmark matching `input.url` with a fresh entry
 * built from `input`. Used by the archive flow to stamp `blossom` +
 * `archive-tier:forever` tags onto a previously-saved private bookmark.
 * Same per-item write as `addToPrivateSet` — the d-tag derives from the
 * URL, so the newer event replaces the older one in every reader's
 * merge, including over stale chunk copies. If no entry exists for the
 * URL the input is simply added. The alias is kept for caller intent.
 */
export const updatePrivateSetEntry = addToPrivateSet;
