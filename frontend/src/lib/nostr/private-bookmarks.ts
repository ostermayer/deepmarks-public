...
export async function fetchOwnPrivateSet(
  ...
  for (const event of events) {
    ...
    if (!result.ok) decryptFailures += 1;
...
function assertPrivateSetRewriteSafe(set: PrivateSet): void {
  if (set.decryptFailures && set.decryptFailures > 0) {
    throw new Error(
      ...
      'not rewriting the set. Reconnect your signer and try again.',
    );
  }
}
...
export async function addToPrivateSet(
  ...
  const entries = withLocalPrivateCacheEntries(set.entries, ownerPubkey, set.deletedUrls);
...
export async function updatePrivateSetEntry(
  ...
  const set = await fetchOwnPrivateSet(ownerPubkey);
  assertPrivateSetRewriteSafe(set);
  const next = withLocalPrivateCacheEntries(set.entries, ownerPubkey, set.deletedUrls);
...
function withLocalPrivateCacheEntries(
  entries: string[][][],
  ownerPubkey: string,
  deletedUrls: Record<string, number> = {},
): string[][][] {
  ...
  //
  // Tombstone guard: a URL deleted on ANOTHER device still sits in this
  // device's cache. Re-injecting it would republish it in a chunk whose
  // created_at beats the tombstone — resurrecting the deleted bookmark
  // everywhere. Skip cache extras whose save time predates the newest
  // tombstone for that URL.
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          for (const b of parsed) {
            ...
            const deletedAt = deletedUrls[b.url];
            const cachedAt = typeof b.savedAt === 'number'
              ? b.savedAt
              : typeof b.publishedAt === 'number'
                ? b.publishedAt
                : 0;
            if (deletedAt !== undefined && deletedAt >= cachedAt) continue;
