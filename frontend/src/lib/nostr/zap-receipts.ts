...
export function watchZapReceipts(
  ...
  const verifiableInvoices = invoices.filter((inv) => inv.zapReceiptVerifiable !== false);
  const invoiceByKey = new Map(verifiableInvoices.map((inv) => [invoiceKey(inv.invoice), inv]));
  ...
  if (!invoiceByKey.size) {
    callbacks.onStatus('zap receipt unavailable from this Lightning provider');
    return { stop };
  }
