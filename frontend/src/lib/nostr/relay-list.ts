...
import type { RelayConfig } from '$lib/stores/user-settings';
...
// Relays used solely to fetch a user's NIP-65 advertised relay list
// (kind:10002). After discovery, NDK uses the user's own list for
// outbox routing. Kept short on purpose — every URL here opens a
// WebSocket and adds startup noise. The previous broader set
// (snort.social, nostr.band, relayable.org, current.fyi, nostr.bg,
// wellorder.net, eden.nostr.land, etc.) included relays that are
// dead, geo-blocked, or routinely closing connections — they
// produced a stream of WebSocket-closed errors on iOS without ever
// returning a kind:10002.
...
export function getRelayList(pubkey: string): Readable<RelayList | null> {
  ...
  const promise = (async () => {
    const ndk = getNdk();
    const relaySet = NDKRelaySet.fromRelayUrls(RELAY_DISCOVERY_RELAYS, ndk, true);
    const event = await ndk.fetchEvent({ kinds: [10002], authors: [pubkey] }, {}, relaySet);
    if (!event) return;
...
/**
 * Publish a fresh kind:10002 advertising the current relay config to
 * other Nostr clients. Lets a user's NIP-65 list stay in sync with
 * their Deepmarks settings when they ask for it — we don't publish
 * automatically since signing prompts and per-edit publishes get
 * annoying on bunker/extension signers.
 */
