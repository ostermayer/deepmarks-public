...
export async function maybeBackfill(
  ...
  try {
    ...
    clearArchiveBackoff(state.pubkey, archivedUrls);
    ...
    if (!archiveByDefault) {
      ...
      archiveBackfillStatus.set({
        ...
        message: archiveCheckSummary(0, 0, refreshed, retryableFailed, missingKeyRetry, mediaQueued, queueStatus) || 'archive-all setting is off',
        updatedAt: Date.now(),
      });
    ...
    const candidates = bookmarks
      .filter((bookmark) => {
        ...
        // The URL failed terminally not long ago (locally observed failure or
        // a server `queued:recent-failure` sentinel) — don't even send the
        // request until the escalating backoff window has lapsed.
        if (isArchiveBackoffActive(state.pubkey!, bookmark.url)) {
          backedOff += 1;
          return false;
        }
        if (bookmark.blossomHash || bookmark.waybackUrl) return false;
        // Same static gate the enqueue API applies — a private/intranet URL
        // would 400 there after consuming a client queue slot.
        return isServerArchivableUrl(bookmark.url);
      })
    ...
    if (candidates.length === 0) {
      ...
      archiveBackfillStatus.set({
        ...
        pubkey: state.pubkey,
        totalMissing: alreadyQueued + backedOff,
        queued: 0,
        skipped: alreadyQueued + backedOff,
        failed: 0,
        ...queueStatusFields(queueStatus),
        message: archiveCheckSummary(alreadyQueued, backedOff, refreshed, retryableFailed, missingKeyRetry, mediaQueued, queueStatus) || 'no missing archives found',
        updatedAt: Date.now(),
      });
    ...
    if (serverOutstanding >= MAX_QUEUE_PER_RUN) {
      ...
      archiveBackfillStatus.set({
        state: 'queued',
        pubkey: state.pubkey,
        totalMissing: candidates.length + alreadyQueued + backedOff + serverOutstanding,
        queued: 0,
    ...
    let firstFailure = '';
    let queued = 0;
    let skipped = alreadyQueued + backedOff;
    const slice = candidates.slice(0, MAX_QUEUE_PER_RUN);
    archiveBackfillStatus.set({
      state: 'queueing',
      pubkey: state.pubkey,
      totalMissing: candidates.length + alreadyQueued + backedOff,
      queued,
      skipped,
      failed: failures,
      ...queueStatusFields(queueStatus),
...
function archiveCheckSummary(
  ...
  if (backedOff > 0) parts.push(`${backedOff} failed archive${backedOff === 1 ? '' : 's'} waiting out retry backoff`);
