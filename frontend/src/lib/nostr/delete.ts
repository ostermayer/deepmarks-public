...
// then we POST it through /publish so the server forwards to relay.deepmarks
// and fans it out to the user's NIP-65 relays. Relays are not required to
// honor kind:5 — copies on well-behaved nodes vanish, others keep the events.
// The settings UI makes this caveat clear.

import type { NDKFilter } from '@nostr-dev-kit/ndk';
import { getNdk } from './ndk.js';
import { KIND } from './kinds.js';
import { publishEvent, publishEventQueued, type PublishEventOptions } from './publish.js';
...
export async function publishBookmarkDeletion(params: {
  ...
  const { pubkey, eventId, url, reason, publishOptions } = params;
  const tags: string[][] = [
    ['e', eventId],
  ...
  const result = publishOptions?.queueBeforePost
    ? await publishEventQueued(template, pubkey, withoutQueueBeforePost(publishOptions))
    : await publishEvent(template, pubkey, publishOptions);
  return {
    deletionEventId: result.eventId,
    relays: result.relays,
  };
...
function withoutQueueBeforePost(
  options: PublishEventOptions,
): Omit<PublishEventOptions, 'queueBeforePost'> {
  const { queueBeforePost: _queueBeforePost, ...rest } = options;
  return rest;
}
