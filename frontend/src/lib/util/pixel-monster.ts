...
function pick<T>(arr: readonly T[], seed: number): T {
  ...
  if (value === undefined) throw new Error('computed pick index was out of range');
  return value;
}
...
export function buildPixelMonster(pubkey: string): PixelMonster {
  ...
  const bodyIdx = hexNibble(seed.charAt(0));
  const bodyColor = pick(PALETTE_BODY, hexNibble(seed.charAt(1)));
  const eyeColor = pick(PALETTE_EYE, hexNibble(seed.charAt(2)));
  const bgColor = pick(PALETTE_BG, hexNibble(seed.charAt(3)));

  const cells = parseTemplate(pick(BODIES, bodyIdx));
