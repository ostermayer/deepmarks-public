  ...
  const stop2 = canSign.subscribe((cs) => {
    ...
    void api.archives
      ...
      .then(async (records) => {
        ...
        if (!pubkey) return;
        // Promote any pending archive keys stashed at archive-creation
        // time into the chunked NIP-51 set so other devices can
        // decrypt the user's private archives. Previously this only
        // ran when the user opened /app/archives directly — visiting
        // any other page left the stash unpublished and the lock
        // icon stuck with "no decryption key" on every other surface.
        const { reconcileArchiveKeys } = await import('$lib/nostr/archive-keys');
        await reconcileArchiveKeys(records, pubkey).catch(() => { /* tolerable */ });
      })
