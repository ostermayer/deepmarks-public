...
import { api, type ArchiveFailureRecord, type ArchiveRecord } from '$lib/api/client';
...
const failuresByUrl = writable<Map<string, ArchiveFailureRecord>>(new Map());
...
/** Owner-only terminal archive failures keyed like myArchives. Lets rows
 * show "site blocked" even after a reload. */
export const myArchiveFailures: Readable<Map<string, ArchiveFailureRecord>> = {
  subscribe: failuresByUrl.subscribe,
};
...
export function rememberArchiveRecord(pubkey: string | null | undefined, rec: ArchiveRecord): void {
  ...
  failuresByUrl.update((current) => {
    const next = new Map(current);
    for (const key of archiveRecordLookupKeys(rec)) next.delete(key);
    return next;
  });
...
export function replaceMyArchiveRecords(
  pubkey: string | null | undefined,
  records: ArchiveRecord[],
  failures: ArchiveFailureRecord[] = [],
): void {
  const map = archiveRecordsByLookupKey(records);
  recordsByUrl.set(map);
  failuresByUrl.set(archiveFailuresByLookupKey(failures, map));
...
async function refreshMyArchiveRecords(pubkey: string, seq: number): Promise<void> {
  ...
  const failures = new Map<string, ArchiveFailureRecord>();
  const limit = 1000;
  try {
    for (let offset = 0; offset <= 50_000; offset += limit) {
      ...
      for (const failure of page.failures ?? []) failures.set(failure.jobId, failure);
      mergeMyArchiveRecords(pubkey, page.archives);
      mergeMyArchiveFailures([...failures.values()]);
    ...
    if (seq !== archiveFetchSeq || pubkey !== lastFetchedPubkey) return;
    replaceMyArchiveRecords(pubkey, records, [...failures.values()]);
  } catch {
...
function mergeMyArchiveFailures(failures: ArchiveFailureRecord[]): void {
  if (failures.length === 0) return;
  failuresByUrl.update((current) => {
    const next = new Map(current);
    const successes = new Set(recordsByUrlSnapshotKeys());
    for (const failure of failures) {
      for (const key of archiveLookupKeys(failure.url)) {
        if (!successes.has(key)) upsertFailureByKey(next, key, failure);
      }
    }
    return next;
  });
...
function upsertArchiveRecord(
  ...
  for (const key of archiveRecordLookupKeys(rec)) upsertArchiveByKey(map, key, rec);
}
...
function archiveFailuresByLookupKey(
  failures: ArchiveFailureRecord[],
  successfulArchives: Map<string, ArchiveRecord>,
): Map<string, ArchiveFailureRecord> {
  const map = new Map<string, ArchiveFailureRecord>();
  for (const failure of failures) {
    for (const key of archiveLookupKeys(failure.url)) {
      if (!successfulArchives.has(key)) upsertFailureByKey(map, key, failure);
    }
  }
  return map;
}

function upsertFailureByKey(
  map: Map<string, ArchiveFailureRecord>,
  key: string | undefined,
  failure: ArchiveFailureRecord,
): void {
  if (!key) return;
  const existing = map.get(key);
  if (!existing || compareArchiveFailuresNewest(failure, existing) < 0) map.set(key, failure);
}
...
function recordsByUrlSnapshotKeys(): string[] {
  let keys: string[] = [];
  recordsByUrl.subscribe((map) => { keys = [...map.keys()]; })();
  return keys;
}
...
function archiveRecordLookupKeys(rec: ArchiveRecord): string[] {
  const keys = new Set<string>();
  for (const key of archiveLookupKeys(rec.url)) keys.add(key);
  for (const key of archiveLookupKeys(rec.originalUrl)) keys.add(key);
  return [...keys];
}

/** Same `yt:<lowercase id>` key format the server's videoContentKey
 *  emits; the shared parser covers every URL form incl. nocookie. */
...
function compareArchiveFailuresNewest(a: ArchiveFailureRecord, b: ArchiveFailureRecord): number {
  const timeline = archiveFailureTimelineSeconds(b) - archiveFailureTimelineSeconds(a);
  if (timeline !== 0) return timeline;
...
function archiveFailureTimelineSeconds(rec: ArchiveFailureRecord): number {
  return rec.bookmarkSavedAt && rec.bookmarkSavedAt > 0 ? rec.bookmarkSavedAt : rec.failedAt;
}
