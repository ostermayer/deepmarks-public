...
function load(): UserSettings {
  ...
  try {
    ...
    return {
      ...
      defaultVisibility: parsed.defaultVisibility === 'public' ? 'public' : 'private',
...
export function pendingLocalSettingsShouldWin(
  ...
  // The API stores second-resolution timestamps, so a remote value with
  // the same timestamp can still be older than a just-saved local change.
