...
function clearStoredNsec() {
  ...
  // Also wipe the iOS share extension's Keychain copy so a signed-out
  // (or switched) account can never be published under from the share
  // sheet. No-op outside iOS native.
  mirrorSharedNsec(null);
}
...
/**
 * Mirror the user's nsec into the iOS shared Keychain so the
 * DeepmarksShare extension can sign + POST /publish on its own — no
 * main-app foreground required. No-op outside iOS native (the JS bridge
 * stubs to nothing on web / Android). Pass null to wipe the copy.
 *
 * This is deliberately decoupled from WebView localStorage: native
 * sign-in keeps the nsec out of localStorage (it lives in the native
 * secure store instead), but the share extension reads its own Keychain
 * copy, so it must be kept in sync regardless of the localStorage policy.
 */
function mirrorSharedNsec(nsecHex: string | null) {
  if (!browser) return;
  void (async () => {
    try {
...
// Bumped by logout/clearSigner to invalidate an in-flight rehydrate().
let sessionEpoch = 0;
...
export const session = {
  ...
  async login(signer: ResolvedSigner, options: LoginOptions = {}) {
    ...
    } else if (signer.kind === 'nsec' && signer.nsecHex) {
      // Native sign-in (persistNsec:false) keeps the nsec out of WebView
      // localStorage — it lives in the native secure store instead. But
      // the iOS share extension reads its own shared-Keychain copy, so
      // still mirror the key there; otherwise the extension can never
      // publish on its own and every mobile share is forced to open the
      // app to finish saving.
      mirrorSharedNsec(signer.nsecHex);
      clearStoredNip46Payload();
      clearStoredAndroidSignerPayload();
  ...
  async logout() {
    // Invalidate any in-flight rehydrate(): without this, a restore that
    // was mid-await when the user logged out resumed afterward, re-wrote
    // the nsec into the iOS shared Keychain, and signed the session back
    // in — violating the cleared-on-logout contract (2026-08-23 review
    // #6, security).
    sessionEpoch += 1;
    writeHint(null);
    clearStoredNsec();
  ...
  async rehydrate() {
    ...
    // signer has appeared while we were async-waiting. The epoch check
    // covers the OPPOSITE race: a logout (or explicit signer clear)
    // during the restore bumps sessionEpoch, so the resumed restore must
    // not re-mirror the nsec or re-attach the signer (2026-08-23 #6).
    const epoch = sessionEpoch;
    const hasSignerNow = () => sessionEpoch !== epoch || get(internal).signer !== null;
      ...
      if (hint.kind === 'nip07' && isNip07Available()) {
        ...
        if (nsecHex) {
          try {
            ...
            // Re-sync the iOS share extension's Keychain copy on every
            // launch so a key written before this mirror existed (or one
            // the OS evicted) is restored — the extension can then
            // publish shares without foregrounding the app.
            mirrorSharedNsec(nsecHex);
            await attachNdkSigner(signer);
            internal.set({ signer, pubkey: signer.pubkey });
            return;
