...
import {
  ...
  mergeReplaceableBookmark,
  replaceableBookmarkWins,
  type ParsedBookmark
...
// ── Relay-observed PUBLIC deletions (cross-device delete propagation) ──
// A kind:5 published from another device prunes the relay feed, but the
// serverPublicBookmarks merge is deliberately merge-only-never-remove —
// without this map the deleted bookmark re-enters from the API cache (or
// this device's localStorage) forever. Scoped to PUBLIC sources only: a
// public delete must not hide a private copy of the same URL.
const RELAY_DELETED_LS_PREFIX = 'deepmarks-relay-deleted:v1:';
const RELAY_DELETED_CAP = 500;
let relayDeletedUrls = new Map<string, number>();

function lsLoadRelayDeleted(pubkey: string): Map<string, number> {
  if (typeof localStorage === 'undefined') return new Map();
  try {
    const raw = localStorage.getItem(RELAY_DELETED_LS_PREFIX + pubkey);
    if (!raw) return new Map();
    const parsed = JSON.parse(raw) as Record<string, number>;
    return new Map(Object.entries(parsed).filter(([, at]) => Number.isFinite(at)));
  } catch {
    return new Map();
  }
}

function lsSaveRelayDeleted(pubkey: string, map: Map<string, number>): void {
  if (typeof localStorage === 'undefined') return;
  try {
    const entries = [...map.entries()]
      .sort(([, a], [, b]) => b - a)
      .slice(0, RELAY_DELETED_CAP);
    if (entries.length === 0) {
      localStorage.removeItem(RELAY_DELETED_LS_PREFIX + pubkey);
    } else {
      localStorage.setItem(RELAY_DELETED_LS_PREFIX + pubkey, JSON.stringify(Object.fromEntries(entries)));
    }
  } catch {
    // Quota/private mode — in-memory pruning still applies this session.
  }
}

function coveredByRelayDeletion(bookmark: ParsedBookmark): boolean {
  const deletedAt = relayDeletedUrls.get(bookmark.url);
  if (deletedAt === undefined) return false;
  // A genuinely newer re-save of the URL wins over the old deletion.
  return (bookmark.eventCreatedAt ?? bookmark.savedAt) <= deletedAt;
}

function withoutRelayDeleted(list: ParsedBookmark[]): ParsedBookmark[] {
  if (relayDeletedUrls.size === 0) return list;
  return list.filter((bookmark) => !coveredByRelayDeletion(bookmark));
}

function handleObservedDeletion(deletion: { pubkey: string; url: string; deletedAt: number }): void {
  if (!activePubkey || deletion.pubkey !== activePubkey) return;
  const pubkey = activePubkey;
  const previous = relayDeletedUrls.get(deletion.url) ?? 0;
  if (deletion.deletedAt > previous) {
    relayDeletedUrls.set(deletion.url, deletion.deletedAt);
    lsSaveRelayDeleted(pubkey, relayDeletedUrls);
  }
  serverPublicBookmarks.update((list) => {
    const next = withoutRelayDeleted(list);
    if (next.length !== list.length) lsSavePublic(pubkey, next);
    return next;
  });
}
...
function clearDeletedUrls(pubkey: string | null | undefined, urls: string[]): void {
  ...
  // A fresh save of the URL supersedes any old deletion record —
  // relay-observed or local.
  let relayChanged = false;
  for (const url of urls) {
    if (relayDeletedUrls.delete(url)) relayChanged = true;
  }
  if (relayChanged && pubkey) lsSaveRelayDeleted(pubkey, relayDeletedUrls);
  locallyDeletedUrls.update((current) => {
...
// Delegates to THE canonical conflict rule in bookmarks.ts.
const shouldReplaceBookmark = (existing: ParsedBookmark, incoming: ParsedBookmark): boolean =>
  replaceableBookmarkWins(existing, incoming);

const mergeBookmarkReplacement = mergeReplaceableBookmark;
...
async function loadServerPublicBookmarks(pubkey: string, token: number): Promise<void> {
  try {
    ...
    if (token === serverLoadToken && activePubkey === pubkey) {
      // The API cache can lag a deletion observed live from the relay —
      // don't let it re-merge a bookmark another device deleted.
      const parsed = withoutRelayDeleted(withoutLocallyDeleted(res.bookmarks.map(publicBookmarkToParsed)));
...
session.subscribe(($session) => {
  ...
  relayDeletedUrls = pubkey ? lsLoadRelayDeleted(pubkey) : new Map();
  relayPublicBookmarks.set(pubkey ? withoutRelayDeleted(cachedBookmarkFeedSnapshot({ authors: [pubkey], limit: 200 })) : []);
  importedBookmarks.set([]);
  const deleted = pubkey ? lsLoadDeleted(pubkey) : new Set<string>();
  locallyDeletedUrls.set(deleted);
  serverPublicBookmarks.set(pubkey ? withoutRelayDeleted(withoutLocallyDeleted(lsLoadPublic(pubkey), deleted)) : []);
...
let deletionObserverRegistered = false;
function registerDeletionObserverOnce(): void {
  if (deletionObserverRegistered) return;
  deletionObserverRegistered = true;
  void import('$lib/nostr/feed').then(({ onBookmarkDeletionObserved }) => {
    onBookmarkDeletionObserved(handleObservedDeletion);
  }).catch(() => {
    deletionObserverRegistered = false; // retry on the next network load
  });
}
...
async function startNetworkLoads(pubkey: string, token: number): Promise<void> {
  ...
  try {
    ...
    if (activePubkey !== pubkey || token !== serverLoadToken) return;
    registerDeletionObserverOnce();
    relayFeedStop = createBookmarkFeed({ authors: [pubkey], limit: 200 }).subscribe((list) => {
      relayPublicBookmarks.set(withoutRelayDeleted(withoutLocallyDeleted(list)));
    });
