...
import { createImportedBookmarksFeed } from '$lib/nostr/imported-bookmarks';
...
export const mySavedUrls: Readable<Set<string>> = derived(
  [publicUrls, importedUrls, privateUrls],
  ([$pub, $imported, $priv]) => {
...
export function startMySavedUrlsLoader(): () => void {
  ...
  const stopSession = session.subscribe((s) => {
    ...
    const imported = createImportedBookmarksFeed({ authors: [s.pubkey], limit: 500 });
