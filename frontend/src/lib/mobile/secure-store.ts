...
export interface PendingSharedBookmark {
  ...
  createdAt?: string;
  createdAtMs?: string;
  /** Set to "1" by the share extension when it has already signed
...
export async function writeUserTagsToAppGroup(tags: string[]): Promise<void> {
  // No-op on web / Android — only the iOS share extension consumes
  // the AppGroup-shared tag list. Capped here as a defense-in-depth
  // measure even though the native side also caps.
  if (
    !isNativeSecureStoreAvailable() ||
    Capacitor.getPlatform() !== 'ios' ||
