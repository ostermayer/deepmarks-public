...
async function handleRequestEvent(event: NostrEvent): Promise<void> {
  ...
  try {
    ...
    if (request.method === 'connect') {
      // NIP-46 `connect` params[2] is the client's REQUESTED permission
      // list. It used to be merged into the stored connection and
      // persisted BEFORE the permission check — so a paired app that was
      // approved for sign_event:1 could silently self-escalate to
      // nip44_decrypt on any reconnect (2026-08-23 review #7). Requested
      // permissions are now ignored: granting happens only where the
      // user approved the pairing. `connect` itself is always allowed by
      // permissionAllows, so reconnects keep working.
      const requested = splitPerms(request.params[2] ?? '');
      const ungranted = requested.filter((perm) => !connection.perms.includes(perm));
      if (ungranted.length > 0) {
        mobileSignerStatus.update((s) => ({
          ...s,
          lastMessage: `${connection.name || shortKey(connection.clientPubkey)} requested unapproved permissions (${ungranted.join(', ')}) — ignored`,
        }));
      }
    }
