...
<section class="intro">
  <p>
    deepmarks is bookmarks for the open web. save a link, tag it, come back later.
    you can tip the people who find great links, and lifetime members can archive pages.
    nothing you save here depends on us staying around — every bookmark is a
    signed nostr event that gets copied to relays you pick, readable by any nostr client.
  </p>
  <p>
...
<section class="faq">
  ...
  <details>
    ...
    <p>
      ...
      your device signs it; deepmarks posts it to nostr relays without exposing your IP
      to those relays.
    </p>
  </details>
  ...
  <details>
    <summary>what are zaps and who gets them?</summary>
    <p>
      a zap is a lightning tip with a public receipt on nostr. when you zap a bookmark,
      your wallet pays one invoice directly to the curator who saved it, using the
      lightning address on their nostr profile. if the curator has no lightning
      address, the invoice goes to deepmarks instead. nothing is custodial — your
      wallet pays the recipient directly. receipts land on the relays you choose, so
      any nostr client can verify the payment.
    </p>
  </details>
  ...
  <details>
    ...
    <p>
      the zap goes to deepmarks. the dialog shows that before you confirm, and it
      still creates only one invoice.
    </p>
  </details>

  <details>
    <summary>do bookmarked websites receive part of a zap?</summary>
    <p>
      no. zaps now create one invoice: either the curator, or deepmarks when the
      curator has no lightning address. that keeps payment flows simple and avoids
      surprising users with extra invoices.
    </p>
  </details>
  ...
  <details>
    ...
    <p>
      bookmarking is free and always will be. archiving is included with lifetime membership.
      we render the page through singlefile and mirror the resulting blob to four blossom
      servers by default. if any page you care about breaks later, the archive has an
      independent copy.
    </p>
  </details>
  ...
  <details>
    ...
    {#if IS_APPLE_BUILD}
      <p>
        lifetime members get unlimited page archives and api access for programmatic reads
        and writes. existing lifetime access works in the app.
      </p>
    {:else}
      <p>
        {config.lifetimePriceSats.toLocaleString('en-US')} sats one time, pay via lightning. unlimited page
  ...
  <details>
    ...
    <p>
      ...
      fanned out to relays you choose. we're a convenience layer on top — a nice
      interface, search, zaps, permanent archives. if we disappear, every
