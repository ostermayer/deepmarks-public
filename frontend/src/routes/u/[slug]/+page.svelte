<script lang="ts">
  ...
  //   collections — public Deepmarks collection events that can be shared
  //                as /u/[slug]/[collection].
  //
  ...
  import { addFriend, friendPubkeys, removeFriend } from '$lib/nostr/friends';
  import {
  ...
  import { mergeCollectionLists, type BookmarkCollection } from '$lib/bookmark-collections';
  import { createPublicCollectionsFeed, ownCollections, refreshOwnCollections } from '$lib/nostr/collections';
  ...
  type Tab = 'bookmarks' | 'posts' | 'collections';
  ...
  const publicCollectionsFeed = writable<BookmarkCollection[]>([]);
  let publicFeedPubkey: string | null = null;
  let publicFeedToken = 0;
  let publicFeedStop: (() => void) | null = null;
  let publicCollectionsStop: (() => void) | null = null;
  ...
  function clearPublicFeed(): void {
    ...
    publicFeedStop?.();
    publicCollectionsStop?.();
    publicFeedStop = null;
    publicCollectionsStop = null;
    publicBookmarks.set([]);
    publicCollectionsFeed.set([]);
  }
  ...
  function startPublicFeed(pk: string): void {
    ...
    publicCollectionsStop = createPublicCollectionsFeed({ authors: [pk], limit: 200 }).subscribe(publicCollectionsFeed.set);
  }
  ...
  $: if (isOwner && pubkey && ownCollectionsLoadedFor !== pubkey) {
    ownCollectionsLoadedFor = pubkey;
    void refreshOwnCollections(pubkey);
  }
  $: if (!isOwner) ownCollectionsLoadedFor = '';
  $: isFollowing = !!pubkey && $friendPubkeys.has(pubkey);
  $: displayFollowing = optimisticFollowing ?? isFollowing;
  $: if (optimisticFollowing !== null && optimisticFollowing === isFollowing && !followWorking) {
  ...
  $: postsEntries = (pubkey
    ? derived([postUrls, postNotes], ([$urls, $notes]) => {
        ...
        merged.sort(comparePostEntriesNewest);
        return merged;
      })
  ...
  function comparePostEntriesNewest(a: PostEntry, b: PostEntry): number {
    const time = postTime(b) - postTime(a);
  ...
  function sortPosts(list: PostEntry[], sort: PostSort): PostEntry[] {
    ...
    next.sort((a, b) => sort === 'oldest'
      ? comparePostEntriesNewest(b, a)
      : comparePostEntriesNewest(a, b));
    return next;
  }
  ...
  async function toggleFollow(): Promise<void> {
    ...
    optimisticFollowing = next;
    followWorking = true;
    followNotice = '';
    try {
      if (!next) await removeFriend(pubkey, $session.pubkey);
      else await addFriend(pubkey, $session.pubkey);
    } catch (e) {
      optimisticFollowing = null;
  ...
  $: publicCollections = $publicCollectionsFeed;
  $: visibleCollections = isOwner
    ? mergeCollectionLists(publicCollections, $ownCollections)
    : publicCollections;

  function collectionHref(collection: BookmarkCollection): string {
    if (collection.publicCount > 0) {
      return `/u/${encodeURIComponent(feedSlug)}/${encodeURIComponent(collection.slug)}`;
    }
    return `/app/collections/${encodeURIComponent(collection.slug)}`;
  }
</script>
        ...
        <div class="meta">
          <div class="name-row">
            ...
            {#if $session.pubkey && !isOwner}
              <button type="button" class:active={displayFollowing} on:click={toggleFollow} disabled={followWorking} aria-pressed={displayFollowing}>
      ...
      <section class="profile-tabs">
        <div class="tab-row">
          ...
          <button
            ...
            on:click={() => setTab('collections')}
          >
            collections
            <span class="count">{visibleCollections.length}</span>
          </button>
        ...
        {:else}
          {#if visibleCollections.length === 0}
            <p class="empty">no public collections yet.</p>
          {:else}
            <div class="collection-grid">
              {#each visibleCollections as collection (collection.slug)}
                <a class="collection-card" href={collectionHref(collection)}>
                  <span class="collection-name">{collection.title}</span>
                  <span class="collection-meta">
                    {#if isOwner && collection.privateCount > 0}
                      {collection.publicCount.toLocaleString()} public · {collection.privateCount.toLocaleString()} private
                    {:else}
                      {collection.publicCount.toLocaleString()} bookmark{collection.publicCount === 1 ? '' : 's'}
                    {/if}
                  </span>
                  <span class="collection-url">
                    {#if collection.publicCount > 0}
                      /u/{feedSlug}/{collection.slug}
                    {:else}
                      private collection
                    {/if}
                  </span>
