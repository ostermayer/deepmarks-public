<script lang="ts">
  ...
  onMount(async () => {
    ...
    // here, then bounce to /app/bookmarks.
  ...
  function pickTier(tier: 'free' | 'lifetime') {
    markTierChosen();
    void goto(tier === 'lifetime' ? '/app/upgrade' : '/app/bookmarks');
  }
</script>
    ...
    <div class="tier-cards">
      <button type="button" class="tier-card default" on:click={() => pickTier('free')}>
        ...
        <p class="tier-card-blurb">
          unlimited bookmarks, public + private. upgrade later if you want page archives.
        </p>
