<script lang="ts">
  ...
  onMount(() => {
    ...
    const loaderTimer = setTimeout(async () => {
      ...
      // Native only: keep the iOS AppGroup-shared tag list in sync with
      // the signed-in user's bookmarks so the Share Extension can
      // autocomplete without hitting the network. Debounced inside.
      if (nativeShell) {
        const [{ ownBookmarks: ownBookmarksStore }, { flushUserTagsForShareExtension }] = await Promise.all([
        ...
        stopArchives = () => { previousArchives?.(); stopTagSync(); };
      }
