<script lang="ts">
  ...
  // Public network matches render as a secondary right rail. They use
  // semantic search first, with lexical search as a fallback when the
  // embedding index has no useful coverage yet.
  ...
  type GlobalSearchMode = 'semantic' | 'lexical';

  let hits: Hit[] = [];
  let total = 0;
  let queryTimeMs = 0;
  let globalMode: GlobalSearchMode = 'semantic';
  let loading = false;
  ...
  async function runGlobalSearch(q: string): Promise<void> {
    ...
    error = null;
    try {
      const semantic = await fetchSearch('semantic', q, 2_500);
      if (token !== searchToken || !globalSearch || q !== query) return;
      if (semantic.total > 0) {
        hits = semantic.hits;
        total = semantic.total;
        queryTimeMs = semantic.query_time_ms;
        globalMode = 'semantic';
      } else {
        const lexical = await fetchSearch('lexical', q);
        if (token !== searchToken || !globalSearch || q !== query) return;
        hits = lexical.hits;
        total = lexical.total;
        queryTimeMs = lexical.query_time_ms;
        globalMode = 'lexical';
      }
    } catch (e) {
      try {
        ...
        if (token !== searchToken || !globalSearch || q !== query) return;
        error = (fallbackError as Error).message || (e as Error).message || 'search failed';
      }
    } finally {
  ...
  async function fetchSearch(
    ...
    q: string,
    timeoutMs = 10_000,
  ): Promise<{ hits: Hit[]; total: number; query_time_ms: number }> {
    const path = mode === 'semantic'
      ? `/search/semantic/public?q=${encodeURIComponent(q)}&limit=50&rerank=false`
      : `/search/public?q=${encodeURIComponent(q)}&limit=50`;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    let res: Response;
    try {
      res = await fetch(`${config.apiBase}${path}`, { signal: controller.signal });
    } finally {
      clearTimeout(timer);
    }
    if (!res.ok) throw new Error(`${mode} search ${res.status}`);
    return (await res.json()) as { hits: Hit[]; total: number; query_time_ms: number };
  }
  ...
  function hitToBookmark(h: Hit): ParsedBookmark {
    return {
      ...
      description: h.doc.llm_summary || h.doc.description,
      tags: h.doc.tags ?? [],
      archivedForever: false,
      savedAt: h.doc.created_at,
      savedAtMs: h.doc.saved_at_ms,
      ...
      <aside class="global-results" aria-label="global results">
        <div class="section-head">
          ...
          {#if total > 0 && !loading && !error}
            <span>{total.toLocaleString()} public · {globalMode} · {queryTimeMs} ms</span>
          {:else}
