<script lang="ts">
  ...
  import CleanupSection from '$lib/components/CleanupSection.svelte';
  ...
  async function updateSyncedSettings(next: UserSettings): Promise<void> {
    const optimistic = {
      ...next,
      pendingSync: true,
      syncedAt: Math.max(next.syncedAt, Date.now()),
    };
...
<div class="page">
  ...
  <UsernameSection {isLifetime} lifetimeLoading={lifetimeStatusLoading} />
  ...
  <section class="settings-band">
    ...
    {#if lifetimeStatusLoading}
      <p class="muted compact">checking archive access...</p>
    {:else if !isLifetime}
