<script lang="ts">
  ...
  type Sort = 'popular' | 'zap-sats' | 'alpha';
  ...
  $: routeSort = (requestedSort === 'alpha' || requestedSort === 'zap-sats' ? requestedSort : 'popular') as Sort;
  ...
  $: nextZapTargetIds = bookmarkZapTargetEventIds($ownBookmarks).sort();
  ...
  $: rawCounts = sortCounts(
    sort === 'zap-sats' ? countTagsWithZapSats($ownBookmarks, zapSatsByEventId) : countTags($ownBookmarks),
    sort,
  );
  ...
  // gets the full set.
  $: cloud = bucketize(
    displayedCounts.slice(0, 80).map((item) => ({
      name: item.name,
      count: sort === 'zap-sats' ? (item.zapSats ?? 0) : item.count,
    })),
  ) as TagCloudItem[];
  ...
  function cacheKey(nextSort: Sort = sort): string {
    return `deepmarks-tags:mine:${nextSort}:v3`;
  }
  ...
  function loadCachedCounts(nextSort: Sort = sort): TagCount[] {
    ...
    try {
      ...
      return parsed.filter((item): item is TagCount =>
        ...
        (item.zapSats === undefined || typeof item.zapSats === 'number'),
      );
    } catch {
      return [];
    }
...
<div class="container">
  <div class="toolbar">
    ...
    <div class="segments">
      ...
      <button type="button" on:click={() => setSort('zap-sats')} class:active={sort === 'zap-sats'}>⚡ sats</button>
    </div>
  </div>
