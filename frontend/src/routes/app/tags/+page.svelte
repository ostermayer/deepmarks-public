<script lang="ts">
  ...
  // Mine: full list — when scope is 'mine' the user is asking to see
  // everything they've used and the cap was hiding their long tail
  // (500+ tags showed only 200). Network: keep a hard cap because
  // that pool is unbounded.
  //
  // The network feed is a kind:39701 subscription (public bookmarks
  // only) and the private set lives on kind:30003 with encrypted
  // content, so private tags can't structurally leak in. We still
  // strip 'private:'-prefixed eventIds out of the network bookmark
  // input defensively in case a future caller wires both feeds in.
  $: networkCounts = countTags($feed.filter((b) => !b.eventId.startsWith('private:'))).slice(0, 80);
  $: mineCounts = countTags($ownBookmarks);
