<script lang="ts">
  ...
  $: normalizedTagSearch = normalizeTag(activeSearchQuery);
  $: searchedTagCounts = activeSearchQuery
    ? tagCounts.filter((tag) => tag.name.includes(normalizedTagSearch))
    : tagCounts;
  $: tagCloud = bucketize(
    searchedTagCounts.slice(0, 48).map((item) => ({
      name: item.name,
  ...
  $: searchSummary = activeSearchQuery
    ? `${mode === 'tags' ? searchedTagCounts.length : searchedBookmarks.length} match${(mode === 'tags' ? searchedTagCounts.length : searchedBookmarks.length) === 1 ? '' : 'es'}`
    : '';
  $: overlaySearchResults = !activeSearchQuery
    ? []
    : mode === 'tags'
      ? searchedTagCounts.slice(0, OVERLAY_RESULT_CAP).map((t) => ({
          id: `tag:${t.name}`,
          title: `#${t.name}`,
          subtitle: `${t.count.toLocaleString()} ${t.count === 1 ? 'bookmark' : 'bookmarks'}`,
          href: tagHref(t.name),
          external: false,
        }))
      : searchedBookmarks.slice(0, OVERLAY_RESULT_CAP).map(bookmarkToSearchResult);
</script>
...
<AppActionBar
  ...
  bind:searchQuery
  addDisabled={true}
  searchPlaceholder={mode === 'tags' ? 'search public tags...' : selectedTag ? `search #${selectedTag} bookmarks...` : 'search public bookmarks...'}
...
{#if mode === 'tags'}
  <main class="tag-explorer">
    <h1>global tags</h1>
    {#if searchedTagCounts.length === 0}
      <div class="empty">{activeSearchQuery ? 'no matching public tags' : 'no public tags yet'}</div>
...
{:else}
  ...
  <BookmarkList
    ...
    summaryBookmarks={activeSearchQuery ? searchedBookmarks : tagSource}
    loading={true}
    paginationKey={`${sort}:${selectedTag}:${windowKind}:${customSince}:${customUntil}:${activeSearchQuery}`}
    emptyMessage={activeSearchQuery ? 'no matching public bookmarks' : sort === 'popular' ? 'no popular bookmarks yet' : 'no public bookmarks yet'}
    tagScope="network"
    {zapSatsByEventId}
