<script lang="ts">
  ...
  type Sort = 'newest' | 'popular' | 'oldest' | 'title-az';
  ...
  $: routeTagSort = ($page.url.searchParams.get('tagSort') === 'alpha' ? 'alpha' : 'popular') as TagSort;
  ...
  function parseSort(raw: string | null): Sort {
    if (raw === 'popular' || raw === 'oldest' || raw === 'title-az') return raw;
    return 'newest';
  }
  ...
  $: bookmarks = sortBookmarks(filteredFeed, sort, $receipts, activeWindow);
  $: tagCounts = sortTagCounts(countTags(tagSource), tagSort);
  $: tagCloud = bucketize(tagCounts.slice(0, 48)) as TagCloudItem[];
