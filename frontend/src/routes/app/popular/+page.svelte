<script lang="ts">
  ...
  import { bookmarkZapTargetEventIds } from '$lib/nostr/bookmark-zap-target';
  import {
  ...
  import { createTargetedZapReceiptFeed, type ZapReceiptRecord } from '$lib/nostr/zap-counts';
  ...
  $: activeRange = computeRange($selectedKind, $customSince, $customUntil);
  $: zapSatsByEventId = tallyReceiptsInWindow($targetedZapReceipts, activeRange.sinceSec, activeRange.untilSec);
  $: {
    const filteredBookmarks = filterBookmarksByWindow($feed, activeRange);
    // Two-tier floor: everything needs score >= 2 (no one-curator noise), and
    // anything not editorially seeded must have > 500 sats zapped against it.
    ranked = applyPopularityFloor(rankByPopularity(filteredBookmarks, zapSatsByEventId), {
      editorialPubkeys: config.deepmarksEditorialPubkeys,
    });
  }
  $: activeSearchQuery = searchOpen ? searchQuery.trim() : '';
  $: searchedRanked = activeSearchQuery
    ? searchLocalBookmarks(ranked, activeSearchQuery, { limit: 10_000 })
    : ranked;
  $: overlaySearchResults = activeSearchQuery
    ? searchedRanked.slice(0, OVERLAY_RESULT_CAP).map(bookmarkToSearchResult)
    : [];
  $: searchSummary = activeSearchQuery
    ? `${searchedRanked.length.toLocaleString()} match${searchedRanked.length === 1 ? '' : 'es'}`
    : '';
</script>

<svelte:head><title>popular — Deepmarks</title></svelte:head>

<Subheader context="global · popular" feedUrl="/feed/popular.xml" feedLabel="Deepmarks popular feed" />
...
<AppActionBar
  ...
  bind:searchQuery
  addDisabled={true}
  searchPlaceholder="search popular bookmarks..."
...
<BookmarkList
  bookmarks={searchedRanked}
  loading={true}
  freezeTagCloud={true}
  paginationKey={`popular:${$selectedKind}:${$customSince}:${$customUntil}:${activeSearchQuery}`}
  emptyMessage={activeSearchQuery ? 'no matching popular bookmarks' : 'no bookmarks in this window yet'}
  tagScope="network"
  {zapSatsByEventId}
/>
