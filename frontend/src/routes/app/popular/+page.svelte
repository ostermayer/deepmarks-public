<script lang="ts">
  ...
  const ranked: Readable<RankedBookmark[]> = derived(
    ...
    ([$f, $r, $kind, $since, $until]) => {
      ...
      // noise), and anything not editorially seeded must have > 500 sats
      // zapped against it. Without this, the list is mostly single-save
      // junk from the broader Nostr firehose.
...
<BookmarkList bookmarks={$ranked} loading={true} freezeTagCloud={true} emptyMessage="no bookmarks in this window yet" tagScope="network" />
