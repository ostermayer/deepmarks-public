<script lang="ts">
  ...
  const PUBLIC_APP_PATHS = ['/app/recent', '/app/popular', '/app/explore', '/app/mobile-signer'];
  let settingsSyncedFor = '';
  let settingsSyncInFlightFor = '';
  let bookmarksRefreshedAt = 0;
  ...
  function refreshBookmarksSoon(): void {
    const now = Date.now();
    if (now - bookmarksRefreshedAt < 15_000) return;
    bookmarksRefreshedAt = now;
    void import('$lib/stores/own-bookmarks').then(({ refreshOwnBookmarks }) => {
      refreshOwnBookmarks();
    }).catch(() => {
      // Bookmarks still render from the current store/cache.
    });
  }
