...
<div class="page">
  <header>
    <h1>archives</h1>
    <p class="lede">
      pages archived by the deepmarks worker and pinned on Blossom.
      private archives are encrypted to your nsec —
      opening or downloading them decrypts locally in this browser.
    </p>
  </header>
          ...
          {:else}
            ...
            >
              {#if a.thumbHash}
                <img
                  class="thumb"
                  src={`${config.blossomUrl.replace(/\/$/, '')}/${encodeURIComponent(a.thumbHash)}`}
                  alt=""
                  loading="lazy"
                />
              {/if}
