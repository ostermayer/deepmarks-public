<script lang="ts">
  ...
  import { archiveMime, archiveTimelineAt, downloadArchiveRecord, fetchArchiveBytes } from '$lib/archives/download';
  ...
  function archiveLabel(rec: ArchiveRecord): string {
    if ((rec.kind === 'video' || rec.kind === 'youtube' || rec.kind === 'media') && rec.videoTitle) {
        ...
        <li class="archive-item">
          {#if a.tier === 'private'}
            <button
              ...
              title={a.kind === 'video' || a.kind === 'youtube' || a.kind === 'media' ? 'Private media archive - click to decrypt + open' : 'Private archive - click to decrypt + open the snapshot'}
            >
