<script lang="ts">
  ...
  import { type BookmarkInput, type ParsedBookmark, safeExternalHref } from '$lib/nostr/bookmarks';
  ...
  async function isLifetimeMember(pubkey: string): Promise<boolean> {
    try {
      ...
      return get(getLifetimeStatus(pubkey)) ?? false;
    }
  }
      ...
      <div class="review-list">
        {#each reviewItems as b, offset (`${reviewStart + offset}:${b.url}`)}
          ...
          <article class="review-row">
            <div class="row-top">
              <a class="row-url" href={safeExternalHref(b.url)} target="_blank" rel="noreferrer">{b.url}</a>
