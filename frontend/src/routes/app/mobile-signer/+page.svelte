  ...
  {#if nativeShell && $mobileSignerStatus.foregroundAvailable}
    <section>
      ...
      <label class="toggle-row">
        ...
        />
        <span>
          <strong>keep mobile signer running</strong>
          <em>Android shows a persistent notification while paired clients can request NIP-46 signatures.</em>
        </span>
      </label>
      {#if !account}<p class="muted">add a mobile signer key first.</p>{/if}
      {#if account && connections.length === 0}<p class="muted">pair a client first.</p>{/if}
    </section>
  {/if}
