<script lang="ts">
  ...
  $: if (connectParam && connectParam !== connectInput) {
    connectInput = connectParam;
    if (account) void pairConnect();
  }
  ...
  async function startScan(): Promise<void> {
    ...
    if (!('BarcodeDetector' in window)) {
      error = 'QR scanning is not available in this WebView yet. Paste the nostrconnect link instead.';
      return;
    }
    try {
  ...
  async function tickVideo(): Promise<void> {
    await new Promise((resolve) => setTimeout(resolve, 0));
    if (videoEl && scanStream) {
  ...
  async function scanLoop(): Promise<void> {
    ...
    const loop = async () => {
      ...
      try {
        ...
        // Keep scanning; some frames fail while the video is warming.
      }
