...
<header class="page-header">
  ...
  <p class="callout">
    <strong>API access is for lifetime members only.</strong> Public bookmarks are still
    readable from Nostr relays without a Deepmarks API key.
  </p>
</header>
...
<section>
  ...
  <p>
    ...
    ({config.lifetimePriceSats.toLocaleString('en-US')} sats, one-time).
...
&#125;</code></pre>
  ...
  <h3>Initiate a lifetime archive</h3>
...
&#125;</code></pre>
  <p>
    API keys are lifetime-only, so this enqueues the archive directly.
    The archive worker renders + stores the page on Blossom; poll the
    job id until it completes.
  </p>

  <h3>Poll an archive job</h3>
<pre><code>GET /api/v1/archives/:jobId</code></pre>
  <p>
    State is <code>pending</code> → <code>enqueued</code> while queued; completed
    archives land in your archives list. Returns 404 for
