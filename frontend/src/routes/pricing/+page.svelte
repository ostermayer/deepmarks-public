<script lang="ts">
  ...
  onMount(() => {
    if (IS_APPLE_BUILD) void goto('/app/bookmarks', { replaceState: true });
  });
</script>
...
<section class="tiers">
  <article class="tier pixel-card">
    ...
    <ul>
      ...
      <li>export everything whenever you want</li>
    </ul>
  ...
  <article class="tier pixel-card accent">
    ...
    <div class="tier-head">
      <h2>lifetime</h2>
      <p class="amount">{config.lifetimePriceSats.toLocaleString('en-US')} sats</p>
    </div>
    <ul>
      <li>unlimited site archives</li>
      <li>duplicated storage worldwide</li>
      <li>API keys + short handles</li>
    </ul>
...
<section class="faq">
  ...
  <details id="duplicated-worldwide">
    ...
    <p>
      Public bookmarks are Nostr <code>kind:39701</code> events signed on your device, sent
      to Deepmarks, then fanned out to your Nostr relays. Private bookmarks are encrypted
      with NIP-44 v2 before they leave your device. Your data lives on relays you control —
      not in our database — so the network stays useful even if Deepmarks disappears.
    </p>
  </details>
  ...
  <details id="archive-forever">
    ...
    <p>
      Lifetime members can snapshot pages through the Deepmarks archive worker. We render
      the page through SingleFile and mirror the resulting blob to four independent Blossom
      servers by default, with the hashes pinned in your own Nostr bookmark event. If
      Deepmarks vanishes, the mirrors don't.
    </p>
  </details>
  ...
  <details id="tip-great-links">
    ...
    <p>
      Send a zap to great bookmarks and your wallet pays one Lightning invoice directly
      to the curator who saved the link. If the curator has no Lightning address, the
      zap goes to Deepmarks instead. Nothing is custodial — your wallet pays the
      recipient directly.
    </p>
  </details>
  ...
  <details>
    ...
    <p>
      Recurring payments lock you into infrastructure decisions you didn't make. Lifetime
      membership is a one-time upgrade for archive storage and convenience features. Your
      bookmarks still live on Nostr relays either way.
    </p>
  </details>

  <details>
    <summary>Can every page be archived?</summary>
    <p>
      We do our best with public pages: Wayback is disabled for production captures,
      Playwright renders the current page, SingleFile packages it, and a quality floor rejects
      obvious bot-block/captcha captures. Some sites still block automation or require an
      account session we cannot access from the server.
    </p>
    <p>
      Media archiving is also best-effort. Deepmarks attempts to archive associated media
      on eligible pages, but provider bot restrictions, login walls, removed media, or
      unsupported players can prevent a complete media archive.
    </p>
  </details>
  ...
  <details id="archive-deletion">
    ...
    <p>
      What we <em>can't</em> do is reach into Primal or any user-added Blossom backup
      server and force it to drop its copy. Those servers pulled the bytes
