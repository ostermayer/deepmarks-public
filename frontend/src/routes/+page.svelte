<script lang="ts">
  ...
  onMount(() => {
    ...
    void (async () => {
      const { loginWithDeepmarksExtension, shouldAttemptDeepmarksAutoLogin } = await import('$lib/nostr/extension-autologin');
      if (!shouldAttemptDeepmarksAutoLogin()) return;
      try {
        await loginWithDeepmarksExtension();
        void goto('/app/bookmarks', { replaceState: true });
      } catch {
        // Stay on the public landing page when the extension is locked or
        // the user declines its one-time approval prompt.
      }
    })();
  });
  ...
  const feed = config.landingFeedPubkeys.length > 0
...
<section class="extension-row" aria-label="browser extensions">
  ...
  <a
    href="https://chromewebstore.google.com/detail/deepmarks/hefiabmeleknfielegnadaapcaacapoo?hl=en-US"
    target="_blank"
    rel="noreferrer"
  >chrome</a>
  <a
    href="https://addons.mozilla.org/en-US/firefox/addon/deepmarks/"
    target="_blank"
    rel="noreferrer"
  >firefox</a>
</section>
