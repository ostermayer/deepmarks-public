<script lang="ts">
  ...
  import {
    ...
    isNip07Available,
    listAvailableAndroidSigners,
    SignerError,
  } from '$lib/nostr/signers';
  import { session } from '$lib/stores/session';
  import { hasChosenTier, markTierChosen } from '$lib/onboarding';
  ...
  import type { AndroidSignerApp, Nip46PairingSession } from '$lib/nostr/signers';

  type Method = 'extension' | 'passkey' | 'mobile' | 'bunker' | 'nsec' | 'android-signer';

  // `revealed` toggles the QR / bunker / nsec input panels (they need user
  // input, can't be one-click). Passkey + extension never set this.
  let revealed: 'mobile' | 'bunker' | 'nsec' | 'android-signer' | null = null;
  ...
  let androidSignerApps: AndroidSignerApp[] = [];
  let androidSignerStatus = '';
  ...
  $: bunkerParam = $page.url.searchParams.get('bunker') ?? '';
  ...
  $: androidNative = nativeShell && nativePlatform() === 'android';
  ...
  onMount(() => {
    mounted = true;
    if (nativeShell) {
      revealed = bunkerParam ? 'bunker' : androidNative ? null : 'nsec';
   ...
   *  sign-ins jump straight to the redirect target. Lifetime accounts
   *  should also skip /welcome on fresh devices after the pubkey check. */
  async function nextRouteAfterLogin(pubkey?: string | null): Promise<string> {
    if (IS_APPLE_BUILD || nativePlatform() === 'ios') {
      markTierChosen();
      return redirectTarget;
    }
    if (hasChosenTier()) return redirectTarget;
    if (pubkey && await isLifetimeMemberOnce(pubkey)) {
      markTierChosen();
      return redirectTarget;
    }
    return '/welcome';
  ...
  async function finishLoginRoute(pubkey?: string | null): Promise<void> {
    await goto(await nextRouteAfterLogin(pubkey));
  }
  ...
  async function loginExt(firstPartyOnly = false) {
    ...
    try {
      ...
      void finishLoginRoute(s.pubkey);
    } catch (e) {
  ...
  async function loginWithPasskey() {
    ...
    try {
      ...
      void finishLoginRoute(signer.pubkey);
    } catch (e) {
  ...
  async function loginAndroidSigner(packageName = '') {
    if (busy) return;
    error = '';
    stopMobilePairing();
    stopNsecScan();
    androidSignerStatus = '';
    workingMethod = 'android-signer';
    try {
      let selectedPackage = packageName;
      if (!selectedPackage) {
        androidSignerApps = await listAvailableAndroidSigners();
        if (androidSignerApps.length === 0) {
          throw new SignerError('Install Amber, Primal, or another Android Nostr signer and try again.', 'no-signer');
        }
        if (androidSignerApps.length > 1) {
          revealed = 'android-signer';
          androidSignerStatus = 'choose which Android signer to use';
          return;
        }
        const onlySigner = androidSignerApps[0];
        if (!onlySigner) {
          throw new SignerError('Install Amber, Primal, or another Android Nostr signer and try again.', 'no-signer');
        }
        selectedPackage = onlySigner.packageName;
      }
      revealed = null;
      const signer = await createAndroidSigner({ packageName: selectedPackage });
      await session.login(signer);
      void finishLoginRoute(signer.pubkey);
    } catch (e) {
      error = e instanceof SignerError ? e.message : (e as Error).message;
  ...
  $: if (mounted && bunkerParam && bunkerParam !== bunkerUri && autoBunkerParam !== bunkerParam) {
    bunkerUri = bunkerParam;
    revealed = 'bunker';
  }

  $: if (mounted && bunkerParam && autoBunkerParam !== bunkerParam && !busy) {
    autoBunkerParam = bunkerParam;
    bunkerUri = bunkerParam;
    revealed = 'bunker';
    void loginBunker();
  }
  ...
  async function loginNsec() {
    ...
    try {
      ...
      if (shouldSaveWithPasskey) {
        if (!hex) throw new Error('nsec key unavailable for passkey setup');
  ...
  function continueAfterPastedNsec() {
    ...
    nsecInput = '';
    void finishLoginRoute($session.pubkey);
  }
...
<div class="page" class:native={nativeShell}>
  ...
  <p class="lede">
    {#if nativeShell}
      {#if androidNative}
        connect an Android signer or paste your recovery key once.
      {:else}
  ...
  <div class="methods">
    ...
    {#if androidNative}
      <button
        type="button"
        class="method"
        class:open={revealed === 'android-signer'}
        class:working={workingMethod === 'android-signer'}
        disabled={busy && workingMethod !== 'android-signer'}
        on:click={() => void loginAndroidSigner()}
      >
        <div class="title">connect Android signer <span class="rec">recommended</span></div>
        <div class="sub">
          {#if workingMethod === 'android-signer'}
            opening your signer…
          {:else}
            choose Amber, Primal, or another NIP-55 signer
          {/if}
        </div>
      </button>
      {#if revealed === 'android-signer'}
        <div class="panel android-signer-panel">
          <p class="info compact">{androidSignerStatus || 'choose which Android signer should approve this login.'}</p>
          <div class="android-signer-list">
            {#each androidSignerApps as app (app.packageName)}
              <button
                class="ghost signer-choice"
                type="button"
                on:click={() => void loginAndroidSigner(app.packageName)}
                disabled={busy}
              >
                <span class="signer-name">{app.appName || app.packageName}</span>
                <span class="signer-package">{app.packageName}</span>
              </button>
            {/each}
          </div>
        </div>
...
<style>
  ...
  .android-signer-panel { margin: 0 0 4px; }
  ...
  .android-signer-list { display: flex; flex-direction: column; gap: 8px; }
  .signer-choice {
    width: 100%;
    border-radius: 8px;
    display: flex;
  ...
  .signer-package { color: var(--ink); font-size: 11px; line-height: 1.35; word-break: break-word; }
