...
//   - API response caching. /bookmarks/public, /profile/:pubkey,
//     etc. are already mirrored into the in-app localStorage stores
//     (ownBookmarks, profileCache) for offline reads; layering an
//     SW cache on top creates a second source of truth that's hard
//     to invalidate when a save happens.
...
sw.addEventListener('fetch', (event) => {
  ...
  // Only intercept same-origin requests. Cross-origin (api.deep
  // marks.org, blossom, relay WSS, etc.) is left to the browser
  // and the in-app caches.
