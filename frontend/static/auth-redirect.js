...
// `goto('/app/bookmarks')`, the marketing content has already painted — that's
...
(function () {
  try {
    ...
    // Native Capacitor shells use capacitor://localhost (iOS) or
    // https://localhost (Android). Let the Svelte app rehydrate the
    // platform secure-store first instead of doing a blind web redirect
    // from a localStorage hint.
    if (window.location.protocol === 'capacitor:' || window.location.hostname === 'localhost') return;
    var hint = localStorage.getItem('deepmarks-session-hint');
    if (!hint) return;
    // location.replace so the marketing page doesn't end up in history.
    window.location.replace('/app/bookmarks');
  } catch (_) {
