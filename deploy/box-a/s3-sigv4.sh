...
# host. Supports virtual-hosted-style object GET/PUT plus read-only bucket
# subresource GETs used by the Object Storage safety probe.
...
s3_signed_curl() {
  ...
  local file="${5:-}"
  local output="${6:-}"
  local canonical_query="${7:-}"
  local display_query="${8:-$canonical_query}"
...
${hashed_canonical}"
  ...
  local url="${scheme}://${request_host}${canonical_uri}"
  if [ -n "$display_query" ]; then
    url="${url}?${display_query}"
  fi
...
s3_get_bucket_versioning() {
  s3_signed_curl GET "$1" "" "$2" "" "${3:-}" "versioning=" "versioning"
}

s3_get_bucket_lifecycle() {
  s3_signed_curl GET "$1" "" "$2" "" "${3:-}" "lifecycle=" "lifecycle"
}

s3_put_bucket_versioning() {
  s3_signed_curl PUT "$1" "" "$2" "$3" "" "versioning=" "versioning"
}

s3_put_bucket_lifecycle() {
  s3_signed_curl PUT "$1" "" "$2" "$3" "" "lifecycle=" "lifecycle"
}
