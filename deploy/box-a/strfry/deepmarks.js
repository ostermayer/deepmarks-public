...
//   24133  NIP-46 nostr-connect messages (api ↔ Box C bunker)
//
// Kind 24133 is the plumbing that keeps nsecs off Box A: api
...
//   30001  NIP-51 deprecated "categorized bookmarks/sets" — the
//          predecessor to 30003 that 2023–2024 clients still wrote.
//          Accepted so the onboarding scanner can mirror a user's
//          legacy bookmark set; without it strfry bounces the import
//          and those users see an empty list. Opaque ciphertext to us.
...
const ALLOWED_KINDS = new Set([0, 5, 39701, 30000, 30003, 30001, 10003, 3, 10000, 10002, 9735, 1985, 24133]);
...
// kind:39701 bookmarks get their own (larger) budget: every write
// already passes the authenticated /publish gate + registered-pubkey
// check, and a first-run import of a few thousand bookmarks is the
// product's headline flow — the old shared 200/h general bucket dropped
// most of a big import after retries were exhausted.
const BOOKMARK_RATE_LIMIT_PER_HOUR = Number.parseInt(
  process.env.STRFRY_BOOKMARK_RATE_LIMIT_PER_HOUR ?? '1000',
  10,
);
...
// api adds real Deepmarks users and followed curators whose
...
function rateLimitOk(pubkey, limit = RATE_LIMIT_PER_HOUR, scope = 'general') {
  ...
  // Count ACCEPTED events only. Counting rejects too made the
  // relay-fanout worker's retries burn the next window before it ever
  // delivered anything — a rejected import could starve itself forever.
...
function rateLimitForEvent(event) {
  if (event.kind === 39701) {
    return { limit: BOOKMARK_RATE_LIMIT_PER_HOUR, scope: 'bookmarks' };
  }
  if (event.kind !== 30003) {
  ...
  if (
    ...
    // Per-item delete tombstones ride alongside chunk rewrites (one per
    // deleted bookmark) — same private-state machinery, same budget.
    // In the general bucket a bulk cleanup lost tombstones, letting
    // deleted private bookmarks resurrect from stale devices.
    d.startsWith('deepmarks-private-item:') ||
    d.startsWith('deepmarks-collection-private:') ||
    d === 'deepmarks-nwc'
  ) {
...
rl.on('line', async (line) => {
  ...
  // api. NIP-46 (24133) bypasses the gate because the
