...
//
//   0      NIP-01 profile metadata for registered users
//   5      NIP-09 deletion requests for registered users
...
// Kinds any REGISTERED user may publish on this relay.
...
//   3      NIP-02 contact list — accepted from registered users so the
...
// bookmarks and team posts only. Registered users CAN still post
// kind:1 through our endpoint (so the mobile/web app can publish to
// one socket), but the writePolicy below pushes the event onto a
// fanout queue and returns shadowReject so strfry never persists it.
// The fanout worker reads the queue and broadcasts to the user's
// NIP-65 advertised relays (Damus, Primal, nos.lol, …).
const ALLOWED_KINDS = new Set([0, 5, 39701, 30003, 10003, 3, 10000, 10002, 9735, 1985, 24133]);
...
// Registered-pubkey gate. Deepmarks-relay-first means we accept events
// from third-party clients, but we only persist them when the author
// is a registered Deepmarks user. Registration is auto-populated by
// payment-proxy on first NIP-98 auth, lifetime markPaid, username
// claim, or first bookmark ingest.
//
// The registry lives in Redis as a set: dm:registered:pubkeys
//   SISMEMBER dm:registered:pubkeys <hex-pubkey>  → 1 if registered
//
...
rl.on('line', async (line) => {
  ...
  // Kind:1 from a non-team registered user is a CROSS-POST through
  // our relay, not a request to store. The user's mobile/web client
  // stays connected to relay.deepmarks.org only; we asynchronously
  // mirror the note out to their NIP-65 advertised relays so it
  // reaches the rest of Nostr (Damus, Primal, nos.lol, …). The note
  // itself is not persisted here — strfry would otherwise turn into
  // a general-purpose text-note relay, which is explicitly out of
  // scope.
  //
  // Flow: registered check first (avoid building a queue of spam
  // from unregistered pubkeys), then push to the fanout queue and
  // shadowReject (relay returns OK to the publisher, never writes
  // to LMDB). The kind:1-fanout worker BRPOPs the queue and
  // broadcasts.
