...
# The PRODUCTION redis, used to distinguish a deletion race from an
# orphaned record when a sampled blob 404s (see sample_archive_blobs).
LIVE_REDIS_CONTAINER="${LIVE_REDIS_CONTAINER:-box-a-redis-1}"
RESTORE_TEST_TMP_PARENT="${RESTORE_TEST_TMP_PARENT:-/var/tmp}"

source "$COMPOSE_DIR/s3-sigv4.sh"
require_var LINODE_ACCESS_KEY
require_var LINODE_SECRET_KEY

mkdir -p "$RESTORE_TEST_TMP_PARENT"
TMPDIR="$(mktemp -d "${RESTORE_TEST_TMP_PARENT%/}/deepmarks-restore-test.XXXXXX")"
...
download_manifest_and_object() {
  ...
  s3_get "$BACKUP_S3_BUCKET" "$latest_key" "$S3_ENDPOINT" "$manifest"
  ...
  echo "  object: s3://${BACKUP_S3_BUCKET}/${object_key}"

  echo "→ downloading $label object"
  s3_get "$BACKUP_S3_BUCKET" "$object_key" "$S3_ENDPOINT" "$object_file"
...
'
  ...
  local import_log="$TMPDIR/strfry-import.log"
  gzip -cd "$archive" | docker run --rm -i \
    ...
    box-a-strfry:latest \
    strfry --config=/etc/strfry.conf import >/dev/null 2>"$import_log" \
    || { tail -20 "$import_log" >&2; exit 1; }

  # strfry import applies NIP-09 deletion processing: a stored kind:5
  # whose a-tag target is also in the export removes that target from
  # the restored copy, so the restored count can legitimately come up
  # short of the manifest's line count. The live relay can retain such
  # targets (first seen 2026-07-29: a deleted→recreated→re-deleted
  # kind:30000 survived live ingest but not re-import). Every missing
  # event must be matched by a "Deleting" line in the import log —
  # anything unexplained still fails the drill.
  local deletions
  deletions="$(grep -c 'INFO| Deleting ' "$import_log" || true)"
  ...
  if [ "$((restored_count + deletions))" != "$expected_count" ]; then
    echo "✗ restored strfry event count mismatch: got $restored_count restored + $deletions deleted-at-import, expected $expected_count" >&2
    grep 'INFO| Deleting ' "$import_log" >&2 || true
    tail -20 "$import_log" >&2
    exit 1
  fi
  if [ "$deletions" != "0" ]; then
    echo "  ✓ restored strfry database exported $restored_count events ($deletions removed at import by stored kind:5 deletions)"
    grep 'INFO| Deleting ' "$import_log" | tail -5
  else
...
' | awk -v limit="$limit" '/^[0-9a-f]{64}$/ && !seen[$0]++ && count < limit { print; count++ }' || true)"
  ...
  while IFS= read -r hash; do
    ...
    if ! curl --fail --silent --show-error --location --max-time 20 -D "$header" -o "$out" "$url"; then
      # The sample set comes from the RESTORED snapshot, so a blob whose
      # archive the owner deleted after the backup ran legitimately has
      # no bytes anymore — that must not fail the drill. A hash the LIVE
      # records still reference is the opposite: an orphaned record
      # (bytes lost while a user still expects them), exactly what this
      # sampling exists to catch. First real catch: 2026-07-17, blob
      # 8620004d… — referenced live, missing from primary and mirrors.
      live_state="$(docker exec -e H="$hash" "$LIVE_REDIS_CONTAINER" sh -c '
        for key in $(redis-cli --scan --pattern "dm:archives:*"); do
          if [ "$(redis-cli HEXISTS "$key" "$H")" = "1" ]; then echo referenced; exit 0; fi
        done
        echo unreferenced' 2>/dev/null | tail -1)"
      if [ "$live_state" = "unreferenced" ]; then
        echo "  skipped $hash: gone from Blossom and no longer referenced live (deleted since snapshot)"
        rm -f "$out" "$header"
        continue
      fi
      echo "✗ archive blob $hash is referenced by LIVE records but missing from Blossom (orphaned record)" >&2
      exit 1
    fi
    local size
