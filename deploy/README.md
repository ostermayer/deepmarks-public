...
- **box-b** (VPC 10.0.0.3) — private worker: archive-worker (Playwright + SingleFile), Dedicated 4 GB with Docker `cpus: "1.5"` and `MAX_CONCURRENT_JOBS=4`
- **box-c** (VPC 10.0.0.4) — private signer: nsecBunker (holds operational nsecs, talks NIP-46 to payment-proxy via strfry)
...
Keep the CPU cap in place unless you are deliberately trading SSH/deploy
headroom for faster backlog drain.
...
latest Redis + strfry backup manifests from Object Storage, checks
sizes and SHA-256 hashes, restores them into throwaway containers, and
samples Blossom archive blobs. Full runbook:
