...
DEDUP_SECONDS=600
# Box C: used by deepmarks-uptime-check to probe Box A's redis for the
# archive-worker heartbeat.
# Box A: used by deepmarks-resource-check to inspect Redis persistence.
# Box A's REDIS_PASSWORD must match.
REDIS_HOST=10.0.0.2
REDIS_PORT=6379
REDIS_PASSWORD=
ARCHIVE_WORKER_HEARTBEAT_KEY=dm:archive:worker-heartbeat
# Box A only — user that owns the backup/restore systemd timers.
BACKUP_SYSTEMD_USER=dan
