...
| 3 | Host resource alerts (disk > 80 %, memory > 90 %, container restart loops, backup/restore failures, Redis persistence failures, Blossom local-storage fallback, macaroon age) | `deepmarks-resource-check` — runs every 15 min via cron on every box |
...
- `deepmarks-resource-check` — disk + memory + container restart + Box A data-durability + macaroon-age checks.
...
Tier-1 app alerts are sent by `payment-proxy` itself, not by the cron
...
If payment-proxy starts with app alerts disabled, its logs include:

```text
[alerter] no recipient configured
```

After changing Box A's `.env`, restart only payment-proxy:

```bash
ssh dan@<box-a> \
  'cd /opt/deepmarks-repo/deploy/box-a && docker compose up -d --no-deps payment-proxy'
```
