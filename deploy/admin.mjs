 ...
 *   archive-rescue [--run] [--force] [--limit=N] [--pubkey=npub]  Find public alternatives for failed archives
 *   archive-retry  [--run] [--force] [--limit=N] [--pubkey=npub]  Re-enqueue terminal archive failures
 *
...
function usage() {
  ...
  console.error('  archive-retry  [--run] [--force] [--limit=N] [--pubkey=npub]');
  console.error('                               re-enqueue terminal archive failures (dry-run unless --run)');
  process.exit(2);
}
...
switch (cmd) {
  ...
  case 'archive-retry': {
    // Re-enqueue terminal archive failures from scratch (PUBLIC webpage/file
    // only — private/media are skipped server-side, they need client retry).
    // Use this to pick up worker-side fixes (browser UA, FixTweet, etc.).
    // Dry-run by default; --run to enqueue, --force to retry recently-tried.
    const run = args.includes('--run');
    const force = args.includes('--force');
    const limitArg = args.find((a) => a.startsWith('--limit='));
    const pubArg = args.find((a) => a.startsWith('--pubkey='));
    ...
    if (limitArg) body.limit = Number.parseInt(limitArg.split('=')[1], 10);
    if (pubArg) body.pubkey = decodePubkey(pubArg.split('=')[1]);
    await nip98Request('POST', '/admin/archive-failures/retry', body);
    break;
  }
  default:
    usage();
