...
- **LLM augmentation:** optional DeepInfra bookmark enrichment, semantic
  search embeddings, import cleanup suggestions, archive summaries, and
  operator alert digests. See
  [`../docs/llm-augmentation.md`](../docs/llm-augmentation.md).
...
| `GET` | `/search/semantic/public` | LLM embedding search over enriched public bookmarks | none/rate-limited |
| `GET` | `/llm/status` | LLM availability, model policy, and active models | none |
| `POST` | `/llm/import/cleanup` | Import cleanup suggestions | NIP-98 |
| `GET` | `/llm/collections/suggest` | User tag/category suggestions from LLM-enriched bookmarks | NIP-98 |
...
As of 2026-06-25 the workers run as **separate containers** off the same
image (`src/worker.ts`, selected by `WORKER_GROUP`), not inside the API
process. The API runs `RUN_WORKERS=none`; set `RUN_WORKERS=all` to fold
the whole fleet back into the API (dev / single-box). Each container hosts
one task group, assembled by `buildWorkerGroup()` in `workers-bootstrap.ts`:

- **`worker-search-indexer`** — `indexer`, `save-count-tracker`, `zap-listener` (relay→Meilisearch writers)
- **`worker-relay-sync`** — `relay-fanout`, `onboarding-scanner`, `follows-ingester`, `profile-resolver`, `pinboard-seeder` (the network-heavy outbox)
- **`worker-enrichment`** — `llm-enrichment`
- **`worker-payments`** — Lightning invoice listener (zap receipts + legacy metered settlement), `lifetime-archive-backfill`, boot-time lifetime-label recovery

Live worker stats heartbeat to Redis (`dm:worker-stats:*`) and are read by
`/admin/dashboard` (no longer in-process memory). What each worker does:
...
- `llm-enrichment` processes bookmark/archive/alert LLM jobs and, when
  explicitly enabled, backfills existing Meilisearch bookmark documents.
...
uploads bytes to Blossom and calls `/archive/callback`; api
...
- Passkey nsec ciphertext is stored encrypted; api cannot
  decrypt it.
...
- The LND invoice macaroon is invoice-only. It can create/read invoices but
