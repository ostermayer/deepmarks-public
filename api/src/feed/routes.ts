// Atom feed routes:
//   /feed/{network,recent,popular}.xml
//   /feed/tags/:tag.xml
//   /feed/user/:slug.xml
//   /feed/friends/:slug.xml
//
...
import {
  FRIENDS_SET_NAME,
  friendPubkeysFromEvents,
  mergeFriendsBookmarks,
  socialLinksToBookmarks,
} from './friends.js';
...
const DEFAULT_PUBLIC_BRAND_PUBKEY =
  '2944e915ba71cf0fc19f5dda048ce053a87c01fd7478b179330a17edca4ce2f4';
const LEGACY_ADMIN_SEEDER_PUBKEY =
...
interface FeedDeps {
  ...
  deepmarksSeederPubkey: string;
  deepmarksEditorialPubkeys: string[];
...
function hidePubkeysSet(deps: FeedDeps): Set<string> {
  return new Set(deps.deepmarksEditorialPubkeys);
}
...
export function applyAttributionPreference(
  ...
  // Compare CANONICAL urls: the seeder stores canonicalized URLs, but a
  // real user's save can carry a trailing slash or ?utm_* — a raw string
  // compare missed those, so the seeded mark was not hidden (2026-08-23
  // review #8; violates the documented attribution rule).
  const urlsFromOthers = new Set<string>();
  for (const b of bookmarks) {
    if (!hidePubkeys.has(b.pubkey)) urlsFromOthers.add(canonicalizeUrlForIndex(b.url));
  }
  return bookmarks.filter(
    (b) => !hidePubkeys.has(b.pubkey) || !urlsFromOthers.has(canonicalizeUrlForIndex(b.url)),
  );
}
...
export function registerFeedRoutes(app: FastifyInstance, deps: FeedDeps): void {
  ...
  async function renderRecent(): Promise<string> {
    ...
    filtered.sort((a, b) => bookmarkSortScore(b) - bookmarkSortScore(a));
  ...
  app.get<{ Params: { tag: string } }>(
    ...
    async (req, reply) => {
      ...
      // Match the app's committed tag tokens: lower-case letters,
      // digits, dots, and hyphens. Slashes stay rejected so each tag
      // remains one path segment.
      ...
      const body = buildAtomFeed(
        {
          ...
          htmlUrl: `${deps.publicBaseUrl}/app/explore?tag=${encodeURIComponent(tag)}`,
  ...
  app.get<{ Params: { slug: string } }>(
    ...
    async (req, reply) => {
      ...
      bookmarks.sort((a, b) => bookmarkSortScore(b) - bookmarkSortScore(a));
      const body = buildAtomFeed(
        {
  ...
  // ── /feed/friends/:slug.xml — links from a user's public Deepmarks friends set
  app.get<{ Params: { slug: string } }>(
    '/feed/friends/:slug.xml',
    async (req, reply) => {
      const slug = req.params.slug;
      ...
      const key = `friends:${hex}`;
      const hit = cache.get(key);
      if (hit) return sendXml(reply, hit);

      const friendsEvents = await fetchEvents(deps.pool, deps.indexerRelay, {
        kinds: [30000],
        authors: [hex],
        '#d': [FRIENDS_SET_NAME],
        limit: 5,
      });
      const friendPubkeys = friendPubkeysFromEvents(friendsEvents).slice(0, 500);
      let bookmarks: BookmarkJson[] = [];
      if (friendPubkeys.length > 0) {
        const [bookmarkEvents, noteEvents] = await Promise.all([
          fetchEvents(deps.pool, deps.indexerRelay, {
            kinds: [39701],
        ...
        bookmarks = mergeFriendsBookmarks(
          applyAttributionPreference(eventsToBookmarks(bookmarkEvents), hidePubkeysSet(deps)),
          socialLinksToBookmarks(noteEvents),
        ).slice(0, DEFAULT_LIMIT);
      }
      const body = buildAtomFeed(
        {
          title: `Deepmarks · Friends of ${slug.length <= 30 ? slug : npub.slice(0, 12) + '…'}`,
          subtitle: 'Public links shared by this user’s Deepmarks friends.',
          htmlUrl: `${deps.publicBaseUrl}/app/friends`,
          feedUrl: `${deps.publicBaseUrl}/feed/friends/${encodeURIComponent(slug)}.xml`,
          id: `${deps.publicBaseUrl}/feed/friends/${hex}`,
        },
        bookmarks,
      );
      cache.set(key, body);
...
export function feedDepsFromEnv(pool: SimplePool): FeedDeps {
  ...
  // PUBLIC_BASE_URL is the API host (api.deepmarks.org) because NIP-98
  // auth and payment callbacks bind to that origin. Feeds are public
  // website assets, so keep their browser/self links on the apex.
  const publicBaseUrl = process.env.FEED_PUBLIC_BASE_URL ?? 'https://deepmarks.org';
  // Public brand/social pubkey drives the Deepmarks social presence and
  // the daily Pinboard bookmark. Attribution filtering keeps the legacy
  // admin/operational seeder key too so old imported rows don't compete
  // with real user saves.
  const deepmarksPubkey = firstNonEmpty(
    process.env.DEEPMARKS_PUBLIC_BRAND_PUBKEY,
    process.env.VITE_DEEPMARKS_PUBKEY,
    DEFAULT_PUBLIC_BRAND_PUBKEY,
  )!;
  const deepmarksSeederPubkey = firstNonEmpty(
    process.env.DEEPMARKS_SEEDER_PUBKEY,
    deepmarksPubkey,
  )!;
  const deepmarksEditorialPubkeys = Array.from(
    new Set([deepmarksSeederPubkey, deepmarksPubkey, LEGACY_ADMIN_SEEDER_PUBKEY].filter(Boolean)),
  );
  return {
    pool,
    indexerRelay,
