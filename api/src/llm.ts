...
export class DeepInfraClient {
  ...
  async enrichBookmark(input: BookmarkEnrichmentInput): Promise<BookmarkEnrichment | null> {
    if (!this.config) return null;
      ...
      messages: [
        {
          ...
          content: [
            ...
            'Tags must be lowercase short topical tokens, not phrases.',
          ].join(' '),
        },
  ...
  private async chatJson<T>(opts: {
    ...
    }>(`${this.config.baseUrl}/chat/completions`, body);
    const content = raw.choices?.[0]?.message?.content;
    if (!content) throw new Error('DeepInfra returned no content');
    let parsed: unknown;
    try {
      parsed = JSON.parse(content);
    } catch (err) {
      throw new Error(`DeepInfra returned invalid JSON: ${(err as Error).message}`);
    }
    return opts.zodSchema.parse(parsed);
  }

  private async fetchJson<T>(url: string, body: unknown): Promise<T> {
    if (!this.config) throw new Error('DeepInfra client is disabled');
    ...
    if (!res.ok) {
      ...
      throw new Error(`DeepInfra ${res.status}: ${text.slice(0, 500)}`);
    }
