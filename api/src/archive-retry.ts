...
export function archiveFailureRetryEligibility(
  ...
  const base = archiveFailureReplayEligibility(failure);
  if (!base.eligible) return base;
  // A permanent failure (page gone, over the size cap) fails identically on
  // a blind retry of the same URL — rescue, not retry, is the recovery path.
  if (isPermanentArchiveFailureReason(failure.reason)) {
    return { eligible: false, reason: 'permanent-failure' };
  }
