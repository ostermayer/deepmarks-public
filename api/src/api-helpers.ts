...
function parseUnixMillis(value: string | undefined, expectedSeconds: number): number | undefined {
  if (!value || !/^\d+$/.test(value)) return undefined;
  const parsed = Number(value);
...
export function bookmarkSortScore(bookmark: Pick<BookmarkJson, 'savedAt' | 'savedAtMs'>): number {
...
export function bookmarkEventToJson(event: NostrEvent): BookmarkJson {
  ...
  const savedAtMs = parseUnixMillis(get('published_at_ms'), savedAt);
  return {
    id: event.id,
    pubkey: event.pubkey,
    url,
