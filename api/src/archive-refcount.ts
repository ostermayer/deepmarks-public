 ...
 *
 *  KNOWN RESIDUAL RACE (2026-08-23 review #12, accepted for now): a
 *  concurrent addArchiveRef can SADD between this returning 0 and the
 *  caller's physical blob delete, orphaning the adder's just-recorded
 *  archive. The window is milliseconds and requires a byte-identical
 *  blob completing for another user during a delete. Closing it needs
 *  a delete-tombstone consulted by the dedupe path (so a racing add
 *  re-uploads instead of ref'ing a dying blob) — tracked as a design
 *  follow-up, not patched ad hoc. */
  ...
  try {
    do {
      ...
      for (const archivesKey of keys) {
        ...
        const records = await redis.hgetall(archivesKey);
        const blobHashes = Object.keys(records);
        ...
        for (const [blobHash, raw] of Object.entries(records)) {
          for (const refHash of archiveRefHashesFromRecord(blobHash, raw)) {
            pipeline.sadd(refsKeyFor(refHash), pubkey);
            refsAdded += 1;
          }
        }
...
function archiveRefHashesFromRecord(primaryBlobHash: string, raw: string): string[] {
  const hashes = new Set<string>();
  if (/^[0-9a-f]{64}$/.test(primaryBlobHash)) hashes.add(primaryBlobHash);
  try {
    const parsed = JSON.parse(raw) as { blobHash?: unknown; files?: unknown };
    if (typeof parsed.blobHash === 'string' && /^[0-9a-f]{64}$/.test(parsed.blobHash)) {
      hashes.add(parsed.blobHash);
    }
    if (Array.isArray(parsed.files)) {
      for (const file of parsed.files) {
        if (!file || typeof file !== 'object') continue;
        const blobHash = (file as { blobHash?: unknown }).blobHash;
    ...
    // Legacy/corrupt record: the hash key is still usable if valid.
  }
  return [...hashes];
}
