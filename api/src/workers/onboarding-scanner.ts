...
export class OnboardingScannerWorker {
  ...
  private async scan(pubkey: string): Promise<void> {
    ...
    if (relays.length === 0) {
      ...
      // Indistinguishable from a discovery-relay timeout — retry tomorrow
      // instead of holding the 30-day done-marker (2026-08-23 review).
      await this.deps.redis.expire(markerKey, 24 * 60 * 60).catch(() => undefined);
      return;
    }
    ...
    if (events.length === 0) {
      ...
      // queryWithTimeout returns [] for both "empty account" and "every
      // relay timed out" — retry tomorrow rather than marking the import
      // done for 30 days on a timeout (2026-08-23 review).
      await this.deps.redis.expire(markerKey, 24 * 60 * 60).catch(() => undefined);
      return;
    }
  ...
  private async fetchBookmarkedNoteTargets(listEvents: NostrEvent[], relays: string[]): Promise<NostrEvent[]> {
    ...
    return fetchBookmarkedKind1Targets(queryRelays, targets.ids, {
