...
import { subscribeSingleRelay } from '../relay-helpers.js';
...
export class ProfileResolver {
  ...
  async start(): Promise<void> {
    ...
    this.bookmarksSub = subscribeSingleRelay(
      this.pool,
      this.deps.relayUrl,
    ...
    this.profilesSub = subscribeSingleRelay(
      this.pool,
      this.deps.relayUrl,
      [{ kinds: [0], since }],
      {
...
export async function cacheProfileEvent(redis: Redis, event: NostrEvent): Promise<void> {
  ...
  // kind:0 is replaceable — never let a LAGGING relay roll the cache
  // back to an older profile (a stale mirror used to overwrite a fresh
  // rename/picture on every ingest pass; 2026-08-23 review). Same-age
  // events keep last-writer-wins.
  try {
    const existingRaw = await redis.get(`dm:profile-event:${pubkey}`);
    if (existingRaw) {
      const existing = JSON.parse(existingRaw) as { created_at?: number };
      if (typeof existing.created_at === 'number' && existing.created_at > event.created_at) return;
    }
  } catch {
    // Unreadable cache entry — overwrite it below.
  }
