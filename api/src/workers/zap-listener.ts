...
export class ZapReceiptListener {
  ...
  private async handleReceipt(event: NostrEvent): Promise<void> {
    ...
    // Web Push side: notify the zap recipient regardless of issuer.
    // Trust gate below is only for our zap-ranking aggregation; for
    // a notification the user wants to know about any zap that hit
    // them. Fire-and-forget so a slow push delivery doesn't block
    // ranking flush.
    if (recipientPubkey && /^[0-9a-f]{64}$/.test(recipientPubkey)) {
      void this.notifyZapRecipient(event, recipientPubkey, targetEventId);
    }
    ...
    // Per-receipt dedup: the resume filter's `since` is inclusive and the
    // checkpoint is the last receipt's created_at, so every restart used
    // to re-count the most recent receipt(s), inflating ranking totals
    // (2026-08-23 review). 48h TTL comfortably covers the 24h resume cap.
    const fresh = await this.deps.redis.set(`dm:zap-seen:${event.id}`, '1', 'EX', 2 * 86_400, 'NX');
    if (fresh !== 'OK') return;
  ...
  private async notifyZapRecipient(
    event: NostrEvent,
    recipientPubkey: string,
    targetEventId: string | undefined,
  ): Promise<void> {
    try {
      const { sendPush, ensureVapid } = await import('../web-push.js');
      if (!ensureVapid()) return;
      const amountMsat = extractAmountMsat(event);
      const sats = amountMsat ? Math.floor(amountMsat / 1000) : 0;
      // Idempotency tag: same zap (same receipt event id) replaces the
      // notification instead of stacking. Also collapses repeated
      // pushes for the same bookmark to one entry in the tray.
      const tag = `zap:${targetEventId ?? event.id}`;
      const title = sats > 0 ? `⚡ ${sats.toLocaleString()} sats received` : '⚡ Zap received';
      const body = sats > 0
        ? `Someone zapped one of your bookmarks ${sats.toLocaleString()} sats.`
        : 'Someone zapped one of your bookmarks.';
      const url = '/app/bookmarks';
      const pushLogger = {
        info: this.deps.logger.info,
        warn: this.deps.logger.error, // zap-listener logger has no `warn`; route to error
      };
      const result = await sendPush(this.deps.redis, recipientPubkey, {
        title, body, url, tag,
      }, pushLogger);
      if (result.delivered > 0) {
        this.deps.logger.info(
          { recipient: recipientPubkey, sats, ...result },
          'zap push delivered',
        );
      }
    } catch (err) {
      this.deps.logger.error({ err }, 'zap push notify failed');
    }
  }
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      // full-document REPLACE): flushing this {id, zap_total} stub
