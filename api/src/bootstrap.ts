...
// (NIP-98 verify, rate-limit, requireAdmin), and returns
...
import { metadataFromBookmarkedUrl } from './bookmark-metadata-fallback.js';
...
import { QdrantSemanticStore, qdrantConfigFromEnv } from './semantic-vector-store.js';
...
//
// Native Capacitor builds load from local app origins, not deepmarks.org.
// Keep those built in so mobile first-paint API reads work even if a box
// only configures the public web origin.
...
const CORS_ORIGIN = Array.from(new Set([
  ...(process.env.CORS_ORIGIN ?? 'https://deepmarks.org')
  .split(',')
  .map((s) => s.trim())
  .filter((s) => s.length > 0),
...
export async function buildDeps(app: FastifyInstance): Promise<Deps> {
  ...
  const requireNip98 = makeRequireNip98(nip98, redis);
  ...
  const userSettingsStore = new UserSettingsStore(redis);
  const mediaArchiveAddonStore = new MediaArchiveAddonStore(redis);
  ...
  let llmConfig: ReturnType<typeof deepInfraConfigFromEnv> = null;
  try {
    llmConfig = deepInfraConfigFromEnv();
  } catch (err) {
    app.log.error({ err }, 'DeepInfra LLM enrichment disabled by model policy');
  }
  const llm = new DeepInfraClient(llmConfig);
  if (llm.enabled) {
    app.log.info({ models: llm.modelSummary() }, 'DeepInfra LLM enrichment enabled');
  } else {
    app.log.warn('DeepInfra LLM enrichment disabled — set DEEPINFRA_TOKEN to enable');
  }
  const archiveRescueSearch = new ArchiveRescueSearchClient(archiveRescueSearchConfigFromEnv());
  if (archiveRescueSearch.enabled) {
    app.log.info({ search: archiveRescueSearch.summary() }, 'archive rescue web search enabled');
  } else {
    app.log.warn('archive rescue web search disabled — set ARCHIVE_RESCUE_SEARCH_PROVIDER/URL to enable');
  }
  const qdrantConfig = qdrantConfigFromEnv();
  const semanticStore = qdrantConfig ? new QdrantSemanticStore(qdrantConfig) : null;
  if (semanticStore) {
    app.log.info({ url: qdrantConfig!.url, collection: qdrantConfig!.collection }, 'Qdrant semantic vector store enabled');
  } else {
    app.log.warn('Qdrant semantic vector store disabled — set QDRANT_URL to enable ANN search');
  }
  ...
  const lnIdentities: Record<string, RemoteSigner> = {
    zap: signers.brand,
    // Legacy address mapping. In current production this signer is also
    // the public brand/social profile used for daily Pinboard posts.
    dan: signers.personal,
  };
  const relayPool = createRelayPool();
  const metadataStore = new MetadataStore(redis, (url) =>
    metadataFromBookmarkedUrl({
      redis,
      relayPool,
    ...
    // Bot-walled hosts (WSJ & co. answer 401 challenges to every datacenter
    // fetch) fall back to the rescue web search for a host+path-bound title.
    archiveRescueSearch,
  );
  ...
  const LIFETIME_LABEL_RELAYS = (process.env.LIFETIME_LABEL_RELAYS ?? 'ws://strfry:7777,wss://nos.lol,wss://relay.primal.net')
  ...
  const rawArchiveWorkerPubkey = (process.env.ARCHIVE_WORKER_PUBKEY ?? '').trim().toLowerCase();
  const ARCHIVE_WORKER_PUBKEY = /^[0-9a-f]{64}$/.test(rawArchiveWorkerPubkey)
    ? rawArchiveWorkerPubkey
    : null;
  if (ARCHIVE_WORKER_PUBKEY) {
    app.log.info(
      { pubkey: ARCHIVE_WORKER_PUBKEY.slice(0, 12) + '…' },
      'archive worker blossom pubkey loaded',
    );
  } else {
    app.log.warn('ARCHIVE_WORKER_PUBKEY is empty or invalid — archive-worker Blossom uploads will require lifetime/standalone access');
  }
