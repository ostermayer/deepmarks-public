...
export function createRelayPool(): SimplePool {
  // enableReconnect is NOT the default: without it a hard close runs
  // closeAllSubscriptions and nothing ever resubscribes, so a strfry
  // restart silently killed every worker subscription until the
  // container restarted (2026-08-23 review #3). With it, nostr-tools
  // reconnects with backoff and re-fires each open subscription with
  // since = lastEmitted + 1 (gap-free resume). enablePing makes a
  // half-dead TCP connection actually reach that reconnect path.
...
// kind:1985 label event from the Deepmarks admin/operational pubkey attesting that
