...
export class ZapStore {
  ...
  /**
   * Atomically claim a pending zap for receipt processing. Returns the
   * record only to the ONE caller that wins the claim (SET NX on a sibling
   * `:claimed` marker); near-simultaneous / redelivered LND callbacks get
   * null so they can't double-publish a kind:9735.
   *
   * Unlike a GET+DEL, this does NOT delete the pending record — the caller
   * must call `finalize()` once the receipt is signed (drops the record;
   * the claim marker's TTL blocks late redelivery) or `release()` if signing
   * failed (e.g. a transient bunker outage) so a redelivered `invoice_updated`
   * can retry while the record + invoice are still live. Deleting before the
   * bunker signed would drop the receipt permanently on any signing error.
   */
  async claim(paymentHash: string): Promise<PendingZap | null> {
    const claimKey = ZAP_PREFIX + paymentHash + ':claimed';
    const claim = await this.redis.set(claimKey, '1', 'EX', ZAP_TTL_SECONDS, 'NX');
    if (claim !== 'OK') return null;
    const rec = await this.get(paymentHash);
    if (!rec) {
  ...
  /** Receipt signed — drop the pending record. The `:claimed` marker is left
   *  to expire on its own TTL, blocking any late redelivery from reprocessing. */
  async finalize(paymentHash: string): Promise<void> {
    await this.redis.del(ZAP_PREFIX + paymentHash).catch(() => undefined);
  }

  /** Signing/publishing failed — release the claim so a redelivered
   *  `invoice_updated` can retry. The pending record is left intact. */
  async release(paymentHash: string): Promise<void> {
    await this.redis.del(ZAP_PREFIX + paymentHash + ':claimed').catch(() => undefined);
  }
}
