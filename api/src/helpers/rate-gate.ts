...
export function makeGateRateLimit(rateLimit: RateLimitFn): GateRateLimitFn {
  return async function gateRateLimit(
    ...
    limit: number,
    windowSeconds: number,
    // `label` is the client-visible error string. The default matches
    // what every route historically sent; routes that distinguish
    // multiple gates in one handler pass e.g. 'rate limit (pubkey)'.
    label = 'rate limit',
  ): Promise<boolean> {
    const r = await rateLimit(bucket, key, limit, windowSeconds);
    if (r.ok) return true;
    reply.header('Retry-After', String(r.retryAfter));
    reply.status(429).send({ error: label, retryAfter: r.retryAfter });
    return false;
  };
}
