...
export function archiveFailureRescueEligibility(
  ...
  if (kind !== 'webpage') return { eligible: false, reason: 'non-webpage-archive' };
  try {
...
async function gatherArchiveRescueCandidates(
  ...
  const wayback = await waybackCandidates(failure.url, fetchImpl, options.timeoutMs ?? VERIFY_TIMEOUT_MS)
    .catch((err) => {
  ...
  const all = [...deterministic, ...socialMirror, ...scholarlyPdf.candidates, ...wayback, ...llmCandidates, ...searchCandidates]
...
async function verifyArchiveRescueCandidate(
  ...
  if (contentType && !isRescuableContentType(contentType)) return null;
...
// ─── Known social-media mirrors ──────────────────────────────────────
// x.com / Twitter render content client-side and block scrapers, so the
// archive worker captures an empty shell. Nitter-style mirrors (xcancel,
// nitter) serve the SAME tweet as static HTML that archives cleanly. We
// only ever mirror to a fixed allowlist of hosts, and only when the tweet
// status id matches — so a mirror candidate can never smuggle in
// unrelated content past candidateAllowedForSource / the SSRF check.
const TWITTER_FAMILY_HOSTS = new Set([
  'x.com', 'twitter.com', 'mobile.twitter.com', 'xcancel.com', 'nitter.net',
]);
const X_MIRROR_HOSTS = ['xcancel.com', 'nitter.net'] as const;

function isTwitterFamilyHost(host: string): boolean {
  return TWITTER_FAMILY_HOSTS.has(normalizedHost(host));
}
...
function socialMirrorCandidates(rawUrl: string): ArchiveRescueCandidate[] {
  ...
  if (!isTwitterFamilyHost(parsed.hostname) || !twitterStatusId(parsed)) return [];
  const sourceHost = normalizedHost(parsed.hostname);
  const out: ArchiveRescueCandidate[] = [];
  for (const host of X_MIRROR_HOSTS) {
    if (sourceHost === host) continue; // don't mirror a host to itself
    const mirror = new URL(parsed.toString());
    mirror.protocol = 'https:';
    mirror.hostname = host;
    mirror.search = '';
    mirror.hash = '';
    addCandidate(out, mirror.toString(), 'social-mirror', `Public ${host} mirror of the same tweet.`, 0.82);
  }
  return dedupeCandidates(out);
}

function isSocialMirrorPair(original: URL, candidate: URL): boolean {
  if (!isTwitterFamilyHost(original.hostname)) return false;
  if (!(X_MIRROR_HOSTS as readonly string[]).includes(normalizedHost(candidate.hostname))) return false;
  const originalId = twitterStatusId(original);
  return originalId !== null && originalId === twitterStatusId(candidate);
}
...
function candidateAllowedForSource(
  ...
  // Known social-media mirror of the SAME tweet (e.g. x.com → xcancel).
  // Not source-gated, so both directly-constructed mirrors and
  // searxng-found mirror URLs pass.
  if (isSocialMirrorPair(original, candidate)) return true;
