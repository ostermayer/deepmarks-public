...
import {
  ...
  archiveFailureReplayEligibility,
  listArchiveFailures,
...
// Tweet policy lives in its own dependency-free module (tweet-policy.ts) so
// the cross-package sync test can import it without this file's dep chain.
// A tweet rescue can't be verified by fetching the page (the x.com URL is an
// empty JS shell) — we confirm the tweet is servable via FixTweet and
...
export async function rescueArchiveFailure(
  ...
  // Rescue deliberately has NO permanent-reason gate on top of the shared
  // eligibility: rescuing a 'not-found' page via Wayback/mirrors is exactly
  // what the rescue pass is for. Only the blind same-URL retry sweep
  // (archiveFailureRetryEligibility) skips permanent failures.
  const eligibility = archiveFailureReplayEligibility(failure);
...
async function verifyArchiveRescueCandidate(
  ...
  // Follow redirects MANUALLY, re-validating the SSRF guard and (for the
  // real path) re-pinning the resolved IP on every hop. The old
  // `redirect: 'follow'` let a candidate 30x to an internal target
  // (10.0.0.x / 169.254.169.254 / Box C) with no re-check — undici issued
  // the internal request before the post-fetch `finalUrl` guard ran, so
  // that guard only prevented *enqueuing* the internal URL, not the request
  // that already fired. When a test injects `options.fetch` we keep using
  // it verbatim; otherwise real fetches go through the IP-pinning safeFetch.
  const doFetch: typeof fetch = fetchImpl === fetch
    ? (((input, init) =>
        safeFetch(input as string | URL, init as Omit<RequestInit, 'dispatcher'>, {
          dnsLookup: options.dnsLookup,
        })) as typeof fetch)
    : fetchImpl;
  const MAX_VERIFY_REDIRECTS = 4;
  let current = parsed;
  let finalUrl: string | undefined;
  let res: Response | null = null;
  for (let hop = 0; hop <= MAX_VERIFY_REDIRECTS; hop++) {
    if (hop > 0) {
      current = await assertSafeResolvedPublicHttpUrl(current.toString(), { dnsLookup: options.dnsLookup });
      if (!candidateAllowedForSource(current.toString(), originalUrl, candidate.source)) return null;
    }
    const r = await doFetch(current.toString(), {
      method: 'GET',
      redirect: 'manual',
  ...
  // finalUrl (if any) was validated + source-checked per hop in the redirect
  // loop above, so no post-fetch re-check is needed here.
  return {
    ...candidate,
    url: parsed.toString(),
    status: res.status,
