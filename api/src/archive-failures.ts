...
export interface ArchiveFailureRecord {
  ...
  /** Structured worker error code ('tweet_deleted', 'anti_bot_wall', …). */
  errorCode?: string;
  failedAt: number;
  ...
  /** Consecutive terminal failures for this owner+URL (reset when a later
   *  archive succeeds via clearArchiveFailure). Drives the escalating
   *  re-enqueue cooldown for retryable reasons. */
...
export async function recordArchiveFailure(
  ...
  const reason = record.reason
    ?? classifyArchiveFailureReason(record.error, record.errorCategory, record.errorCode);
  const previous = await getArchiveFailure(redis, record.ownerPubkey, record.url).catch(() => null);
  ...
  // Don't resurrect a record a NEWER success just cleared: this write is
  // a read-modify-write, so a success callback's clearArchiveFailure can
  // land between our read and hset (2026-08-23 review). The clear leaves
  // a timestamped tombstone; a failure that predates it is stale.
  const clearedRaw = await redis
    .get(clearedTombstoneKey(record.ownerPubkey, record.url))
    .catch(() => null);
  if (clearedRaw && Number(clearedRaw) >= failure.failedAt) return;
  await redis.hset(
...
export async function clearArchiveFailure(redis: Redis, ownerPubkey: string, url: string): Promise<void> {
  ...
  // Tombstone for the recordArchiveFailure RMW race above. 6h covers any
  // realistic in-flight callback window; genuinely newer failures carry
  // a later failedAt and still write.
  await redis
    .set(
      clearedTombstoneKey(ownerPubkey, url),
...
function clearedTombstoneKey(ownerPubkey: string, url: string): string {
  return `dm:archive-failure-cleared:${ownerPubkey}:${archiveFailureField(url)}`;
}
...
export function parseArchiveFailureRecord(
  ...
  try {
    ...
    return {
      ...
      consecutiveFailures: normalizeStrikeCount(parsed.consecutiveFailures),
    };
  } catch {
    return null;
  }
...
/** Worker error codes (RenderError/PermanentError `.code`) whose meaning is
 *  unambiguous — when the callback carries one of these, it decides the
 *  reason outright and the message-string sniffing below never runs. Codes
 *  not listed here (http_error, verify_failed, …) intentionally fall through
 *  to the string heuristics, which read the detail the code lacks. */
const REASON_BY_WORKER_ERROR_CODE: Record<string, ArchiveFailureReason> = {
  tweet_deleted: 'not-found',
  anti_bot_wall: 'site-blocked',
  file_too_large: 'too-large',
  output_too_large: 'too-large',
  browser_capture_too_large: 'too-large',
  timeout: 'timeout',
  no_response: 'timeout',
};

export function classifyArchiveFailureReason(
  error: unknown,
  category: unknown,
  errorCode?: unknown,
): ArchiveFailureReason {
  if (typeof errorCode === 'string') {
    const byCode = REASON_BY_WORKER_ERROR_CODE[errorCode];
  ...
  // Content that genuinely isn't there to download (yt-dlp "no video could
  // be found", no playable formats) is a permanent content outcome, not a
  // "timeout" — the reason feeds the permanent-failure re-enqueue gate, and
  // 'timeout' would keep automated backfills re-queuing the URL forever.
  if (isMediaContentUnavailable(text)) return 'not-found';
  if (
...
export function failureTimelineSeconds(record: Pick<ArchiveFailureRecord, 'bookmarkSavedAt' | 'failedAt'>): number {
  return normalizeUnixSeconds(record.bookmarkSavedAt) ?? normalizeUnixSeconds(record.failedAt) ?? 0;
}

function normalizeStrikeCount(value: unknown): number | undefined {
  return typeof value === 'number' && Number.isInteger(value) && value > 0 ? value : undefined;
}

/** Reasons a blind retry of the same URL cannot fix: the content is gone or
 *  over the size cap. 'site-blocked' stays replayable — blocks lift, and the
 *  rescue pass can recover via Wayback/mirrors. */
export function isPermanentArchiveFailureReason(reason: ArchiveFailureReason): boolean {
  return reason === 'not-found' || reason === 'too-large';
}

/** How long a permanent failure suppresses automated re-enqueue of its URL.
 *  A deleted page then costs one quiet job a month instead of a fresh
 *  MAX_ATTEMPTS job every backfill cycle (12 duplicates of one deleted tweet
 *  were found in the prod queue on 2026-07-05). */
export const PERMANENT_FAILURE_REENQUEUE_WINDOW_SECONDS = 30 * 24 * 60 * 60;

export async function getArchiveFailure(
  redis: Redis,
  ownerPubkey: string,
  url: string,
): Promise<ArchiveFailureRecord | null> {
  const raw = await redis.hget(failureKey(ownerPubkey), archiveFailureField(url)).catch(() => null);
  return raw ? parseArchiveFailureRecord(raw, ownerPubkey) : null;
}

/** True when this owner+URL failed for a permanent reason recently enough
 *  that automated enqueue paths (backfills, sweeps) should skip it. */
export async function hasRecentPermanentArchiveFailure(
  redis: Redis,
  ownerPubkey: string,
  url: string,
  nowSeconds = Math.floor(Date.now() / 1000),
): Promise<boolean> {
  const failure = await getArchiveFailure(redis, ownerPubkey, url);
  if (!failure || !isPermanentArchiveFailureReason(failure.reason)) return false;
  return nowSeconds - failure.failedAt < PERMANENT_FAILURE_REENQUEUE_WINDOW_SECONDS;
}

/** Base cooldown before automated enqueue paths may retry a URL whose last
 *  terminal failure was RETRYABLE (site-blocked / timeout / failed). Doubles
 *  with each consecutive failure, capped at the permanent window. The
 *  permanent-only gate above stopped the 2026-07-17 duplicate-job loop, but
 *  left retryable failures ungated: on 2026-08-21 (Wayback down, so its
 *  fallback couldn't rescue dead URLs) a client backfill re-minted a fresh
 *  MAX_ATTEMPTS job every ~14 minutes per failing URL — 92 duplicates of one
 *  2002 URL — until the SLA failure spike paged the uptime alerter. */
export const RETRYABLE_FAILURE_REENQUEUE_BASE_SECONDS = 6 * 60 * 60;

/** How long this failure suppresses automated re-enqueue of its URL:
 *  30 days for permanent reasons, an escalating 6h·2^(n-1) for retryable
 *  ones (n = consecutive failures), capped at the permanent window. */
export function archiveFailureReenqueueWindowSeconds(
  failure: Pick<ArchiveFailureRecord, 'reason' | 'consecutiveFailures'>,
): number {
  if (isPermanentArchiveFailureReason(failure.reason)) {
    return PERMANENT_FAILURE_REENQUEUE_WINDOW_SECONDS;
  }
  const strikes = Math.max(1, failure.consecutiveFailures ?? 1);
  const scaled = RETRYABLE_FAILURE_REENQUEUE_BASE_SECONDS * 2 ** Math.min(strikes - 1, 8);
  return Math.min(scaled, PERMANENT_FAILURE_REENQUEUE_WINDOW_SECONDS);
}

/** The failure record for owner+URL when it is recent enough that automated
 *  enqueue paths (backfills, sweeps, looping clients) should skip the URL —
 *  or null when a retry is allowed. Deliberate operator retries
 *  (archive-retry / archive-rescue) do not consult this gate. */
export async function getRecentArchiveFailure(
  redis: Redis,
  ownerPubkey: string,
  url: string,
  nowSeconds = Math.floor(Date.now() / 1000),
  ...
  return nowSeconds - failure.failedAt < archiveFailureReenqueueWindowSeconds(failure)
    ? failure
    : null;
}

/** Shared gate for re-driving a recorded failure — used by BOTH the terminal
 *  retry sweep and the rescue pass so the two can't drift on what is
 *  replayable (they had already diverged on the reason string). */
export function archiveFailureReplayEligibility(
  ...
  // Public webpages AND direct files (PDFs, docs) are both replayable from
  // alternative public sources. Media (yt-dlp) stays excluded: its
  // "alternatives" are a different problem handled by the client-side media
  // retry path.
