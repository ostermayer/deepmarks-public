...
/** YouTube bot-wall / cookie-expiry signature. Unlike a plain "no video"
 *  media failure, this IS operator-actionable: the yt-dlp cookies have most
 *  likely expired and need re-exporting. Used to fire a distinct, rate-limited
 *  alert that overrides the best-effort media-failure suppression. */
export function isYoutubeBotWallError(error: unknown, url?: string): boolean {
  const text = typeof error === 'string' ? error.toLowerCase() : '';
  if (!/sign in to confirm|not a bot|cookies?\b.{0,24}(expired|invalid)|use --cookies/.test(text)) {
    return false;
  }
