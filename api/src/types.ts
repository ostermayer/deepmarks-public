...
export interface ArchiveJob {
  ...
  videoContentKey?: string;
  /** Private browser-view capture payload. Omitted from retained job
   *  metadata so large DOM snapshots do not live in Redis for 30 days. */
  capturedHtmlBase64?: string;
  capturedTitle?: string;
  capturedContentType?: string;
  capturedAt?: number;
...
export interface ArchiveJobMetadata {
  ...
  enqueuedAt: number;
  bookmarkSavedAt?: number;
  /** Original bookmark URL when `url` is a public rescue URL. */
  originalUrl?: string;
  kind?: 'webpage' | 'youtube' | 'video' | 'media' | 'file';
  videoId?: string;
  videoContentKey?: string;
