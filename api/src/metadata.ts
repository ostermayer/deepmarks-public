...
import { assertSafeResolvedPublicHttpUrl, type PublicDnsLookup } from './safe-url.js';
...
async function fetchOembedMetadata(
  ...
  if (!endpoint || !(await isSafePreviewFetchUrl(endpoint, dnsLookup))) return null;
  try {
    const res = await fetch(endpoint, {
...
async function fetchPreviewResource(
  ...
  try {
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      if (!(await isSafePreviewFetchUrl(current, dnsLookup))) return null;
...
async function isSafePreviewFetchUrl(url: string, dnsLookup?: PublicDnsLookup): Promise<boolean> {
  if (!parseAllowedUrl(url)) return false;
  try {
    await assertSafeResolvedPublicHttpUrl(url, { dnsLookup });
    return true;
  } catch {
    return false;
  }
