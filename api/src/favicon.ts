...
import { assertSafeResolvedPublicHttpUrl, safeFetch } from './safe-url.js';
...
async function fetchBytes(url: string, timeoutMs = FETCH_TIMEOUT_MS): Promise<Uint8Array | null> {
  ...
  try {
    ...
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
      if (!(await isPublicFetchUrl(current))) return null;
      // safeFetch pins the validated IP into the connect (DNS-rebinding
      // guard); the per-hop isPublicFetchUrl check above still bounds the
      // redirect chain to public hosts.
