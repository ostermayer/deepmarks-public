...
// bytes through api for cache hits. That keeps the API process
...
import { readBoundedText, readBytesWithin } from './bounded-read.js';
import { pickBestIconHref } from './metadata.js';
import { assertSafeResolvedPublicHttpUrl, followRedirectsSafely, safeFetch } from './safe-url.js';
 ...
 *  Each hop's destination is re-validated and DNS-checked. */
async function fetchBytes(url: string, timeoutMs = FETCH_TIMEOUT_MS): Promise<Uint8Array | null> {
  try {
    // Per-hop isPublicFetchUrl (DNS-resolving) bounds the redirect chain to
    // public hosts; safeFetch pins the validated IP into the connect.
    ...
    // Streamed + capped: never buffer an unbounded body just to size-check
    // it afterward (the old `arrayBuffer()` here was exactly that bug).
    const buf = await readBytesWithin(res, MAX_FAVICON_BYTES);
...
async function isPublicFetchUrl(url: string): Promise<boolean> {
  try {
    await assertSafeResolvedPublicHttpUrl(url);
    return true;
  } catch {
    return false;
  }
 ...
 *  redirects with a per-hop DNS/public-IP check (same posture as fetchBytes). */
export async function findIconFromHomepage(host: string, timeoutMs = FETCH_TIMEOUT_MS): Promise<string | null> {
  try {
    ...
    // need the <head>.
    const html = await readBoundedText(res, HOMEPAGE_MAX_HTML_BYTES);
    const $ = cheerio.load(html);
    // Prefer larger icons when multiple are declared — one shared policy
    // with the /metadata preview extractor.
    const bestHref = pickBestIconHref($);
    if (!bestHref) return null;
    try {
      return new URL(bestHref, `https://${host}/`).toString();
    } catch {
      return null;
    }
  } catch {
