// NIP-46 client — how api requests signatures from the Box C
...
/** Abstract signer so the rest of api doesn't know if signing
...
export interface BunkerSignerOpts {
   ...
   *  api's docker network — same string strfry uses). */
  relayUrl: string;
  logger: Logger;
}
...
export class BunkerSigner implements RemoteSigner {
  ...
  constructor(opts: BunkerSignerOpts) {
    ...
    // routes responses by subId can disambiguate when api
    // holds multiple signer roles (legacy brand/admin + personal) on the same ws.
...
export interface SignerSet {
  /** Legacy "brand" role — admin/operational signer for zap@ + lifetime labels. */
  brand: RemoteSigner;
  /** Legacy "personal" role — production maps this to the public brand/social profile key. */
