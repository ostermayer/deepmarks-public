...
export function attachInvoiceHandler(deps: InvoiceHandlerDeps): InvoiceSubHandle | null {
  ...
  invoiceSub.on('invoice_updated', async (inv: {
    ...
    // 1) Zap?
    try {
      // Claim (not consume): the pending record stays until the receipt is
      // signed, so a transient bunker outage during buildZapReceipt doesn't
      // drop the receipt permanently — release() lets a redelivered
      // invoice_updated retry. The NX claim still blocks double-publish.
      const pending = await zaps.claim(inv.id);
      if (pending) {
        ...
        if (pending.paymentHash !== inv.id) {
          ...
          await zaps.finalize(inv.id); // corrupt record — drop it, don't loop
          return;
        }
        ...
        if (!signer) {
          ...
          await zaps.finalize(inv.id); // deterministic — retry can't help, drop it
          return;
        }
        let signed = false;
        try {
          ...
          // Signed → drop the record. Not signed (bunker down/timeout) →
          // release the claim so a redelivered callback can retry.
          if (signed) await zaps.finalize(inv.id);
          else await zaps.release(inv.id);
        }
