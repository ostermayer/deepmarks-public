...
import { registerRawJsonBodyParser } from './json-body-parser.js';
import { feedDepsFromEnv, registerFeedRoutes } from './feed/routes.js';
import { buildWorkerGroup, type WorkerGroupHandle } from './workers-bootstrap.js';
import { workerDepsFromDeps, isWorkerGroup } from './worker-deps.js';
...
import * as blossomAuth from './routes/blossom-auth.js';
import * as admin from './routes/admin.js';
import * as adminRelayStats from './routes/admin-relay-stats.js';
import * as adminDashboard from './routes/admin-dashboard.js';
import * as search from './routes/search.js';
import * as publicBookmarks from './routes/public-bookmarks.js';
import * as popularTags from './routes/popular-tags.js';
import * as profile from './routes/profile.js';
import * as webPushRoutes from './routes/web-push.js';
import * as videoArchive from './routes/youtube-archive.js';
import * as contacts from './routes/contacts.js';
import * as publishRoute from './routes/publish.js';
import * as apiV1 from './routes/api-v1.js';
import * as socialBookmarks from './routes/social-bookmarks.js';
import * as llmRoutes from './routes/llm.js';
...
async function start(): Promise<void> {
  ...
  await app.register(cors, {
    // Include DELETE/PUT: the web app makes cross-origin calls to revoke API
    // keys (DELETE /api/v1/keys/:id), delete the account (DELETE /account),
    // release a username (DELETE /account/username), and save settings
    // (PUT /account/settings). With only GET/POST the browser blocked those
    // preflights, so those security-relevant self-service actions failed.
    origin: CORS_ORIGIN,
  ...
  // parsed object; downstream routes don't change. Malformed JSON is a
  // 400 client error, not an unhandled 500 (see json-body-parser.ts).
  ...
  // Background workers. By default (RUN_WORKERS=all) the API process
  // hosts the whole fleet in-process — the single-box / dev shape. In
  // production each box sets RUN_WORKERS=none here and runs the worker
  // groups as their own containers (see deploy/box-a/compose.yml and
  // src/worker.ts). RUN_WORKERS may also name a single group.
  const RUN_WORKERS = (process.env.RUN_WORKERS ?? 'all').trim();
  let workerGroup: WorkerGroupHandle | null = null;
  ...
  blossomAuth.register(deps);
  admin.register(deps);
  adminRelayStats.register(deps);
  adminDashboard.register(deps);
  search.register(deps);
  publicBookmarks.register(deps);
  popularTags.register(deps);
  profile.register(deps);
  webPushRoutes.register(deps);
  videoArchive.register(deps);
  contacts.register(deps);
  publishRoute.register(deps);
  apiV1.register(deps);
  socialBookmarks.register(deps);
  llmRoutes.register(deps);
  ...
  try {
    ...
    if (RUN_WORKERS !== 'none') {
      if (!isWorkerGroup(RUN_WORKERS)) {
        app.log.error(
          { RUN_WORKERS },
          'invalid RUN_WORKERS — expected none|all|search-indexer|relay-sync|enrichment|payments; running no workers in-process',
        );
      } else {
        workerGroup = buildWorkerGroup(workerDepsFromDeps(deps), RUN_WORKERS);
        workerGroup.start();
        stopWorkerHeartbeat = startStatsHeartbeat(deps.redis, workerGroup.statsSources, app.log);
        app.log.info({ group: RUN_WORKERS }, 'in-process workers started');
      }
    } else {
      app.log.info('RUN_WORKERS=none — background workers run in separate processes');
    }
    // One-time backfill: pull every existing lifetime member, username
    // holder, and bookmark-cache author into the relay's registered
    // pubkey set so the writePolicy plugin doesn't reject events from
    // pre-registry users after this lands.
    void import('./registry.js').then(({ backfillRegistry }) =>
      backfillRegistry(deps.redis, {
        info: app.log.info.bind(app.log),
        warn: app.log.warn.bind(app.log),
        error: app.log.error.bind(app.log),
      }),
    ).catch((err) => app.log.error({ err }, 'registry backfill threw'));
    // One-shot lazy backfill of cross-user archive refcounts so the
    // very first delete after this module ships can't unilaterally
    // destroy a shared blob.
    void import('./archive-refcount.js').then(({ backfillFromExistingArchives }) =>
      backfillFromExistingArchives(deps.redis, {
        info: app.log.info.bind(app.log),
        warn: app.log.warn.bind(app.log),
      }),
    ).catch((err) => app.log.error({ err }, 'archive-refs backfill threw'));
  } catch (err) {
    app.log.error(err);
    process.exit(1);
  }
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    try {
      stopWorkerHeartbeat();
      if (workerGroup) await workerGroup.stop();
      await app.close();
