// /admin/dashboard — one JSON aggregate for the operator UI.
//
// The frontend at /app/admin/dashboard polls this every few seconds.
// Pulled into one route so the page makes one round-trip per refresh
// instead of fanning out to N admin endpoints.
//
// What we report:
//   - boxes: which subsystems api is in contact with right
//     now (Redis, Meilisearch, strfry, the archive worker via its
//     callback HMAC pulse, Voltage LN). Each carries an `ok` boolean +
//     a one-line status string + latencyMs where it makes sense.
//   - relay: event counts per kind on our local strfry,
//     relay-allowed-pubkey count, watched-contact count.
//   - queues: depth of every Redis LIST used as a worker queue
//     (onboarding, lifetime-archive, pending-publish, archive).
//   - workers: each worker's public `stats` object (defined on the
//     class). Adding a new worker stat shows up here automatically.
//   - alerts: most-recent N alerts the Tier-1 alerter has fired.
//
// Auth: NIP-98 admin gate. The page is at /app/admin/dashboard and
// signs requests with the operator's own nsec (no separate admin
// session — the same admin pubkey set that gates other /admin/*).

import type { Deps } from '../route-deps.js';
import { readWorkerStats } from '../worker-stats.js';
import { computeLifetimePriceSats } from '../lifetime.js';
import { MEDIA_ARCHIVE_ADDON_PRICE_SATS } from '../media-archive-addon.js';
import { collectArchiveHealth, type ArchiveHealthSummary, type ArchiveAuditSummary } from '../archive-health.js';

const RELAY_QUERY_TIMEOUT_MS = 3_000;
const DAY_SECONDS = 86_400;

interface BoxStatus {
  ok: boolean;
  status: string;
  latencyMs?: number;
}

interface DashboardResponse {
  ts: number;
  metrics: {
    growth: {
      registeredPubkeys: number;
      usernameClaims: number;
      lifetimeMembers: number;
      mediaArchiveAddons: number;
      lifetimeConversionPct: number;
      publicBookmarks: number;
      privateBookmarkChunks: number;
      serverKnownBookmarkRecords: number;
      publicBookmarksPerRegisteredPubkey: number;
      watchedContacts: number;
    };
    revenue: {
      estimatedLifetimeSats: number;
      mediaArchiveSats: number;
      estimatedTotalSats: number;
      ranges: PaidMetricRange[];
      monthly: MonthlyMetric[];
      notes: string[];
    };
    health: {
      okBoxes: number;
      totalBoxes: number;
      queuedJobs: number;
      recentAlerts: number;
    };
  };
  boxes: {
    redis: BoxStatus;
    meilisearch: BoxStatus;
    strfry: BoxStatus;
    voltage: BoxStatus;
    archiveWorker: BoxStatus;
    bunker: BoxStatus;
  };
  relay: {
    url: string;
    registeredPubkeys: number;
    watchedContacts: number;
    eventCounts: Record<string, number>;
  };
  queues: Record<string, number>;
  archiveQueue: ArchiveHealthSummary;
  archiveAudit: ArchiveAuditSummary | null;
  workers: Record<string, unknown>;
  alerts: Array<{ key: string; severity: string; subject: string; sentAt: number }>;
}

interface PaidMember {
  pubkey: string;
  paidAt: number;
}

interface PaidMetricRange {
  label: string;
  from: number | null;
  to: number;
  lifetimeMembers: number;
  mediaArchiveAddons: number;
  estimatedRevenueSats: number;
}

interface MonthlyMetric {
  month: string;
  from: number;
  to: number;
  lifetimeMembers: number;
  mediaArchiveAddons: number;
  estimatedRevenueSats: number;
}

/** Probe Redis with a PING; return ok + roundtrip ms. */
async function pingRedis(redis: Deps['redis']): Promise<BoxStatus> {
  const t0 = Date.now();
  try {
    const reply = await redis.ping();
    return { ok: reply === 'PONG', status: reply === 'PONG' ? 'connected' : `unexpected: ${reply}`, latencyMs: Date.now() - t0 };
  } catch (err) {
    return { ok: false, status: (err as Error).message ?? 'ping failed', latencyMs: Date.now() - t0 };
  }
}

/** Probe Meilisearch /health. */
async function pingMeili(meili: Deps['meili']): Promise<BoxStatus> {
  const t0 = Date.now();
  try {
    const ok = await meili.healthy();
    return { ok, status: ok ? 'healthy' : 'unhealthy', latencyMs: Date.now() - t0 };
  } catch (err) {
    return { ok: false, status: (err as Error).message ?? 'unreachable', latencyMs: Date.now() - t0 };
  }
}

/** Probe strfry: query a single recent event with a tiny limit.
 *  If the relay is up, EOSE comes back fast; if it's down, the timeout fires. */
async function pingStrfry(
  pool: Deps['relayPool'],
  relayUrl: string,
): Promise<BoxStatus> {
  const t0 = Date.now();
  return new Promise((resolve) => {
    let resolved = false;
    let sub: { close: () => void } | null = null;
    const finish = (s: BoxStatus): void => {
      if (resolved) return;
      resolved = true;
      try { sub?.close(); } catch { /* already closed */ }
      resolve(s);
    };
    const timer = setTimeout(() => finish({ ok: false, status: 'EOSE timeout', latencyMs: Date.now() - t0 }), RELAY_QUERY_TIMEOUT_MS);
    try {
      sub = pool.subscribeMany(
        [relayUrl],
        { kinds: [1], limit: 1 } as never,
        {
          onevent: () => undefined,
          oneose: () => { clearTimeout(timer); finish({ ok: true, status: 'EOSE received', latencyMs: Date.now() - t0 }); },
        },
      );
    } catch (err) {
      clearTimeout(timer);
      finish({ ok: false, status: (err as Error).message ?? 'subscribe failed', latencyMs: Date.now() - t0 });
    }
  });
}

/** Voltage status. The invoice-only macaroon can't call getInfo (missing
 *  info:read scope), so a per-poll handshake would be too heavy. Instead
 *  we just report whether the lnd handle is initialised and whether the
 *  startup handshake has stamped the most-recent connection-OK time. */
async function voltageStatus(
  lnd: Deps['lnd'],
  redis: Deps['redis'],
): Promise<BoxStatus> {
  if (!lnd) return { ok: false, status: 'not configured' };
  try {
    const last = await redis.get('dm:voltage:last-ok');
    if (!last) return { ok: true, status: 'configured, no handshake stamp yet' };
    const ageMs = Date.now() - Number.parseInt(last, 10);
    if (ageMs > 6 * 60 * 60 * 1000) return { ok: false, status: `last handshake ${Math.round(ageMs / 60_000)} min ago` };
    return { ok: true, status: `last handshake ${Math.round(ageMs / 60_000)} min ago` };
  } catch {
    return { ok: !!lnd, status: 'handle initialised' };
  }
}

/** The bunker (Box C) is reachable only by signing a NIP-46 RPC. The
 *  cheapest health check is "do we have a signer with a known pubkey?"
 *  — the signer set is built at boot and rejects sign attempts if the
 *  bunker connection is down. If signers.brand.pubkey is set, the
 *  handshake succeeded. */
function bunkerStatus(signers: Deps['signers']): BoxStatus {
  const brand = signers.brand?.pubkey;
  const personal = signers.personal?.pubkey;
  if (!brand || !personal) return { ok: false, status: 'signers not initialised' };
  return { ok: true, status: `signers brand=${brand.slice(0, 8)}… personal=${personal.slice(0, 8)}…` };
}

/** Count events of each kind on strfry. Bounded query (limit 1) per
 *  kind — we use COUNT via Redis-cached counters where they exist, and
 *  fall back to a small per-kind probe. */
async function relayEventCounts(
  redis: Deps['redis'],
): Promise<Record<string, number>> {
  // api maintains kind:39701 author counts in
  // dm:public-bookmarks:author:<pubkey> ZSETs; we don't have a single
  // global counter. Use the relay-stats helper sets if present,
  // otherwise return what we can pull cheaply from Redis-cached
  // tallies. Operator can drill into per-pubkey via /admin/relay-stats.
  const [pubBookmarks, privChunks, profiles, contacts, nip65, archiveLabels, zapReceipts, registrations] = await Promise.all([
    redis.get('dm:relay-counter:39701').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:30003').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:0').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:3').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:10002').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:1985').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:9735').then((v) => Number(v ?? 0)).catch(() => 0),
    redis.get('dm:relay-counter:24133').then((v) => Number(v ?? 0)).catch(() => 0),
  ]);
  return {
    '39701_public_bookmarks': pubBookmarks,
    '30003_private_chunks': privChunks,
    '0_profiles': profiles,
    '3_contacts': contacts,
    '10002_nip65': nip65,
    '1985_labels': archiveLabels,
    '9735_zap_receipts': zapReceipts,
    '24133_nip46': registrations,
  };
}

async function queueDepths(redis: Deps['redis']): Promise<Record<string, number>> {
  // These are the keys the workers actually use. The old
  // 'dm:pending-publish:queue' entry watched a key nothing writes, so
  // the publish gauge read 0 even during a total relay-write outage.
  const lists = [
    'dm:onboarding:queue',
    'dm:lifetime-archive:queue',
    'dm:publish-relay:queue',
    'dm:publish-relay:dead',
    'dm:kind1-fanout:queue',
    'dm:archive:queue',
  ];
  const lens = await Promise.all(
    lists.map((q) => redis.llen(q).catch(() => 0)),
  );
  const out: Record<string, number> = {};
  lists.forEach((q, i) => { out[q] = lens[i] ?? 0; });
  // Delayed retries live in a sorted set, not a list.
  out['dm:publish-relay:delayed'] = await redis.zcard('dm:publish-relay:delayed').catch(() => 0);
  return out;
}

async function recentAlerts(redis: Deps['redis']): Promise<DashboardResponse['alerts']> {
  // The alerter stamps recent alerts into a capped Redis list.
  // If the key doesn't exist (e.g. fresh boot), return empty.
  try {
    const raw = await redis.lrange('dm:alerter:recent', 0, 19);
    return raw
      .map((s) => {
        try { return JSON.parse(s) as DashboardResponse['alerts'][number]; }
        catch { return null; }
      })
      .filter((x): x is DashboardResponse['alerts'][number] => !!x);
  } catch {
    return [];
  }
}

function buildMetrics(args: {
  registeredPubkeys: number;
  usernameClaims: number;
  watchedContacts: number;
  eventCounts: Record<string, number>;
  queues: Record<string, number>;
  boxes: DashboardResponse['boxes'];
  alerts: DashboardResponse['alerts'];
  lifetimeMembers: PaidMember[];
  mediaArchiveMembers: PaidMember[];
  nowMs: number;
}): DashboardResponse['metrics'] {
  const publicBookmarks = args.eventCounts['39701_public_bookmarks'] ?? 0;
  const privateBookmarkChunks = args.eventCounts['30003_private_chunks'] ?? 0;
  const queuedJobs = Object.values(args.queues).reduce((sum, depth) => sum + depth, 0);
  const boxValues = Object.values(args.boxes);
  const okBoxes = boxValues.filter((box) => box.ok).length;
  const estimatedLifetimeSats = sumLifetimeRevenue(args.lifetimeMembers);
  const mediaArchiveSats = args.mediaArchiveMembers.length * MEDIA_ARCHIVE_ADDON_PRICE_SATS;

  return {
    growth: {
      registeredPubkeys: args.registeredPubkeys,
      usernameClaims: args.usernameClaims,
      lifetimeMembers: args.lifetimeMembers.length,
      mediaArchiveAddons: args.mediaArchiveMembers.length,
      lifetimeConversionPct: pct(args.lifetimeMembers.length, args.registeredPubkeys),
      publicBookmarks,
      privateBookmarkChunks,
      serverKnownBookmarkRecords: publicBookmarks + privateBookmarkChunks,
      publicBookmarksPerRegisteredPubkey: ratio(publicBookmarks, args.registeredPubkeys),
      watchedContacts: args.watchedContacts,
    },
    revenue: {
      estimatedLifetimeSats,
      mediaArchiveSats,
      estimatedTotalSats: estimatedLifetimeSats + mediaArchiveSats,
      ranges: paidRanges(args.lifetimeMembers, args.mediaArchiveMembers, args.nowMs),
      monthly: monthlyPaidMetrics(args.lifetimeMembers, args.mediaArchiveMembers, args.nowMs),
      notes: [
        'Lifetime revenue is estimated from the server price schedule at each paidAt timestamp; manual admin comps are indistinguishable from paid records.',
        'Private bookmark item counts stay client-private. The dashboard can count public bookmark events and encrypted private bookmark-set chunks, not exact private items.',
      ],
    },
    health: {
      okBoxes,
      totalBoxes: boxValues.length,
      queuedJobs,
      recentAlerts: args.alerts.length,
    },
  };
}

function paidRanges(lifetimeMembers: PaidMember[], mediaMembers: PaidMember[], nowMs: number): PaidMetricRange[] {
  const now = Math.floor(nowMs / 1000) + 1;
  const today = utcDayStart(nowMs);
  const starts: Array<{ label: string; from: number | null }> = [
    { label: 'today', from: today },
    { label: '7 days', from: now - 7 * DAY_SECONDS },
    { label: '30 days', from: now - 30 * DAY_SECONDS },
    { label: '90 days', from: now - 90 * DAY_SECONDS },
    { label: 'all time', from: null },
  ];
  return starts.map(({ label, from }) => paidMetricRange(label, from, now, lifetimeMembers, mediaMembers));
}

function paidMetricRange(
  label: string,
  from: number | null,
  to: number,
  lifetimeMembers: PaidMember[],
  mediaMembers: PaidMember[],
): PaidMetricRange {
  const lifetime = inRange(lifetimeMembers, from, to);
  const media = inRange(mediaMembers, from, to);
  return {
    label,
    from,
    to,
    lifetimeMembers: lifetime.length,
    mediaArchiveAddons: media.length,
    estimatedRevenueSats: sumLifetimeRevenue(lifetime) + media.length * MEDIA_ARCHIVE_ADDON_PRICE_SATS,
  };
}

function monthlyPaidMetrics(lifetimeMembers: PaidMember[], mediaMembers: PaidMember[], nowMs: number): MonthlyMetric[] {
  const now = new Date(nowMs);
  const currentMonth = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1);
  const out: MonthlyMetric[] = [];
  for (let offset = 11; offset >= 0; offset--) {
    const startDate = new Date(currentMonth);
    startDate.setUTCMonth(startDate.getUTCMonth() - offset);
    const endDate = new Date(startDate.getTime());
    endDate.setUTCMonth(endDate.getUTCMonth() + 1);
    const from = Math.floor(startDate.getTime() / 1000);
    const to = Math.floor(endDate.getTime() / 1000);
    const lifetime = inRange(lifetimeMembers, from, to);
    const media = inRange(mediaMembers, from, to);
    out.push({
      month: startDate.toISOString().slice(0, 7),
      from,
      to,
      lifetimeMembers: lifetime.length,
      mediaArchiveAddons: media.length,
      estimatedRevenueSats: sumLifetimeRevenue(lifetime) + media.length * MEDIA_ARCHIVE_ADDON_PRICE_SATS,
    });
  }
  return out;
}

function inRange(records: PaidMember[], from: number | null, to: number): PaidMember[] {
  return records.filter((record) => (from === null || record.paidAt >= from) && record.paidAt < to);
}

function sumLifetimeRevenue(records: PaidMember[]): number {
  return records.reduce((sum, member) => (
    sum + computeLifetimePriceSats(new Date(member.paidAt * 1000))
  ), 0);
}

function utcDayStart(ms: number): number {
  const date = new Date(ms);
  return Math.floor(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()) / 1000);
}

function pct(numerator: number, denominator: number): number {
  return denominator > 0 ? Number(((numerator / denominator) * 100).toFixed(2)) : 0;
}

function ratio(numerator: number, denominator: number): number {
  return denominator > 0 ? Number((numerator / denominator).toFixed(2)) : 0;
}

/** Worker groups whose live `.stats` the dashboard surfaces. Each worker
 *  heartbeats its stats to Redis (see worker-stats.ts) so this works
 *  whether the worker runs in-process (RUN_WORKERS=all) or in its own
 *  container. */
const WORKER_STAT_NAMES = [
  'relayFanout',
  'onboardingScanner',
  'lifetimeArchiveBackfill',
  'followsIngester',
  'llmEnrichment',
] as const;

async function collectWorkerStats(
  redis: Deps['redis'],
): Promise<Record<string, Record<string, unknown>>> {
  const entries = await Promise.all(
    WORKER_STAT_NAMES.map(async (name) => {
      const s = await readWorkerStats(redis, name).catch(() => null);
      // Never null — the frontend does Object.entries(stats). A missing
      // heartbeat (worker down / not deployed) shows as offline.
      if (!s) return [name, { offline: 1 }] as const;
      const { heartbeatAt, ...rest } = s;
      const ageSeconds = typeof heartbeatAt === 'number'
        ? Math.round((Date.now() - heartbeatAt) / 1000)
        : -1;
      return [name, { ...rest, heartbeatAgeSeconds: ageSeconds }] as const;
    }),
  );
  return Object.fromEntries(entries);
}

export function register(deps: Deps): void {
  const {
    app, redis, meili, relayPool, lnd, signers,
    lifetimeStore, mediaArchiveAddonStore,
    INDEXER_RELAY_URL_FOR_API, requireAdmin,
  } = deps;

  app.get('/admin/dashboard', async (request, reply) => {
    const auth = await requireAdmin({
      headers: request.headers,
      url: '/admin/dashboard',
      method: 'GET',
    });
    if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });

    const [
      redisStatus, meiliStatus, strfryStatus, voltageStatusBox,
      archiveHealth, eventCounts, registered, usernameClaims, watched, queues, alerts,
      lifetimeMembers, mediaArchiveMembers, workerStats,
    ] = await Promise.all([
      pingRedis(redis),
      pingMeili(meili),
      pingStrfry(relayPool, INDEXER_RELAY_URL_FOR_API),
      voltageStatus(lnd, redis),
      collectArchiveHealth(redis),
      relayEventCounts(redis),
      redis.scard('dm:registered:pubkeys').catch(() => 0),
      redis.hlen('dm:username:bypubkey').catch(() => 0),
      redis.scard('dm:contacts:watched').catch(() => 0),
      queueDepths(redis),
      recentAlerts(redis),
      lifetimeStore.listMembers().catch(() => []),
      mediaArchiveAddonStore.listMembers().catch(() => []),
      collectWorkerStats(redis),
    ]);

    const boxes = {
      redis: redisStatus,
      meilisearch: meiliStatus,
      strfry: strfryStatus,
      voltage: voltageStatusBox,
      archiveWorker: {
        ok: archiveHealth.ok,
        status: archiveHealth.status,
      },
      bunker: bunkerStatus(signers),
    };
    const body: DashboardResponse = {
      ts: Date.now(),
      metrics: buildMetrics({
        registeredPubkeys: registered,
        usernameClaims,
        watchedContacts: watched,
        eventCounts,
        queues,
        boxes,
        alerts,
        lifetimeMembers,
        mediaArchiveMembers,
        nowMs: Date.now(),
      }),
      boxes,
      relay: {
        url: INDEXER_RELAY_URL_FOR_API,
        registeredPubkeys: registered,
        watchedContacts: watched,
        eventCounts,
      },
      queues,
      archiveQueue: archiveHealth,
      archiveAudit: archiveHealth.lastAudit,
      workers: workerStats,
      alerts,
    };

    reply.header('cache-control', 'no-store');
    return body;
  });
}
