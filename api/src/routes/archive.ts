...
export function register(deps: Deps): void {
  ...
  app.post('/archive/callback', async (request, reply) => {
    ...
    const failureReason = classifyArchiveFailureReason(payload.error, payload.errorCategory);
    ...
    const alertOperator = shouldAlertArchiveFailure(failureReason, payload.error, expectedJob.kind);
    const failureLog = {
      jobId: payload.jobId,
      error: payload.error,
    ...
    void rescueArchiveFailure(deps, failureRecord).catch((err) => {
