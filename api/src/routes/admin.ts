...
import { rescueFailedArchives } from '../archive-rescue.js';
import { retryTerminalArchiveFailures } from '../archive-retry.js';
...
const ArchiveRescueRunSchema = z.object({
  pubkey: z.string().regex(/^[0-9a-f]{64}$/i).optional(),
  limit: z.number().int().positive().max(50).optional(),
  dryRun: z.boolean().optional(),
  force: z.boolean().optional(),
});

const ArchiveRetryRunSchema = z.object({
  pubkey: z.string().regex(/^[0-9a-f]{64}$/i).optional(),
...
export function register(deps: Deps): void {
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      for (const action of parsed.data.actions) {
        ...
        switch (action.kind) {
          ...
          case 'hash_blocklist':
            ...
            // Future hardening: also DELETE the blob from blossom-server
            // via its admin API after the operator confirms mirror policy.
            break;
          case 'url_blocklist':
      ...
      // Future hardening: send a statement-of-reasons email to the
      // affected user once account email verification is available.
  ...
  app.post<{ Body: { pubkey?: string; paidAt?: number } }>(
    ...
    async (request, reply) => {
      ...
      void backfillLifetimePublicArchives(deps, pubkey).catch((err) => {
        app.log.warn({ err, pubkey }, 'manual lifetime public archive backfill failed');
      });
  ...
  app.post('/admin/media-archive/reconcile', async (request, reply) => {
    const auth = await requireAdmin({
      headers: request.headers,
      url: '/admin/media-archive/reconcile',
      method: 'POST',
      rawBody: (request as { rawBody?: Buffer }).rawBody,
    });
    if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });
    ...
    for (let pageIdx = 0; pageIdx < MAX_PAGES; pageIdx++) {
      ...
      for (const inv of page) {
        ...
        const product = typeof inv.metadata?.deepmarksProduct === 'string'
          ? inv.metadata.deepmarksProduct
          : undefined;
        const isAddon = inv.metadata?.mediaArchiveAddon === true;
        if (product !== 'video-archive' || !isAddon) { skipped++; continue; }
        const pubkey = typeof inv.metadata?.deepmarksPubkey === 'string'
          ? inv.metadata.deepmarksPubkey.toLowerCase()
          : undefined;
        if (!pubkey || !/^[0-9a-f]{64}$/.test(pubkey)) { skipped++; continue; }
        if (!(await mediaArchiveAddonStore.isPaid(pubkey))) {
          await mediaArchiveAddonStore.markPaid(pubkey, inv.expirationTime);
          stamped++;
        }
      }
      if (page.length < PAGE) break;
    ...
    if (skip >= MAX_PAGES * PAGE) truncated = true;
    app.log.info(
      { scanned, stamped, skipped, truncated, admin: auth.pubkey },
      'media archive add-on reconcile complete',
    );
    return { scanned, stamped, skipped, truncated };
  });

  app.get('/admin/media-archive/members', async (request, reply) => {
    const auth = await requireAdmin({
      headers: request.headers,
      url: '/admin/media-archive/members',
      method: 'GET',
    });
    if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });
    const members = await mediaArchiveAddonStore.listMembers();
    return { count: members.length, members };
  });

  app.post<{ Body: { pubkey?: string; limit?: number; offset?: number } }>(
    '/admin/lifetime/backfill-archives',
    async (request, reply) => {
      const auth = await requireAdmin({
        headers: request.headers,
        url: '/admin/lifetime/backfill-archives',
        method: 'POST',
        rawBody: (request as { rawBody?: Buffer }).rawBody,
      });
      if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });

      const { pubkey, limit, offset } = request.body ?? {};
      const maxMembers = typeof limit === 'number' && Number.isInteger(limit) && limit > 0
        ? Math.min(limit, 500)
        : 500;
      const start = typeof offset === 'number' && Number.isInteger(offset) && offset >= 0 ? offset : 0;
      const members = pubkey
        ? [{ pubkey, paidAt: 0 }]
        : (await lifetimeStore.listMembers()).slice(start, start + maxMembers);

      if (pubkey && !/^[0-9a-f]{64}$/i.test(pubkey)) {
        return reply.status(400).send({ error: 'pubkey (hex, 64 chars) required' });
      }
      if (pubkey && !(await lifetimeStore.isPaid(pubkey))) {
        return reply.status(402).send({ error: 'pubkey is not a lifetime member' });
      }
      ...
      for (const member of members) {
        const result = await backfillLifetimePublicArchives(deps, member.pubkey);
        results.push({ pubkey: member.pubkey, ...result });
      ...
      app.log.info(
        { admin: auth.pubkey, members: members.length, enqueued, skipped, scanned, locked },
        'lifetime public archive backfill requested',
      );
      return { members: members.length, offset: start, enqueued, skipped, scanned, locked, results };
    },
  );

  app.post<{ Body: { pubkey?: string; limit?: number; dryRun?: boolean; force?: boolean } }>(
    '/admin/archive-rescue/run',
    async (request, reply) => {
      const auth = await requireAdmin({
        headers: request.headers,
        url: '/admin/archive-rescue/run',
        method: 'POST',
        rawBody: (request as { rawBody?: Buffer }).rawBody,
      });
      if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });

      const parsed = ArchiveRescueRunSchema.safeParse(request.body ?? {});
      if (!parsed.success) return reply.status(400).send({ error: 'invalid archive rescue request' });

      const result = await rescueFailedArchives(deps, {
        ownerPubkey: parsed.data.pubkey,
  ...
  app.post<{ Body: { pubkey?: string; limit?: number; dryRun?: boolean; force?: boolean } }>(
    '/admin/archive-failures/retry',
    async (request, reply) => {
      const auth = await requireAdmin({
        headers: request.headers,
        url: '/admin/archive-failures/retry',
        method: 'POST',
        rawBody: (request as { rawBody?: Buffer }).rawBody,
      });
      if (!auth.ok) return reply.status(auth.status ?? 401).send({ error: auth.reason });

      const parsed = ArchiveRetryRunSchema.safeParse(request.body ?? {});
      if (!parsed.success) return reply.status(400).send({ error: 'invalid archive retry request' });

      const result = await retryTerminalArchiveFailures(deps, {
        ownerPubkey: parsed.data.pubkey,
        limit: parsed.data.limit,
        dryRun: parsed.data.dryRun ?? true,
        force: parsed.data.force,
      ...
      app.log.info(
        ...
        'terminal archive retry run requested',
      );
      return result;
    },
  );
