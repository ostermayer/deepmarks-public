...
export function register(deps: Deps): void {
  const { app, ciphertextStore, passkeyStore, redis, gateRateLimit, requireNip98, PUBLIC_BASE_URL } = deps;
  ...
  app.post('/account/nsec-ciphertext', async (request, reply) => {
    ...
    if (!(await gateRateLimit(reply, 'ciphertext-pk', authCheck.pubkey, 30, 60))) return reply;
  ...
  app.get<{ Querystring: { pubkey?: string; token?: string } }>(
    ...
    async (request, reply) => {
      ...
      const token = bearerToken(request.headers.authorization) ?? request.query.token;
      if (!token) {
