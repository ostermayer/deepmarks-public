...
export function register(deps: Deps): void {
  const { app, accounts, privateMarks, gateRateLimit, requireNip98, requireSession, PUBLIC_BASE_URL } = deps;
  ...
  app.put<{ Params: { id: string }; Body: PrivateMarkCiphertext }>(
    ...
    async (request, reply) => {
      ...
      if (!(await gateRateLimit(reply, 'private-marks-put', authCheck.pubkey, 600, 60))) return reply;
