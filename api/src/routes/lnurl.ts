  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      if (request.query.nostr) {
        ...
        // NIP-57: clients only honor a kind:9735 receipt whose signer
        // matches the recipient's advertised nostrPubkey — which for this
        // address is `signer.pubkey`. A zap request whose p-tag names any
        // other pubkey would be invoiced and PAID but its receipt ignored
        // by every client — sats accepted for a zap we can never make
        // count (2026-08-23 review). Fail the callback up front instead.
        const recipient = zapRequest.tags.find((t) => t[0] === 'p')?.[1]?.toLowerCase();
        if (recipient !== signer.pubkey.toLowerCase()) {
          return reply.status(400).send(
            lnurlError('zap request p tag must be the recipient pubkey advertised by this address'),
          );
        }
