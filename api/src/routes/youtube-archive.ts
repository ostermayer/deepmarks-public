...
import { claimPendingArchiveJob, releasePendingArchiveJob } from '../archive-dedupe.js';
import { getRecentArchiveFailure } from '../archive-failures.js';
import { markPaidAndEnqueue, type MarkPaidAndEnqueueResult } from '../purchase-settlement.js';
...
export function register(deps: Deps): void {
  ...
  for (const path of ENQUEUE_PATHS) {
    app.post<{
      ...
      // Same terminal-failure cooldown as /archive/lifetime: a media URL
      // whose last job terminally failed fails identically on immediate
      // re-enqueue, and the client's automated media retry re-submits a
      // bot-walled video every cycle otherwise (2026-08-21). Old clients
      // already treat the `queued:` prefix as already-queued/skipped.
      const recentFailure = await getRecentArchiveFailure(redis, auth.pubkey, normalized.url);
      if (recentFailure) {
        app.log.info(
          {
            url: normalized.url,
          ...
          'media archive enqueue skipped — recent terminal failure',
        );
        const sentinel = 'queued:recent-failure';
        return {
          paymentHash: sentinel,
          jobId: sentinel,
          amountSats: 0,
      ...
      // Same pending-claim dedupe as private lifetime archives (this
      // route had none at all — the 2026-07-17 duplicate flood's top
      // offenders were media jobs queued 40+ times). While a media job
      // for this owner+URL is pending, duplicates get the `queued:`
      // sentinel; the terminal callback releases the claim. Keyed on the
      // canonical URL so youtu.be/watch?v= variants coalesce too.
      const claim = await claimPendingArchiveJob(redis, 'media', auth.pubkey, normalized.url, jobId);
      if (!claim.claimed) {
        app.log.info(
          { url: normalized.url, user: auth.pubkey, pendingJobId: claim.existingJobId },
          'media archive enqueue coalesced — job already pending',
        );
      ...
      let settled: MarkPaidAndEnqueueResult;
      try {
        ...
        // Shared core: markPaid → enqueue, rolling back to pending on
        // enqueue failure.
        settled = await markPaidAndEnqueue({ purchases, paymentHash: jobId });
      } catch (err) {
        // A create/markPaid failure used to strand the pending claim for
        // its full 7-day TTL, locking the owner+URL out of media
        // archiving (2026-08-23 review).
        await releasePendingArchiveJob(redis, 'media', auth.pubkey, normalized.url, jobId)
          .catch(() => undefined);
        throw err;
      }
      if (settled.status === 'enqueue-failed') {
        await releasePendingArchiveJob(redis, 'media', auth.pubkey, normalized.url, jobId)
          .catch(() => undefined);
        throw settled.error;
      }
      if (settled.status === 'not-found') {
        // Never enqueued — don't hold the owner+URL hostage for the TTL.
        await releasePendingArchiveJob(redis, 'media', auth.pubkey, normalized.url, jobId)
          .catch(() => undefined);
      }
      return {
