...
import { DELISTED_EVENTS } from '../blocklist.js';
import { z } from 'zod';
...
import { archiveFilesForRecord, compareArchiveRecordsNewest, parseArchiveRecord } from '../archive-lifecycle.js';
import { archiveFailureMessage, classifyArchiveFailureReason } from '../archive-failures.js';
import {
  SignedEventSchema,
  bookmarkEventToJson,
  publishToRelays,
...
export function register(deps: Deps): void {
  ...
  app.post('/api/v1/keys', async (request, reply) => {
    ...
    if (!(await gateRateLimit(reply, 'apikey-mint', pubkey, 5, 60 * 60, 'rate limit (key mint)'))) return reply;

    // Gate on the LIVE lifetime source (the Redis marker the BTCPay
    // settlement webhook stamps — same source /archive/lifetime uses).
    // The old gate read accounts.isLifetimeMember, but the email-account
    // subsystem's create() was never wired (the email flow was removed
    // when passkeys shipped, docs/login.md), so no account record can
    // exist and every paid lifetime member got 402 here (2026-08-23
    // review follow-up).
    ...
    if (!isLifetime) {
      return reply.status(402).send({
        ...
        upgradeUrl: publicWebUrl(PUBLIC_BASE_URL, '/pricing'),
      });
    }
  ...
  app.get<{
    ...
    if (useMeili) {
      ...
      if (!(await gateRateLimit(reply, 'apikey-search', auth.pubkey, 60, 60))) return reply;
      const delistedEventIds = new Set<string>(
        (await redis.smembers(DELISTED_EVENTS)) ?? [],
      );
  ...
  // ── POST /api/v1/archives — enqueue a lifetime archive ──────────────
  // API keys are lifetime-only, so this route never creates a Lightning
  // invoice. It validates the URL, rate-limits the key, and pushes the
  // job directly onto the archive queue.
  app.post('/api/v1/archives', async (request, reply) => {
    ...
    const normalizedMirrorUrls = normalizeMirrorUrls(parsed.data.mirrorUrls);
    ...
    // authenticated, a leaked key should not be able to pin Playwright
    // workers by enqueuing unlimited renders.
    if (!(await gateRateLimit(reply, 'archive-pk', auth.pubkey, 10, 60, 'rate limit (pubkey)'))) return reply;
    if (!(await gateRateLimit(reply, 'archive-apikey', auth.hash, 30, 60, 'rate limit (api key)'))) return reply;
    const result = await enqueueLifetimeArchive({
      purchases,
      url: parsed.data.url,
      userPubkey: auth.pubkey,
      tier: parsed.data.tier,
      archiveKey: parsed.data.archiveKey,
      mirrorUrls: normalizedMirrorUrls.urls,
      bookmarkSavedAt: normalizeBookmarkSavedAt(parsed.data.bookmarkSavedAt),
    });
    return {
  ...
  app.get<{ Params: { jobId: string } }>(
    ...
    async (request, reply) => {
      const auth = await authenticateApiKey(request, reply);
      if (!auth) return;
      const [record, jobMetadata] = await Promise.all([
        purchases.get(request.params.jobId),
        purchases.getArchiveJobMetadata(request.params.jobId).catch(() => null),
      ]);
      if (!record && !jobMetadata) return reply.status(404).send({ error: 'job not found' });
      if (record && record.userPubkey !== auth.pubkey) {
        return reply.status(404).send({ error: 'job not found' });
      }
      if (jobMetadata && jobMetadata.ownerPubkey !== auth.pubkey) {
        return reply.status(404).send({ error: 'job not found' });
      }
      const doneRaw = await redis.get(`dm:archive:done:${request.params.jobId}`);
      if (doneRaw) {
        try {
          ...
          // Fall through to the queue-state response below.
        }
      }
      return {
        jobId: request.params.jobId,
...
function normalizeBookmarkSavedAt(value: number | undefined): number | undefined {
  if (value === undefined) return undefined;
  const now = Math.floor(Date.now() / 1000);
  return value <= now + 10 * 60 ? value : undefined;
}
...
function meiliHitToBookmarkJson(doc: import('../search.js').BookmarkDoc): {
  ...
  tags: string[]; savedAt: number; savedAtMs?: number; saveCount: number; zapTotal: number;
} {
  return {
    id: doc.id,
