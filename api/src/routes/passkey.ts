...
export function register(deps: Deps): void {
  ...
  async function passkeyIpGate(request: FastifyRequest, reply: FastifyReply): Promise<boolean> {
    return gateRateLimit(reply, 'passkey-ip', request.ip, 60, 60);
  }
  ...
  // Public check — does this pubkey have a usable passkey sign-in? A
  // credential alone is not enough; interrupted registrations can leave
  // a passkey record without the encrypted nsec ciphertext.
  app.get<{ Querystring: { pubkey?: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!(await gateRateLimit(reply, 'passkey-exists', request.ip, 60, 60))) return reply;
      const normalized = pubkey.toLowerCase();
      const hasCredential = await passkeyStore.hasPasskey(normalized);
      let hasCiphertext = false;
      if (hasCredential && ciphertextStore) {
        try {
          hasCiphertext = await ciphertextStore.exists(normalized);
        } catch (err) {
          app.log.warn({ err, pubkey: normalized }, 'passkey ciphertext status check failed');
        }
      }
      return {
        exists: hasCredential && hasCiphertext,
        hasCredential,
        hasCiphertext,
      };
    },
