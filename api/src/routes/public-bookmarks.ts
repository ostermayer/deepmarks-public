...
import { bookmarkFiletype, bookmarkHasPdf, isScholarlyBookmark } from '../search.js';
...
export function register(deps: Deps): void {
  ...
  app.get('/bookmarks/public', async (request, reply) => {
    ...
    if (!(await gateRateLimit(reply, 'public-bookmarks-read-ip', request.ip, 120, 60))) return reply;
  ...
  app.post('/bookmarks/public', async (request, reply) => {
    ...
    if (!(await gateRateLimit(reply, 'public-bookmarks-ingest-ip', request.ip, 180, 60))) return reply;
    if (!(await gateRateLimit(reply, 'public-bookmarks-ingest-pk', event.pubkey, 120, 60))) return reply;
...
export async function meiliBookmarkDoc(event: NostrEvent, deps: Pick<Deps, 'redis'>): Promise<import('../search.js').BookmarkDoc | null> {
  const bookmark = bookmarkEventToJson(event);
  const url = bookmark.url;
  if (!url) return null;

  // Only index http(s) bookmarks. A kind:39701 arriving via relay fanout
  // could carry a javascript:/data: d-tag; keeping it out of the search
  // index prevents it surfacing in /search/public and being rendered as
  // an href by a client.
  let domain = 'unknown';
  let parsedUrl: URL;
  ...
  return {
    ...
    is_scholarly: isScholarlyBookmark({ url, title, description, tags }),
