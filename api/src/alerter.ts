  ...
  return {
    async alert(opts: AlertOpts): Promise<void> {
      ...
      try {
        ...
        // Mirror the most recent N alerts into a capped Redis list so
        // /admin/dashboard can render them without re-reading emails.
        try {
          ...
          await cfg.redis.lpush('dm:alerter:recent', entry);
          await cfg.redis.ltrim('dm:alerter:recent', 0, 49);
        } catch { /* dashboard mirror is best-effort */ }
      } catch (err) {
        // Alerting that throws is worse than alerting that silently
        // fails — never propagate. But DO release the dedup claim: it
        // was stamped before the send, so a failed send used to
        // suppress every retrigger for the whole debounce window — a
        // provider outage masked the very alert reporting it
        // (2026-08-23 review).
        if (dedupClaimed) await cfg.redis.del(dedupKey).catch(() => {});
