...
const isApple = process.env.VITE_APPLE_BUILD === '1';
...
export default defineManifest({
  ...
  //
  // Safari MV3 also accepts background.scripts and it's significantly
  // more reliable than background.service_worker on macOS — the
  // service-worker form intermittently fails to register (Safari logs
  // WKWebExtensionContextErrorBackgroundContentFailedToLoad and the
  // extension appears alive in the toolbar but content-script
  // messages never reach a listener). The scripts form runs as a
  // non-persistent background page that wakes on events, which is
  // what we need for NIP-07 / WebLN routing from third-party sites.
  background: (browser === 'firefox' || isApple)
  ...
  permissions: [
    ...
    'alarms',      // background lifetime-member private archive backfill
  ],
  host_permissions: [
    'https://*/*', // metadata scrape and NIP-07 support for normal web pages
    'http://*/*',  // bookmarking still needs to work on non-TLS pages
  ],
  content_scripts: [
    {
