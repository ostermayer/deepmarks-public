    ...
    return (
      <div>
        <div style={emptyHint}>
          ...
          this wallet after local approval. The NWC secret is encrypted with
          your Deepmarks password on this device.
        </div>
        <button
          style={revokeBtn}
  ...
  return (
    <div>
      <div style={emptyHint}>
        ...
        The secret is encrypted with your Deepmarks password and never leaves
        this browser.
      </div>
      <textarea
        value={draft}
...
export function ArchiveDefaultRow({ value, isLifetime, nsecHex, locked, onChange, onUpgraded }: {
  ...
  async function handleToggle(next: boolean) {
    ...
    if (IS_APPLE_BUILD) {
      setError('Archive is only available for premium members.');
      return;
    }
    if (locked || !nsecHex) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <ToggleRow label="Archive new bookmarks by default" value={value} onChange={handleToggle} />
