...
export function ArchiveDefaultRow({ value, isLifetime, nsecHex, locked, onChange, onUpgraded }: {
  ...
  async function handleToggle(next: boolean) {
    ...
    if (IS_APPLE_BUILD) {
      // Apple build copy must never reference the membership or any
      // purchase made outside the app (App Review guideline 3.1.3(b)).
      setError('Archiving is not enabled for this account.');
      return;
    }
    if (locked || !nsecHex) {
  ...
  if (isLifetime === null) return IS_APPLE_BUILD ? null : <div style={emptyHint}>Checking lifetime status...</div>;

  if (!isLifetime) {
    // Apple build: render nothing rather than copy that names the
    // membership — that wording is what App Review flags as accessing
    // "content purchased outside the app" (guideline 3.1.3(b)).
    if (IS_APPLE_BUILD) return null;
    return (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {busy && !IS_APPLE_BUILD && <div style={{ fontSize: 12, color: '#827d72' }}>preparing checkout...</div>}
      {error && <ErrorBox>{error}</ErrorBox>}
      {checkout && !IS_APPLE_BUILD && <CheckoutPanel checkout={checkout} onCancel={() => setCheckout(null)} />}
    </div>
  );
...
export function SecuritySection({ state, onStateChange }: {
  ...
  async function showQr() {
    ...
    try {
      const qrDataUrl = await nsecQrDataUrl(reveal.nsec, 180);
      setReveal((current) => (
