...
export function SignRequest() {
  ...
  async function decide(decision: 'approve' | 'reject') {
    ...
    try {
      ...
      // Don't navigate away when the queue empties. A common NIP-07
      // flow (Stacker News, Coracle, etc.) issues getPublicKey
      // immediately followed by signEvent; if we hop to 'recent' the
      // second request only surfaces as a badge increment and the
      // user has to manually click the pennant to see it. Stay here
      // and let the 1.5s poll surface anything that lands. The empty
      // state (<NoRequests/>) shows a "back to bookmarks" link so
      // the user can leave whenever they want.
    } catch (e) {
