...
export interface EditTarget {
  ...
  /** Immutable bookmark/import time. Edits must carry this forward. */
  savedAt: number;
  savedAtMs?: number;
  publishedAt?: number;
  ...
  eventId?: string;
  /** Archive linkage + metadata carried through the edit — kind:39701
   *  is replaceable, so a rebuild that drops these strips them from
   *  the event every other device reads. */
  lightning?: string;
  blossomHash?: string;
  waybackUrl?: string;
}
