...
//
// Header -> body (h1 + subtitle, three numbered bullets, "new to Deepmarks?"
...
export function Onboarding() {
  return (
    <div style={page}>
      ...
      <div style={body}>
        <h1 style={h1}>Your bookmarks, your identity.</h1>
        <p style={lede}>
          Save any page to Nostr with a key that stays on this device.
        </p>

        <ol style={bullets}>
          <Bullet n="01" text="Create or import a Nostr identity." />
          <Bullet n="02" text="One click saves the current page; tags + archive optional." />
          <Bullet n="03" text="Use Deepmarks as a signer on Nostr-enabled websites." />
        </ol>

        <div style={sandPanel}>
          <div style={sandLabel}>NEW TO DEEPMARKS?</div>
          <p style={sandBody}>
            We'll help you create an identity and back up the recovery key. Already use
            Nostr? Import your existing <code style={mono}>nsec1...</code> key instead.
          </p>
        </div>
      </div>

      <footer style={footer}>
        <button style={primaryBtn} onClick={() => navigate('login')}>Create or import identity</button>
      </footer>
    </div>
  );
}
