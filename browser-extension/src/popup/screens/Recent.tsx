...
} from '../../lib/bookmark-merge-core.js';
...
interface ParsedBookmark {
  ...
  eventId: string;
  /** Archive linkage + metadata that MUST survive an edit round-trip —
   *  kind:39701 is replaceable, so a rebuild that drops these strips
   *  them from the event every other device reads. */
  lightning?: string;
  blossomHash?: string;
  waybackUrl?: string;
...
function sortNewest(list: ParsedBookmark[]): ParsedBookmark[] {
  return [...list].sort(compareBookmarksNewest);
}
...
function upsertBookmark(
  ...
  const incoming = mergeReplaceableBookmark(cachedByUrl.get(bookmark.url), bookmark);
  const existing = byUrl.get(bookmark.url);
  if (!existing || replaceableBookmarkWins(existing, incoming)) {
    byUrl.set(bookmark.url, mergeReplaceableBookmark(existing, incoming));
  }
}
...
export function Recent({ state }: { state: NsecState }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      nsecHex
        ? fetchPrivateBookmarks(nsecHex, pubkey)
            .then((entries) => entries.map((e): ParsedBookmark => ({
              ...
              blossomHash: e.blossomHash,
              waybackUrl: e.waybackUrl,
            })))
            .then((items) => ({ items, failed: false }))
            .catch(() => ({ items: [] as ParsedBookmark[], failed: true }))
      ...
      for (const b of filterDeletedBookmarks(priv.items, deleted)) {
        const incoming = mergeReplaceableBookmark(cachedByUrl.get(b.url), b);
        const existing = byUrl.get(b.url);
        byUrl.set(b.url, mergeReplaceableBookmark(existing, incoming));
      }
  ...
  const lockedNotice = state.pubkey && !state.nsecHex;
  ...
  return (
    <div style={page}>
      {lockedNotice && (
        ...
        >
          deepmarks is locked — private bookmarks are hidden until you
