...
function parse(event: NostrEvent): ParsedBookmark | null {
  ...
  return {
    ...
    savedAt: publishedAt ?? event.created_at,
    publishedAt,
    eventId: event.id,
    visibility: 'public',
  };
...
export function Recent({ state }: { state: NsecState }) {
  ...
  useEffect(() => {
    ...
    type FeedResult = { items: ParsedBookmark[]; failed: boolean };
    void Promise.all<FeedResult>([
      ...
      if (cancelled) return;
      if (pub.failed && priv.failed) return;
      const cachedByUrl = new Map(cachePrime.map((b) => [b.url, b]));
      const byUrl = new Map<string, ParsedBookmark>();
      for (const b of pub.items) byUrl.set(b.url, preserveKnownSaveTime(cachedByUrl.get(b.url), b));
      for (const b of priv.items) byUrl.set(b.url, b);
  ...
  function removeBookmarkFromList(bookmark: ParsedBookmark) {
    ...
    setBookmarks((prev) => {
      const next = (prev ?? []).filter((x) => x.url !== bookmark.url);
