...
import { getLifetimeStatus, startBrowserCaptureArchive, startLifetimeArchive } from '../../lib/archive.js';
import { scheduleArchiveKeyReconcileSoon } from '../../lib/archive-key-reconciler.js';
import { generateArchiveKey, publishPendingArchiveKey, stashPendingKey } from '../../lib/archive-keys.js';
import { captureActiveTabHtml } from '../../lib/browser-capture.js';
import { maybeStartMediaArchiveForBookmark } from '../../lib/media-archive.js';
...
export function Add({ state: account }: { state: NsecState }) {
  ...
  const [captureBrowserView, setCaptureBrowserView] = useState(false);
  ...
  const originalSavedAtMsRef = useRef<number | undefined>(undefined);
  // Archive linkage + metadata the edit must carry forward — a rebuild
  // that drops these strips them from the replaceable event relay-wide.
  const originalLightningRef = useRef<string | undefined>(undefined);
  const originalBlossomHashRef = useRef<string | undefined>(undefined);
  const originalWaybackUrlRef = useRef<string | undefined>(undefined);
    ...
    void (async () => {
      ...
      if (edit) {
        applyDraft({
          ...
          originalSavedAtMs: edit.savedAtMs,
          originalLightning: edit.lightning,
          originalBlossomHash: edit.blossomHash,
          originalWaybackUrl: edit.waybackUrl,
        });
        return;
      }
      ...
      const activeTabPromise = readActiveTab();
      const localSettings = await withTimeout(getSettings(), 1000, DEFAULT_SETTINGS);
      const settingsPromise = account.nsecHex
        ? syncSettingsAndPublishedRelays(account.pubkey, account.nsecHex).catch(() => localSettings)
        : Promise.resolve(localSettings);

      const [t, s, lifetime] = await Promise.all([
        activeTabPromise,
        withTimeout(settingsPromise, 2500, localSettings),
        withTimeout(
          account.pubkey
      ...
      if (t) {
        ...
        void withTimeout(fetchUrlMetadata(t.url), 3500, null).then((meta) => {
          if (cancelled) return;
          if (meta?.suggestedTags?.length) setSuggestedTags(meta.suggestedTags);
        });
      } else {
        setTabError('Deepmarks could not read the active Safari tab. Close this popup, reload the page, and click Deepmarks again.');
      }
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      void saveAddBookmarkDraft({
        ...
        originalSavedAtMs: originalSavedAtMsRef.current,
        originalLightning: originalLightningRef.current,
        originalBlossomHash: originalBlossomHashRef.current,
        originalWaybackUrl: originalWaybackUrlRef.current,
      });
    }, 150);
  ...
  function applyDraft(draft: AddBookmarkDraft) {
    ...
    setCaptureBrowserView(draft.captureBrowserView ?? false);
    ...
    originalSavedAtMsRef.current = draft.originalSavedAtMs;
    originalLightningRef.current = draft.originalLightning;
    originalBlossomHashRef.current = draft.originalBlossomHash;
    originalWaybackUrlRef.current = draft.originalWaybackUrl;
    draftReadyRef.current = true;
  }
  ...
  async function onSave() {
    ...
    setError(null);
    try {
      const savedAtMs = editing
        ? (originalSavedAtMsRef.current ?? ((originalSavedAtRef.current ?? Math.floor(Date.now() / 1000)) * 1000))
        : Date.now();
      ...
      const input = {
        ...
        tags,
        publishedAt: savedAt,
        publishedAtMs: savedAtMs,
        // Preserve archive linkage through edits; never invent it on a
        // fresh save (the refs are only populated by the edit handoff).
        lightning: editing ? originalLightningRef.current : undefined,
        blossomHash: editing ? originalBlossomHashRef.current : undefined,
        waybackUrl: editing ? originalWaybackUrlRef.current : undefined,
      ...
      if (archive) {
        ...
        // the key keyed by jobId before navigating) but not
        ...
        try {
          ...
          if (lifetime.isLifetimeMember) {
            ...
            if (visibility === 'private' && archiveKey) {
              await stashPendingKey(r.jobId, archiveKey).catch(() => { /* tolerable */ });
              await publishPendingArchiveKey(r.jobId, archiveKey, account_.nsecHex, account_.pubkey).catch(() => { /* tolerable */ });
        ...
        if (captureBrowserView) {
          try {
            const latestSettings = await getSettings();
            const browserCaptureKey = generateArchiveKey();
            const capture = await captureActiveTabHtml(tab.url);
            const r = await startBrowserCaptureArchive(
              {
                url: tab.url,
                eventId: visibility === 'public' ? result.event.id : undefined,
                archiveKey: browserCaptureKey,
                htmlBase64: capture.htmlBase64,
                title: capture.title || title.trim(),
                mirrorUrls: latestSettings.backupBlossomServers,
                bookmarkSavedAt: savedAt,
              },
              account_.nsecHex,
            );
            await stashPendingKey(r.jobId, browserCaptureKey).catch(() => { /* tolerable */ });
            await publishPendingArchiveKey(r.jobId, browserCaptureKey, account_.nsecHex, account_.pubkey).catch(() => { /* tolerable */ });
            scheduleArchiveKeyReconcileSoon();
          } catch {
            // Browser-view capture is a private fallback. A failed
            // capture should not block the normal saved bookmark or
            // standard server archive job above.
          }
        }
      }
      void maybeStartMediaArchiveForBookmark({
        url: tab.url,
  ...
  return (
    <div style={page}>
      ...
      <div style={body}>
        ...
        {tabError && <div style={errorRow}>{tabError}</div>}
      ...
      <footer style={footer}>
        ...
        {showArchiveToggle && archive && (
          <label style={browserCaptureRow}>
            <input
              type="checkbox"
              checked={captureBrowserView}
              onChange={(e) => setCaptureBrowserView(e.currentTarget.checked)}
            />
            <span>
              <span style={archiveTitle}>Archive current browser view</span>
              <span style={browserCaptureSub}>Private fallback for pages that block server archiving</span>
            </span>
          </label>
        )}
        <div style={btnRow}>
...
const browserCaptureRow: React.CSSProperties = {
...
const browserCaptureSub: React.CSSProperties = {
  display: 'block',
