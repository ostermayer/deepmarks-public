...
export interface AddBookmarkDraft {
  ...
  originalSavedAt?: number;
  originalSavedAtMs?: number;
  /** Archive linkage + metadata an edit must carry forward — a rebuild
   *  that drops these strips them from the replaceable event. */
  originalLightning?: string;
  originalBlossomHash?: string;
  originalWaybackUrl?: string;
}
...
export async function saveAddBookmarkDraft(draft: AddBookmarkDraft): Promise<void> {
  await setSessionDraft(ADD_DRAFT_KEY, {
    ...
    originalSavedAt: typeof draft.originalSavedAt === 'number' ? draft.originalSavedAt : undefined,
    originalSavedAtMs: typeof draft.originalSavedAtMs === 'number' ? draft.originalSavedAtMs : undefined,
    originalLightning: draft.originalLightning || undefined,
    originalBlossomHash: draft.originalBlossomHash || undefined,
    originalWaybackUrl: draft.originalWaybackUrl || undefined,
  });
}
...
function isAddBookmarkDraft(value: unknown): value is AddBookmarkDraft {
  ...
  return (
    ...
    (v.captureBrowserView === undefined || typeof v.captureBrowserView === 'boolean') &&
    ...
    (v.originalSavedAt === undefined || typeof v.originalSavedAt === 'number') &&
    (v.originalSavedAtMs === undefined || typeof v.originalSavedAtMs === 'number')
  );
}
