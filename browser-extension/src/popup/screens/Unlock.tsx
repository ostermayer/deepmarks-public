...
import { syncSettingsFromServer } from '../../lib/settings-store.js';
...
export function Unlock({ state, onUnlocked }: {
  ...
  async function unlock() {
    ...
    try {
      ...
      if (next.nsecHex) void syncSettingsFromServer(next.nsecHex).catch(() => undefined);
  ...
  async function recoverWithNsec() {
    ...
    try {
      ...
      await clearNwc().catch(() => undefined);
      if (next.nsecHex) void syncSettingsFromServer(next.nsecHex).catch(() => undefined);
      void chrome.runtime.sendMessage({ kind: 'archive-backfill-run' }).catch(() => undefined);
      onUnlocked(next);
    } catch (e) {
