...
import {
  getSettings, importNip65Relays, patchSettings, pushSettingsToServer, revokeLogin, savedLoginLabel, syncSettingsFromServer,
...
export function Settings({ state, onStateChange, onSignOut }: {
  ...
  const autoRelayScanFor = useRef<string | null>(null);

  useEffect(() => {
    void getSettings().then(setSettings);
    if (state.nsecHex) {
      setSyncStatus('syncing settings...');
      void syncSettingsFromServer(state.nsecHex)
        .then((next) => {
          setSettings(next);
  ...
  useEffect(() => {
    if (!state.pubkey || !settings || autoRelayScanFor.current === state.pubkey) return;
    autoRelayScanFor.current = state.pubkey;
    void findRelays({ quiet: true });
  }, [state.pubkey, settings]);
  ...
  async function findRelays(opts: { quiet?: boolean } = {}) {
    ...
    try {
      ...
      const added = discovered.filter((r) => settings?.relays.some((existing) => existing.url === r.url) === false).length;
      if (added > 0 || !opts.quiet) {
        setRelayScanStatus(added > 0
          ? `added ${added} relay${added === 1 ? '' : 's'} from NIP-65`
          : 'NIP-65 relays already in settings');
      }
    } catch (e) {
