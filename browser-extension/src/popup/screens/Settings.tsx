...
export function Settings({ state, onStateChange, onSignOut }: {
  ...
  // Two-tap inline confirm for sign-out. window.confirm() is blocked
  // inside Safari Web Extension popups (the dialog never appears and
  // confirm() returns false), so we replace it with an in-popup
  // "click again to confirm" state. Auto-resets after 4s.
  ...
  async function update(patch: Partial<SettingsT>) {
    ...
    const next = await patchSettings(shouldSync
      ? { ...patch, pendingSync: true, syncedAt: Date.now() }
      : patch);
    setSettings(next);
  ...
  return (
    <div style={page}>
      ...
      <div style={body}>
        ...
        {/* Apple build: the whole section disappears for non-members.
            An "Archiving" heading over membership copy is what App
            Review flagged (guideline 3.1.3(b)); members just see the
            working controls with no purchase reference. */}
        {(!IS_APPLE_BUILD || isLifetime) && (
        ...
        <Section title="Account">
          ...
          >
            {signOutArmed ? 'Click again to confirm — clears your nsec' : 'Sign out'}
          </button>
        </Section>
      </div>
    </div>
