...
async function openApprovalUi(): Promise<void> {
  // Toolbar popup is the cleanest surface when the browser allows it.
  // Some NIP-07 calls are not considered user gestures by Chrome/Firefox,
  // so openPopup can reject; fall back to a small extension window so
  // login/sign prompts are visible without relying on the badge.
  if (chrome.action.openPopup) {
    try {
      await chrome.action.openPopup();
      return;
    } catch {
      // Fall through to the standalone approval window.
    }
  }
  ...
  approvalWindowId = typeof win.id === 'number' ? win.id : null;
}
