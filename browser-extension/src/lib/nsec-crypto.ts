...
/** Encrypt an arbitrary UTF-8 secret with an already-derived account
 *  key. Used for child secrets such as NWC, so they unlock and expire
 *  with the same password-cache policy as the nsec. */
export async function encryptTextWithKey(plaintext: string, key: CryptoKey): Promise<EncryptedPayload> {
  const iv = crypto.getRandomValues(new Uint8Array(IV_BYTES));
  const ctBuf = await crypto.subtle.encrypt(
    { name: 'AES-GCM', iv: iv as BufferSource },
    key,
...
export async function decryptTextWithKey(blob: EncryptedPayload, key: CryptoKey): Promise<string> {
  if (blob.cipher !== 'aes-gcm-256') throw new Error('unsupported encrypted payload');
  const ptBuf = await crypto.subtle.decrypt(
    { name: 'AES-GCM', iv: b64ToBytes(blob.ivB64) as BufferSource },
    key,
    b64ToBytes(blob.ciphertextB64) as BufferSource,
