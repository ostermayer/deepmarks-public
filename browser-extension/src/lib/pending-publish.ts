...
export async function drainPendingPublishes(nsecHex: string, pubkey: string): Promise<DrainResult> {
  ...
  for (let i = 0; i < slice.length; i += DRAIN_CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map(async (item) => {
      try {
        await postSignedEventViaDeepmarks(drainEvent(item.event, nsecHex), nsecHex);
...
/** Replaceable (d-tag) events are state snapshots — the relay keeps only
 *  the newest copy per d-tag. Replaying a queued one with its original
 *  created_at after any newer save of the same key gets it silently
 *  discarded as "older". Re-sign with drain time so the queued state
 *  actually lands. */
export function drainEvent(event: NostrEvent, nsecHex: string): NostrEvent {
  if (!event.tags.some((tag) => tag[0] === 'd')) return event;
  const createdAt = Math.max(Math.floor(Date.now() / 1000), event.created_at + 1);
  if (createdAt === event.created_at) return event;
  try {
    return finalizeEvent(
      { kind: event.kind, tags: event.tags, content: event.content, created_at: createdAt },
      hexToBytes(nsecHex),
    );
  } catch {
    return event; // bad key material — send the original rather than nothing
  }
}
