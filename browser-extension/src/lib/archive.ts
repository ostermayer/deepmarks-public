...
import { IS_APPLE_BUILD } from './build-flags.js';

const API_BASE = 'https://api.deepmarks.org';

// Apple build strings must never reference the membership or any
// purchase made outside the app (App Review guideline 3.1.3(b)) —
// IS_APPLE_BUILD is a build-time literal, so only one string ships.
const ARCHIVE_402_MESSAGE = IS_APPLE_BUILD
  ? 'archiving is not enabled for this account'
  : 'lifetime membership required to archive pages';
...
export async function startLifetimeArchive(
  ...
  if (res.status === 402) throw new Error(ARCHIVE_402_MESSAGE);
...
export interface ArchiveStatus {
  ...
  blobHash?: string;
  blossomHash?: string;
  reason?: 'site-blocked' | 'not-found' | 'timeout' | 'failed';
  message?: string;
  error?: string;
}
...
/** The account's outstanding server-side archive jobs. The backfill
 *  loop uses this as backpressure — mirroring the web app's behavior —
 *  so a slow server queue is waited out instead of enqueued into. */
export async function getArchiveQueueStatus(nsecHex: string): Promise<ArchiveQueueStatus> {
  const path = '/account/archive-queue';
  const url = `${API_BASE}${path}`;
  const auth = await buildNip98AuthHeader(url, 'GET', nsecHex);
  const res = await fetch(url, { headers: { Authorization: auth } });
  if (!res.ok) throw new Error(`archive queue status ${res.status}: ${await res.text().catch(() => '')}`);
  const body = (await res.json()) as { pending?: number; running?: number };
  return { pending: body.pending ?? 0, running: body.running ?? 0 };
}
