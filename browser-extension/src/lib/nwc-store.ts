...
// It is intentionally not synced to relays or other devices: an NWC
// app secret is a wallet spending credential, so each client must be
// connected to the wallet separately.
//
...
export async function clearNwc(_opts: { sync?: boolean } = {}): Promise<void> {
  await clearLocalNwc();
}
