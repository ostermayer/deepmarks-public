...
export async function loadNwc(): Promise<NwcConnection | null> {
  ...
  if (account.nsecHex && account.pubkey) {
    const remote = stored
      ? await withTimeout(loadSyncedNwcBestEffort(account.nsecHex, account.pubkey), 750)
      : await loadSyncedNwcBestEffort(account.nsecHex, account.pubkey);
    if (remote.kind === 'deleted') {
      await clearLocalNwc();
      return null;
    }
    if (remote.kind === 'connected') {
      const key = await getCachedAccountEncryptionKey();
      if (key) await saveNwcWithKey(remote.connection, key);
...
export async function saveNwc(conn: NwcConnection): Promise<void> {
  ...
  const account = await nsecStore.getState();
  if (account.nsecHex && account.pubkey) {
    void publishSyncedNwc(conn, account.nsecHex, account.pubkey).catch(() => undefined);
  }
}
...
export async function clearNwc(opts: { sync?: boolean } = {}): Promise<void> {
  if (opts.sync !== false) {
    const account = await nsecStore.getState();
    if (account.nsecHex && account.pubkey) {
      void publishSyncedNwc(null, account.nsecHex, account.pubkey).catch(() => undefined);
    }
  }
  await clearLocalNwc();
}

async function withTimeout<T>(promise: Promise<T>, ms: number): Promise<T | { kind: 'missing' }> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([
      promise,
      new Promise<{ kind: 'missing' }>((resolve) => {
        timer = setTimeout(() => resolve({ kind: 'missing' }), ms);
      }),
    ]);
  } finally {
...
async function clearLocalNwc(): Promise<void> {
  await chrome.storage.local.remove(KEY);
}

async function loadSyncedNwcBestEffort(
  nsecHex: string,
  pubkey: string,
): Promise<
  | { kind: 'missing' }
...
> {
  try {
    const synced = await fetchSyncedNwc(nsecHex, pubkey);
