...
// frontend/src/lib/nostr/nsec-backup.ts and
// browser-extension/src/lib/nsec-backup.ts, guarded by
...
export async function nsecQrDataUrl(nsec: string, width = 220): Promise<string> {
  // Width is a per-surface display choice (web backup card vs the
  // narrower extension popup) — callers pass theirs so this file can
  // stay byte-identical on both surfaces.
