...
//     device or from the web app. Pending jobs are published as
//     job:<jobId> until reconciliation promotes them to blobHash.
//
...
import {
  ...
  archiveKeyJobMapKey,
  fetchArchiveKeyMap,
  removeArchiveKeyFromSet,
} from './archive-keys-sync.js';

const STORAGE_KEY = 'deepmarks-archive-keys';
// Pre-completion stash: holds plaintext keys keyed by jobId
...
interface PendingMap {
  // jobId → plaintext archive key (base64). Plaintext rather
...
/**
 * Stash a freshly-minted archive key under its jobId *before*
 ...
 * the user's nsec.
 */
export async function stashPendingKey(jobId: string, archiveKey: string): Promise<void> {
  ...
  existing[jobId] = { archiveKey, createdAt: Math.floor(Date.now() / 1000) };
...
export async function publishPendingArchiveKey(
  ...
  if (!jobKey) return;
  await addArchiveKeyToSet(jobKey, archiveKey, nsecHex, ownPubkey);
}
...
export async function countPendingArchiveKeys(): Promise<number> {
  return Object.keys(await readPendingMap()).length;
}

/**
 * Walk the user's archives and reconcile any pending keys (jobId
 ...
 * a save→close→reopen cycle. Only touches archives whose jobId
...
export async function getArchiveKey(
  ...
  const jobKey = archiveKeyJobMapKey(jobId);
  const plaintext = map[blobHash] ?? (jobKey ? map[jobKey] : undefined);
