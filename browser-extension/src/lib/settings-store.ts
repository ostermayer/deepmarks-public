...
export interface Settings {
  ...
  syncedAt: number;
  /** Local synced setting changed while the server write was pending
   *  or failed. Remote settings should only lose to local state while
   *  this is true. */
  pendingSync: boolean;
  relays: RelayConfig[];
  defaultTags: string[];
   ...
   *  this; user can flip per-bookmark. Default: public unless the user
   *  explicitly switches the account default to private. */
...
export async function getSettings(): Promise<Settings> {
  ...
  return {
    ...
    defaultVisibility: value.defaultVisibility === 'private' ? 'private' : 'public',
...
export async function syncSettingsFromServer(nsecHex: string): Promise<Settings> {
  ...
  const current = await getSettings();
  if (current.pendingSync && remote.updatedAt <= current.syncedAt) {
    return pushSettingsToServer(nsecHex, current);
  }
...
/** Replace settings.relays with the current published NIP-65 relay list.
 *  Behavior:
 *    - The latest kind:10002 is the source of truth; old relays that
 *      only came from previous scans are removed.
 *    - URLs get their NIP-65 read/write markers.
...
export async function importNip65Relays(
  ...
  return mutateSettings((settings) => {
    ...
    return { ...settings, relays: normalizeRelays([...byUrl.values()]) };
  });
}
