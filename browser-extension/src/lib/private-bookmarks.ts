...
/** Inner entries key on the same `d` tag as the set events. */
const entryUrl = dTag;
...
function selectCompletePrivateSetEvents(events: NostrEvent[]): NostrEvent[] {
  ...
  // Last-resort union (ported from the web app): no single version is
  // complete — a partial save failed mid-publish, or two clients started
  // overlapping rewrites. Rather than treat the set as empty (which a
  // later save would turn into a destructive rewrite), take the latest
  // event per chunk index across all versions.
  if (records.length === 0) return [];
  const unionByIndex = new Map<number, NostrEvent>();
  for (const record of records) {
...
async function fetchCurrentPrivateSet(
  ...
  return { ...mergePrivateSetEntries(decryptedEvents), decryptFailures, eventCount: events.length };
}

/** Refuse a whole-set rewrite built on an incomplete view of relay state. */
function assertPrivateSetRewriteSafe(set: PrivateSet): void {
  if (set.decryptFailures && set.decryptFailures > 0) {
    throw new Error(
      `couldn't decrypt ${set.decryptFailures} private bookmark chunk(s) — ` +
      'not rewriting the set. Try again, or open deepmarks.org to recover.',
    );
  }
}

const KNOWN_SET_STORAGE_PREFIX = 'deepmarks-private-set-known:';

/** Last-known non-empty set size, persisted across popup sessions. A
 *  fetch that comes back EMPTY while we know the set was non-empty means
 *  the read silently failed (relay timeout returns [] without throwing) —
 *  rewriting on top of it would wipe the library. */
async function readKnownSetCount(pubkey: string): Promise<number> {
  try {
    if (typeof chrome === 'undefined' || !chrome?.storage?.local) return 0;
    const key = `${KNOWN_SET_STORAGE_PREFIX}${pubkey}`;
    const got = await chrome.storage.local.get(key);
    const value = got?.[key] as { count?: number } | undefined;
...
async function writeKnownSetCount(pubkey: string, count: number): Promise<void> {
  try {
    if (typeof chrome === 'undefined' || !chrome?.storage?.local) return;
    const key = `${KNOWN_SET_STORAGE_PREFIX}${pubkey}`;
    await chrome.storage.local.set({ [key]: { count, updatedAt: Date.now() } });
  } catch {
    // Storage quota / detached context — the guard is best-effort.
  }
}

async function assertFetchedSetPlausible(set: PrivateSet, pubkey: string): Promise<void> {
  if ((set.eventCount ?? 0) > 0 || set.entries.length > 0) return;
  const known = await readKnownSetCount(pubkey);
  if (known > 0) {
    throw new Error(
      'private bookmarks could not be loaded from the relay — ' +
      'not rewriting the set. Check your connection and try again.',
    );
  }
}
...
export async function publishPrivateBookmark(
  ...
  // 1. Fetch + decrypt the current chunked private set. Refuse to
  //    continue on any signal that this is an incomplete view — a
  //    whole-set rewrite built on it erases bookmarks relay-wide.
  const set = await fetchCurrentPrivateSet(pool, readRelays, nsecHex, ownPubkey);
  assertPrivateSetRewriteSafe(set);
  await assertFetchedSetPlausible(set, ownPubkey);
  ...
  if (ok.length > 0) await writeKnownSetCount(ownPubkey, next.length);
...
export async function fetchPrivateBookmarks(
  ...
  const readRelays = await getReadRelays();
  const set = await fetchCurrentPrivateSet(pool, readRelays, nsecHex, ownPubkey);
  // Successful read: remember the set size so a later silent-empty fetch
  // (relay timeout yields [] without throwing) can't feed a destructive
  // rewrite in publish/delete.
  if ((set.eventCount ?? 0) > 0 && !set.decryptFailures) {
    await writeKnownSetCount(ownPubkey, set.entries.length);
  }
