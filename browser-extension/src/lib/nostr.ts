...
import { enqueuePendingPublish, pendingPublishRelay, removePendingPublish } from './pending-publish.js';
...
const DEEPMARKS_RELAY = pendingPublishRelay;
...
export interface BookmarkInput {
  ...
  /** Detected site Lightning address. Maps to the `lightning` tag. */
  lightning?: string;
  /** Set when the bookmark has a permanent archive — the worker's
   *  Blossom hash. Maps to the `blossom` tag. */
  blossomHash?: string;
  /** Wayback Machine snapshot URL backing an archive. Maps to the
   *  `wayback` tag. */
  waybackUrl?: string;
...
export async function postSignedEvent(
  ...
  if (mode === 'direct') {
    ...
    const result = await publishSignedEventDirect(event, relays, opts.pool ?? sharedPool(), opts.timeoutMs ?? 8000);
    if (result.ok.length === 0) {
      const reason = result.failed.map((f) => f.reason).filter(Boolean).join('; ') || 'direct publish failed';
      const queued = await enqueuePendingPublish(event, reason);
      if (queued) return { ok: [DEEPMARKS_RELAY], failed: [] };
    } else if (!result.ok.some((u) => normalizeRelayUrl(u) === normalizeRelayUrl(DEEPMARKS_RELAY))) {
      // Partial success that missed the canonical relay: nos.lol having
      // the event doesn't make it show up in Deepmarks (server cache,
      // search, other devices read relay.deepmarks.org). Queue a retry
      // targeted at the canonical path; report the real partial result.
      await enqueuePendingPublish(event, 'canonical relay missed in direct publish');
    }
    return result;
  }
...
async function postSignedEventViaDeepmarks(
  ...
  const queued = await enqueuePendingPublish(event, 'queued before network publish');
  try {
    ...
    if (queued) await removePendingPublish(event);
