...
- **`lib/nwc-store.ts`** — one optional Nostr Wallet Connect record.
  ...
  cached account key as the nsec. Legacy plaintext NWC records migrate
  the next time the password is unlocked.
...
- Paying Lightning invoices through the optional WebLN bridge backed
  by the user's NWC wallet connection
...
password-derived account key that protects the user's nsec. Removing
password protection disconnects NWC instead of writing the wallet
