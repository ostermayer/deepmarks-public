...
Chromium smoke test for the media archive enqueue path:

```bash
npm run smoke:chrome
```

This builds the Chrome extension, loads a temporary copy into
Chrome/Chromium, and verifies that supported media URLs enqueue archive
jobs through `chrome.storage.local`. Set `CHROME_BIN` if Chrome is not
in a standard location. Minimal CI hosts without Chrome print a `SKIP`
