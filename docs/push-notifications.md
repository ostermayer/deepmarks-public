...
One pubkey can register several devices; the per-pubkey subscription set is
capped at `MAX_SUBS_PER_PUBKEY = 10` (`web-push.ts`). Stale endpoints are
dropped automatically when a vendor returns 404/410 on the next push.
...
3. Redeploy Box A (targeted — a bare `--only a` triggers a full
   `deploy.sh a` that cold-recompiles strfry from C++ source and can OOM
   the box; name the service instead):
   ```bash
