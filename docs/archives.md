...
small chooser so the user can pick which file to view. The row's privacy
chip (next to the URL) conveys whether the archive is encrypted. Pending
...
later chunks). Multi-file private archives publish the same archive key
for every file blob in the record. See
...
for video and audio stay private even when the original bookmark is
public.
