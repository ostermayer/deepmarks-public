...
After a public `webpage` job reaches terminal failure, Box A also starts
an archive rescue pass. Rescue is public-only: private archive failures
are not reprocessed because the server no longer has the user's archive
key. The pass combines deterministic URL variants, Wayback availability,
known public domain migrations, optional LLM hints, and live web search
through the configured archive-rescue search provider. The LLM can
...
- *Webpage archive*: fetch the tweet JSON and render a clean, self-contained
  ...
  disappears. Falls back to the normal Playwright render if no provider serves
  the tweet.
...
**YouTube.** YouTube gates most formats behind a BotGuard **PO token**; without
...
yt-dlp is installed via a pip venv so the plugin loads, with **deno** on the
image as yt-dlp's JS runtime. Cookies are *not* used by default — a PO token
...
YouTube bot-detection trips so the cookies can be refreshed.
...
(lowered from 4 in `deploy/box-b/compose.yml` so large blob uploads and
mirror fanout stay under Linode's outbound-bandwidth threshold; the
`.env.example` default is 2). If many users archive at once, jobs queue
instead of running unbounded browser processes.
...
Worker chunks are large (up to the 150 MB capture cap (media add-on blobs up to 2 GB) and 150 MB
