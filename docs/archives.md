...
"format not available", interrupted jobs) re-request after ~2 days so a
server-side fix is picked up quickly, while genuinely-gone media (no
media / private / removed) backs off to monthly. Since 2026-08-21 the media
enqueue route also enforces the server-side terminal-failure cooldown: once
consecutive failures widen the escalating window past the client's cadence,
re-requests are answered with the `queued:recent-failure` sentinel instead
of a new job.
...
- *Deleted tweets fail permanently, not retryably.* When a provider answers
  with a definitive 404 (HTTP 404/410 or a FixTweet `{code:404}` body) and no
  provider succeeded or failed transiently, the worker throws
  `PermanentError('tweet_deleted')`: the job fails once as reason `not-found`,
  which does **not** page the operator (`shouldAlertArchiveFailure`) and does
  not retry. This classification still matters: `not-found` suppresses
  automated re-enqueue for 30 days outright, while a deleted tweet
  misclassified as *retryable* burns a fresh 3-attempt job per
  escalating-cooldown window (6h doubling per consecutive failure up to the
  same 30-day cap — `archiveFailureReenqueueWindowSeconds`) before converging
  on the same monthly quiet retry. Before the 2026-08-21 terminal-failure
  gate, that misclassification meant a fresh job — and one operator alert —
  every backfill cycle, forever.
- *A dead provider is "no signal", not "transient".* `api.fixupx.com`
  dropped out of DNS entirely in mid-2026 (NXDOMAIN; the apex domain still
  resolves). `fetchTweet` treats an unresolvable provider hostname
  (`ENOTFOUND`) as contributing no evidence either way: it can't veto a live
  provider's 404 (the bug that caused the deleted-tweet alert loop above),
  and if *every* provider is unresolvable (box DNS broken) the job stays
  retryable instead of being misread as "tweet deleted". The dead host is
  kept in the provider list on purpose — it costs one instant DNS miss and
  self-heals if FxEmbed ever restores the subdomain.
...
backup Blossom servers (three community mirrors since 2026-07-08). Users
can add their own trusted or paid Blossom servers from settings. Those
user-supplied mirrors are included on new archive jobs. Failed mirror
legs retry with backoff for ~34 h via the worker's
`dm:archive:mirror:retry` queue; permanently-rejecting legs (all-4xx
signatures, e.g. servers that refuse encrypted blobs) are dropped
immediately. Mirror-leg failures never email the operator — the primary
has its own uptime probe (`GET /health/blossom`). Details and history in
[blossom.md](blossom.md#mirror-fanout-bud-04) and
...
That client-side courtesy proved insufficient on its own: in July 2026
one client's backfill loop re-submitted ~2,380 private URLs 40-58 times
each (a fresh archiveKey per retry), growing the queue to 20.5k jobs —
88% duplicates — and delaying everyone's archives for a week
([`incidents/2026-07-17-archive-queue-duplicate-flood.md`](incidents/2026-07-17-archive-queue-duplicate-flood.md)).
The server now holds a **pending-archive claim** per (family, owner,
url) for a job's entire queued/in-flight lifetime
([`api/src/archive-dedupe.ts`](../api/src/archive-dedupe.ts)): private
lifetime enqueues and media add-on enqueues that duplicate a pending job
get the `queued:already-pending` sentinel instead of a new job. The
claim is released by the terminal callback (success or failure) with a
7-day TTL backstop. Webpage and media claims are separate namespaces —
archiving a video's page and its media are both legitimate. Duplicates
are deliberately answered with the sentinel rather than the pending
jobId: binding a retry's freshly-generated key to an old job would make
the eventual archive undecryptable for the client.

Since 2026-08-21 a second server gate sits in front of the pending
claim: the **terminal-failure cooldown**
([`api/src/archive-failures.ts`](../api/src/archive-failures.ts)). An
owner+URL whose last terminal failure is recent is refused with
`queued:recent-failure` (or `queued:permanent-failure`) on **all**
tiers — public, private, and the media add-on — with an escalating
window: 6h doubling per consecutive failure, capped at the 30-day
permanent window, cleared by a later success. This closed the *serial*
variant of the duplicate flood: the pending claim stops concurrent
duplicates but is released by the terminal callback, so a looping
client could re-mint a failed URL every cycle
([`incidents/2026-08-21-archive-backfill-retry-loop.md`](incidents/2026-08-21-archive-backfill-retry-loop.md)).
The client mirrors the same curve locally
([`frontend/src/lib/nostr/archive-backoff.ts`](../frontend/src/lib/nostr/archive-backoff.ts))
so backed-off URLs are skipped without a request. Deliberate operator
retries (`archive-retry`, `archive-rescue`) bypass the gate.
...
It forces another queue pass for missing-key records; plain failed
records instead follow the escalating failure backoff (client ledger in
[`archive-backoff.ts`](../frontend/src/lib/nostr/archive-backoff.ts)
with the server cooldown behind it), so mashing retry cannot re-flood
the queue.
