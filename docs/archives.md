...
- **Social-media mirrors** — `x.com` / `twitter.com` render client-side
  and block scrapers, so the worker captures an empty shell. Nitter-style
  mirrors (`xcancel.com`, `nitter.net`) serve the same tweet as static
  HTML that archives cleanly.
...
2. **Same-item binding** — a candidate is only accepted when it is a
   ...
   scholarly PDF (same DOI/PII), or an *allowlisted* social mirror of the
   *same tweet id*. An LLM hallucination or a poisoned search result on an
   unrelated domain is dropped.
