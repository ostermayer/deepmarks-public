...
archiving the same URL — or the same YouTube video — produce identical
blob hashes and share one set of bytes in Blossom.

Without a refcount, the first user to delete their archive would also
take everyone else's copy with them. The `dm:archive-refs:<contentKey>`
Redis SET tracks every pubkey that references a blob:

- `contentKey` for webpage archives = the `blobHash` itself.
- `contentKey` for YouTube archives = `yt:<videoId>` so the same video
  pasted as `youtu.be/X`, `watch?v=X`, `embed/X`, and `shorts/X` all
  collapse to one refcount bucket.
