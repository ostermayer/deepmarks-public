...
| Component | Role |
|---|---|
| **LND node** | Our own self-hosted LND instance (`ln.lnzap.org`). Receives zap payments directly and backs BTCPay. |
| **BTCPay Server** (self-hosted, `https://btcpay.lnzap.org`) | Hosted checkout for user-initiated lifetime and add-on purchases. Talks to the same LND node over gRPC. |
| **`api`** | Fastify service on Box A. Builds invoices for Deepmarks-hosted LNURL addresses, verifies settlement, signs zap receipts, serves the LNURL-pay endpoint. |
...
For Deepmarks-hosted LNURL addresses, the api talks directly
to the LND gRPC endpoint at `ln.lnzap.org:10009` (not the REST 8080) using
...
`deepmarks handshake`. Seeing one such request when `api`
...
`subscribeToInvoices` in api fans out settled invoices to
two handlers:
...
The `SubscriptionCircuitBreaker` reconnects on transient LND-node
...
| `dan@deepmarks.org` | operator personal signer | Public operator Lightning address (also used as the brand/social profile). Always registered in `api/src/bootstrap.ts` (`lnIdentities.dan = signers.personal`) — not env-gated. |
...
change in `api/src/bootstrap.ts` (the `lnIdentities` map) plus a
new nsec on Box C.
...
5. api verifies the NIP-57 zap request, creates an invoice
   with `description_hash = sha256(<exact zap request JSON bytes>)`,
   and returns the BOLT-11 string.
6. On invoice settlement, the api picks the matching signer
...
tests locking it down (`tests/api/nostr.test.ts`). Signing
...
with round-trip tests in `tests/frontend/lib/nostr/zap.test.ts`.
...
2. api enqueues an `ArchiveJob` with a synthetic
...
4. User pays on BTCPay's hosted page. BTCPay settles via its store's
   Lightning connection (our same self-hosted LND node).
...
| Var | Scope | Notes |
|---|---|---|
| `VOLTAGE_REST_URL` | Box A | Host:port for gRPC, e.g. `ln.lnzap.org:10009` |
| `VOLTAGE_INVOICE_MACAROON` | Box A | hex-encoded, invoice-only |
| `VOLTAGE_TLS_CERT` | Box A | the self-hosted LND node's TLS cert — self-signed (nix-bitcoin), so this is **required** |
| `BUNKER_CLIENT_NSEC` | Box A | api's ephemeral key to the bunker; **not** the brand or personal nsec |
...
| `BTCPAY_STORE_ID` | Box A | the store pointing at our LND node |
...
| `ARCHIVE_WORKER_PUBKEY` | Box A | archive-worker Blossom write key allowed by api's dynamic Blossom auth gate |
