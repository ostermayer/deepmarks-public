...
- **relay** — canonical URL, registered-pubkey count
  (`dm:registered:pubkeys`), watched-contacts count
  (`dm:contacts:watched`), per-kind event counters
  (`dm:relay-counter:<kind>`).
- **queues** — `LLEN` of the four worker queues
  (`dm:onboarding:queue`, `dm:lifetime-archive:queue`,
  `dm:pending-publish:queue`, `dm:archive:queue`).
...
against what payment-proxy's Redis cache thinks it has. Built for
...
```bash
curl -H "Authorization: Nostr $(node admin.mjs nip98 GET /admin/relay-stats)" \
