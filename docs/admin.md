...
## 2026-07 reliability updates

Full stories in [`incidents/`](incidents/); the operational surface:

- **Health probes** (all watched by `deepmarks-uptime-check`, Box C
  cron, critical after 3 consecutive failures):
  - `GET /health/blossom` (public, no auth) — asks the blossom-server
    container directly over the compose network; any HTTP answer < 500
    is alive. Needed because `blossom.deepmarks.org/` is a static Caddy
    landing page that stays green with the container dead.
  - `GET /health/archive` — 503 while the archive queue's oldest job is
    > 24 h old (among other conditions). A sustained 503 here means a
    backlog, not necessarily a dead worker — check `pending` and
    `oldestQueuedAgeSeconds` in the body. The webpage retryable-failure
    spike only 503s when it spans ≥25 distinct URLs with ≥10 of them
    OUTSIDE the most-affected owner; one user mass-failing (e.g. a
    dead-bookmark import) — even with a stray failure from someone else
    next to it — shows up in `warnings` instead (2026-08-21 incident +
    2026-08-22 refinement).
  - Box C bunker — probed via its Docker healthcheck
    (`docker inspect`), not a host port; the bunker deliberately
    publishes none.
- **Monitoring PATH gotcha (the big one)**: cron's default PATH lacks
  `/usr/local/bin`, which silenced *every* alert both check scripts
  tried to send since installation. Both scripts now export PATH
  themselves. If `/var/lib/deepmarks-uptime/*.fail` counters are large
  while the inbox is quiet, test `deepmarks-alert` by hand first —
  and remember a manual (login-shell) run can succeed where cron
  fails. `DEDUP_SECONDS=3600` on Box C: a persisting critical pages
  hourly.
- **Mirror-fanout alerts are gone**: failed Blossom mirror legs are
  log-only (`archive complete with partial Blossom mirror fanout` in
  api logs) — the worker's retry queue handles them; only primary
  Blossom trouble pages, via the probe above.
- **Pending-archive claims**: private lifetime + media add-on enqueues
  dedupe against in-flight jobs (`dm:archive-pending:item:*` keys,
  released by terminal callbacks, 7-day TTL backstop). Duplicate
  submissions return the `queued:already-pending` sentinel. If a user
  reports "can't re-archive a URL," check whether a stale claim is
  holding it: the key is
  `dm:archive-pending:item:<family>:<pubkey>:<sha256(url)>`.
- **Queue surgery precedent**: the 2026-07-17 compaction (RENAME the
  queue aside, rebuild, LPUSH back) is the template for any future
  mass queue repair — see the duplicate-flood incident report for the
  exact procedure and its safety reasoning.
