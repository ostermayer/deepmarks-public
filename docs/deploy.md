...
./deploy/push-deploy.sh --only a --service payment-proxy
...
```bash
ssh dan@<box-a> '/opt/deepmarks-repo/deploy/deploy.sh a payment-proxy'
```

That runs `docker compose build payment-proxy` and
`docker compose up -d --no-deps payment-proxy`, avoiding an unnecessary
...
- `CORS_ORIGIN` must include browser and native app origins. Production
  ...
  payment-proxy also appends the native origins in code so mobile API
