...
| DELETE | `/account` | NIP-98 | Full tombstone — username release + API-key revoke + synced-settings wipe + archive blob deletion + mirror delete queue + passkey wipe + ciphertext wipe. Lifetime-payment record deliberately preserved. |
...
completed a fresh assertion can read their own blob. The assertion token is
single-purpose; API keys are opaque `dmk_` tokens (never JWTs — the
email-era session-JWT machinery was removed 2026-08-23).
...
settings, the username claim, any API keys, and archive
records/blobs. Mirror Blossom
