...
Test suite grew from 900 to 1,022 along the way; every fix below has a
...
- **Live replication**: a `strfry stream` sidecar on Box A pushes every
...
2. **Progressive media playback** — v2 chunks decrypt with bounded
   memory but still download fully before play; Range + MSE streaming
   (which needs fMP4 remuxing on the worker) is the next step.
   Worker-side file-to-file streaming encrypt would drop its remaining
   ~2× memory peak. `strfry stream` is deprecated upstream in favor of
   `strfry router` — migrate when convenient.
3. **Export coverage for imported e-tag note bookmarks** — they render
   and search now, but /app/export still covers native kinds only.
4. **Merge-helper unification** — feed.ts vs own-bookmarks vs the
   extension's Recent still carry near-duplicate "which copy wins"
   policies with real behavioral differences; a design pass, not
   mechanical (the extension's tie-break still keeps the higher id).
