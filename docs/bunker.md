...
Box C's NIP-46 signing service. Holds operational signer nsecs and
answers signing requests from api over a Nostr relay — so no
nsec lives on Box A.
...
compromise of the api host granted the attacker total
control of the operational Deepmarks signer on Nostr — they could forge
receipts and labels. Moving
...
|---|---|
| Box A compromise | Can request signatures on the allowlist (legacy `brand` role × {9735, 1985, 39701}; personal × {9735, 39701, 1}) using the ephemeral `BUNKER_CLIENT_NSEC`. **Cannot** exfiltrate any nsec. **Cannot** sign kind:0 profile edits, kind:5 deletions, kind:10002 relay lists, or any kind not on the allowlist. Rotation: generate a new client keypair, update Box A's `BUNKER_CLIENT_NSEC` + Box C's `BUNKER_CLIENT_PUBKEY`, redeploy both. Old client's future requests start being rejected immediately. |
| Box C compromise | Can read operational signer nsecs from disk (chmod 400 is defense-in-depth, not a boundary against root). Full control of those operational identities on Nostr. Rotation: generate new nsecs, drop into `/opt/deepmarks-bunker/nsecs/`, update Box A's `BUNKER_BRAND_PUBKEY` / `BUNKER_PERSONAL_PUBKEY` envs. LNURL metadata will start advertising the new pubkeys; old receipts become unverifiable by NIP-57-strict clients. |
...
|---|---|---|---|---|
| `brand` (legacy role name) | `npub10jeec…` | `/opt/deepmarks-bunker/nsecs/brand.nsec` | `9735`, `1985`, `39701` | Admin/operational signer: zap receipts for `zap@deepmarks.org`, NIP-32 lifetime labels on settlement, and legacy operational bookmark publishing |
| `personal` (legacy role name) | `npub199z…` in production | `/opt/deepmarks-bunker/nsecs/personal.nsec` | `9735`, `39701`, `1` | Public brand/social signer: daily Pinboard bookmark plus matching social note; also legacy zap receipts for the mapped LNURL identity |

Unauthorized client pubkeys and disallowed kinds both get audited as
`rejected`. There is **no path** to sign kind:0 (profile), kind:5
(deletion), kind:10002 (relay list), or any kind not on the list for
its identity. The only kind:1 path is the public-profile daily
Pinboard cross-post through `personal`.
...
```
api                              strfry (10.0.0.2:7777)                              bunker (Box C)
─────────────                              ──────────────────────                              ──────────────
  kind:24133 event
...
echo "BUNKER_CLIENT_PUBKEY=<api's client pubkey>" \
...
BUNKER_BRAND_PUBKEY=<legacy brand/admin operational nsec's pubkey, hex>
...
50 tests in `tests/bunker/*.test.ts` (the `bunker/vitest.config.ts` `dir` points there; only stale compiled `bunker/dist/*.test.js` artifacts sit under `bunker/`):
...
- `handler.test.ts` — full pipeline: decrypt → dispatch → permission
  ...
  brand *cannot* sign kind:1, personal *cannot* sign kind:1985, unaddressed
...
- [lightning.md](lightning.md#lnurl-pay--hosted-deepmarks-addresses) —
