...
When the collection has a description, it is emitted as the
`summary` tag on the public event. The `client` (NIP-89) tag is
stamped unconditionally by `buildPublicCollectionEvent`
(`bookmark-collections.ts`).
```
...
- stripping single and double quotes (`'` and `"`)
- lowercasing
