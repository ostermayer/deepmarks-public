...
- **Bottom tab bar** lives in `frontend/src/lib/components/NativeTabBar.svelte`
  ...
  bookmarks, collections, friends, search, and save. Tags, posts, read later,
  ...
  search action buttons remain available in the horizontal view toolbar
  and search only within the active view by default.
...
- **Settings cleanup is shared.** The cleanup panel uses the normal
  Settings route in native shells. It can scan duplicate canonical URLs,
  failed archive jobs from imports, and archive-missing bookmarks. Missing
  archives are queued again; only duplicate and failed-archive candidates
  are eligible for bulk delete through the same signer-backed delete paths
  as the web app.
...
- **Collections:** `/app/collections` is a native-tab destination by
  default. The row-level collection action opens as a tap sheet above the
  bottom tab bar, and collection-specific adds route through
  `/app/save?collection=<collection>&collectionVisibility=<public|private>&returnTo=...`
  so the user returns to the collection after saving. Collections are
  explicit bookmark lists, not tags.
...
directly from the share sheet with NIP-98 auth — **the bookmark reaches
the relays and shows up on the web without ever opening the app**:
...
The two platforms read the signing key from different on-device stores,
because the share sheet runs outside the WebView:

- **iOS** reads the nsec from a shared Keychain item (`KeychainSharedStore`,
  accessible after first unlock) that the main app must mirror there. That
  mirror is kept in sync on every nsec login **and** on app relaunch
  (restore), independent of the WebView's localStorage policy, and is
  wiped on logout/account switch — see `mirrorSharedNsec` in
  `frontend/src/lib/stores/session.ts`. Without this, the extension has no
  key and every share falls back to opening the app.
- **Android** reads the nsec from the app's encrypted secure store
  (`NativeSecureValueStore`, AES/GCM via Android Keystore), written on
  native login by `saveMobileSignerNsec`. The Zapstore build is the same
  signed `app-release.apk`; nothing about sharing differs.

Only local-key sign-ins (nsec / passkey / recovery key) get the no-open
path. Remote-signer sign-ins (NIP-46 / bunker / Android NIP-55) have no
key on the device, so those shares fall back to the host app — as do
native publish failures — with the edited payload queued for `/app/save`.
Queued saves already posted by the native share sheet are marked
`published: "1"`, so the host app updates local state without posting a
duplicate event.

**Fast metadata autofill.** The share sheets (and the in-app save form
when running natively) request `/metadata?...&enrich=0`, which skips the
server's inline LLM enrichment so title/description/suggested-tags
populate as fast as possible — the page metadata is Redis-cached and the
saved bookmark is still LLM-enriched by the backend backfill once it
publishes. See [llm-augmentation.md](llm-augmentation.md).
