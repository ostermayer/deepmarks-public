...
- **Foreground refresh hook**: `MobileAppLockGate.svelte` registers a
  ...
  app.
- **Publish timeout** is bumped to 12 s on native (vs 4.5 s on web)
  because WKWebView WebSocket cold-starts can take 5–8 s before the
  first relay acks. Lives in `frontend/src/lib/nostr/publish.ts`.
- **Save flow durability**: the durable publish queue auto-retries
  any save whose initial publish didn't reach `relay.deepmarks.org`.
  See [durable-publish.md](durable-publish.md). This is how saves
  made offline or on flaky LTE eventually land.
