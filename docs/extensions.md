...
Public saves publish directly to the user's relays. Private saves are
encrypted to self with NIP-44 v2. Large private bookmark sets use the
same chunked, versioned `kind:30003` format as the web app, so saving
from the extension cannot collapse a large web import back to a single
partial set.
...
default tags, default visibility, archive-by-default, and backup
Blossom servers, plus whether archive-by-default was manually
overridden. Device-local secrets stay local: NWC credentials, saved
website grants, and encrypted nsec records are not sent through the
settings endpoint.

## NWC / WebLN

Users can add a Nostr Wallet Connect URI. The NWC secret is encrypted
with the same password-derived key used for the user's nsec. The
extension can expose a WebLN bridge so supported sites can request
Lightning payments through the user's connected wallet.
