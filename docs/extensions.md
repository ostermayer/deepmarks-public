...
device-local. NWC credentials are not sent through `/account/settings`;
they sync separately as a self-encrypted `kind:30003` record with
`d="deepmarks-nwc"`.
...
also published to the user's relays as a NIP-44 self-encrypted
`kind:30003` record. That lets web, mobile, and extension surfaces use
the same wallet connection after the user adds it once. The extension can
expose a WebLN bridge so supported sites can request Lightning payments
through the user's connected wallet.
