...
**Safari** is the third store target (Edge is out of scope). The Safari
build wraps the same source as a Safari Web Extension: `npm run
build:safari-app` / `package:safari-app` in `browser-extension/`
produces the app shell in `browser-extension/safari/`, which is opened
in Xcode and submitted to the App Store like any native macOS/iOS app.
All three targets ship the same extension version (currently 2.2.10).
