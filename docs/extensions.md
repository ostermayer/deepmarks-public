...
Extension readers also understand native mobile private-item events
(`kind:30003`, `d=deepmarks-private-item:<sha256(url)>`) and merge them
with the chunked private set. Private deletes publish an encrypted
tombstone item for the URL so a private share saved from mobile cannot
