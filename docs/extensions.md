...
server-mediated path the web app and iOS app use. The server writes the
accepted event to `relay.deepmarks.org` and the relay-fanout worker
propagates it to the user's NIP-65 write relays (see
[`relay-policy.md`](relay-policy.md)). Private saves are encrypted to
self with NIP-44 v2 and their signed `kind:30003` set updates use the
same `/publish` path. Large private bookmark sets use the same chunked,
versioned format as the web app, so saving from the extension cannot
collapse a large web import back to a single partial set.
