...
|---|---:|---:|---|
| `api/src/` | 86 | 29 | broad helper coverage; route-module wiring still thin |
| `archive-worker/src/` | 14 | 12 | crypto/Blossom/Wayback/mirrors/media/scholarly helpers covered; worker audit/media paths have foothold coverage; queue/renderer still thin |
| `bunker/src/` | 8 | 5 | core signing pipeline covered; relay/config thin |
| `browser-extension/src/` | 42 | 5 | first tests landed; background/content-script paths thin |
| `frontend/src/lib/` | 154 | 42 | high relative coverage; UI regressions still need browser checks |

Verification counts at the time of this snapshot: 373 api tests, 377
frontend tests, 50 bunker tests, 63 archive-worker tests, and 23
browser-extension tests, for **886 tests** total. (As of 2026-08-21 the
suites have grown to 60/72/5/24/8 test files and ~1,293 passing tests —
see [`../tests/README.md`](../tests/README.md) for the live catalog.)
...
### B. api route layer (thin direct tests)
...
- Auth-gate wiring (NIP-98 vs Bearer API keys; requireSession existed then) is only tested in production
...
| File | Risk | Why no test is bad |
|---|---|---|
| `queue.ts` | HIGH | Recovering dead processing lists while skipping live active workers is covered. `MAX_ATTEMPTS` (5 at the time; 3 today), corrupt JSON skip, and `complete` pipeline error surfacing still need focused tests. |
| `renderer.ts` | MEDIUM | Playwright-driven; hard to test cleanly. The subresource SSRF block (batch 9) and the output cap (50 MB at the time; 150 MB today) are both unverified. |
| `worker.ts` | MEDIUM | Partial direct coverage now exists for media success/failure, pending failure-callback retry, and archive-audit stale-job requeue. The long-running `runLoop`, audit cursor progression across multiple capped passes, and full render/upload/callback integration still need tests. |
| `safe-url.ts` | HIGH | DNS-resolve recheck path. Mirror of `api/src/safe-url.ts` which IS tested — would be cheap to mirror the test suite. *(Since added: [`tests/archive-worker/safe-url.test.ts`](../tests/archive-worker/safe-url.test.ts).)* |
...
- `api/src/route-deps.ts` — pure type wiring, no behavior to test
- `api/src/types.ts` — pure types
- `api/src/index.ts` — bootstrap; integration test would be more useful than unit
- `api/src/bootstrap.ts` — same
- `api/src/workers-bootstrap.ts` — same
- `api/src/email.ts` — thin Resend wrapper; production warning logic added in batch 15 isn't tested but the function is 4 lines
...
| June 3 archive-icon incident: completed archives hidden on mobile | A frontend store test for paged `/account/archives` loads plus URL-variant lookup. Targeted tests now cover trailing-slash and YouTube-video-key lookup; bookmark rows no longer let local missing-key state hide completed `blossom` tags. |
| June 3 stale public archive jobs | Archive-worker audit tests for metadata replay and capped audit cursor progression. Metadata replay is now covered; cursor progression still needs a multi-pass fake-Redis test. |
| June 3 OUP HTTP 403 capture failure | Browser/worker smoke fixture that asserts blocked publishers produce terminal failed state and do not remain stuck as `taken`. |
...
3. **`api/src/queue.test.ts`** + **`invoice-handler.test.ts`** — atomic markPaid claim, rollback path, double-callback dedup, underpaid refusal. Mock `ioredis` at the `multi/exec` boundary. ~400 lines, ~20 tests. The financial paths deserve coverage on par with auth.
