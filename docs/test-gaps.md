...
Current verification count snapshot: 373 api tests, 377
frontend tests, 50 bunker tests, 63 archive-worker tests, and 23
browser-extension tests, for **886 tests** total.
...
| File | Risk | Why no test is bad |
|---|---|---|
| `queue.ts` | HIGH | Recovering dead processing lists while skipping live active workers is covered. `MAX_ATTEMPTS=5`, corrupt JSON skip, and `complete` pipeline error surfacing still need focused tests. |
...
| `safe-url.ts` | HIGH | DNS-resolve recheck path. Mirror of `api/src/safe-url.ts` which IS tested — would be cheap to mirror the test suite. |
