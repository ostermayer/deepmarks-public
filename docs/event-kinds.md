...
| 30003  | NIP-51 | Private bookmark sets, archive keys, and explicit collections | Deepmarks-native + third-party |
| 30001  | NIP-51 | Deprecated bookmark sets (predecessor of 30003)  | Legacy third-party (imported only) |

This page covers only the bookmark + friend-discovery kinds. For the
complete list of every Nostr kind Deepmarks touches (profiles, zaps,
labels, bunker plumbing, NWC, etc.) see [`nostr.md`](nostr.md).
...
- **Anyone can sign** — but our writePolicy only persists events
  ...
  outbox worker. There are two exceptions outside this table: (1) kind:1
  text notes authored by `dm:contacts:watched` pubkeys that contain an
  `http(s)` URL are persisted (so `/app/friends` can render those link
  rows from the local relay), and (2) a kind:1 note whose id a watched
  friend's NIP-51 bookmark list references (`dm:bookmarked-note-target:<id>`)
  is persisted even from an unregistered author, so a bookmarked post
  renders without re-querying third-party relays; see
  [`relay-policy.md`](relay-policy.md) for both exception paths.
...
Generic NIP-51 bookmark/list set. Deepmarks uses it for **three**
distinct things:

### A) The user's private bookmarks

The normal web/app save path stores private bookmarks as **chunked**
encrypted lists because NIP-44 v2 caps plaintext at ~65 KB and a heavy
bookmarker has thousands of entries:
...
order. Older private entries can lack those tags; readers then derive a
stable fallback from the encrypted set event's `created_at` plus the
entry's position inside the set, so order is browser-independent without
treating local cache time as canonical. The selector in
`private-bookmarks.ts`
...
Mobile share extensions can also publish one encrypted private-item
event without opening the host app:
...
```jsonc
{
  ...
  "created_at": 1779100000,
  "tags": [
    ["d", "deepmarks-private-item:<sha256(url)>"]
  ],
  "content": "<NIP-44 v2 ciphertext, base64>",
  "sig": "<64-byte signature>"
}
...
Decrypting a private item yields the same JSON array-of-inner-tag-arrays
as a chunk, normally with one bookmark entry. Readers fetch both the
chunked set and the latest private item per `d` tag, then merge by URL
using the newer containing event. Deleting a private bookmark also
publishes an encrypted tombstone item for that URL so an old mobile item
cannot reappear after the chunked set removes it.

### B) Explicit collections

Deepmarks collections are not tag-derived. Public collections are
addressable bookmark sets whose `d` tag starts with
`deepmarks-collection:` and whose member URLs are public `r` tags:
...
```jsonc
{
  ...
  "tags": [
    ...
    ["visibility", "public"],
    ["r", "https://example.com/paper", "Example Paper", "1779100000"]
  ],
  "content": "",
  "sig": "<64-byte signature>"
}
...
Private collections use
`d=deepmarks-collection-private:<sha256(collection-slug)>`. Their
collection name and member URLs live in NIP-44 encrypted content so the
public `d` tag does not leak the list name.

The NIP-51 importer skips both `deepmarks-collection:` prefixes so
collection members do not appear as loose imported bookmarks.

### C) The archive-keys set

A second kind:30003 set with `d=deepmarks-archive-keys`. Same chunking
pattern. Maps each archived blob hash, and each in-flight
`job:<jobId>`, to its private archive AES decryption key:

```json
{
  "<blob-hash-1>": "<base64 AES key>",
  "job:<job-id>": "<base64 AES key>"
}
```

Lets a user decrypt private archives on any device they're signed in
on without the AES keys ever crossing a server boundary. After the
archive completes, reconciliation promotes the `job:<jobId>` key to the
final blob-hash entries.
...
- **Public or private refs** — public bookmark refs live in tags.
  Private NIP-51 refs can live in encrypted `content` as JSON tag
  arrays. Current clients should use NIP-44; Deepmarks also attempts
  NIP-04 fallback for legacy `?iv=` payloads.
...
notes from Damus / Primal show up the moment they sign in. Encrypted
private refs are mirrored as ciphertext, decrypted on the user's
device after sign-in, then sent as explicit target ids to the server's
social-bookmark prefetch endpoint so the referenced `kind:1` notes can
be imported into our relay. After that, the writePolicy keeps accepting
their kind:10003 publishes from any client, and the fanout worker
propagates them to the user's NIP-65 write relays.

## kind:30001 — deprecated bookmark sets (NIP-51, legacy)

`kind:30001` was the original addressable "categorized bookmarks/sets"
kind, since deprecated in favour of `kind:30003`. Plenty of 2023–2024
clients (older Amethyst / Nostrudel) still wrote bookmark sets here,
commonly with `d:"bookmark"`. Deepmarks treats `30001` exactly like
`30003`: the onboarding scanner imports it (requiring a non-empty `d`
tag), the writePolicy allowlist accepts it, and the frontend importer
extracts public + decrypted-private `r`/`e` refs from it. It is
**import-only** — Deepmarks never authors a new `30001`; user edits
write `kind:39701` or the encrypted `30003` set. Without this, users
whose only bookmark set is a legacy `30001` would see an empty list.
...
sign locally, then POST signed event(s) to api.deepmarks.org/publish
   ↓
api queues and forwards to relay.deepmarks.org
   ↓
...
10003 / 30000 / 30003 / 30001 / 10002 / 3 / 0 events and forwards them
to our relay, so a long-time Damus user's bookmark history, follow
sets, and relay list show up the moment they sign up.
