...
- **Section nav tab** — `read later` tab at the top of
  `/app/bookmarks` filters to `toread`-tagged items. Reachable on web
  and mobile.
...
- **One-click toggle** — every owner row in `BookmarkCard` has a
  ...
  tag in/out and republishes the bookmark. The toggle is
  optimistic: the local cache flips immediately, the relay publish
  happens in the background, and a failed publish reverts.
- **Save form** — every save form (web, iOS share extension, browser
  extension popup) has a "Read later" toggle that pre-toggles based
  on the `mark new bookmarks as read later by default` setting.
...
- The durable publish queue catches the publish if `relay.deepmarks.org`
  doesn't ack within timeout.
