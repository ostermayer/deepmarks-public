...
- Terminal archive failures must not lose the Box A callback. If
  `api` does not accept the failure callback, the worker writes
...
- `tests/api/redis-exec.test.ts` covers Redis command-level
  pipeline errors and null exec results.
- `tests/archive-worker/queue.test.ts` covers dead processing-list recovery
  while skipping live active workers.
- `tests/archive-worker/worker.test.ts` covers terminal failure records
...
- Extract shared API/archive contracts across frontend, browser
  extension, api, and archive-worker so archive record fields
