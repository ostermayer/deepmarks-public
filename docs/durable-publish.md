...
api forwards to strfry and the relay-fanout worker retries
...
is ADDITIVE: it reads the relay's current state first and publishes only
what is missing — one kind:39701 template per missing public URL, one
encrypted per-item event per missing private URL. It never rewrites the
chunked set, never resurrects tombstoned URLs, and skips entirely when
the relay is unreachable, so a stale device cannot re-assert old state.
   regular save uses.
