...
|---|---|---|
| Public source release | GitHub tags/releases on `deepmarks-public`, for example `v0.8.1` | A sanitized source snapshot for audit, contributors, and public release notes. |
...
The public GitHub repository is a sanitized source mirror. Its active
release train is the `v0.x` series (currently `v0.8.x`).
...
- use the next `v0.x` public source tag (e.g. `v0.8.x`)
...
- publish a public source release, using the next public source tag
  such as `v0.8.1`, with notes that mention the installed app/extension
  version line
