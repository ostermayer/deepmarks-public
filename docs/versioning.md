...
|---|---|---|
| Public source release | GitHub tags/releases on `deepmarks-public`, for example `v0.9.0` | A sanitized source snapshot for audit, contributors, and public release notes. |
...
release train is the `v0.x` series (currently `v0.9.x`).
...
- use the next `v0.x` public source tag (e.g. `v0.9.x`)
...
- publish a public source release, using the next public source tag
  such as `v0.9.0`, with notes that mention the installed app/extension
  version line
