...
States: `pending-payment` · `enqueued` · `done` · `failed`. Successful
completion also shows up in `GET /api/v1/archives` below. Done records
may include a `files` array when an archive has multiple blobs, such as
a scholarly article captured as both rendered HTML and PDF.
