...
Programmatic access to your bookmarks and archives. **Lifetime-tier members only** (21,000-sat one-time payment). The sovereignty story is that anyone can use the app, talk to relays directly, write a native Nostr client, and get all their data out for free. The API is a *convenience* that Deepmarks operates for lifetime members.
...
Sign a `kind:27235` event with `u` and `method` tags. Deepmarks clients
also include a random `nonce` tag. Body-bearing routes must include a
`payload` tag equal to the SHA-256 hex digest of the exact request body
bytes:
...
```json
{
  ...
  "tags": [
    ...
    ["payload", "sha256-body-hex-for-body-bearing-routes"]
  ],
  "content": "",
  "pubkey": "...",
  "id": "...",
...
Base64-encode the full JSON string and pass as
`Authorization: Nostr <base64>`. The server rejects events that are
stale (>60 s clock skew), replayed within the freshness window, missing
the required body hash, or whose `u` / `method` tags do not exactly
match the request.
...
All paths are relative to `https://api.deepmarks.org` in production, `http://localhost:4000` in local dev.
...
```bash
curl -X POST https://api.deepmarks.org/api/v1/keys \
...
```bash
curl https://api.deepmarks.org/api/v1/keys \
...
```bash
curl -X DELETE https://api.deepmarks.org/api/v1/keys/$KEY_ID \
...
- `archived=true` — only entries with an `archive-tier` intent tag
  (simple-list mode only). This is not a completed-archive guarantee;
  completed snapshots are tracked by archive job status/account archive
  records.

```bash
# Simple list — newest 50 with the bitcoin tag
curl "https://api.deepmarks.org/api/v1/bookmarks?tag=bitcoin&limit=50" \
  -H "Authorization: Bearer $DMK_KEY"

# Full-text search — "rust async" with two AND'd tags
curl "https://api.deepmarks.org/api/v1/bookmarks?q=rust+async&tag=tokio&tag=hyper&limit=20" \
  -H "Authorization: Bearer $DMK_KEY"
```
  ...
  "bookmarks": [
    {
      ...
      "title": "An Article", "description": "A short summary",
      "tags": ["rust", "async"],
      "savedAt": 1700000000, "savedAtMs": 1700000000123,
      "saveCount": 3, "zapTotal": 1500
    }
  ],
...
```bash
curl "https://api.deepmarks.org/api/v1/search/public?q=bitcoin&tag=lightning&site=stacker.news" \
  -H "Authorization: Bearer $DMK_KEY"
```
...
```bash
curl -X POST https://api.deepmarks.org/api/v1/bookmarks \
  -H "Authorization: Bearer $DMK_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE https://api.deepmarks.org/api/v1/bookmarks/$EVENT_ID \
  -H "Authorization: Bearer $DMK_KEY" \
  -H "Content-Type: application/json" \
...
Start a lifetime archive job. API keys are only issued to lifetime
members, so this endpoint never returns or creates a Lightning invoice.
Poll `GET /api/v1/archives/:jobId` until the job completes. Large imports
may spend time queued on Box B; archives are background preservation work,
not realtime availability.

```bash
curl -X POST https://api.deepmarks.org/api/v1/archives \
  -H "Authorization: Bearer $DMK_KEY" \
  -H "Content-Type: application/json" \
  -d '{"url":"https://example.com/article","mirrorUrls":["https://your-blossom.example"]}'
```

Optional `mirrorUrls` adds user-owned backup Blossom servers to the
operator mirror set for that archive. Entries must be public `https`
Blossom server roots. Up to 8 backup servers are accepted per request.

Optional body fields:
- `tier` — `"public"` (default) or `"private"`. Private archives are
  AES-256-GCM encrypted in the worker before Blossom upload.
- `archiveKey` — **required when `tier` is `"private"`**. The base64
  AES key (43 chars, optional trailing `=`) the worker encrypts with;
  it is zeroed after encryption and never persisted server-side. Keep
  it client-side (you need it to decrypt). Omitting it on a private
  request is a 400.
- `bookmarkSavedAt` — optional unix-seconds timestamp used for ordering;
  values more than 10 minutes in the future are ignored.

**200 OK**:
```json
{
  "jobId": "lifetime:<job-id>",
  "paymentHash": "lifetime:<job-id>",
  "amountSats": 0
}
```
...
Poll status for an archive job.

```bash
curl https://api.deepmarks.org/api/v1/archives/$JOB_ID \
  -H "Authorization: Bearer $DMK_KEY"
```

States: `queued` (accepted, no purchase record yet) · `pending-payment`
· `paid` · `enqueued` · `expired` · `done` · `failed`. (`queued` is the
fallback when only the job has been recorded; `paid`/`enqueued`/`expired`
are passthrough purchase-record statuses.) Successful completion also
shows up in `GET /api/v1/archives` below. Done records may include a
`files` array when an archive has multiple blobs, such as a scholarly
article captured as both rendered HTML and PDF.
...
```bash
curl "https://api.deepmarks.org/api/v1/archives?limit=50" \
  -H "Authorization: Bearer $DMK_KEY"
```
...
The `blobHash` is a SHA-256 you can fetch from any Blossom server hosting the archive (current default: `https://blossom.deepmarks.org/<blobHash>`). When `files` is present, `blobHash` remains the primary archive file and every `files[].blobHash` is independently content-addressed and refcounted.

### NIP-98 sibling: `GET /account/archives` and `DELETE /account/archives/:blobHash`

Authed via NIP-98 (kind:27235 HTTP-Auth) instead of Bearer keys. The web
app's `/app/archives` page, settings zip download, bookmark-row download
buttons, and the browser extension's archived tab all use this endpoint.
The record shape matches `GET /api/v1/archives` but this route also
returns a top-level `failures` array and, per record, `thumbHash`,
`kind`, `videoId`, `videoContentKey`, `videoTitle`, `videoChannel`, and
`videoDurationSeconds`.

Query parameters:
- `limit` — max records, 1–5000, default 100
- `offset` — pagination offset, 0–50 000, default 0

`DELETE /account/archives/:blobHash` removes the archive from your account:
1. drops the entry from `dm:archives:<authPubkey>` so the GET sibling stops returning it
2. decrements the per-blob cross-user refcount; the primary S3 blob is
   `deleteObject`'d **only when no other pubkey still references it**
   (byte-identical public archives are shared). A still-referenced blob is
   retained and `sharedReferences` reports how many owners remain
3. queues a best-effort BUD-01 `DELETE` to known mirrors (`mirrorDeleteQueued`)
4. (client-side, separately) for private archives the browser extension purges the AES key from `chrome.storage.local` and re-publishes the user's NIP-51 `deepmarks-archive-keys` set without the entry — so even mirror copies of the ciphertext become mathematically unreadable
...
```json
{
  ...
  "tier": "private",
  "primaryDeleted": true,
  "mirrorDeleteQueued": true,
  "sharedReferences": 0
}
```

`primaryDeleted` is `false` (with `sharedReferences > 0`) when another user
still references the same blob. Mirror deletes are queued as best-effort
BUD-01 requests — Deepmarks asks mirror operators to drop the blob, but
honoring it is up to each operator, so public archives should be treated as
effectively permanent once mirrored; private archives become unreadable
after key purge regardless. See `/pricing#archive-deletion` for the full FAQ.
...
- **Private bookmarks.** Stored as NIP-51 (`kind:30003`) sets encrypted to your own pubkey via NIP-44 v2. Large libraries are chunked across `deepmarks-private*` replaceable sets to stay below NIP-44 plaintext limits. The server holds ciphertext only and the API has no decrypt path. They never appear in any `/api/v1/*` response.
