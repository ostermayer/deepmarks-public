...
Same data shape as `GET /api/v1/archives`, but authed via NIP-98
(kind:27235 HTTP-Auth) instead of Bearer keys. The web app's
`/app/archives` page, settings zip download, bookmark-row download
buttons, and the browser extension's archived tab all use this endpoint.

Query parameters:
- `limit` — max records, 1–500, default 100
- `offset` — pagination offset, 0–10 000, default 0
...
`mirrorsRetained` is always `true` today — we have no protocol mechanism to instruct mirror operators like Primal or user-added Blossom backups to drop blobs. Public archives are effectively permanent once mirrored; private archives become unreadable after key purge. See `/pricing#archive-deletion` for the full FAQ.
