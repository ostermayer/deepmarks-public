...
flowchart LR
  ...
  relay --> indexer[search-indexer worker]
  indexer --> meili[Meilisearch public index]
...
flowchart TB
  ...
  subgraph boxA[Box A: public edge]
    ...
    strfry[strfry relay]
    strfryStream[strfry-stream replication router]
  ...
  strfry --> publicRelays
  strfryStream --> strfryReplica
  autoheal -.->|restart if unhealthy| strfry
```
...
archive.today snapshot, an open-access scholarly PDF, a FixTweet-rebuilt
copy of an `x.com` / `twitter.com` status (by status id — no nitter/xcancel
mirror is ever fetched), or a mirror surfaced by a self-hosted searxng
search and an open-source LLM — and archives that instead. Every candidate is
