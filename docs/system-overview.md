...
Box A is the public edge and stateful API host. Box B is isolated so
headless browser work cannot take down the API. Box C holds only the
server-owned signing keys needed for zap receipts, lifetime labels, and
brand events.
...
flowchart TB
  ...
  publicChoice -->|private| privateSet[Update encrypted kind:30003 set]
