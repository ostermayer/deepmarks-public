...
Deepmarks collections are explicit single-author NIP-51 bookmark
sets — see [`collections.md`](collections.md) for the full event
model (kind:30003 with `d=deepmarks-collection:<slug>` / private
SHA256 variant), slug rules, share URLs, owner UI, and mobile
behavior.

Key invariants that affect cross-feature behavior:

- Collections stay single-author. Nostr replaceable events are signed
  by exactly one author, so Deepmarks does not model multi-user
  mutation of one list event.
- Member entries are `r` URL tags only — the collection writer/reader
  does not handle `e`/`a`/`t` member kinds. Other Nostr clients'
  NIP-51 lists may include those kinds; Deepmarks ignores them when
  importing as collections.
- Canonical public Nostr identity is
  `30003:<owner-pubkey>:deepmarks-collection:<collection-slug>`,
  exposed on the web as
  `https://deepmarks.org/u/<user>/<collection-slug>`.
