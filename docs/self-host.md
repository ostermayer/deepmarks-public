...
| Server | Public? | Runs |
|---|---:|---|
| Box A | yes | Caddy, api (HTTP only) + four `worker-*` containers, strfry relay, Blossom server, Redis, Meilisearch, qdrant, searxng |
...
| Server | Public? | Runs |
|---|---:|---|
| Box A | yes | public edge, API (HTTP only) + four `worker-*` containers, relay, Blossom, Redis, Meilisearch, qdrant, searxng |
| Box B | no inbound | archive-worker with Playwright/Chromium + bgutil PO-token sidecar |
...
- The Box A Docker services are healthy: `caddy`, `api`, the four
  ...
  `worker-enrichment`, `worker-payments`), `strfry`, `blossom-server`,
  `redis`, `meilisearch`, `qdrant`, and `searxng`. (`api` runs
