...
| [`api/`](../api) | Fastify API. Handles LNURL/zaps, lifetime membership, search, favicon cache, API keys, account settings, passkey ciphertext storage, archive queueing, and archive callbacks. |
...
flowchart LR
  ...
  subgraph minimum[Minimum public instance]
    boxA[Box A: API + workers, relay, Redis, Meilisearch, qdrant, searxng, Blossom]
    boxC[Box C: optional NIP-46 bunker]
  end

  subgraph full[Full archive instance]
    boxB[Box B: archive-worker]
  end
...
That starts Redis, api, archive-worker, and frontend with
...
| Server | Public? | Runs |
|---|---:|---|
| Box A | yes | Caddy (`deepmarks-caddy:ratelimit` build), api (HTTP only, `RUN_WORKERS=none`) + four `worker-*` containers, strfry relay + `strfry-stream` replication sidecar (`replication` profile), Blossom server, Redis, Meilisearch, qdrant, searxng, `autoheal` (restarts unhealthy containers — strfry recovery path) |
...
| Server | Public? | Runs |
|---|---:|---|
| Box A | yes | public edge, Caddy (`deepmarks-caddy:ratelimit` build), API (HTTP only, `RUN_WORKERS=none`) + four `worker-*` containers, strfry relay + `strfry-stream` replication sidecar (`replication` profile), Blossom, Redis, Meilisearch, qdrant, searxng, `autoheal` (strfry recovery path) |
| Box B | no inbound | archive-worker with Playwright/Chromium + bgutil PO-token sidecar + `wg` WireGuard residential-egress sidecar (archive-worker shares its netns for a last-ditch YouTube egress path); `strfry-replica` container (VPC-only `10.0.0.3:7777`, fed by Box A's `strfry-stream`, no profile gate — runs whenever Box B compose is up) |
...
| `api.example.org` | Box A Caddy | api API |
...
5. Configure Box A.
   ...
   use. For a small instance, leave BTCPay/LND (`VOLTAGE_*`) fields empty until you
...
| BTCPay / LND | Bookmarking, search, relay, and app login still work. Lifetime purchases and hosted Lightning invoices are disabled. |
...
| Meilisearch | Personal local search still works in the frontend. Global public keyword search is unavailable or degraded. |
| qdrant | Keyword search still works. Semantic / natural-language bookmark search falls back to the Redis-vector path or is unavailable. |
| searxng | Archives still work. Archive rescue loses its web-search candidate source (it still tries URL variants, Wayback, scholarly PDFs, and FixTweet for tweets). |
| Four `worker-*` containers | The API serves HTTP but no background jobs run: search indexing, the relay outbox (fanout/onboarding/follows), LLM enrichment, and Lightning settlement all stall. Run them, or set `RUN_WORKERS=all` on a single-box dev/fallback to fold the fleet back into the API process. |
...
- The Box A Docker services are healthy: `caddy`, `api`, the four
  `worker-*` containers (`worker-search-indexer`, `worker-relay-sync`,
  `worker-enrichment`, `worker-payments`), `strfry`, `strfry-stream`
  (only with `--profile replication`), `blossom-server`, `redis`,
  `meilisearch`, `qdrant`, `searxng`, and `autoheal`. (`api` runs
  `RUN_WORKERS=none`, so without the worker containers indexing, the
  relay outbox, enrichment, and payment settlement are silently dead.
  `autoheal` is the recovery path when `strfry` reports unhealthy.)
...
- `/account/archives` returns within a few seconds on a lifetime
  account with many archives. The server caps `limit` at 5000 per page
  (default 100); the web client pages at 1000 entries per request.
