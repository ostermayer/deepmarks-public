...
- Deepmarks only acts as a signer for other apps when the mobile-signer
  feature is enabled. While it is off (the default), incoming
  `nostrsigner:` requests are declined without bringing Deepmarks to the
  foreground, and the signer content provider returns nothing — so
  opening another app that happens to have Deepmarks selected as its
  external signer (e.g. Amethyst) no longer interrupts you with a
  Deepmarks sign-in prompt.
...
> Plus `Nip46ForegroundApprovalStore`, `NostrSignerActivity`,
> `ShareActivity`, and `PendingShareStore` which sit alongside these
> classes under `frontend/android/app/src/main/java/org/deepmarks/app/`.
>
> The Android `BOOT_COMPLETED` receiver (Phase 3 below) is **not yet
> implemented** — the service does not auto-restart after device reboot.
