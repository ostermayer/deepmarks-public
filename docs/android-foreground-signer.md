...
Unsupported or unauthorized methods must return an encrypted error
response rather than crashing or leaking plaintext.
...
Potential future approval policy:

- Default: approve every sensitive request in the app.
- Optional per-client "remember approval" for narrow methods/kinds.
- A remembered policy must show the method/kind scope clearly.
- Never remember approvals for unbounded `sign_event` without an
  explicit warning.
...
iOS resubmission is not required for this feature unless shared web UI
copy changes in a way that ships inside the iOS bundle.
...
- Decide whether future builds should approve every `sign_event`
  visibly, or continue allowing background signing for already-paired
  clients and allowed kinds.
