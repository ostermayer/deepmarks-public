...
```bash
./scripts/publish-public.sh --dry-run --keep-export --tag-version --version 1.0.0 -m "Prepare Deepmarks public release"
```
...
```bash
./scripts/publish-public.sh --tag-version --version 1.0.0 -m "Prepare Deepmarks public release"
```
...
```bash
SIGN_WITH="bunker://pubkey?relay=wss://relay.example.com&secret=..." zsp publish --quiet zapstore.yaml
```
...
npm version 2.2.9                  # bumps package.json + auto-commits
git tag extension-v2.2.9           # match the tag prefix the workflow watches
...
### Safari marketing version vs build number

`npm run package:safari-app` stamps `MARKETING_VERSION` from
`package.json` and `CURRENT_PROJECT_VERSION` from `SAFARI_BUILD_NUMBER`
(**pass it explicitly** — the default is the marketing version, which
collides). The build number is a **plain integer counter, bumped by 1
per App Store upload** — never a version string. macOS requires it to
be strictly higher than the last upload regardless of marketing
version. History: early uploads used dotted builds (2.2.8 shipped as
build "2.2.9" after the 3.1.3(b) rejection cycle); the stream switched
to integers at v2.2.9, which ships as build **29**. Apple compares
dotted strings component-wise, so any integer ≥ 3 outranks the old
dotted builds — the next upload is 30.

```bash
SAFARI_BUILD_NUMBER=29 npm run package:safari-app
```
