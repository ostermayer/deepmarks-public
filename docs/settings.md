...
- **Private vs public** — initial visibility for the save form. Stored
  as `defaultVisibility` in synced settings. New accounts default to
  public saves unless the user has explicitly switched the default to
  private.
- **Mark new bookmarks as read later by default** — flips the
  ...
  and pre-toggles accordingly. Default: off.
...
- **Extra archive copies** — lifetime-only. Add personal Blossom
  ...
  defaults.
- **Browser extensions** — install links for Chrome / Firefox / Safari.
...
private bookmarks missing from `relay.deepmarks.org`.
...
The top-level "cleanup" section is library maintenance, not a normal
feed view. It scans the local signed-in library and shows selectable
candidates for:

- duplicate canonical URLs, where Deepmarks keeps the newest or already
  archived copy and preselects the redundant copies
- archive jobs that failed during import or later archive-status refresh

The delete action is explicit selection plus a second confirmation
click. Public bookmarks publish NIP-09 deletion requests; private
bookmarks publish encrypted per-item tombstones; imported third-party
NIP-51 URL bookmarks rewrite the original list without that `r` tag.

Bookmarks that are merely missing an archive are not cleanup candidates:
while archive-by-default is enabled, the cleanup panel counts them as
"archives to queue" and exposes a queue action that re-enters them into
the lifetime archive backfill path. A missing archive becomes a cleanup
candidate only after the archive queue records a failure.

The cleanup panel intentionally does not probe arbitrary URLs directly
from the browser. Dead-domain and redirect-target checks still need a
server-side worker with the same SSRF controls as archive capture. See
[roadmap.md](roadmap.md#broken-link-cleanup).
...
`updatedAt`, using millisecond timestamps so rapid toggles across
devices have a deterministic winner. Secrets are deliberately excluded:
NWC credentials, saved signer grants (extension-only), passkey state,
and recovery keys are not stored in this document. NWC credentials also
stay out of relay-backed settings because they are wallet spending
credentials.
