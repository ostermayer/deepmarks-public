...
it is synced separately as a NIP-44 self-encrypted `kind:30003` record
with `d="deepmarks-nwc"`, so the user adds it once and all Deepmarks
surfaces can load it while the server and relays only see ciphertext.
...
keys are not stored in this document. NWC sync uses a separate
self-encrypted `kind:30003` record.
