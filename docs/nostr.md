...
- **Server-mediated publish model.** Clients sign locally and POST the
  ...
  including the registered-pubkey writePolicy and onboarding scan.
...
| 10002 | [65](https://github.com/nostr-protocol/nips/blob/master/65.md) | read + publish | user's relay list (outbox model); fanout uses write relays |
...
| 10003 | [51](https://github.com/nostr-protocol/nips/blob/master/51.md) | read + fanout | NIP-51 legacy single bookmark list. Damus / Primal / Amethyst / Snort use this when a user pins a kind:1 note. Our onboarding scanner imports the user's existing kind:10003 on first registration; the writePolicy keeps accepting forward-published kind:10003 from any client. Rendered in `/app/posts` via `createImportedNoteRefsFeed` |
| 30003 | [51](https://github.com/nostr-protocol/nips/blob/master/51.md) | read + publish | NIP-51 categorized sets, encrypted to self via NIP-44 v2. Two `d` tags in use: `deepmarks-private` (private bookmark entries) and `deepmarks-archive-keys` (per-blob AES keys for private archives, mapped by blobHash). Onboarding scanner also imports third-party kind:30003 bookmark sets (any with `e` / `a` / `r` tags) so a user's `"reading"` / `"research"` categories from another client ride along |
...
- **relay.deepmarks.org** — [strfry](https://github.com/hoytech/strfry)
  ...
  gates by registration and kind. Registered pubkeys can publish:
