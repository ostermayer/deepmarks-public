...
- We only publish kind:1 text notes when the user explicitly composes
  ...
  notes are fanned out to the user's relays and are generally not stored
  on our relay — with one exception: notes authored by
  `dm:contacts:watched` pubkeys that contain an `http(s)` URL are stored
  so `/app/friends` can render link-only rows without re-querying old
  third-party relays. See
  [`relay-policy.md`](relay-policy.md).
...
| 7 | [25](https://github.com/nostr-protocol/nips/blob/master/25.md) | planned | reactions are not read or published yet; kind:7 only appears in the relay's TEAM_EXTENDED_KINDS allowance for the brand pubkey |
...
| 10063 | [BUD-03](https://github.com/hzrd149/blossom/blob/master/buds/03.md) | delete scan only | legacy/planned Blossom user-server list kind (Blossom BUD-03, not a NIP); account deletion includes it if found |
| 24242 | [BUD-01](https://github.com/hzrd149/blossom/blob/master/buds/01.md) | publish (server) | Blossom media-type write auth — used by archive-worker to request upload/download/has/list/delete permission. The api's `/blossom/check-auth` validates the `t` (op) tag of each BUD-01 event and only permits the archive-worker pubkey |
| 1985 | [32](https://github.com/nostr-protocol/nips/blob/master/32.md) | publish | lifetime-membership labels (see below) |
| 24133 | [46](https://github.com/nostr-protocol/nips/blob/master/46.md) | publish/read | nostr-connect bunker messages between api and Box C — never user-facing, relay-only |
...
Shape (see `api/src/api-helpers.ts` and
...
On startup, api queries our own label events from relays and
...
- **relay.deepmarks.org** — [strfry](https://github.com/hoytech/strfry)
  ...
  - `kind:24133` — NIP-46 bunker messages (api ↔ Box C)
  ...
  queued for fanout, and are not stored by our relay — with two
  exceptions the write-policy persists: a note authored by a
  `dm:contacts:watched` pubkey that contains an `http(s)` URL (so
  `/app/friends` can render friends' link posts from the local relay),
  and a note whose id a watched friend's NIP-51 bookmark list references
  (`dm:bookmarked-note-target:<id>`) — the latter is accepted even from
  an unregistered author so a bookmarked post renders without re-querying
  third-party relays. All other kinds from non-team pubkeys are rejected.
  The relay is deliberately narrow — it's not a general-purpose public
  relay.
- **strfry internal** (`ws://strfry:7777` from inside the docker
  network, `ws://10.0.0.2:7777` from the VPC) — used by api's
  internal workers, the `/archive/lifetime` + webhook-settlement
...
- **User nsecs**: *never* touch our servers. NIP-07 extensions (Alby,
  ...
  The api only ever receives already-signed events.

## How the server signs without holding keys

api speaks NIP-46 (nostr-connect) to Box C's bunker, which
holds the operational signer nsecs. Every time api needs to
...
5. api pairs the response by request id, publishes the now-
...
The canonical copy lives on the user's relays; cross-device
reconciliation re-fetches from relays (the api-side encrypted cache
that once existed was part of the email-account subsystem removed on
2026-08-23). Decryption always happens client-side.
...
| Zap request validation + receipt template | [`api/src/nostr.ts`](../api/src/nostr.ts) |
| NIP-32 lifetime label template + publish | [`api/src/nostr.ts`](../api/src/nostr.ts) |
| NIP-46 bunker client (api → Box C) | [`api/src/signer.ts`](../api/src/signer.ts) |
...
| NIP-98 verification | [`api/src/auth.ts`](../api/src/auth.ts) (`verifyNip98`) |
