...
| 30003 | [51](https://github.com/nostr-protocol/nips/blob/master/51.md) | read + publish | NIP-51 categorized sets, encrypted to self via NIP-44 v2. Deepmarks `d` tags in use: `deepmarks-private` (private bookmark entries), `deepmarks-archive-keys` (per-blob AES keys for private archives, mapped by blobHash), and `deepmarks-nwc` (synced NWC wallet connection). Onboarding scanner also imports third-party kind:30003 bookmark sets (any with `e` / `a` / `r` tags) so a user's `"reading"` / `"research"` categories from another client ride along |
