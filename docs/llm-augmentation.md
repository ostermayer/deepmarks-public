...
- chat/enrichment/import/alert/summary:
  `deepseek-ai/DeepSeek-V4-Flash` (DeepInfra) with a **Cerebras
  `gpt-oss-120b` fallback** — `chatJson` fails over to Cerebras on a
  DeepInfra timeout/error so a spike becomes slower-but-successful rather
  than a dropped job. Both are open-weight.
- archive rescue: `deepseek-ai/DeepSeek-V4-Pro`
- embedding: `Qwen/Qwen3-Embedding-8B` (DeepInfra only, no fallback)
- rerank: `Qwen/Qwen3-Reranker-4B` (DeepInfra only, no fallback)

Chat uses a longer timeout than the native/embedding path:
`DEEPINFRA_CHAT_TIMEOUT_MS` (20 s) vs `DEEPINFRA_TIMEOUT_MS` (8 s), since
occasional latency spikes — not slow models — were clipping chat jobs.
...
DEEPINFRA_CHAT_TIMEOUT_MS=20000
# Chat fallback (open-weight). Empty CEREBRAS_TOKEN disables the fallback.
CEREBRAS_TOKEN=
...
- `GET /llm/status` returns `{ enabled, policy, models, queueDepth,
  semanticStore, archiveRescueSearch }` (the last is the searxng
  rescue-search summary).
