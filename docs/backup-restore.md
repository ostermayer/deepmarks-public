...
| qdrant | Not separately backed up | Semantic-search vectors (`qdrant-data` volume), derived from bookmarks + embeddings. Rebuild by re-enriching/re-indexing if lost. |
...
credentials are the api bucket credentials:
...
**Status: LIVE since 2026-07-17.** The Box B Cloud Firewall rule (TCP
7777 from 10.0.0.2) exists, the sidecar is running, and
`COMPOSE_PROFILES=replication` is set in `deploy/box-a/.env` so every
plain `docker compose` / `deploy.sh` invocation manages the sidecar
like any other service — without that env var a compose command that
omits `--profile replication` would silently ignore it. The stream had
been OFF from the 2026-06-09 pid-namespace incident until the firewall
rule landed; the replica drifted to nightly-export staleness in
between (caught by `strfry-replica-stale` on 2026-07-17 and
reconciled — see
[`incidents/2026-07-17-silent-monitoring.md`](incidents/2026-07-17-silent-monitoring.md)).
...
incident). The flip side: a relay crash kills the sidecar too, and
Docker's restart of the sidecar can race the relay's own restart and
strand it `Exited` (2026-08-04 incident). `deepmarks-resource-check`
on Box A auto-revives and reconciles within 15 min, and Box B alerts
`strfry-replica-stale` after 1h without a replicated event — see
`deploy/monitoring/README.md` ("Replication stream — defense in
depth").
