...
Since the structural-projects release, Box A's `strfry-stream` sidecar
pushes every newly-stored event to a VPC-only strfry replica on Box B
(`10.0.0.3:7777`) within seconds — the recovery point for a Box A disk
loss is no longer the last nightly export.
...
- Reconcile a gap (replica downtime):
  `cd /opt/deepmarks-repo/deploy/box-a && docker compose run --rm strfry-stream strfry sync ws://10.0.0.3:7777 --dir up`
