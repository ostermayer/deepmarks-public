...
- **Provider blocks:** hosted media extraction is only as good as the
  ...
  days. The primary YouTube unlock is the **`bgutil-provider` PO-token
  sidecar** on Box B (the yt-dlp `bgutil-ytdlp-pot-provider` plugin),
  which lets public videos archive without cookies. A read-only
  `YTDLP_COOKIES_FILE` is only needed for the age-restricted /
  "confirm you're not a bot" subset — and invalid cookies are
  counterproductive (they can zero out formats even with a valid PO
  token), so it stays blank unless a valid file is mounted. Either way
  it's an operator choice, not a user-account feature, and does not
...
5. On future bookmark saves, web/mobile/extension clients detect eligible
   ...
   bookmarkSavedAt }`. A URL with a media job already pending is answered
   with the `queued:already-pending` sentinel, and one whose last terminal
   failure is inside the escalating cooldown gets `queued:recent-failure`
   instead of a new job (see the terminal-failure gate in
...
1. Add a route under `api/src/routes/` for hosted checkout,
