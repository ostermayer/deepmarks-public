...
| `api` | ../../api | 10.0.0.2:4000 (VPC) | Fastify app: LNURL, zaps, lifetime archive enqueue, BTCPay webhook, /api/v1, /admin, search, `/favicon` cache, `/account/archives` (GET + DELETE). VPC binding lets Box B's archive-worker callback in without going through Caddy. |
...
- `blossom.deepmarks.org` → static archive-storage info page at `/`
  and `blossom-server:3000` for blob reads/writes.
...
`/publish` for fanout only and are not stored by strfry.
...
> Process topology: as of 2026-06-25 the `indexer` and `relay-fanout`
> worker (and the rest of the background fleet) run as **separate worker
> containers** on Box A, not inside the API process (`RUN_WORKERS=none`).
> The data flow above is unchanged — only the hosting process differs.
...
those relays or waiting on flaky third-party relays at save time. See
