...
```
  ...
  (CF Pages,         (Box A, Caddy)     (Box A, strfry)
   static SPA)                          blossom.deepmarks.org
        ...
        │          (archive blobs, backup bucket, favicons, ciphertext)
        │
        │                 Box C (VPC, SSH-only)
        │       ┌───────────────────────────┐
...
payment-proxy, which only allows the archive-worker pubkey. Lifetime
...
lists (10002), NIP-51 bookmark/follow lists (10003 / 30000 / 30003),
