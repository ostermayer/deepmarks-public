...
accepts the supported Deepmarks user kinds from registered pubkeys:
profiles (0), contacts (3), deletions (5), mute lists (10000), relay
lists (10002), NIP-51 bookmark lists (10003 / 30003), public
bookmarks (39701), zap receipts (9735), and bunker messages (24133).
...
```
browser/iOS/extension (signer) ─signs kind:39701─> POST /publish
                                                           │
                                                           ▼
                                                payment-proxy queue + forward
                                                           │
                                                           ▼
                                               strfry writePolicy plugin
                                                1. kind in ALLOWED_KINDS?
                                                2. SISMEMBER dm:registered:pubkeys author?
                                                3. rate limit OK?
                                                           │ accept
                                                           ▼
                                                    strfry persists
...
[`relay-policy.md`](relay-policy.md) for the registered-pubkey gate,
fan-out worker, onboarding scanner, and lifetime-archive backfill.
