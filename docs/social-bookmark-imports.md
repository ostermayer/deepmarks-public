...
- `kind:30003` — NIP-51 addressable bookmark set. Deepmarks imports
  ...
  signed event is otherwise valid and has a `d` tag. Deepmarks-owned
  collection sets whose `d` tag starts with `deepmarks-collection:` or
  `deepmarks-collection-private:` are skipped here; those are rendered by
  the collection UI, not imported as loose bookmarks.
