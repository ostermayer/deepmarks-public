...
| `api/` | Fastify API, LNURL, account settings, `/publish`, search, workers |
...
`api`, `archive-worker`, and `frontend`.
...
1. [`system-overview.md`](system-overview.md) for the product, Nostr,
   archive, and deployment diagrams.
2. [`architecture.md`](architecture.md) for the system diagram and Box
   A/B/C responsibilities.
3. [`relay-policy.md`](relay-policy.md) for the server-mediated publish
   and relay fan-out model.
4. [`bookmarks.md`](bookmarks.md) for public/private bookmark behavior.
5. [`settings.md`](settings.md) for synced settings and device-local
   secrets.
6. [`self-host.md`](self-host.md) for running your own instance.
