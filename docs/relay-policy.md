...
- **The onboarding scanner** runs once per newly relay-allowed pubkey
  ...
  (kind:39701 / 10003 / 30000 / 30003) from their NIP-65 set into our
...
3. Otherwise `LPUSH`es the raw event JSON onto
   `dm:kind1-fanout:queue` (capped at 10,000 entries to bound the
   keyspace).
4. Returns `shadowReject` for fanout-only notes — strfry tells the
...
`payment-proxy/src/workers/follows-ingester.ts` watches kind:3 contact
lists from relay-allowed Deepmarks users. For each contact list:

1. Store the user's followed pubkeys in `dm:follows:by-user:<pubkey>`.
2. Union those followed pubkeys into `dm:contacts:watched`.
3. Periodically pick watched pubkeys whose last ingest is oldest.
4. Resolve each curator's NIP-65 relays, then query public bookmark
   kinds `[39701, 30003, 10003]` plus kind:1 notes that contain
   `http(s)` links.
5. Register the curator in `dm:registered:pubkeys` before forwarding
...
payment-proxy and trigger repeated startup handshakes.
...
2. Queries those relays for events: `kinds:[39701, 10003, 30000, 30003]`,
...
username claim, or first kind:39701 publish via our API)
  ...
  onboarding-scanner worker BRPOPs the queue
   ...
   - Query kinds:[39701,10003,30000,30003] from those relays
