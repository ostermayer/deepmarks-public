...
- Actual product/user metrics should use account, payment, and username
  stores: `dm:registered:pubkeys`, `dm:lifetime:*`, `dm:username:bypubkey`, BTCPay
...
3. Keep kind:30000 follow sets only when they are addressable (`d`
   tag present).
4. Fan out any addressable kind:30003 set (`d` tag present). We do not
   inspect tags for `e` / `a` / `r` references — Amethyst-style private
   bookmarks keep their refs in the NIP-44-encrypted `content`, so
   requiring plaintext refs would drop them.
...
once a `kind:0` event is seen — but only after re-verifying the event
signature (these come from third-party relays, so a forged `kind:0` for
another pubkey is dropped rather than cached). That keeps friend pickers
and feed
...
2. Queries those relays for events: `kinds:[39701, 10003, 30000, 30003, 30001, 10002, 3, 0]`,
...
username claim, or first kind:39701 publish via our API)
  ...
  onboarding-scanner worker BRPOPs the queue
   ...
   - Query kinds:[0,3,10002,39701,10003,30000,30003,30001] from those relays
...
Rate-limit scopes in `deploy/box-a/strfry/deepmarks.js`:

| Scope | Applies to | Default | Env wiring in `deploy/box-a/compose.yml` |
|---|---|---|---|
| `bookmarks` | kind:39701 | 1000/h (`STRFRY_BOOKMARK_RATE_LIMIT_PER_HOUR`) — imports died in the old shared 200/h bucket | **not wired** — env var is read by the plugin but compose.yml does not pass it; override requires editing compose.yml or the image default |
| `private-state` | kind:30003 `deepmarks-private*` (chunks AND `deepmarks-private-item:` per-item events/tombstones), `deepmarks-collection-private:*`, `deepmarks-archive-keys*`, `deepmarks-nwc` | 5000/h | `STRFRY_PRIVATE_STATE_RATE_LIMIT_PER_HOUR` |
| `general` | everything else | 200/h | `STRFRY_RATE_LIMIT_PER_HOUR` |
