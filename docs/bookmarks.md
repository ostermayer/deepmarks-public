...
**Filtering** (what subset to show) is decoupled from sorting and lives
in the section nav tabs above the sort row: `bookmarks` (all),
`posts`, `read later`, `archives`. On native shell the `archives` tab
is suppressed because the bottom tab bar already covers it. The
underlying mechanism is a `?view=archived` or `?view=readlater` query
param the section-nav links carry.
...
[archives.md](archives.md)). Pending/running archive intent stays out
of normal rows and is shown in the archives-tab progress panel. The
right-rail archived count is the completed `/account/archives` count,
not the number of bookmarks that have an `archive-tier` intent tag.
...
row. The "read later" section-nav tab filters to the same set. New
saves include `toread` by default — controlled by the **mark new
bookmarks as read later by default** toggle in settings, which lives
on top of the same `defaultTags` field that already syncs across
web/iOS/extension.

## Cross-client sync

Saves are durable. After every publish the client checks whether
`relay.deepmarks.org` specifically acked the event. If not, the signed
template lands in a per-pubkey localStorage queue
(`deepmarks-pending-publish:<pubkey>`) that drains on:
...
`drainPendingPublishes` runs four publishes in parallel, dedupes
replaceable events by `kind + d-tag`, and drops items after 30
attempts or 30 days so the queue can't grow forever. The full
