...
flowchart LR
  url[URL save]
  choice{Visibility}
  publicEvent[kind:39701 public bookmark]
  privateEvent[kind:30003 encrypted private set]
  publish[API /publish]
  relay[relay.deepmarks.org]
  fanout[User write relays]
  local[Local decrypted library]

  url --> choice
  choice -->|public| publicEvent
  choice -->|private| privateEvent
  publicEvent --> publish
  privateEvent --> publish
  publish --> relay
  relay --> fanout
  privateEvent --> local
```
...
['client', 'deepmarks']            // NIP-89 client tag — stamped unconditionally
```

(The `client` tag is added by `buildBookmarkEvent` on every published
bookmark; readers ignore unknown tags. See
[`frontend/src/lib/nostr/bookmarks.ts`](../frontend/src/lib/nostr/bookmarks.ts).)
...
Every surface (web, extension, iOS/Android share sheets) saves a
private bookmark as a single encrypted PER-ITEM event:
`kind:30003`, `d=deepmarks-private-item:<sha256(url)>`, content =
NIP-44 encrypted JSON containing the same inner tag array. The chunked
`deepmarks-private*` set remains the bulk-import write path and the
read base. Readers merge the latest per-item events with the chunked
set, preferring the newer containing event for duplicate URLs — so a
save touches exactly one URL and concurrent multi-device edits cannot
clobber each other.
...
`friends`, `posts`, `collections`, `read later`, `archives`. On iOS and
Android, the duplicated section nav is hidden and the native bottom tab
system owns mobile navigation. `Posts` and `Read later` are available
from the More menu by default and can be added to the visible tab row;
`Archived` is opened from each bookmark row's archive controls. The
underlying mechanism is a `?view=` query param: `?view=archived`,
`?view=readlater`, and (sidebar link only — not in the section nav)
`?view=media` for media-add-on archives.
...
1. **Title line** — the page title plus a small `private`/`public`
   ...
   decorative lock/globe symbol) so the row's visual vocabulary stays
   narrow.
...
3. **Actions row** — holds archive open/download, zap, post-to-Nostr,
   collection assignment, edit, and read-later toggle. Follow/unfollow is
   **not** here — it lives on the
   curator's `/u/<npub>` profile page so feed rows aren't crowded
   with chrome the user isn't trying to act on.

The favicon is a stable left column that spans the row's main text,
which reduces row jitter and makes dense lists easier to scan.

## Collections

Collections are explicit bookmark lists. Adding a bookmark to a
collection updates the collection's `kind:30003` event; it does not add a
tag and does not republish the bookmark. Public collections can be shared
at `/u/<user>/<collection-slug>`, while private collections stay inside
`/app/collections`.

See [collections.md](collections.md) for the full route, privacy, mobile,
and NIP-51 direction.

## Read later

The `toread` tag (pinboard convention) marks a bookmark for later
reading. Rows with `toread` get a light-orange tint, a `read later` pill
in the facts row, and a one-tap `read` toggle in the actions row. The
`read later` navigation entry filters to the same set. New saves include
`toread` only when the user enables **mark new bookmarks as read later by
default** in settings. That toggle lives on top of the same `defaultTags`
field that already syncs across web, iOS, Android, and the browser
extension.
...
Editing a private bookmark publishes a fresh per-item event for that
URL.

Deleting a public bookmark publishes a NIP-09 `kind:5` deletion request.
Deleting a private bookmark publishes an encrypted per-item tombstone
for the URL; the merge in every reader drops a URL whose newest record
is a tombstone, including copies still present in older chunk
generations.
...
- `/app/collections` — signed-in user's public/private collection index
- `/app/collections/<collection>` — signed-in collection view with
  collection-scoped search, tag cloud, and add-bookmark flow
- `/app/posts` — Nostr social posts (bookmarked `kind:1` notes) and
  Nostr note URLs the user saved. Each post row carries the same actions
  as a bookmark row — tags, read-later, archive/download, zap, share —
  via `PostCard`; the note content stays read-only. See
  [social-bookmark-imports.md](social-bookmark-imports.md#post-bookmark-actions).
