...
is suppressed to keep mobile navigation focused; archived items are
opened from each bookmark row's archive controls. The underlying
mechanism is a `?view=archived` or `?view=readlater` query param the
section-nav links carry.
...
of normal rows and is shown in the web archives/progress view or the
server archive status surfaces. The right-rail archived count is the
completed `/account/archives` count, not the number of bookmarks that
have an `archive-tier` intent tag.
...
web, iOS, Android, and the browser extension.
