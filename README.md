...
| ---------------- | --------- | --------------- | ----- |
| `frontend/`      | ✅ 0 err  | ✅ **299 / 299** | SvelteKit SPA, Cloudflare Pages |
| `payment-proxy/` | ✅ 0 err  | ✅ **305 / 305** | Fastify, hosts LNURL + /api/v1 |
| `archive-worker/`| ✅ 0 err  | ✅ **35 / 35**   | Playwright + SingleFile + BUD-04 |
| `bunker/`        | ✅ 0 err  | ✅ **50 / 50**   | NIP-46 signing service |
| `browser-extension/` | ✅ 0 err | ✅ **18 / 18** | Chrome/Firefox/Safari extension |
...
**Combined: 707 passing tests; 0 typecheck errors.**
...
- **`frontend/`** — SvelteKit + TypeScript SPA. All Nostr signing happens
  ...
  remote signers, recovery-key paste, or passkey-encrypted nsec storage
...
link (via their `lud16`) when they have a Lightning address. If they
do not, the zap goes to Deepmarks instead. Nothing is custodial — the
wallet pays one Lightning invoice directly.
