...
| ---------------- | --------- | --------------- | ----- |
| `frontend/`      | passing   | 433 / 433       | SvelteKit SPA, Cloudflare Pages |
| `payment-proxy/` | passing   | 403 / 403       | Fastify, hosts LNURL + /api/v1 |
| `archive-worker/`| builds    | 67 / 67 (+74 env-gated live) | Playwright + SingleFile + BUD-04 |
| `bunker/`        | passing   | 50 / 50         | NIP-46 signing service |
| `browser-extension/` | passing | 29 / 29       | Chrome/Firefox/Safari extension |
...
**Combined: 982 passing tests; 0 typecheck errors in the latest local
release pass.**
...
- **Public source/server releases** use GitHub tags on
  `deepmarks-public`, currently the `v0.7.x` series (legacy `v2.0.x`
...
- **Android, iOS, Zapstore, and browser-extension releases** use
  monotonically increasing store/package versions such as `2.0.7`.
...
```mermaid
flowchart LR
  signer[User signer] -->|signed event| api[payment-proxy /publish]
...
- **`payment-proxy/`** — Fastify API on Box A. LNURL-pay + NIP-57 zap
  ...
  registration + nsec-ciphertext storage. Hosts background workers for
  indexing, zap receipts, save counts, watched-contact ingest, onboarding
  import, relay fanout, archive backfill, and the daily Pinboard post.
