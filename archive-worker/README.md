...
notifies api for account bookkeeping.
...
src/
  ...
  podcast.ts        — podcast RSS enclosure resolution + episode fetch
  mse-codecs.ts     — MediaSource codec allowlist for streamed media captures
  media-helpers.ts  — content-type + filename helpers for media vs. page captures
  archive-errors.ts — PermanentError vs. retryable categorization + cause map
  tweet-embed.ts    — tweet/nostr capture as standalone HTML archive
  safe-url.ts       — SSRF guard for page/file fetches (DNS-rebinder-resistant)
  safe-http-proxy.ts — egress HTTP/CONNECT proxy that pins validated DNS answers
  ...
  redis-exec.ts     — execOrThrow wrapper so failed pipelines surface
```
...
Requires a Redis instance at `REDIS_URL` and a api at `PAYMENT_PROXY_URL` that can accept `POST /archive/callback`.
...
Container runs on Box B (Linode Dedicated 4GB). Production compose caps the worker at `cpus: "1.5"` with `MAX_CONCURRENT_JOBS=1` so large archive blobs and mirror fanout stay under the Linode bandwidth notification threshold. This is intentionally background long-tail preservation, not realtime capture.
...
- `MAX_CONCURRENT_JOBS` — parallel job-processing loops per worker container. Code default is 4; production sets 1 because archive uploads plus mirror fanout are bandwidth-bound.
...
- `MEDIA_ARCHIVE_MAX_BYTES` — max bytes for media add-on captures,
  including hosted yt-dlp downloads, direct media files, and podcast
  enclosures. Default 2 GB.
- `MEDIA_ARCHIVE_TMPDIR` — writable disk-backed scratch directory for
  media downloads, remux output, and encrypted chunked blobs. Production
  mounts this separately from the small `/tmp` browser scratch tmpfs.
...
| Direct files | PDF, captions, feeds/manifests, ebooks, Office/OpenDocument files, text/CSV/JSON/XML, and common compressed archives via direct downloader after safe-url validation | public or private tier from queued job |
| Podcast episode page | RSS/Atom enclosure discovery, then direct audio download | always private through media add-on |
| Media add-on | disk-backed direct media capture, podcast enclosure download, or yt-dlp + ffmpeg/remux for supported image/video/audio pages, with best-effort metadata/thumbnail/subtitle sidecars | always private |
...
- No account updates — api writes the archive record in the user's account.
