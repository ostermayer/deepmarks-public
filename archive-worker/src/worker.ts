...
import { PermanentError, categorize } from './archive-errors.js';
...
export class Worker {
  ...
  private async processJob(job: ArchiveJob, queue: ArchiveQueue): Promise<void> {
    ...
    try {
      ...
      if (!plaintext && isTweetUrl(job.url)) {
        ...
        if (built) {
          ...
          archiveKind = 'webpage';
          this.log.info(
            { jobId: job.jobId, bytes: plaintext.byteLength, provider: built.provider },
      ...
      // YouTube watch pages serve a "Sign in to confirm you're not a bot"
      // wall to headless Chromium from a datacenter IP, so the Playwright
      // render path permanently fails every YouTube URL (and pages the
      // operator). The paid media add-on (kind:'media', yt-dlp + PO token)
      // is the separate path for downloading the actual video file. For the
      // webpage-archive path we instead build a small self-contained HTML
      // "video card" from YouTube's no-auth oEmbed API — title, channel,
      // thumbnail inlined as a data URI, link back to the live page. The
      // archive succeeds, the user keeps their bookmark, no operator page.
      // Same pattern as the tweet-embed branch above.
      if (!plaintext && isYoutubeVideoUrl(job.url)) {
        ...
        if (built) {
          ...
          // oEmbed failed too (rare: API down / private video / regional
          // block). Fall through to the live render, which will hit the
          // bot wall and fail permanently — but that's now a non-alerting
      ...
      const directFile = plaintext ? null : await tryDownloadDirectFileArchive(job.url).catch(async (err) => {
        ...
        // The live host is dead, slow, TLS-broken, or 4xx/5xx for a real
        // direct-file URL (e.g. a link-rotted gov/academic PDF). Try the
        // Internet Archive snapshot before failing the job so it archives
        // on the first pass instead of waiting for the async rescue sweep.
  ...
  private async processVideoJob(job: ArchiveJob, queue: ArchiveQueue): Promise<void> {
    ...
    try {
      ...
      const mirrorResults = mirrorUrls.length > 0
        ? encryptedPath
          ? await this.blossom.mirrorFile(uploadResult.blobHash, mirrorUrls, encryptedPath, 'application/octet-stream')
