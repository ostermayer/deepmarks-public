...
import { encryptBlob, encryptBlobChunked, zeroize } from './crypto.js';
...
export class Worker {
    ...
    do {
      ...
      for (const { meta, done } of rows) {
        ...
        if (liveStateReliable && isMetadataReplayableArchive(meta)) {
          ...
          if (marked) summary.markedLostFailed++;
          else summary.skippedNonRescuable++;
        } else {
          summary.skippedNonRescuable++;
        }
      }
  ...
  private async processVideoJob(job: ArchiveJob, queue: ArchiveQueue): Promise<void> {
    ...
    try {
      ...
      const encrypted = encryptBlobChunked(result.blob, job.archiveKey);
      zeroize(result.blob);
