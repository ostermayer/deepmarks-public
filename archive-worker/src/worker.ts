...
export class Worker {
  ...
  private async processJob(job: ArchiveJob, queue: ArchiveQueue): Promise<void> {
    ...
    try {
      ...
      // archive from the FixTweet API (no credentials). Falls through to the
      // normal render path below if no provider can serve it.
      if (!plaintext && isTweetUrl(job.url)) {
        const built = await buildTweetArchiveHtml(job.url).catch((err) => {
          this.log.warn(
            ...
            'tweet-embed build failed; falling back to render',
          );
          return null;
        });
        if (built) {
...
function resultFromDirectMediaFile(
...
function requiredMediaBuffer(result: VideoArchiveResult): Buffer {
  if (!result.blob) throw new Error('media archive result has no in-memory bytes');
  return result.blob;
}

function mediaResultByteLength(result: VideoArchiveResult): number {
