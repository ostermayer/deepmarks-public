...
const config: WorkerConfig = {
  ...
  // Whole-render ceiling. Must comfortably exceed nav (30s) + settle (10s) +
  // the wall-clock-bounded scroll + settle (8s) + SingleFile, so heavy/long
  // pages aren't timed out before SingleFile runs. The scroll pass self-caps
  // against this value (renderer.ts), reserving headroom for SingleFile.
  playwrightRenderTimeoutMs: num('PLAYWRIGHT_RENDER_TIMEOUT_MS', 120_000),
