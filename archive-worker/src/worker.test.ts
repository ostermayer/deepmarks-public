...
import type { ArchiveFileRecord, ArchiveJob, DoneRecord } from './queue.js';
...
describe('Worker archive audit summary', () => {
  it('summarizes failed public webpage archives without blocking on Wayback repair', async () => {
    ...
    await expect(redis.get('dm:archive-audit:last')).resolves.toContain('"rescueDeferred":1');
  ...
  it('writes a summary for completed jobs that need callback re-notify', async () => {
    ...
    expect(worker.notifyPaymentProxy).not.toHaveBeenCalled();
    await expect(redis.get('dm:archive-audit:last')).resolves.toContain('"renotifyDeferred":1');
  });
