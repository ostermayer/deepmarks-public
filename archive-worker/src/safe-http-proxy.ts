...
export async function startSafeHttpProxy(opts: { localAddress?: string } = {}): Promise<SafeHttpProxy> {
  // When set (a wg0 source IP), outbound sockets bind to it so a Box B source
  // route sends this proxy's egress out the residential WireGuard tunnel. The
  // SSRF public-host validation runs regardless, so this never weakens it.
  const localAddress = opts.localAddress;
  const server = http.createServer();

  server.on('request', (req, res) => {
    void handleHttpRequest(req, res, localAddress).catch((err) => {
  ...
  server.on('connect', (req, clientSocket, head) => {
    ...
    void handleConnect(req, socket, head, localAddress).catch((err) => {
