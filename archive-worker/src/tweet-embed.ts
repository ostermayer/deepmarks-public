...
import { PermanentError } from './archive-errors.js';

// FixTweet-compatible providers (same JSON shape). Tried in order.
// Exported for the cross-package sync test — api/src/archive-rescue.ts
// carries a deliberate duplicate (separate Docker build contexts).
export const PROVIDERS = ['https://api.fxtwitter.com', 'https://api.fixupx.com'] as const;
// FixTweet asks callers to identify themselves in the User-Agent.
export const UA = 'Deepmarks-Archive/1.0 (+https://deepmarks.org/bot)';
...
export async function buildTweetArchiveHtml(
  ...
  const { tweet, notFound } = await fetchTweet(parts, fetchImpl);
  if (!tweet) {
    // A deleted/gone tweet (every FixTweet provider says 404) will never
    // rebuild — fail permanently as "not found" so the job stops after the
    // first attempt and the owner's card reads "not found", instead of
    // burning MAX_ATTEMPTS and paging the operator with a bogus "timeout".
    // A transient miss (timeout / 5xx / parse error) still returns null so
    // the caller retries.
    if (notFound) {
      throw new PermanentError('tweet_deleted', `tweet not found — deleted or unavailable: ${rawUrl}`);
    }
    return null;
  }
...
interface FetchTweetResult {
  tweet: NormalizedTweet | null;
  /**
   * True only when at least one provider returned a definitive not-found
   * (HTTP 404/410, or a FixTweet `{code:404}` body) and none succeeded or
   * failed transiently — i.e. the tweet is deleted/gone, not momentarily
   * unreachable. Lets the caller fail such a job permanently instead of
   * burning MAX_ATTEMPTS on a tweet that will never come back. A timeout,
   * 5xx, 429, or parse error clears it so the job stays retryable; a
   * provider whose hostname no longer resolves contributes no signal.
   */
  notFound: boolean;
}

async function fetchTweet(parts: TweetParts, fetchImpl: typeof fetch): Promise<FetchTweetResult> {
  // The tweet is declared gone only on positive evidence: at least one
  // provider answered with a definitive 404 and none succeeded or failed
  // transiently. A provider whose hostname doesn't resolve (dead domain) is
  // no signal in either direction — it must not veto a live provider's 404
  // (api.fixupx.com fell out of DNS in 2026 and turned every deleted tweet
  // into a retryable failure that re-alerted the operator forever), and it
  // must not count toward "gone" when DNS is broken for every provider.
  ...
  for (const base of PROVIDERS) {
    try {
      ...
      if (!res.ok) {
        // 404/410 = tweet genuinely gone; any other status (429/5xx/…) is
        // transient, so it is not a clean deletion signal.
        if (res.status === 404 || res.status === 410) sawNotFound = true;
      ...
      if (norm) return { tweet: norm, notFound: false };
      // 2xx with no usable tweet: FixTweet uses {code:404} for gone tweets;
      // treat only that as a deletion, anything else as a transient miss.
      if (json?.code === 404) sawNotFound = true;
      else sawTransient = true;
    } catch (err) {
      // Timeout / network / parse error — transient, not a deletion. An
      // unresolvable hostname is the exception: that provider is dead, not
      // the tweet, so it contributes no signal at all.
      if (!isDnsUnresolvable(err)) sawTransient = true;
    }
  }
  return { tweet: null, notFound: sawNotFound && !sawTransient };
}

/** Node's fetch wraps a DNS NXDOMAIN in a TypeError whose `cause` carries
 *  code ENOTFOUND. EAI_AGAIN (flaky resolver) intentionally stays transient. */
function isDnsUnresolvable(err: unknown): boolean {
