...
interface FetchTweetResult {
  ...
  /**
   * True only when every provider returned a definitive not-found (HTTP
   * 404/410, or a FixTweet `{code:404}` body) and none succeeded or failed
   * transiently — i.e. the tweet is deleted/gone, not momentarily
   ...
   * 5xx, 429, or parse error clears it so the job stays retryable.
   */
  notFound: boolean;
}

async function fetchTweet(parts: TweetParts, fetchImpl: typeof fetch): Promise<FetchTweetResult> {
  // Starts optimistic-gone; the first non-404 signal (a success, a transient
  // error, or a non-404 HTTP status) clears it.
  let notFound = true;
  for (const base of PROVIDERS) {
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status !== 404 && res.status !== 410) notFound = false;
      ...
      if (json?.code !== 404) notFound = false;
    } catch {
      // Timeout / network / parse error — transient, not a deletion.
      notFound = false;
    }
  }
  return { tweet: null, notFound };
}
