...
export interface ArchiveJob {
  ...
  attempts: number;
  enqueuedAt: number;
  /** Original bookmark save time, unix seconds. Archive records use
   *  this as their timeline timestamp so archive ordering mirrors
   *  bookmark ordering even if the worker completes later. */
  bookmarkSavedAt?: number;
  /** Original bookmark URL when `url` is a public rescue/mirror URL. */
  originalUrl?: string;
  /** 'webpage' (default) for the existing Playwright/SingleFile path,
   *  'media'/'video' (and legacy 'youtube') for the yt-dlp media add-on. */
  kind?: 'webpage' | 'youtube' | 'video' | 'media' | 'file';
  /** 11-char YouTube video ID when the source can be canonicalized. */
  videoId?: string;
  /** Stable source key from payment-proxy, retained as archive metadata. */
...
export class ArchiveQueue {
  ...
  private startHeartbeat(_job: ArchiveJob, raw: string): void {
    ...
    this.heartbeat = setInterval(() => {
      ...
      this.redis.expire(KEYS.active(this.workerId), ACTIVE_HEARTBEAT_TTL_SECONDS).catch(() => {});
