...
import type { ArchiveFileRecord, ArchiveJob } from './archive-wire.js';
...
const PROCESSING_PREFIX = 'dm:archive:processing:';
export const ACTIVE_HEARTBEAT_TTL_SECONDS = 90;
...
export const KEYS = {
  ...
  processing: (wid: string) => `${PROCESSING_PREFIX}${wid}`,
  ...
  /** Best-effort BUD-01 DELETE queue for known mirror copies after a
   *  user deletes one archive or tombstones their account. */
  deleteQueue: 'dm:archive:delete:queue',
} as const;
 ...
 *  here so the rest of the worker keeps importing them from queue.js. */
export type { ArchiveJob, ArchiveDeleteJob, ArchiveFileRecord } from './archive-wire.js';
...
export interface DoneRecord {
  ...
  /** True when the worker recorded a terminal result but Box A has not
   *  acknowledged the matching /archive/callback yet. The audit loop
   *  retries delivery and clears this flag after the callback succeeds. */
 ...
 *
 *  Only `retryable` failures consume attempts (see worker.ts shouldRetry);
 *  permanent failures — including HTTP 403/404 from page.goto and direct
 *  file fetches — are dropped on the first try and never re-fetched. The
 *  remaining retries are for transient cases (timeouts, 5xx, closed
 *  browser). Lowered 5 -> 3 so a persistently-timing-out URL gives up
 *  after 2 retries instead of 4, cutting wasted re-fetch bandwidth. */
...
export class ArchiveQueue {
  ...
  async takeJob(timeoutSeconds: number): Promise<ArchiveJob | null> {
    ...
    // stale threshold.
    await this.redis.set(KEYS.active(this.workerId), raw, 'EX', ACTIVE_HEARTBEAT_TTL_SECONDS);
    this.startHeartbeat(job, raw);
    return job;
  }

  /** Move JSON entries left in dead workers' processing lists back onto
   *  the main queue. Live workers keep refreshing dm:archive:active:<wid>;
   *  those processing lists are intentionally skipped so overlapping
   *  deploys do not duplicate another process's in-flight job. */
  async recoverOrphans(): Promise<{ recovered: number }> {
    ...
    do {
      ...
      for (const key of keys) {
        ...
        const active = workerId ? await this.redis.exists(KEYS.active(workerId)) : 0;
  ...
  private startHeartbeat(_job: ArchiveJob, raw: string): void {
    ...
    this.heartbeat = setInterval(() => {
      // SET (not EXPIRE): EXPIRE on an already-expired key is a no-op, so
      // one Redis hiccup longer than the TTL permanently orphaned the
      // lease — and, contrary to the old comment here, NOTHING on Box A
      // reclaims active leases; recovery only sweeps processing lists
      // whose active key is GONE, which risks double-processing the
      // still-running job (2026-08-23 review). SET recreates the lease
      // with the same raw payload + TTL.
      this.redis
        .set(KEYS.active(this.workerId), raw, 'EX', ACTIVE_HEARTBEAT_TTL_SECONDS)
        .catch(() => {});
  ...
  async complete(record: DoneRecord): Promise<void> {
    ...
    // job hanging in the active-job key, looking to the supervisor like
    // a worker that quietly died.
    try {
      await execOrThrow(pipeline);
    } finally {
      this.stopHeartbeat();
...
function workerIdFromProcessingKey(key: string): string | null {
  if (!key.startsWith(PROCESSING_PREFIX)) return null;
