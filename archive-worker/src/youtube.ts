...
export async function downloadVideoArchive(
  ...
  try {
    ...
    const meta = await ytDlpMetadata(sourceUrl, workdir, logger, proxy.url);

    const sidecars = await collectYtDlpSidecars(sourceUrl, workdir, logger, proxy.url).catch((err) => {
      logger.warn({ err }, 'yt-dlp sidecar collection failed');
      return [];
    });

    const blobPath = await downloadBestPlayableMedia(sourceUrl, workdir, logger, proxy.url);
...
async function runYtDlp(
  ...
  await runYtDlpCapture(args, timeoutMs, undefined, logger, proxyUrl);
}
...
async function runYtDlpCapture(
  ...
  return new Promise<string>((resolve, reject) => {
    const child = spawn('yt-dlp', withOperatorCookies(withSafeProxy(args, proxyUrl)), {
      cwd,
