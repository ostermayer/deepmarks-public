...
export async function encryptFileChunked(
  ...
  base64Key: string,
  chunkBytes = CHUNK_PLAINTEXT_BYTES,
): Promise<{ plaintextBytes: number; encryptedBytes: number }> {
  const key = Buffer.from(base64Key, 'base64');
  if (key.byteLength !== 32) {
    throw new Error(`archive key must be 32 bytes, got ${key.byteLength}`);
  }
  ...
  let encryptedBytes = header.byteLength;
  let completed = false;
  ...
  try {
    await write(header);
    const chunkCount = Math.max(1, Math.ceil(inputStat.size / chunkBytes));
    let position = 0;
    for (let index = 0; index < chunkCount; index += 1) {
      const requested = Math.min(chunkBytes, Math.max(0, inputStat.size - position));
      const plaintext = Buffer.allocUnsafe(requested);
      const { bytesRead } = await input.read(plaintext, 0, requested, position);
      if (bytesRead !== requested) {
        throw new Error(`archive input truncated while encrypting ${inputPath}`);
      }
      position += bytesRead;

      const nonce = randomBytes(NONCE_BYTES);
      const cipher = createCipheriv('aes-256-gcm', key, nonce);
      cipher.setAAD(chunkAad(index, index === chunkCount - 1));
      ...
      await write(body);
      await write(tag);
      encryptedBytes += nonce.byteLength + body.byteLength + tag.byteLength;
    }
    completed = true;
  } finally {
    zeroize(key);
    ...
    if (completed) {
      // The success path MUST surface a failed flush/close: the final
      // chunk's 16-byte GCM tag can still be buffered here, and a
      // swallowed ENOSPC used to yield a "completed, verified" archive
      // that fails auth at the tail on every decrypt (2026-08-23 review
      // #13). On the error path we still swallow so the original error
      // propagates unmasked.
      await flush;
    } else {
