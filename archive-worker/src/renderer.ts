...
export class PageRenderer {
  ...
  async render(url: string): Promise<{ html: Buffer; screenshot: Buffer | null }> {
    ...
    const context: BrowserContext = await this.browser.newContext({
      ...
      permissions: [],
      // Many production sites use CSP that blocks inline scripts.
      // SingleFile is our trusted capture script, injected by the
      // archive worker after the page loads; bypass the page's CSP so
      // strict sites don't fail at page.addScriptTag().
      bypassCSP: true,
