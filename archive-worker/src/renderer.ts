...
import { isYoutubeHost } from './youtube-embed.js';
...
// Present as a real browser. Many sites (Nitter behind Cloudflare/Anubis,
// some publishers) serve real HTML to browsers but a bot challenge / 403 to a
// crawler UA — so a URL that opens in a browser would otherwise capture as a
// thin challenge page. Matches the rescue verifier's UA on Box A so a mirror
// that verifies there also renders here.
...
export class PageRenderer {
  ...
  async render(url: string): Promise<{ html: Buffer; screenshot: Buffer | null }> {
    ...
    try {
      ...
      // Use domcontentloaded for the main navigation. Some publishers
      // keep long-lived ad/analytics/image requests open long enough
      // that Playwright never reaches the full "load" event inside the
      // navigation timeout, even though the article DOM is ready. The
      // settle, scroll, lazy-image, and SingleFile passes below still
      // give subresources a chance to resolve before capture.
      ...
      if (response.status() >= 400) {
        ...
        // 5xx + 429 (rate-limited) + 408/425 (request timeout / too-early) are
        // transient — a retry (and ultimately a Wayback/rescue pass) can still
        // succeed. Other 4xx (404/403/410) are the page's real state.
        const transient = status >= 500 || status === 429 || status === 408 || status === 425;
        throw new RenderError(
          'http_error',
          `page returned HTTP ${status}`,
          transient ? 'retryable' : 'permanent',
        );
      }
      ...
      // Anti-bot / consent / sign-in interstitial detection. Some hosts
      // (notably YouTube on datacenter IPs) serve a "sign in to confirm
      // you're not a bot" wall, or redirect to a consent/login page,
      // instead of the real content. Capturing that wall and serving it
      // as the archive is worse than a clean "blocked" result — fail
      // permanently here so the bot page is never stored or served. The
      // text probe is scoped to YouTube hosts to avoid false-positives on
      // articles that merely mention the wall.
      {
        let finalHost = '';
        try { finalHost = new URL(page.url()).hostname.toLowerCase(); } catch { /* ignore */ }
        const gateHosts = ['consent.youtube.com', 'consent.google.com', 'accounts.google.com'];
        if (gateHosts.some((h) => finalHost === h || finalHost.endsWith(`.${h}`))) {
          throw new RenderError('anti_bot_wall', `redirected to gate host ${finalHost}`, 'permanent');
        }
        // One in-page probe: visible text, title, and a head of the markup
        // for known challenge fingerprints.
        const probe = await page.evaluate(() => ({
          text: document.body?.innerText?.slice(0, 4000) ?? '',
          title: (document.title || '').slice(0, 200).trim().toLowerCase(),
          html: document.documentElement?.innerHTML?.slice(0, 20000) ?? '',
        })).catch(() => ({ text: '', title: '', html: '' }));

        let origHost = '';
        try { origHost = new URL(url).hostname.toLowerCase(); } catch { /* ignore */ }
        const isYouTube = isYoutubeHost(origHost) || isYoutubeHost(finalHost);
        if (isYouTube && (/sign in to confirm (?:you|your age)/i.test(probe.text)
          || /confirm you[’'`]?re not a bot/i.test(probe.text))) {
          // No cookies in the renderer → won't pass on retry; permanent so the
          // rescue pass (which can pull a Wayback copy) runs now.
          throw new RenderError('anti_bot_wall', 'YouTube anti-bot sign-in wall', 'permanent');
        }

        // Host-agnostic bot-challenge interstitials (Cloudflare / DataDome /
        // PerimeterX). High-confidence fingerprints only — a real article will
        // not ship the challenge-platform script or have these exact titles —
        // so this won't reject legitimate pages. Retryable: a later attempt or
        // the Wayback/rescue pass may reach the real page. Without it, the
        // multi-KB challenge shell sails past the 5KB floor and is stored as a
        // "successful" archive.
        const isChallenge =
          /\/cdn-cgi\/challenge-platform\/|cf-browser-verification|__cf_chl_|cf-turnstile|challenges\.cloudflare\.com/i.test(probe.html)
          || /captcha-delivery\.com/i.test(probe.html)
          || /px-captcha|_pxhd|perimeterx\.net/i.test(probe.html)
          || probe.title === 'just a moment...'
          || probe.title === 'attention required! | cloudflare'
          || /checking your browser before accessing|enable javascript and cookies to continue/i.test(probe.text);
        if (isChallenge) {
          throw new RenderError('anti_bot_wall', `bot-challenge interstitial on ${finalHost}`, 'retryable');
        }
      }
      ...
      // time to fetch before we move on. Bounded by a WALL-CLOCK budget
      // (not just an iteration count) computed from the remaining render
      // timeout minus a reserve for the final settle + SingleFile + the
      // screenshot — so an infinite-scroll / very long page can't eat the
      // whole budget and starve SingleFile (which previously timed the
      // whole job out and failed it on every attempt).
      const scrollBudgetMs = Math.max(
        5_000,
        this.options.renderTimeoutMs - (Date.now() - renderStartedAt) - 35_000,
      );
      await page.evaluate(async (budgetMs) => {
        ...
        window.scrollTo(0, 0);
      }, scrollBudgetMs).catch(() => { /* tolerate eval failure; we still capture what we have */ });
