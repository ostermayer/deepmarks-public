 ...
 * Nostr key. That pubkey must be in the blossom-server
 * /app/config.yml storage.rules allowlist on Box A.
 */
...
export class BlossomClient {
  ...
  async uploadFile(filePath: string, contentType = 'text/html'): Promise<BlossomUploadResult> {
    return this.uploadFileTo(this.primaryUrl, filePath, contentType);
  }
  ...
  private async uploadTo(
    ...
    const res = await fetch(`${baseUrl}/upload`, {
      ...
      // Scale with blob size: a fixed 60s ceiling made every large
      // media upload abort and re-download the whole video on retry.
      signal: AbortSignal.timeout(uploadTimeoutMs(blob.byteLength)),
    });
  ...
  private async uploadFileTo(
    serverUrl: string,
    filePath: string,
    contentType = 'text/html',
  ): Promise<BlossomUploadResult> {
    const [{ size }, blobHash] = await Promise.all([
      stat(filePath),
      sha256File(filePath),
    ]);
    const baseUrl = serverUrl.replace(/\/$/, '');
    ...
    try {
      const res = await fetch(`${baseUrl}/upload`, {
        ...
        duplex: 'half',
        signal: AbortSignal.timeout(uploadTimeoutMs(size)),
      ...
      return { blobHash, primaryUrl: baseUrl, size };
    } finally {
      // undici does not destroy a request-body stream when the fetch
      // fails without consuming it (timeout abort, connection error,
      // early server response). The open fd then pins the file's disk
      // space even after the job's temp-dir cleanup unlinks it.
      body.destroy();
    }
  }
      ...
      mirrorUrls.map(async (mirrorUrl) => {
        ...
        try {
          ...
          const res = await fetch(`${baseUrl}/mirror`, {
            ...
            signal: AbortSignal.timeout(uploadTimeoutMs(blob?.byteLength ?? 0)),
          });
          if (!res.ok) {
    ...
    const results = await Promise.all(
      mirrorUrls.map(async (mirrorUrl) => {
        ...
        try {
          ...
          if (res.ok) return { url: mirrorUrl, ok: true };
          try {
            await this.uploadFileTo(baseUrl, filePath, contentType);
            return { url: mirrorUrl, ok: true };
          } catch (uploadErr) {
            const uploadMessage = uploadErr instanceof Error ? uploadErr.message : String(uploadErr);
            return {
  ...
  private async buildAuth(
    ...
    const tags: string[][] = [
      ...
      // Uniqueness salt: a /mirror attempt and its direct-upload fallback
      // land within the same epoch second, so without this their auth
      // events serialize identically — same event id — and servers that
      // track spent auth ids reject the fallback with 401 "Auth event
      // already used" (cdn.nostrcheck.me, observed 2026-07-08). Servers
      // ignore unknown tags per BUD-01.
      ['nonce', randomUUID()],
    ];
    if (serverUrl) {
...
/** ≥60s, plus a generous 1 MiB/s floor for big blobs, capped at 15 min. */
export function uploadTimeoutMs(byteLength: number): number {
  return Math.min(15 * 60_000, Math.max(60_000, Math.ceil(byteLength / (1024 * 1024)) * 1_000));
}
