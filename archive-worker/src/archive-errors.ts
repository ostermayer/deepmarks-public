...
/** Structured code from our own error classes ('tweet_deleted',
 *  'anti_bot_wall', 'file_too_large', …), or undefined for foreign errors.
 *  Shipped in the failure callback so Box A classifies by code instead of
 *  re-sniffing the message string — string matchers on both sides drifted
 *  whenever upstream (yt-dlp, Playwright) wording changed. */
export function errorCodeOf(err: unknown): string | undefined {
  if (err instanceof PermanentError || err instanceof RenderError) return err.code;
  return undefined;
}
